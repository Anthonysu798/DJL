...
const createBuildConfig = Effect.fn("createBuildConfig")(function* (
  ...
  const configuredRemoteRelayUrl = process.env.DJL_REMOTE_RELAY_URL?.trim();
  if (configuredRemoteRelayUrl) {
    try {
      const parsed = new URL(configuredRemoteRelayUrl);
      if (
        ...
        throw new Error(
          "Relay URLs must use ws/wss and may not contain credentials or query data.",
        );
      }
    } catch (cause) {
      return yield* new BuildScriptError({
        message: "DJL_REMOTE_RELAY_URL is not a safe WebSocket relay URL.",
        cause,
      });
    }
  }
  ...
  const buildConfig: Record<string, unknown> = {
    ...
    productName,
    artifactName: "DJL-${version}-${arch}.${ext}",
    // Runtime dependencies ship compatible N-API/prebuilt binaries. Rebuilding
    // every transitive native package makes Windows releases require a local
    // Visual Studio toolchain and can replace those verified prebuilds.
    npmRebuild: false,
    directories: {
  ...
  const macAfterSignHook =
    platform === "mac" && signed
      ? join(yield* RepoRoot, "scripts", "notarize-macos-after-sign.cjs")
      : undefined;
  ...
  const platformBuildConfigInput = {
    ...
    target,
    signed,
    ...(platform === "mac" && !signed ? { macIdentity: "-" } : {}),
    ...(macAfterSignHook ? { macAfterSignHook } : {}),
...
const buildDesktopArtifact = Effect.fn("buildDesktopArtifact")(function* (
  ...
  const resolvedServerDependencies = yield* Effect.try({
    try: () =>
      resolveCatalogDependencies(
        omitBundledWorkspaceDependencies(serverDependencies),
        rootPackageJson.workspaces.catalog,
        "apps/server",
      ),
    catch: (cause) =>
  ...
  const stageInstallArgs = resolveDesktopStageInstallArgs({
    platform: options.platform,
    arch: options.arch,
  });
  yield* Effect.log(
    `[desktop-artifact] Installing staged production dependencies for ${options.platform}/${options.arch}...`,
  );
  yield* runCommand(
    ChildProcess.make("bun", stageInstallArgs, {
      cwd: stageAppDir,
      ...commandOutputOptions(options.verbose),
      // Windows needs shell mode to resolve .cmd shims (e.g. bun.cmd).
      shell: process.platform === "win32",
  ...
  yield* runCommand(
    ChildProcess.make({
      ...
      env: buildEnv,
      ...commandOutputOptions(options.verbose),
      // Windows needs shell mode to resolve package-manager commands.
      shell: process.platform === "win32",
  ...
  if (options.platform === "mac") {
    ...
    if (options.signed && options.target === "dmg") {
      const dmgPath = path.join(stageDistDir, `DJL-${appVersion}-${options.arch}.dmg`);
      if (!(yield* fs.exists(dmgPath))) {
        return yield* new BuildScriptError({
          message: `Signed macOS build did not produce the expected DMG at ${dmgPath}`,
        });
      }
      const dmgNotarizationScript = path.join(repoRoot, "scripts", "notarize-macos-dmg.cjs");
      yield* Effect.log(`[desktop-artifact] Notarizing and stapling ${path.basename(dmgPath)}...`);
      yield* runCommand(
        ChildProcess.make({
          cwd: repoRoot,
          env: buildEnv,
          ...commandOutputOptions(options.verbose),
        })`node ${dmgNotarizationScript} ${dmgPath}`,
      );

      yield* Effect.log(
        "[desktop-artifact] Regenerating final DMG blockmap and update metadata...",
      );
      yield* Effect.tryPromise({
        try: () => finalizeMacDmgUpdateMetadata({ stageDistDir, dmgPath }),
        catch: (cause) =>
          new BuildScriptError({
            message: "Final notarized DMG metadata regeneration failed.",
            cause,
          }),
      });
    }
...
const buildDesktopArtifactCli = Command.make("build-desktop-artifact", {
  ...
  Command.withDescription("Build a desktop artifact for DJL."),
