...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  ...
  describe("createDevRunnerEnv", () => {
    it.effect("defaults DJL_HOME to ~/.djl while exporting the legacy compatibility alias", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.DJL_HOME, resolve(homedir(), ".djl"));
        assert.equal(env.SYNARA_HOME, env.DJL_HOME);
      }),
    );
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.DJL_HOME, resolve("/tmp/custom-synara"));
        assert.equal(env.SYNARA_HOME, env.DJL_HOME);
