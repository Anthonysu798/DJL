...
export function updateMacUpdateManifestZipEntry(
  ...
  return updateMacUpdateManifestArtifactEntry(manifest, zipFileName, metadata, {
    removeBlockMapSize: true,
  });
}

export function updateMacUpdateManifestArtifactEntry(
  manifest: string,
  artifactFileName: string,
  metadata: MacUpdateZipMetadata,
  options: { readonly removeBlockMapSize?: boolean } = {},
): string {
  ...
  const nextLines = lines.flatMap((line) => {
    ...
    if (fileUrlMatch?.[1]) {
      inTargetFile = stripYamlSingleQuotes(fileUrlMatch[1]) === artifactFileName;
      if (inTargetFile) {
    ...
    if (
      options.removeBlockMapSize === true &&
      inTargetFile &&
      line.match(/^    blockMapSize:\s*\d+$/)
    ) {
      return [];
    }
    ...
    if (topLevelPath !== null) {
      topLevelPathMatches = topLevelPath === artifactFileName;
  ...
  if (!targetFileFound || !targetShaUpdated || !targetSizeUpdated) {
    throw new Error(`Could not update ${artifactFileName} entry in macOS update manifest.`);
  }

  if (matchingTopLevelPathFound && !topLevelShaUpdated) {
    throw new Error(
      `Could not update top-level sha512 for ${artifactFileName} in macOS update manifest.`,
    );
  }
...
export function validateMacUpdateManifestZipMetadata(
  ...
  return validateMacUpdateManifestArtifactMetadata(manifest, zipFileName, metadata);
}

export function validateMacUpdateManifestArtifactMetadata(
  manifest: string,
  artifactFileName: string,
  metadata: MacUpdateZipMetadata,
): MacUpdateManifestZipValidation {
  return {
    manifestHasZipPath: manifest.includes(artifactFileName),
...
export function assertMacUpdateManifestZipMetadata(
  ...
  return assertMacUpdateManifestArtifactMetadata(manifest, zipFileName, metadata);
}

export function assertMacUpdateManifestArtifactMetadata(
  manifest: string,
  artifactFileName: string,
  metadata: MacUpdateZipMetadata,
): MacUpdateManifestZipValidation {
  const validation = validateMacUpdateManifestArtifactMetadata(
    manifest,
    artifactFileName,
    metadata,
  );
  if (
    ...
    throw new Error(`macOS update manifest does not match ${artifactFileName} metadata.`);
  }
  return validation;
}
