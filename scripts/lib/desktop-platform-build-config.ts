...
export const MICROPHONE_USAGE_DESCRIPTION =
  "DJL needs microphone access so you can record voice notes and transcribe them into the chat composer.";
...
export interface CreateDesktopPlatformBuildConfigInput {
  ...
  readonly macIdentity?: string;
  readonly macAfterSignHook?: string;
...
export function createDesktopPlatformBuildConfig(
  ...
  if (input.platform === "mac") {
    if (input.signed && !input.macAfterSignHook) {
      throw new Error("Signed macOS builds require the repository-owned notarization hook.");
    }
    const mac = {
      target: input.target === "dmg" ? [input.target, "zip"] : [input.target],
      icon: MAC_DMG_ICON_PATH,
      ...(input.macIdentity ? { identity: input.macIdentity } : {}),
      // Electron Builder's built-in `notarize: true` uses one blocking
      // notarytool invocation. The repository hook owns submit/poll/staple so
      // polling retries cannot create duplicate submissions.
      notarize: false,
    ...
    return {
      ...nativePackaging,
      ...(input.signed ? { afterSign: input.macAfterSignHook } : {}),
      ...(input.signed ? { dmg: { sign: true } } : {}),
      mac,
    };
  }
