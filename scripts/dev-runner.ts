...
import { resolveDjlHome } from "@synara/shared/djlStoragePaths";
...
const HomeConfig = Config.all({
  djlHome: optionalStringConfig("DJL_HOME"),
  legacyHome: optionalStringConfig("SYNARA_HOME"),
}).pipe(Config.map(({ djlHome, legacyHome }) => djlHome ?? legacyHome));
...
export function runDevRunnerWithInput(input: DevRunnerCliInput) {
  return Effect.gen(function* () {
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.SYNARA_PORT)} webPort=${String(env.PORT)} baseDir=${String(env.DJL_HOME)}`,
    );
