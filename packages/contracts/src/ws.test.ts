...
it.effect("accepts local model install requests", () =>
  Effect.gen(function* () {
    const parsed = yield* decode(WebSocketRequest, {
      id: "req-local-model-install-1",
      body: {
        _tag: WS_METHODS.localModelsInstallModel,
        runtime: "ollama",
        modelId: "qwen3-coder:30b",
      },
    });
    assert.strictEqual(parsed.body._tag, WS_METHODS.localModelsInstallModel);
  }),
);

it.effect("accepts managed Ollama runtime install requests", () =>
  Effect.gen(function* () {
    const parsed = yield* decode(WebSocketRequest, {
      id: "req-local-runtime-install-1",
      body: {
        _tag: WS_METHODS.localModelsInstallRuntime,
        runtime: "ollama",
      },
    });
    assert.strictEqual(parsed.body._tag, WS_METHODS.localModelsInstallRuntime);
  }),
);
...
it.effect("accepts local model snapshot pushes", () =>
  Effect.gen(function* () {
    const parsed = yield* decode(WsResponse, {
      type: "push",
      sequence: 5,
      channel: WS_CHANNELS.localModelEvent,
      data: {
    ...
    assert.strictEqual(parsed.channel, WS_CHANNELS.localModelEvent);
  }),
);
