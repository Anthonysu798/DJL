...
import { STREAMING_UPLOAD_MAX_FILE_BYTES, STREAMING_UPLOAD_MAX_TOTAL_BYTES } from "./document";
...
const decodeSubscribeDomainEventsInput = Schema.decodeUnknownEffect(
  OrchestrationSubscribeDomainEventsInput,
);

it.effect("decodes explicit confirmed command receipts", () =>
  Effect.gen(function* () {
    const receipt = yield* decodeDispatchResult({
      commandId: "command-mobile-1",
      status: "confirmed",
      sequence: 42,
    });
...
it.effect("accepts an optional reconnect cursor for domain event subscriptions", () =>
  Effect.gen(function* () {
    assert.deepEqual(yield* decodeSubscribeDomainEventsInput({}), {});
    assert.deepEqual(yield* decodeSubscribeDomainEventsInput({ fromSequenceExclusive: 41 }), {
      fromSequenceExclusive: 41,
    });
  }),
);

it.effect("accepts the OpenCode accept-edits runtime mode", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeRuntimeMode("accept-edits");
    assert.equal(parsed, "accept-edits");
  }),
);

it.effect("accepts the OpenCode auto-approval runtime mode", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeRuntimeMode("auto-approval");
    assert.equal(parsed, "auto-approval");
  }),
);
...
it.effect("rejects legacy providers for new turns while historical payloads remain readable", () =>
  Effect.gen(function* () {
    ...
    assert.match(String(error), /DJL/u);
  }),
);
...
it.effect("accepts provider-scoped model options in thread.turn.start", () =>
  Effect.gen(function* () {
    ...
    assert.strictEqual(parsed.modelSelection?.provider, "opencode");
    assert.strictEqual(parsed.modelSelection?.options?.variant, "high");
...
it.effect("accepts immutable streaming attachment references in client Work turns", () =>
  Effect.gen(function* () {
    const parsed = yield* decodeClientOrchestrationCommand({
      type: "thread.turn.start",
      commandId: "cmd-client-streaming-attachment",
      threadId: "thread-1",
      message: {
        messageId: "msg-client-streaming-attachment",
        role: "user",
    ...
    if (!attachment || !("uploadMethod" in attachment)) {
      throw new Error("Expected a streaming attachment reference");
    }
    assert.strictEqual(attachment.uploadMethod, "stream");
  }),
);

it.effect("rejects a client turn whose attachments exceed the total byte budget", () =>
  Effect.gen(function* () {
    const count =
      Math.floor(STREAMING_UPLOAD_MAX_TOTAL_BYTES / STREAMING_UPLOAD_MAX_FILE_BYTES) + 1;
    const failed = yield* decodeClientOrchestrationCommand({
      type: "thread.turn.start",
      commandId: "cmd-client-total-attachment-limit",
      threadId: "thread-1",
      message: {
        messageId: "msg-client-total-attachment-limit",
        role: "user",
