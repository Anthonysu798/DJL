...
import { WorkTask, WorkTaskAction } from "./work";
import {
  STREAMING_UPLOAD_MAX_FILES_PER_TURN,
  STREAMING_UPLOAD_MAX_FILE_BYTES,
  STREAMING_UPLOAD_MAX_TOTAL_BYTES,
...
export const NewTaskProviderStartOptions = ProviderStartOptions.check(
  Schema.makeFilter(
    ...
    { identifier: "NewTaskProviderStartOptions" },
  ),
);
...
export const PROVIDER_SEND_TURN_MAX_ATTACHMENTS = STREAMING_UPLOAD_MAX_FILES_PER_TURN;
export const PROVIDER_SEND_TURN_MAX_IMAGE_BYTES = STREAMING_UPLOAD_MAX_FILE_BYTES;
export const PROVIDER_SEND_TURN_MAX_FILE_BYTES = STREAMING_UPLOAD_MAX_FILE_BYTES;
...
export const ChatImageAttachment = Schema.Struct({
  ...
  contentHash: Schema.optional(AttachmentContentHash),
  uploadMethod: Schema.optional(Schema.Literal("stream")),
});
...
const makeAttachmentTotalBytesCheck = <Attachment extends object>() =>
  Schema.makeFilter<ReadonlyArray<Attachment>>(
    (attachments) =>
      attachments.reduce(
        (total, attachment) =>
          total +
      ...
      new SchemaIssue.InvalidValue(Option.some(attachments), {
        message: `Attachment total exceeds ${STREAMING_UPLOAD_MAX_TOTAL_BYTES} bytes`,
      }),
    { identifier: "AttachmentTotalBytesCheck" },
  );
const ChatAttachmentList = Schema.Array(ChatAttachment).check(
  Schema.isMaxLength(PROVIDER_SEND_TURN_MAX_ATTACHMENTS),
  makeAttachmentTotalBytesCheck<ChatAttachment>(),
);
const UploadChatAttachmentList = Schema.Array(UploadChatAttachment).check(
  Schema.isMaxLength(PROVIDER_SEND_TURN_MAX_ATTACHMENTS),
  makeAttachmentTotalBytesCheck<UploadChatAttachment>(),
);
...
export const OrchestrationThread = Schema.Struct({
  ...
  workTask: Schema.optional(Schema.NullOr(WorkTask)),
...
export const ProjectCreateCommand = Schema.Struct({
  ...
  defaultModelSelection: Schema.optional(Schema.NullOr(NewTaskModelSelection)),
  isPinned: Schema.optional(Schema.Boolean).pipe(Schema.withDecodingDefault(() => false)),
  createdAt: IsoDateTime,
});
...
export const ThreadWorkTaskTransitionCommand = Schema.Struct({
  type: Schema.Literal("thread.work-task.transition"),
  commandId: CommandId,
  threadId: ThreadId,
...
export type ThreadWorkTaskTransitionCommand = typeof ThreadWorkTaskTransitionCommand.Type;
...
const DispatchableClientOrchestrationCommand = Schema.Union([
  ...
  ThreadWorkTaskTransitionCommand,
...
export const ThreadWorkTaskTransitionedPayload = Schema.Struct({
  threadId: ThreadId,
  task: WorkTask,
  action: WorkTaskAction,
});
export type ThreadWorkTaskTransitionedPayload = typeof ThreadWorkTaskTransitionedPayload.Type;
...
export const OrchestrationEvent = Schema.Union([
  ...
  Schema.Struct({
    ...EventBaseFields,
    type: Schema.Literal("thread.work-task-transitioned"),
    payload: ThreadWorkTaskTransitionedPayload,
  }),
]);
...
export const DispatchResult = Schema.Struct({
  // Optional when decoding older servers; current servers always return both.
  commandId: Schema.optional(CommandId),
  status: Schema.optional(Schema.Literal("confirmed")),
  sequence: NonNegativeInt,
});
