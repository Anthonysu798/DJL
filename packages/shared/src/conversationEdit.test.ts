...
describe("conversationEdit", () => {
  it("uses an interrupted or failed latest turn to replay a cancelled tail prompt", () => {
    expect(
      resolveTailReplayTurnId({
        sessionIsRunning: false,
        sessionActiveTurnId: null,
        latestTurn: { turnId: "turn-cancelled", state: "interrupted" },
      }),
    ).toBe("turn-cancelled");
    expect(
      resolveTailReplayTurnId({
  ...
  it("does not treat a completed turn as an active tail replay", () => {
    expect(
      resolveTailReplayTurnId({
        sessionIsRunning: false,
        sessionActiveTurnId: null,
