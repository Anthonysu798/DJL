...
export function resolveTailReplayTurnId<TTurnId extends string>(input: {
  readonly sessionIsRunning: boolean;
  readonly sessionActiveTurnId: TTurnId | null | undefined;
  readonly latestTurn:
    | {
  ...
  if (input.sessionIsRunning && input.sessionActiveTurnId) {
    return input.sessionActiveTurnId;
  }
  if (input.latestTurn?.state === "interrupted" || input.latestTurn?.state === "error") {
