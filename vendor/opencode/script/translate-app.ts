...
const desktopLocales = new Set<Locale>(
  locales.filter((locale) => locale !== "th" && locale !== "tr"),
);
...
export function parseTranslationArgs(args: string[]) {
  ...
  if (!parsed.values.help && parsed.positionals.length !== 1)
    throw new Error("Pass one locale or 'all'.");
...
export function findDrift(source: Dictionary, target: Dictionary) {
  return {
    ...
    placeholders: Object.keys(source).filter(
      (key) =>
        Object.hasOwn(target, key) && tokens(source[key]).join() !== tokens(target[key]).join(),
    ),
  };
}
...
export function modelVariants(output: string, model: string) {
  ...
  if (!isRecord(metadata) || !isRecord(metadata.variants))
...
`);
  ...
  if (incomplete.length)
    console.error(
      `Translation remains incomplete for: ${incomplete.map((plan) => plan.locale).join(", ")}`,
    );
  if (escaped.length)
    console.error(`Translation changed files outside its locale targets: ${escaped.join(", ")}`);
  process.exitCode = 1;
}
...
async function inspect(locale: Locale) {
  const domains = await Promise.all(
    targetFiles(locale).map(async (target) => {
      ...
      return {
        name: target.includes("packages/app/")
          ? "app"
...
async function translate(
  ...
  if (result[2] !== 0)
    return { locale: plan.locale, stdout: result[0], stderr: result[1], code: result[2] };
  ...
  const mismatch =
    observed.length === 0 ||
    observed.some((item) => item.model !== model || item.variant !== variant);
  const actual = Array.from(
    new Set(observed.map((item) => `${item.model} (${item.variant ?? "default"})`)),
  ).join(", ");
  return {
    locale: plan.locale,
...
async function resolveModelVariant(model: string, variant: string) {
  ...
  if (!provider || !model.includes("/"))
    throw new Error(`Model must use provider/model syntax: ${model}`);
  const env = isolatedEnvironment();
  env.OPENCODE_DISABLE_PROJECT_CONFIG = "1";
  ...
  if (!Object.hasOwn(variants, variant))
    throw new Error(`Variant '${variant}' is not configured for ${model}.`);
