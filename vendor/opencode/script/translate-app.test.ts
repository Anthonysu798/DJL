...
`;
  ...
  test("disables side effects and scopes edits for the translation agent", () => {
    const config = translationConfig("translate-app-fr", "opencode/gpt-5.5", [
      "packages/app/src/i18n/fr.ts",
    ]);
  ...
  test("detects edits outside the locale targets", () => {
    ...
    expect(
      unexpectedChanges({ "already-dirty.ts": "before" }, { "already-dirty.ts": "after" }, []),
    ).toEqual(["already-dirty.ts"]);
  });
});
