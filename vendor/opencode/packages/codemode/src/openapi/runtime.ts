  ...
  Effect.gen(function* () {
    ...
    const response = yield* client
      ...
      .pipe(
        Effect.catch((cause) =>
          Effect.fail(
            toolError(
              `${plan.operation.method} ${plan.operation.path} failed: transport error`,
              cause,
            ),
          ),
        ),
    ...
    if (response.status < 200 || response.status >= 300) {
      ...
      return yield* Effect.fail(
        toolError(
          `${plan.operation.method} ${plan.operation.path} failed with HTTP ${response.status}: ${summary}`,
        ),
      );
    }
...
const buildRequest = (
  ...
  Effect.gen(function* () {
    ...
    const missing = plan.fields.find(
      (field) =>
        field.required && field.location !== "path" && own(input, field.inputName) === undefined,
    );
    ...
    const setBody = (value: unknown, mediaType: string) =>
      HttpClientRequest.bodyJson(request, value).pipe(
        ...
        Effect.mapError((cause) =>
          toolError(
            `Invalid JSON body for ${plan.operation.method} ${plan.operation.path}.`,
            cause,
          ),
        ),
      );
...
const serializeQuery = (
  ...
  if (field.style === "deepObject") {
    ...
    return Object.entries(value).reduce<HttpClientRequest.HttpClientRequest | ToolError>(
      (current, [name, item]) => {
        if (current instanceof ToolError) return current;
        if (item === undefined || (item !== null && typeof item === "object")) {
          return toolError(
            `Deep-object parameter '${field.inputName}' contains an unsupported nested value.`,
          );
        }
  ...
  if (Array.isArray(value)) {
    ...
    if (value.some((item) => item === undefined || (item !== null && typeof item === "object"))) {
      return toolError(
        `Query parameter '${field.inputName}' contains an unsupported nested value.`,
      );
    }
    return value.reduce(
  ...
  const rendered = serializeSimple(field, value, String);
  return rendered instanceof ToolError
    ? rendered
    : HttpClientRequest.appendUrlParam(request, field.name, rendered);
};
...
const readResponseBody = (
  ...
  Effect.gen(function* () {
    ...
    if (declaredSize !== undefined && declaredSize > maxResponseBodyBytes) {
      return yield* Effect.fail(
        toolError(`${plan.operation.method} ${plan.operation.path} response exceeds 50 MiB.`),
      );
    }
    ...
    yield* Stream.runForEach(response.stream, (chunk) => {
      ...
      Effect.catch((cause) => {
        ...
        return Effect.fail(
          toolError(
            `${plan.operation.method} ${plan.operation.path} failed while reading the response body.`,
            cause,
          ),
        );
      }),
