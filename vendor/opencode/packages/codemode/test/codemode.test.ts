...
describe("CodeMode public contract", () => {
  ...
  test("keeps one-shot and reusable execution equivalent", async () => {
    ...
    expect(
      Schema.decodeUnknownSync(CodeMode.Result)(JSON.parse(JSON.stringify(reusable))),
    ).toStrictEqual(reusable);
  });
  ...
  test("empty query lists everything alphabetically by path", async () => {
    ...
    if (exhausted.ok)
      expect(exhausted.value).toStrictEqual({ items: [], remaining: 0, next: null });
  });
