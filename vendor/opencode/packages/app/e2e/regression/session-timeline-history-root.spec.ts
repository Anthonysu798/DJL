...
for (const scenario of scenarios) {
  test(`keeps the latest user turn visible through ${scenario.name}`, async ({ page }) => {
        ...
        const sample = () => {
          if (state.armed) {
            ...
            const visible = (partID: string) => {
              const part = viewport?.querySelector<HTMLElement>(
    ...
    expect(
      await page.evaluate(() => {
        const state = (
          window as Window & { __historyRootProbe?: { hidden: boolean; stop: boolean } }
        ).__historyRootProbe!;
        state.stop = true;
        return state.hidden;
      }),
...
async function waitForProbeSamples(page: Page, after: number) {
  await page.waitForFunction(
    (after) =>
      (window as Window & { __historyRootProbe?: { samples: number } }).__historyRootProbe!
        .samples >=
      after + 3,
    after,
  );
