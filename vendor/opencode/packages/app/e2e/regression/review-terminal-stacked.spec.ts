...
test("keeps the review tree and terminal sized when both panels are open", async ({ page }) => {
  ...
  const treeViewport = page.locator(
    '#review-panel [data-slot="session-review-v2-sidebar-tree"] .scroll-view__viewport',
  );
  await treeViewport.hover();
  await page.mouse.wheel(0, 100_000);
  await expect
    .poll(() =>
      treeViewport.evaluate(
  ...
  const bottomGap = await lastFile.evaluate((element) => {
    const viewport = element
      .closest<HTMLElement>(".scroll-view__viewport")!
  ...
  const lazyDiff = page.waitForRequest((request) => {
    ...
    return (
      ...
      url.searchParams.get("directory")?.replaceAll("\\", "/").endsWith("/src/branch/d00027") ===
        true
    );
  });
...
async function expectMountedTree(page: Page, total: number) {
  ...
  await expect
    .poll(() =>
      tree.evaluate((element) => element.querySelectorAll('[data-slot="file-tree-v2-row"]').length),
    )
    .toBeGreaterThan(0);
  const state = await tree.evaluate((element) => ({
    root: element.getBoundingClientRect().height,
    viewport: element.closest<HTMLElement>(".scroll-view__viewport")!.getBoundingClientRect()
      .height,
