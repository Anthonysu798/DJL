...
async function installSessionSwitchProbe(
  ...
  await page.evaluate(
    ({ destinationIDs, sourceIDs, lastID, requiredPartID, requireBottomAnchor, href }) => {
      ...
      const sample = () => {
        ...
        setTimeout(() => {
          ...
          const reviewFile = reviewPanel?.querySelector(
          ...
          if (root) {
            ...
            const requiredPartVisible = requiredPartID
              ? [...root.querySelectorAll<HTMLElement>("[data-timeline-part-id]")].some(
                  (element) => {
      ...
      return (
        ...
        stable.every(
          (sample) =>
            ...
            (sample.bottomAnchorRequired === false ||
...
export async function measureSessionSwitch(
  ...
  try {
    ...
    await page.evaluate(() => {
      (
        window as Window & { __sessionSwitchProbe?: SessionSwitchProbe }
      ).__sessionSwitchProbe?.stop();
    });
  }
}
