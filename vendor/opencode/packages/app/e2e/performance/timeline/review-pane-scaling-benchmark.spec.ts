  ...
  for (const fileCount of fileCounts) {
    ...
    benchmark(
      ...
      async ({ page, report }) => {
        ...
        const patchByteLimit = Number(
          process.env.REVIEW_PANE_PATCH_BYTE_LIMIT ?? Number.POSITIVE_INFINITY,
        );
        if (Number.isNaN(patchByteLimit) || patchByteLimit < 0)
          throw new Error(
            `Invalid REVIEW_PANE_PATCH_BYTE_LIMIT: ${process.env.REVIEW_PANE_PATCH_BYTE_LIMIT}`,
          );
...
async function measureBroadReviewSearch(page: Page, expectedRows: number) {
  ...
  await filter.evaluate((element) => {
    element.addEventListener(
      "input",
      () => {
        (window as Window & { __reviewSearchStartedAt?: number }).__reviewSearchStartedAt =
          performance.now();
      },
  ...
  return page.evaluate((expectedRows) => {
    const startedAt = (window as Window & { __reviewSearchStartedAt?: number })
      .__reviewSearchStartedAt!;
    return new Promise<{ stableMs: number; logicalRows: number; renderedRows: number }>(
      (resolve) => {
        let previous = -1;
        let streak = 0;
        const sample = () => {
          const tree = document.querySelector<HTMLElement>(
            '#review-panel [data-component="file-tree-v2"]',
          );
          const rows = [
            ...document.querySelectorAll<HTMLElement>(
              '#review-panel [data-slot="file-tree-v2-row"]',
            ),
          ];
          ...
          const ready =
            ...
            rows.length > 0 &&
            rows.every((row) => row.textContent?.includes("file-"));
          streak = ready && rows.length === previous ? streak + 1 : ready ? 1 : 0;
          previous = rows.length;
          if (streak >= 3) {
            resolve({
              stableMs: performance.now() - startedAt,
              logicalRows,
...
function createScalingDiffs(fileCount: number, patchByteLimit: number) {
  ...
  return Array.from({ length: fileCount }, (_, index) => {
    ...
    if (index === 0 && fullPatch.length > patchByteLimit)
      throw new Error(
        `REVIEW_PANE_PATCH_BYTE_LIMIT must include the active patch (${fullPatch.length} bytes)`,
      );
    const patch =
      !capped && patchBytes + fullPatch.length <= patchByteLimit
        ? fullPatch
        : emptyReviewPatch(file);
...
async function measureReviewPaneLoad(
  page: Page,
  input: { expectedFile: string; expectedRows: number },
) {
  ...
  await page.waitForFunction(
    () =>
      (window as Window & { __reviewPaneScalingProbe?: ReviewPaneScalingProbe })
        .__reviewPaneScalingProbe?.stableReadyMs !== undefined,
    undefined,
    { timeout: completionTimeoutMs },
  );

  return page.evaluate(() => {
    const probe = (window as Window & { __reviewPaneScalingProbe?: ReviewPaneScalingProbe })
      .__reviewPaneScalingProbe!;
    probe.stop();
    ...
    const longTasks = probe.longTasks.filter(
      (entry) =>
        entry.startTime >= startedAt && entry.startTime <= startedAt + probe.stableReadyMs!,
    );
    const frameGaps = probe.frameTimesMs.map(
      (time, index) => time - (probe.frameTimesMs[index - 1] ?? 0),
    );
    ...
    return {
      ...
      responseToStableMs: resource
        ? probe.stableReadyMs! - (resource.responseEnd - startedAt)
        : null,
...
async function installReviewPaneScalingProbe(
  page: Page,
  input: { expectedFile: string; expectedRows: number },
) {
  await page.evaluate(
    ({ expectedFile, expectedRows, stableFrames }) => {
      ...
      const sample = (time: number) => {
        ...
        const diffLines = viewers.reduce(
          (sum, viewer) =>
            sum +
            (viewer.querySelector("diffs-container")?.shadowRoot?.querySelectorAll("[data-line]")
              .length ?? 0),
          0,
        );
        ...
        if (probe.firstTreeRowMs === undefined && rows.length > 0)
          probe.firstTreeRowMs = observedAtMs;
        if (probe.logicalTreeReadyMs === undefined && logicalRows === expectedRows)
          probe.logicalTreeReadyMs = observedAtMs;
        if (probe.firstDiffRenderMs === undefined && diffLines > 0)
          probe.firstDiffRenderMs = observedAtMs;
      ...
      (
        window as Window & { __reviewPaneScalingProbe?: ReviewPaneScalingProbe }
      ).__reviewPaneScalingProbe = probe;
    },
