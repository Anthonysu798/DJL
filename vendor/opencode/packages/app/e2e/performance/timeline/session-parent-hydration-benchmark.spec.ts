...
if (mode !== "natural" && mode !== "candidate")
  throw new Error(`Unknown parent hydration benchmark mode: ${mode}`);
const userID = "msg_parent_hydration_user";
const user = {
  ...fixture.messages[fixture.targetID][0]!,
  info: {
    ...fixture.messages[fixture.targetID][0]!.info,
    id: userID,
...
benchmark(
  "hydrates an orphaned latest turn after a cold session click",
  async ({ browser, report }, testInfo) => {
    ...
    const timing = results
      .map((result) => result.metrics.firstCorrectObservedMs!)
      .sort((a, b) => a - b);
    report(
      {
        results: results.map((result) => ({
          ...result.metrics,
          historyGateCount: result.historyGateCount,
        })),
        summary: {
...
async function trial(page: Page, mode: ParentHydrationBenchmarkMode) {
  ...
  await mockOpenCodeServer(page, {
    ...
    beforeMessagesResponse: (request) => {
      if (mode !== "candidate" || request.sessionID !== fixture.targetID || !request.before)
        return Promise.resolve();
      historyGates++;
      return history!.promise;
    },
    onMessage: (request) => {
      if (request.sessionID === fixture.targetID && request.messageID === userID)
    ...
    pageMessages: (sessionID, limit, before) => {
      ...
      return {
        ...
        cursor: start > 0 ? items[start]!.info.id : undefined,
      };
    },
  });
