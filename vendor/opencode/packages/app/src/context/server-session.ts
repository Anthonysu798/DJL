...
export function createServerSession(client: OpencodeClient, options?: { retry?: typeof retry }) {
  ...
  const resetMessageLoad = (
    sessionID: string,
    load: MessageLoadState,
    baseline?: MessageLoadBaseline,
  ) => {
  ...
  const loadMessages = async (
    ...
    try {
      ...
      if (mode !== "prepend") {
        const users = new Set([
          ...
          ...(data.message[sessionID] ?? [])
            .filter((message) => {
              ...
              return (
                load.touchedMessages.has(message.id) && (!item || item.confirmedMessage === true)
              );
            })
        ...
        const parentIDs = [
          ...new Set(
            page.session.flatMap((message) =>
              message.role === "assistant" && !users.has(message.parentID)
                ? [message.parentID]
                : [],
            ),
          ),
        ...
        for (const parentID of parentIDs) {
          ...
          if (parent.message.role !== "user")
            throw new Error(`Assistant parent is not a user message: ${parentID}`);
          parents.push(parent);
        }
      }
      if (generations.get(sessionID) !== active) return;
      ...
      const preserveUnfetched =
        mode === "prepend" ||
        (!result.complete && (!first || ((message: Message) => cmpMessage(message, first) < 0)));
      applyMessagePage(
        sessionID,
        result,
