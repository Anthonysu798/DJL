...
const btn = (accent?: boolean) =>
  ({
    ...
    background: accent
      ? "var(--v2-background-bg-contrast, #242424)"
      : "var(--v2-background-bg-base, #fff)",
...
function Stage(props: { count: number }) {
  ...
  const seed = () =>
    messages.slice(0, props.count).map((text, index) => ({ id: `rolled-${index}`, text }));
    ...
    <div style={{ display: "grid", gap: "16px", "max-width": "720px" }}>
      ...
      <div class="text-[12px] text-v2-text-text-faint">
        ...
        <For each={seed()}>
          {(item) => (
            <span>
              {store.items.some((current) => current.id === item.id) ? "" : `“${item.text}” `}
            </span>
          )}
        </For>
      </div>
