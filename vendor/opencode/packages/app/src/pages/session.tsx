...
export default function Page() {
  ...
  const desktopV2ReviewOpen = createMemo(
    () => newSessionDesign() && desktopReviewOpen() && !!params.id,
  );
  ...
  const desktopSidePanelOpen = createMemo(
    () => desktopSessionResizeOpen() || desktopFileTreeOpen(),
  );
  ...
  const loadReviewDiff = async (
    ...
    try {
      ...
      console.debug("[session-review] failed to load bounded vcs diff", {
        mode,
        file,
        root,
        error,
