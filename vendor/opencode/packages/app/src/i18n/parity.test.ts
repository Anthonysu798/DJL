...
describe.skipIf(!!process.env.CI)("i18n parity", () => {
  ...
  test("non-English locales preserve English placeholders", async () => {
    for (const domain of domains) {
      ...
      for (const locale of domain.locales) {
        ...
        expect({ domain: domain.name, locale, mismatched }).toEqual({
          domain: domain.name,
          locale,
          mismatched: [],
        });
  ...
  test("changed-file summary keys preserve rendered English copy and localize complete phrases", async () => {
    ...
    expect(source["ui.sessionTurn.diffs.changed.one"].replace("{{count}}", "1")).toBe(
      "1 Changed file",
    );
    expect(source["ui.sessionTurn.diffs.changed.other"].replace("{{count}}", "2")).toBe(
      "2 Changed files",
    );
