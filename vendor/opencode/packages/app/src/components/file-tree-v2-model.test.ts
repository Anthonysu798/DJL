...
describe("file tree v2 model", () => {
  test("builds sorted depth-first rows", () => {
    const model = buildFileTreeV2Model([
      "src/z.ts",
      "src/lib/b.ts",
      "src/lib/a.ts",
      "README.md",
    ...
    expect(
      flattenFileTreeV2(model, () => true).map((row) => [row.node.path, row.node.type, row.level]),
    ).toEqual([
      ["docs", "directory", 0],
  ...
  test("omits descendants of collapsed directories", () => {
    ...
    expect(
      flattenFileTreeV2(model, (path) => path !== "src/lib").map((row) => row.node.path),
    ).toEqual(["src", "src/lib", "src/z.ts"]);
  });
  ...
  test("normalizes separators and duplicate paths", () => {
    ...
    expect(rows.find((row) => row.node.path === "src/lib/a.ts")?.node.originalPath).toBe(
      "src\\lib\\a.ts",
    );
  });
