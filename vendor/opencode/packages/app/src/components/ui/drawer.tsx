...
const DrawerHeader: Component<ComponentProps<"div">> = (props) => {
  ...
  return (
    <div
      class={props.class}
      classList={{ "grid gap-1.5 p-4 text-center sm:text-left": true }}
      {...rest}
    />
  );
};
...
const DrawerFooter: Component<ComponentProps<"div">> = (props) => {
  const [, rest] = splitProps(props, ["class"]);
  return (
    <div class={props.class} classList={{ "mt-auto flex flex-col gap-2 p-4": true }} {...rest} />
  );
};
...
const DrawerTitle = <T extends ValidComponent = "div">(
  ...
  return (
    <DrawerPrimitive.Label
      class={props.class}
      classList={{
        "text-base font-[530] leading-none tracking-[-0.04px] text-v2-text-text-base": true,
      }}
      {...rest}
    />
  );
