...
export const DialogSelectModelUnpaidV2: Component<{ model?: ModelState }> = (props) => {
  ...
  const modelKey = (item: ReturnType<ModelState["list"]>[number]) =>
  ...
  const openProviders = (provider?: string) => {
    void import("./dialog-connect-provider").then((x) => {
      ...
      void dialog.show(() => (
        <x.DialogConnectProvider controller={controller} directory={directory} />
      ));
    });
  };
              ...
              <For each={model.list()}>
                {(item) => (
                  <TooltipV2
                    ...
                    openDelay={0}
                    value={
                      <ModelTooltip model={item} latest={item.latest} free={isFree(item)} v2 />
                    }
                  >
                    <button
                      type="button"
                  ...
                  >
                    {(provider) => (
                      ...
                      >
                        <ProviderIcon
                          id={provider.id}
                          class="size-4 shrink-0 text-v2-icon-icon-muted"
                        />
