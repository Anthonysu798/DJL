...
async function listGeoDaily(opts?: {
  provider?: string;
  model?: string;
}): Promise<GeoStatMetric[]> {
  const scope =
...
export const getStatsModelComparisonData: (
  ...
  secondModel: string,
) => Effect.Effect<StatsModelComparisonData, DatabaseError, ModelStatRepo> = Effect.fn(
  "StatsModelComparison.getData",
