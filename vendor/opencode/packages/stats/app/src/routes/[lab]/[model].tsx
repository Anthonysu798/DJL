...
function ModelOverview(props: { catalog: ModelCatalogEntry | null }) {
  ...
  const specs = createMemo(() => [
    ...
    },
    {
      ...
      value: formatCatalogMonth(
        props.catalog?.knowledge,
          ...
          <div data-slot="model-trend-plot">
                      ...
                      >
                        <defs>
                          <linearGradient id={areaGradientId} x1="0%" y1="0%" x2="0%" y2="100%">
                            <stop
                              offset="0%"
                              stop-color="var(--model-trend-active)"
                              stop-opacity="0.1"
                            />
                            <stop
                              offset="66%"
                  ...
                  <Show when={activeLineClip()}>
                    {(clip) => (
                      ...
                      >
                        ...
                        <path
                          data-slot="model-trend-line-active"
                          d={path()}
              ...
              <For each={usage()}>
                {(point, index) => (
                  <div
                    ...
                    data-muted={
                      highlightBars() && activeIndex() !== undefined && activeIndex() !== index()
                        ? "true"
                        : undefined
                    }
                    style={
                      {
                        "--model-trend-token-height": modelUsageStripHeight(
                          props.value(point),
                          valueMax(),
                        ),
                      } as JSX.CSSProperties
            ...
            <Show when={activeTooltip()} keyed>
              {(active) => (
                ...
                >
                  <strong>
                    {formatModelUsageTooltipDate(active.point.date, props.data?.updatedAt ?? null)}
                  </strong>
                  <span>
        ...
        {(data) => (
          <>
            ...
            <div data-component="model-efficiency-grid">
              <MetricCard
                label={i18n.t("model.totalSpendLabel")}
...
function PeerRow(props: { peer: ModelPeerEntry; active: boolean }) {
  ...
  return (
    <li>
      ...
      >
        <span
          data-slot="model-peer-rank"
          aria-label={props.active ? `Rank ${props.peer.rank}` : undefined}
        >
...
function modelComparisonPairs(
  ...
  const catalogPairs = (
    catalogEntry && catalog
      ? (catalog.labs.find((lab) => lab.id === catalogEntry.lab)?.models ?? [])
      : []
  )
...
function momentumChart(data: ModelUsagePoint[], updatedAt: string | null) {
  ...
  const points =
    data.length > 1
      ? data
      : [data[0] ?? emptyUsagePoint(fallbackDate), data[0] ?? emptyUsagePoint(fallbackDate)];
...
function smoothLinePath(points: { x: number; y: number }[]) {
  ...
  if (points.length === 1)
    return `M${formatSparklinePoint(points[0].x)} ${formatSparklinePoint(points[0].y)}`;
  return points
    .map((point, index) => {
...
function parseMomentumDate(date: string, updatedAt: string | null): Date {
  ...
  const month = label
    ? shortMonths.findIndex((item) => item.toLowerCase() === label[1].toLowerCase())
    : -1;
  ...
  const year =
    month > anchor.getUTCMonth() + 1 ? anchor.getUTCFullYear() - 1 : anchor.getUTCFullYear();
...
function modelUsageLinePath(points: ModelUsageLinePoint[]) {
  if (points.length === 0) return "";
  if (points.length === 1)
    return `M ${formatUsagePathNumber(points[0].x)} ${formatUsagePathNumber(points[0].y)}`;
