          ...
          <Show when={categories().length > 0}>
            <section id="model-comparison" data-section="compare-home-related">
              ...
              <div data-component="compare-home-card-grid">
                <For each={categories()}>
                  {(category) => <CompareHomeCard category={category} />}
                </For>
              </div>
            </section>
          </Show>
...
function CompareModelSelectModal(props: {
  ...
  const availableModels = createMemo(() =>
    props.models.filter((model) => model.id !== props.blockedId),
  );
  const filteredModels = createMemo(() => {
    const terms = search().trim().toLowerCase().split(/\s+/).filter(Boolean);
    if (terms.length === 0) return availableModels();
    return availableModels().filter((model) =>
      terms.every((term) => modelSearchText(model).includes(term)),
    );
  });
  ...
  createEffect(() => {
    ...
    const fallback =
      selected && selected.id !== props.blockedId ? selected.id : filteredModels()[0]?.id;
    if (!fallback) return;
      ...
      <dl data-slot="compare-model-modal-facts">
        ...
        <div data-slot="compare-model-modal-fact">
          ...
          <dd data-slot="compare-model-modal-uptime">
            <For each={uptimeBars}>
              {(bar) => <span data-active={bar < 7 ? "true" : undefined} />}
            </For>
          </dd>
        </div>
        <div data-slot="compare-model-modal-fact">
...
function categoryCandidates(
  kind: (typeof categoryTemplates)[number]["kind"],
  models: ModelCatalogEntry[],
) {
  if (kind === "affordable")
    return models
  ...
  if (kind === "image")
    return models
      .filter((model) =>
        model.modalities.output.some((modality) => modality.toLowerCase().includes("image")),
      )
...
function recentModelSort(a: ModelCatalogEntry, b: ModelCatalogEntry) {
  return (
    displayDateTime(b.releaseDate) - displayDateTime(a.releaseDate) || a.name.localeCompare(b.name)
  );
}
...
function modelCost(model: ModelCatalogEntry) {
  return (
    (model.cost?.input ?? Number.POSITIVE_INFINITY) +
    (model.cost?.output ?? Number.POSITIVE_INFINITY)
  );
}
