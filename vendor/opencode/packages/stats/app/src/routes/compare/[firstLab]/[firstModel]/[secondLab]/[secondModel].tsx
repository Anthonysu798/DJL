...
const getComparisonData = query(
  async (firstLab: string, firstModel: string, secondLab: string, secondModel: string) => {
    ...
    return runtime.runPromise(
      getStatsModelComparisonData(firstLab, firstModel, secondLab, secondModel),
    );
  },
  "getStatsModelComparisonData",
);
...
export default function ModelComparePair() {
  ...
  const firstCatalog = createMemo(() =>
    resolvedCatalogEntry(catalog(), firstLabParam(), firstModelParam()),
  );
  const secondCatalog = createMemo(() =>
    resolvedCatalogEntry(catalog(), secondLabParam(), secondModelParam()),
  );
  ...
  const canonicalUrl = createMemo(() =>
    new URL(
      canonicalPath(),
      event?.request.url ??
        (typeof window === "undefined" ? compareFallbackUrl : window.location.href),
    ).toString(),
  );
...
function ComparisonTable(props: {
  models: readonly [ComparisonModel, ComparisonModel];
  rows: ComparisonRow[];
}) {
  return (
...
function buildComparisonRows(first: ComparisonModel, second: ComparisonModel): ComparisonRow[] {
  return [
    ...
    comparisonRow(
      ...
      {
        value: first.stats ? formatPercent(first.stats.tokenShare) : "No usage",
        score: first.stats?.tokenShare,
      },
      {
        value: second.stats ? formatPercent(second.stats.tokenShare) : "No usage",
        score: second.stats?.tokenShare,
      },
      "higher",
    ),
    comparisonRow(
      ...
      "Recent token volume.",
      {
        value: first.stats ? formatTokens(first.stats.totals.tokens) : "No usage",
        score: first.stats?.totals.tokens,
      },
      {
    ...
    ...alternatives.slice(0, 3).flatMap((model, index) => [
      {
        first: current[0],
        second: model,
        detail: index === 0 ? "Nearby alternative" : "Related comparison",
      },
      {
        first: current[1],
        second: model,
