...
export function ComparisonSelector(props: {
  ...
  const [secondId, setSecondId] = createSignal(
    differentModelId(props.models, firstId(), props.secondId),
  );
  ...
  createEffect(() => {
    ...
    const next =
      props.firstId && modelById().has(props.firstId) ? props.firstId : props.models[0]?.id;
  ...
  return (
    ...
    >
      <label>
        ...
        <select
          value={firstId()}
          onInput={(event) => setFirstId(event.currentTarget.value)}
          required
        >
      ...
      <label>
        ...
        <select
          value={secondId()}
          onInput={(event) => setSecondId(event.currentTarget.value)}
          required
        >
          <option value="" disabled>
            Select model
...
function differentModelId(
  ...
  if (preferredId && preferredId !== firstId && models.some((model) => model.id === preferredId))
    return preferredId;
