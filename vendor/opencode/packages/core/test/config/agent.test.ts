...
const it = testEffect(
  AppNodeBuilder.build(LayerNode.group([AgentV2.node, FSUtil.node, Global.node])),
);
...
describe("ConfigAgentPlugin.Plugin", () => {
  it.effect("matches POSIX paths against home-relative permissions", () =>
    Effect.gen(function* () {
      ...
      expect(
        PermissionV2.evaluate("external_directory", "/home/test/p/opencode/src/*", permissions)
          .effect,
      ).toBe("allow");
      expect(
        PermissionV2.evaluate("external_directory", "/home/test/cache/files/*", permissions).effect,
      ).toBe("deny");
      expect(PermissionV2.evaluate("external_directory", "/some/~/path", permissions).effect).toBe(
        "deny",
      );
      expect(
        PermissionV2.evaluate("external_directory", "$HOMELESS/private/*", permissions).effect,
      ).toBe("deny");
    ...
    Effect.gen(function* () {
      ...
      expect(
        PermissionV2.evaluate(
          "external_directory",
          "C:\\Users\\test\\p\\opencode\\src\\*",
          permissions,
        ).effect,
      ).toBe("allow");
      expect(
        PermissionV2.evaluate("external_directory", "C:\\Users\\test\\cache\\files\\*", permissions)
          .effect,
      ).toBe("deny");
    }),
  );
