...
// DJL runs OpenCode as an embedded, isolated backend. In managed mode only credentials written
// through OpenCode's auth API may connect a provider; host env/config secrets must stay inert.
const DJL_MANAGED_AUTH = process.env.DJL_MANAGED_AUTH === "1";
      ...
      Effect.gen(function* () {
        ...
        for (const [providerID, provider] of configProviders) {
          ...
          const parsed: Info = {
            ...
            options: DJL_MANAGED_AUTH
              ? omit(mergeDeep(existing?.options ?? {}, provider.options ?? {}), [
                  "apiKey",
                  "token",
                  "accessToken",
                  "secretAccessKey",
        ...
        // load env (disabled for DJL's managed runtime so host credentials cannot leak in)
        if (!DJL_MANAGED_AUTH) {
        ...
        for (const [id, provider] of configProviders) {
          ...
          if (provider.options) {
            partial.options = DJL_MANAGED_AUTH
              ? omit(provider.options, ["apiKey", "token", "accessToken", "secretAccessKey"])
              : provider.options;
          }
