...
const addServer = Effect.fnUntraced(function* (
  name: string,
  url: string,
...
mcpTest.instance("browser launch receives the discovered authorization URL", () =>
  Effect.gen(function* () {
    ...
    const status = yield* awaitWithTimeout(
      mcp.authenticate("test-oauth-server-3", (url) =>
    ...
    expect(
      server.requests.some(
        (request) =>
          request.pathname === "/mcp" && request.headers.get("x-custom-header") === "custom-value",
      ),
    ).toBe(true);
  }),
);
