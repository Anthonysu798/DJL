...
const stopOAuthCallback = Effect.addFinalizer(() =>
...
mcpTest.instance("first connect to OAuth server shows needs_auth instead of failed", () =>
  Effect.gen(function* () {
    ...
    expect((result.status as Record<string, { status: string }>)["test-oauth"]).toEqual({
      status: "needs_auth",
    });
  }),
);
...
mcpTest.instance(
  "pending provider does not expose or overwrite existing credentials before commit",
  () =>
    Effect.gen(function* () {
      const auth = yield* McpAuth.Service;
