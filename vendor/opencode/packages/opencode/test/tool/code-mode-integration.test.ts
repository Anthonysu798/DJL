...
async function buildTool() {
  ...
  return {
    tool: await Effect.runPromise(
      CodeModeTool.pipe(Effect.flatMap(Tool.init), Effect.provide(layer)),
    ),
...
describe("code mode integration (real MCP server)", () => {
  ...
  test("runs calls in parallel and accumulates every attachment", async () => {
    ...
    expect(out.metadata.toolCalls.map((c) => c.tool)).toEqual([
      "fixtures.screenshot",
      "fixtures.screenshot",
    ]);
  });
  ...
  test("propagates an MCP isError into the program as a catchable error", async () => {
    const out = await run(
      "try { await tools.fixtures.boom({}) } catch (e) { return 'caught: ' + e.message }",
    );
  ...
  test("asks permission for each MCP call, keyed by the flat catalog name", async () => {
    ...
    const permCtx: Tool.Context = {
      ...ctx,
      ask: (req: any) => Effect.sync(() => void asked.push(req.permission)),
    };
  ...
  test("streams running/completed metadata for child calls over a real transport", async () => {
    ...
    await Effect.runPromise(
      tool.execute(
        { code: "await tools.fixtures.add({ a: 1, b: 2 }); return 'done'" },
        recordingCtx,
      ),
    );
