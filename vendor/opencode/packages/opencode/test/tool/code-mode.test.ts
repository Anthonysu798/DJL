...
function harness(input: {
  ...
  return Layer.mergeAll(
    Layer.mock(Plugin.Service, {
      trigger:
        input.trigger ??
        (((_name, _input, output) => Effect.succeed(output)) as Plugin.Interface["trigger"]),
    }),
    ...
    Layer.mock(MCP.Service, {
      ...
      clients: () =>
        Effect.succeed(Object.fromEntries(input.servers.map((name) => [name, {} as any]))),
    }),
  );
}
...
describe("code mode execute", () => {
  ...
  test("groups multi-underscore server names by longest matching prefix", () => {
    const description = describeFor({ my_server_do_thing: mcpTool("do_thing", () => "") }, [
      "my_server",
    ]);
  ...
  test("large catalogs inline a budgeted PARTIAL list plus runtime search", async () => {
    ...
    const out = await Effect.runPromise(
      tool.execute(
        {
          code: "return await tools.$codemode.search({ query: 'only tool', limit: 3, offset: 0 })",
        },
        ctx,
      ),
    );
    ...
    expect(out.metadata.toolCalls).toEqual([
      {
        tool: "$codemode.search",
        status: "completed",
        input: { query: "only tool", limit: 3, offset: 0 },
      },
    ]);
  });
  ...
  test("Object.keys(tools) enumerates the MCP server and CodeMode namespaces", async () => {
    ...
    expect(JSON.parse(output.output)).toEqual({
      namespaces: ["github", "linear", "$codemode"],
      count: 3,
    });
  });
  ...
  test("calls a namespaced MCP tool and flows its text result back into the program", async () => {
    ...
    const output = await Effect.runPromise(
      tool.execute(
        { code: "const r = await tools.greeter.hello({ name: 'world' }); return r.toUpperCase()" },
        ctx,
      ),
    );
  ...
  test("reports an unknown tool as a failed execution", async () => {
    ...
    const error = await failure(
      tool.execute({ code: "return await tools.known.missing({})" }, ctx),
    );
  ...
  test("asks permission before each child tool call", async () => {
    ...
    await Effect.runPromise(
      tool.execute(
        { code: "await tools.a.tool({}); await tools.b.tool({}); return 'done'" },
        permissionCtx,
      ),
    );
  ...
  test("a denied permission fails the child call with a catchable message, not the whole execute", async () => {
    const denyCtx: Tool.Context = {
      ...ctx,
      ask: () => Effect.die(new Error("permission denied by user")),
    };
    const called: string[] = [];
    const tool = await build({
      a_tool: mcpTool("a", () => {
    ...
    const output = await Effect.runPromise(
      tool.execute(
        { code: "try { await tools.a.tool({}) } catch (e) { return 'denied: ' + e.message }" },
        denyCtx,
      ),
    );
  ...
  test("child calls fire plugin tool.execute hooks with the MCP key and synthetic parent/N call ids", async () => {
    ...
    const out = await Effect.runPromise(
      tool.execute(
        { code: "await tools.a.tool({ x: 1 }); await tools.b.tool({}); return 'done'" },
        ctx,
      ),
    );
  ...
  test("a failing before hook fails only that child call as a catchable in-program error", async () => {
    const trigger = ((name: unknown, input: any, output: unknown) => {
      if (name === "tool.execute.before" && input.tool === "a_tool")
  ...
  test("streams live per-call metadata as a call starts and finishes", async () => {
    ...
    await Effect.runPromise(
      tool.execute(
        { code: "await tools.greeter.hello({ name: 'Ada' }); return 'done'" },
        recordingCtx,
      ),
    );
  ...
  test("accumulates stripped media as execute attachments the sandbox never sees", async () => {
    ...
    const out = await Effect.runPromise(
      tool.execute({ code: "return await tools.shot.take({})" }, ctx),
    );
    expect(JSON.parse(out.output)).toEqual({ name: "shot.png" });
    expect(out.attachments).toEqual([
      { type: "file", mime: "image/png", url: "data:image/png;base64,PNGDATA" },
    ]);
  ...
  test("media-only markers distinguish all-image from mixed attachments", async () => {
    const tool = await build({
      ...
      media_mixed: mcpTool("mixed", () => ({
        content: [
          ...
          {
            type: "resource",
            resource: { uri: "file:///tmp/report.pdf", mimeType: "application/pdf", blob: "PDF1" },
          },
        ],
      })),
    });
    ...
    expect(out.attachments).toEqual([
      ...
      { type: "file", mime: "image/png", url: "data:image/png;base64,PNG3" },
      {
        ...
        url: "data:application/pdf;base64,PDF1",
        filename: "report.pdf",
      },
    ]);
  });
  ...
  test("resource links flow to the program as text, never as attachments", async () => {
    ...
    const out = await Effect.runPromise(
      tool.execute({ code: "return await tools.docs.find({})" }, ctx),
    );
  ...
  test("a pre-aborted signal cancels before the program runs", async () => {
    ...
    const output = await Effect.runPromise(
      tool.execute(
        { code: "return await tools.host.touch({})" },
        { ...ctx, abort: controller.signal },
      ),
    );
    expect(output.output).toBe("Execution cancelled.");
  ...
  test("appends logs after the result on success and after the message on error", async () => {
    ...
    const ok = await Effect.runPromise(
      tool.execute(
        { code: "console.log('step one'); console.warn('careful'); return 'done'" },
        ctx,
      ),
    );
...
describe("code mode permission visibility", () => {
  const deny = (permission: string): PermissionV1.Rule => ({
    permission,
    pattern: "*",
    action: "deny",
  });
  const askRule = (permission: string): PermissionV1.Rule => ({
    permission,
    pattern: "*",
    action: "ask",
  });
  ...
  test("an ask-level tool remains callable and still prompts via ctx.ask", async () => {
    ...
    const askCtx: Tool.Context = {
      ...ctx,
      ask: (req) => Effect.sync(() => void asked.push(req.permission)),
    };
    const tool = await build(
      { github_list_issues: mcpTool("list_issues", ok) },
      ["github"],
