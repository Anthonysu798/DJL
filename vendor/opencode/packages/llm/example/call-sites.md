...
CloudflareAIGateway.configure({ accountId, gatewayId, gatewayApiKey, apiKey }).model(
  "openai/gpt-4o",
);
...
const configureAzure = (input: AzureConfig = {}) => {
  const route = azureResponses.with({
    ...
    auth: input.apiKey
      ? Auth.header("api-key", input.apiKey)
      : Auth.envHeader("api-key", "AZURE_OPENAI_API_KEY"),
  });
...
const configureCloudflareAIGateway = (input: CloudflareAIGatewayConfig) => {
  const route = cloudflareOpenAIChat.with({
    ...
    auth: Auth.bearerHeader("cf-aig-authorization", input.gatewayApiKey).andThen(
