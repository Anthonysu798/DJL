...
describe("normalizeGeminiCapabilityProbeResult", () => {
  it("treats authenticated ACP sessions without model discovery as ready", () => {
    expect(
      normalizeGeminiCapabilityProbeResult({
        ...
        models: [],
        message:
          "Gemini CLI is installed, but DJL could not verify authentication or discover models. Gemini ACP session started, but it did not report any available models.",
      }),
    ).toEqual({
      status: "ready",
      auth: { status: "authenticated" },
      models: [],
      message:
        "Gemini CLI is installed and authenticated, but it did not report any available models. DJL will use its built-in Gemini model list.",
    });
  });
  ...
  it("preserves warnings when authentication is still unknown", () => {
    const result = {
      ...
      models: [],
      message:
        "Gemini CLI is installed, but DJL could not verify authentication or discover models. Timed out while starting Gemini ACP session.",
    };
