...
const GEMINI_HEADLESS_AUTH_GUIDANCE = `Use Gemini API key or Vertex AI auth for DJL: set ${GEMINI_API_KEY_ENV_HINT} in \`~/.gemini/.env\`, or set Vertex AI env (${GEMINI_VERTEX_ENV_HINT}), then refresh provider status.`;
const GEMINI_CODE_ASSIST_MIGRATION_AUTH_MESSAGE = `Gemini is not authenticated because Google Code Assist OAuth for individual accounts appears to require Antigravity. For DJL, use ${GEMINI_API_KEY_ENV_HINT} or Vertex AI auth (${GEMINI_VERTEX_ENV_HINT}); use Antigravity for individual Code Assist OAuth until Gemini CLI exposes a compatible path.`;
...
function formatGeminiDiscoveryWarning(detail: string): string {
  return `Gemini CLI is installed, but DJL could not verify authentication or discover models. ${detail}`;
}
...
function formatGeminiModelDiscoveryFallbackMessage(): string {
  return "Gemini CLI is installed and authenticated, but it did not report any available models. DJL will use its built-in Gemini model list.";
}
        ...
        const finalizeOAuthBrowserPrompt = () => {
          finalize({
            ...
            message: formatGeminiAuthMessage(
              "Gemini attempted to start an OAuth browser flow during a DJL status check. Run `gemini` in a terminal to sign in.",
            ),
          });
        };
