...
export const OPENCODE_CLI_SPEC: OpenCodeCompatibleCliSpec = {
  defaultBinaryPath: resolveDjlOpenCodeBinaryPath(),
...
function pooledOpenCodeServerKey(input: {
  ...
  // Working directory is scoped by createOpenCodeSdkClient() on each caller. It must not split
  // the managed server pool: Settings discovery and the first chat otherwise boot separate
  // OpenCode processes during cold start.
  // DJL owns this isolated config root. Including the atomic config file's revision in
  // the pool key lets newly acquired discovery/session servers see local-model changes
  // immediately without interrupting an active server that still has callers attached.
  const managedConfigRevision = input.managedRootDir
    ? (() => {
        try {
          const stat = statSync(
            join(input.managedRootDir, "config", cliSpec.dataDirectoryName, "opencode.json"),
          );
...
const makeOpenCodeRuntime = Effect.gen(function* () {
  ...
  const runOpenCodeCommand: OpenCodeRuntimeShape["runOpenCodeCommand"] = (input) =>
    Effect.gen(function* () {
      const invocation = buildOpenCodeProcessInvocation(input.binaryPath, input.args);
  ...
  const connectToOpenCodeServer: OpenCodeRuntimeShape["connectToOpenCodeServer"] = (input) => {
    ...
    return Effect.gen(function* () {
      ...
      const pooledServer = yield* acquirePooledServer({
        ...
        ...(input.managedRootDir !== undefined ? { managedRootDir: input.managedRootDir } : {}),
