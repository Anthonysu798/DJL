...
import {
  ...
  type OpenCodeModelProviderConnection,
...
function resolveManagedServerValue(
  ...
  return adapterConfig.provider === "opencode" ? undefined : candidate?.trim() || undefined;
}

function sdkServerPasswordOption(
  server: OpenCodeServerConnection,
  externalPassword?: string,
): { readonly serverPassword?: string } {
  const serverPassword = server.serverPassword ?? (server.external ? externalPassword : undefined);
...
function resolveLatestAssistantText(previousText: string | undefined, nextText: string): string {
  if (previousText && previousText.length >= nextText.length && previousText.includes(nextText)) {
    return previousText;
  }
  return nextText;
}
...
function isMissingOpenCodeSessionError(error: OpenCodeRuntimeError): boolean {
  return isOpenCodeSessionNotFoundDetail(error.detail);
}
...
function resolveOpenCodeProcessingLocality(providerId: string): "local" | "remote" | "unknown" {
  ...
  if (LOCAL_OPENCODE_PROVIDER_IDS.has(normalized)) return "local";
  if (REMOTE_OPENCODE_PROVIDER_IDS.has(normalized)) return "remote";
  return "unknown";
}

function openCodeModalityCapabilities(
  model: OpenCodeInventoryProvider["models"][string] | undefined,
): Pick<
...
> {
  ...
  const modalityOrder = ["text", "audio", "image", "video", "pdf"] as const;
  return {
    ...(input
      ? {
          inputModalities: modalityOrder.filter((modality) => input[modality] === true),
          supportsVision: input.image === true,
          supportsPdf: input.pdf === true,
        }
      : {}),
    ...(output
    ...
    ...(typeof model?.capabilities?.toolcall === "boolean"
      ...
      : {}),
    ...(asPositiveInteger(model?.limit?.context)
      ? { contextLimitTokens: asPositiveInteger(model?.limit?.context) }
      : {}),
  };
}
...
function toOpenCodeModelDescriptor(input: {
  ...
  return {
    ...
    ...openCodeModalityCapabilities(input.model),
    processingLocality: resolveOpenCodeProcessingLocality(input.provider.id),
  ...
  return resolvePreferredOpenCodeModelProviders(input)
    .flatMap((provider) =>
      Object.values(provider.models)
        .filter((model) => isSupportedOfficialDeepSeekCatalogModel(provider.id, model.id))
        .flatMap((model) => {
          if (
...
export function flattenOpenCodeProviderConnections(input: {
  ...
  return input.providers
    .flatMap((provider) => {
      const supportsApiKey = supportsSimpleOpenCodeApiKey(
        provider,
      ...
      return [
        ...
        } satisfies OpenCodeModelProviderConnection,
      ];
    })
    .toSorted(
...
export function supportsSimpleOpenCodeApiKey(
  provider: Provider,
    ...
    Effect.gen(function* () {
              ...
              Effect.gen(function* () {
                ...
                const workMcpRegistration: WorkMcpSessionRegistration | null =
                  provider === "opencode" && workMcpServer
                    ...
                    : null;
                if (workMcpRegistration && workMcpServer) {
                  yield* Effect.addFinalizer(() =>
                  ...
                  yield* runOpenCodeSdk("mcp.add", () =>
                    client.mcp.add({
                      ...
                      config: {
                        type: "remote",
                        url: workMcpRegistration.url,
                        enabled: true,
                        headers: {
                          Authorization: `Bearer ${workMcpRegistration.bearerToken}`,
                        },
                        oauth: false,
                        timeout: 15_000,
                      },
                ...
                const createSessionId = resumedSessionId
                  ? // Verify that the persisted native id still belongs to this OpenCode
                    ...
                    // restart removes native sessions; treating that 404 as a successful
                    // resume deferred the failure until the user's first prompt.
                    runOpenCodeSdk("session.get", () =>
                      ...
                      Effect.flatMap(() =>
                        ...
                        // permission ruleset explicitly. Older servers may reject this field,
                        // but the session was already verified and full-access asks are still
                        // auto-approved in the event pump.
                      ...
                      Effect.catch((error) =>
                        isMissingOpenCodeSessionError(error)
                          ? Effect.logWarning(
                            ...
                            ).pipe(Effect.andThen(createSession()))
                          : Effect.fail(error),
                      ),
                    )
                  : createSession();
      ...
      const withDiscoveryClient = <A>(
        ...
        Effect.gen(function* () {
          ...
          const requestedServerUrl = resolveManagedServerValue(
            adapterConfig,
          ...
          return yield* Effect.scoped(
            Effect.gen(function* () {
              const serverUrl = resolveManagedServerValue(
                adapterConfig,
                input.serverUrl ?? undefined,
              );
              const serverPassword = resolveManagedServerValue(
          ...
          Effect.catchCause((cause) =>
            Effect.uninterruptible(
              Effect.logWarning("OpenCode discovery request interrupted", {
                provider,
          ...
          Effect.tapError((error) =>
            Effect.logWarning("OpenCode discovery request failed", {
              provider,
      ...
      const listModels: NonNullable<OpenCodeAdapterShape["listModels"]> = (input) => {
        ...
        return Effect.gen(function* () {
          ...
          if (Exit.isSuccess(inventoryExit)) {
            ...
            yield* Effect.sync(() => recordProviderModelDocumentCapabilities(provider, models));
            return {
              models,
              source:
                cliModels.length > 0
          ...
          ({ client, inventory, credentialProviderIDs }) =>
            runOpenCodeSdk("provider.auth", () => client.provider.auth()).pipe(
              Effect.map((response) => {
                ...
                return {
                  providers,
                  configuredProviderCount: providers.filter((entry) => entry.connected).length,
                  modelCount: providers
                    .filter((entry) => entry.connected)
                    .reduce((total, entry) => total + entry.modelCount, 0),
                } satisfies OpenCodeListModelProvidersResult;
              }),
              Effect.mapError(toAdapterRequestError),
            ),
        );
