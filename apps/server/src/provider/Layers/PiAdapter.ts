    ...
    const makePiExtensionUIContext = (context: PiSessionContext): ExtensionUIContext => {
      ...
      const warnUnsupported = (method: string) => {
        ...
        offerRuntimeEvent({
          ...
          payload: {
            message: `Pi extension UI API '${method}' is not supported in DJL yet.`,
            detail: { method },
          },
          raw: {
            source: "pi.sdk.event",
      ...
      const uiContext: ExtensionUIContext = {
        ...
        setTheme() {
          return { success: false, error: "DJL does not expose Pi themes." };
        },
        getToolsExpanded() {
          return false;
        },
      ...
      Effect.gen(function* () {
        ...
        if (loadedExtensions.length > 0) {
          ...
          offerRuntimeEvent({
            ...
            payload: {
              message:
                "Pi extensions are loaded with DJL's limited UI bridge. select/confirm/input/notify/status are supported; TUI-only widgets and editor hooks are ignored.",
              detail: {
    ...
    const respondUnsupported = (threadId: ThreadId, method: string) =>
      Effect.fail(
        new ProviderAdapterRequestError({
          provider: PROVIDER,
          method,
          detail: `Pi does not expose DJL approval/user-input requests for thread ${threadId}.`,
        }),
      );
