          ...
          Effect.gen(function* () {
            if (!(yield* runtime.supportsSessionFork)) {
              return yield* new ProviderAdapterValidationError({
                ...
                operation: "forkThread",
                issue:
                  "This Droid ACP version does not advertise session/fork; DJL will rebuild the fork from its retained transcript.",
              });
            }
        ...
        const forked = activeSource
          ...
          : yield* Effect.gen(function* () {
              ...
              const runtime = yield* makeDroidAcpRuntime({
                ...
                clientInfo: { name: "DJL Fork", version: "0.0.0" },
              });
              yield* runtime.start();
    ...
    const listModels: NonNullable<DroidAdapterShape["listModels"]> = (input) =>
      Effect.gen(function* () {
        ...
        const runtime = yield* makeDroidDiscoveryRuntime({
          ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
          cwd,
          clientName: "DJL Model Discovery",
        });
        yield* runtime.start();
    ...
    const listCommands: NonNullable<DroidAdapterShape["listCommands"]> = (input) =>
      Effect.gen(function* () {
        ...
        const runtime = yield* makeDroidDiscoveryRuntime({
          ...(input.binaryPath ? { binaryPath: input.binaryPath } : {}),
          cwd,
          clientName: "DJL Command Discovery",
        });
        yield* runtime.start();
