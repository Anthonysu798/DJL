...
function mockSpawnerLayer(
  ...
  options?: { readonly rawCommand?: boolean },
) {
  return Layer.succeed(
    ...
    ChildProcessSpawner.make((command) => {
      ...
      if (!options?.rawCommand) {
        if (/[\\/]cmd\.exe$/i.test(commandName)) {
          const commandLine = commandArgs.at(-1) ?? "";
          const quotedParts = [...commandLine.matchAll(/"([^"]*)"/g)].map(
            (match) => match[1] ?? "",
          );
          const executable = quotedParts.shift();
          if (executable) {
            commandName = executable;
            commandArgs = quotedParts;
          }
        }
        if (
          /[\\/]AppData[\\/]Roaming[\\/]npm[\\/]/i.test(commandName) ||
          /[\\/]node_modules[\\/]\.bin[\\/]/i.test(commandName)
        ) {
          commandName =
            commandName
              .split(/[\\/]/)
              .at(-1)
              ?.replace(/\.(?:cmd|exe)$/i, "") ?? commandName;
        }
      }
      return Effect.succeed(
        mockHandle(handler(commandArgs, commandName, cmd.options?.env, cmd.options)),
      );
    }),
  );
}
...
const cachedReadyOpenCodeStatus = {
  ...
  checkedAt: "2026-06-16T12:00:00.000Z",
  message: "DJL's managed OpenCode runtime is ready.",
} satisfies ServerProviderStatus;
...
it.layer(NodeServices.layer)("ProviderHealth", (it) => {
  describe("disabled provider handling", () => {
    ...
    it("suppresses cached update advisories when automatic update checks are disabled", () => {
      ...
      assert.strictEqual(opencode?.version, "0.129.0");
      assert.strictEqual(opencode?.versionAdvisory?.status, "unknown");
      assert.strictEqual(opencode?.versionAdvisory?.latestVersion, null);
      assert.strictEqual(opencode?.versionAdvisory?.canUpdate, false);
      assert.strictEqual(opencode?.versionAdvisory?.updateCommand, null);
    });
    ...
    it.effect("does not expose cached ready statuses for disabled providers", () =>
      Effect.gen(function* () {
        ...
        yield* writeProviderStatusCache({
          filePath: cachePath,
          provider: cachedReadyOpenCodeStatus,
        });
        ...
        const opencode = statuses.find((status) => status.provider === "opencode");
        const cachedOpenCode = yield* readProviderStatusCache(cachePath);

        assert.strictEqual(opencode?.available, false);
        assert.strictEqual(opencode?.message, "Provider is disabled in DJL settings.");
        assert.deepStrictEqual(cachedOpenCode, cachedReadyOpenCodeStatus);
      }),
    );
    ...
    it.effect("projects cached ready status when a disabled provider is re-enabled", () =>
      Effect.gen(function* () {
        ...
        yield* Effect.gen(function* () {
          ...
          const disabledOpenCode = disabledStatuses.find(
            (status) => status.provider === "opencode",
          );

          assert.strictEqual(disabledOpenCode?.available, false);
          assert.strictEqual(disabledOpenCode?.message, "Provider is disabled in DJL settings.");
          ...
          const currentOpenCode = currentStatuses.find((status) => status.provider === "opencode");
          assert.strictEqual(currentOpenCode?.available, true);
          assert.strictEqual(currentOpenCode?.authStatus, "unknown");
          assert.notStrictEqual(currentOpenCode?.message, "Provider is disabled in DJL settings.");
    ...
    it.effect("rejects one-click updates for disabled providers", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(error.reason, "Provider is disabled in DJL settings.");
      }).pipe(Effect.provide(disabledProviderHealthLayer)),
    );
  });
  ...
  describe("checkCodexProviderStatus", () => {
    ...
    it.effect("returns unavailable when codex is below the minimum supported version", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Codex CLI v0.36.0 is too old for DJL. Upgrade to v0.37.0 or newer and restart DJL.",
        );
      }).pipe(
        Effect.provide(
          mockSpawnerLayer((args) => {
  ...
  describe("checkOpenCodeProviderStatus", () => {
    it.effect("returns ready when opencode is installed", () =>
      Effect.gen(function* () {
        const status = yield* makeCheckOpenCodeProviderStatus("/managed/opencode");
  ...
  describe("checkPiProviderStatus", () => {
    ...
    it.effect("uses configured Pi binary and agent dir without SDK registry reads", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Pi CLI is installed. DJL will use Pi agent dir /tmp/pi-agent.",
        );
      }).pipe(
        Effect.provide(
          mockSpawnerLayer((args, command) => {
    ...
    it.effect("keeps Pi usable when the advisory CLI probe is missing", () =>
      Effect.gen(function* () {
        ...
        assert.strictEqual(
          status.message,
          "Pi SDK is bundled, but the Pi CLI (`pi`) is not on PATH, so DJL could not verify the installed CLI version.",
        );
