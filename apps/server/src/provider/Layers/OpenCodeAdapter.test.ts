...
describe("mergeOpenCodeAssistantText", () => {
  it("ignores a partial suffix snapshot already emitted by session.next streaming", () => {
    const previousText = "Embeded runtimes eliminate dependency drift across every deployment.";
    const partialSnapshot = "ded runtimes eliminate dependency drift across every deployment.";

    expect(mergeOpenCodeAssistantText(previousText, partialSnapshot)).toEqual({
      latestText: previousText,
      deltaToEmit: "",
    });
  });

  it("still emits the suffix from a cumulative snapshot", () => {
    expect(
      mergeOpenCodeAssistantText("Embedded runtimes", "Embedded runtimes stay stable"),
    ).toEqual({
      latestText: "Embedded runtimes stay stable",
      deltaToEmit: " stay stable",
    });
  });
});
...
describe("flattenOpenCodeProviderConnections", () => {
  it("returns API-key providers with credential state and model counts only", () => {
    const providers = flattenOpenCodeProviderConnections({
      providers: [
        ...
        makeProvider({ id: "github-copilot", name: "GitHub Copilot" }),
      ],
      connectedProviderIds: ["openai"],
      authMethods: {
        openai: [{ type: "api", label: "API key" }],
        "github-copilot": [{ type: "oauth", label: "Sign in" }],
      },
    });
  ...
  it("includes providers that declare one conventional API-key variable", () => {
    ...
    expect(supportsSimpleOpenCodeApiKey(deepseek, [])).toBe(true);
    expect(
      flattenOpenCodeProviderConnections({
        providers: [deepseek],
  ...
  it("counts only supported V4 models for the official DeepSeek provider", () => {
    const providers = flattenOpenCodeProviderConnections({
      providers: [
        makeProvider({
          id: "deepseek",
    ...
    expect(providers).toMatchObject([{ id: "deepseek", connected: true, modelCount: 2 }]);
  });
});
...
function createMockOpenCodeRuntime(options?: {
  ...
  readonly authMethods?: Record<string, ReadonlyArray<{ type: "api" | "oauth"; label: string }>>;
  ...
  const authSetCalls: Array<Record<string, unknown>> = [];
  const authRemoveCalls: Array<Record<string, unknown>> = [];
  ...
  const runtime: OpenCodeRuntimeShape = {
    ...
    loadOpenCodeCredentialProviderIDs: () => Effect.succeed(options?.credentialProviderIDs ?? []),
  };
...
describe("resolvePreferredOpenCodeModelProviders", () => {
  it("can require explicit DJL-managed credentials", () => {
    const providers = resolvePreferredOpenCodeModelProviders({
      inventory: {
        providerList: {
          connected: ["openai", "opencode", "github-copilot"],
          all: [
            makeProvider({ id: "openai", name: "OpenAI" }),
            makeProvider({ id: "opencode", name: "OpenCode" }),
            makeProvider({ id: "github-copilot", name: "GitHub Copilot" }),
          ],
        },
        consoleState: { consoleManagedProviders: ["github-copilot"] },
      },
  ...
  it("keeps localhost providers available without synthetic API credentials", () => {
    const providers = resolvePreferredOpenCodeModelProviders({
      inventory: {
        providerList: {
          connected: ["ollama", "lmstudio", "openai"],
          all: [
...
describe("flattenOpenCodeModels", () => {
  it("exposes only V4 models from the official authenticated DeepSeek catalog", () => {
    const models = flattenOpenCodeModels({
      ...
      credentialProviderIDs: ["deepseek"],
      requireCredentials: true,
    });
  ...
  it("exposes model modalities, tool support, context limit, and local processing", () => {
    const models = flattenOpenCodeModels({
      inventory: {
        providerList: {
          connected: ["ollama"],
...
describe("OpenCodeAdapter runtime lifecycle", () => {
  it("writes and removes API keys through OpenCode without returning the secret", async () => {
    ...
    expect(result).toEqual({
      saved: { providerId: "openai", connected: true },
      removed: { providerId: "openai", connected: false },
    });
    expect(JSON.stringify(result)).not.toContain("secret-value");
    expect(runtime.authSetCalls).toEqual([
      { providerID: "openai", auth: { type: "api", key: "secret-value" } },
    ]);
    expect(runtime.authRemoveCalls).toEqual([{ providerID: "openai" }]);
  });

  it("lists only credential-backed OpenCode inventory models", async () => {
    const runtime = createMockOpenCodeRuntime({
      cliModels: [
        {
          slug: "opencode/minimax-m2.5-free",
  ...
  it("does not expose CLI models when authoritative inventory discovery fails", async () => {
    const runtime = createMockOpenCodeRuntime({
  ...
  it("registers the server-owned Work MCP endpoint for OpenCode sessions only", async () => {
    ...
    const workMcpServer = {
      ...
      registerSession: ({
        ...
        Effect.succeed({
          name: `djl_work_${threadId}`,
          url: "http://127.0.0.1:41821/mcp",
          bearerToken: "server-only-token",
          authorizedRoot,
        }),
      unregisterSession: (bearerToken: string) =>
        Effect.sync(() => {
          unregisterCalls.push(bearerToken);
        }),
    };
    const cwd = process.cwd();
    ...
    await Effect.runPromise(
      Effect.gen(function* () {
        const adapter = yield* OpenCodeAdapter;
        const threadId = asThreadId("thread-work-mcp");
        yield* adapter.startSession({
          provider: "opencode",
          threadId,
          runtimeMode: "full-access",
  ...
  it("replaces a missing persisted OpenCode session before the first prompt", async () => {
    ...
    const result = await Effect.runPromise(
      Effect.gen(function* () {
        ...
        return yield* adapter.startSession({
          provider: "opencode",
          threadId: asThreadId("thread-stale-opencode-resume"),
          runtimeMode: "full-access",
          resumeCursor: {
            openCodeSessionId: "ses_removed_by_opencode",
  ...
  it("does not report a session ready when resume validation has an unrelated error", async () => {
    const runtime = createMockOpenCodeRuntime({
      sessionGetError: new Error("OpenCode server connection refused"),
    });
    ...
    ).rejects.toThrow("OpenCode server connection refused");
  ...
  it.each([
    ...
    "re-applies the %s permission ruleset when resuming OpenCode sessions",
    async (runtimeMode, permission) => {
      const runtime = createMockOpenCodeRuntime();
  ...
  it("sends plan-mode prompt instructions and captures tagged markdown as a proposed plan", async () => {
    ...
    expect(runtime.promptCalls[0]?.parts).toEqual([
      {
        type: "text",
        text: expect.stringContaining("DJL plan mode is active."),
      },
    ]);
    expect(result.map((event) => event.type)).toEqual([
      "session.started",
  ...
  it("ignores a stale built-in plan-agent override when DJL is in build mode", async () => {
    const runtime = createMockOpenCodeRuntime();
  ...
  it.each([
    ["approval-required", "bash", "rm -rf *", "command_execution_approval"],
    ["accept-edits", "webfetch", "https://example.com", "dynamic_tool_call"],
    ["auto-approval", "external_directory", "/outside/project/**", "dynamic_tool_call"],
  ] as const)(
    "surfaces approvals in %s mode for OpenCode %s permission asks",
    async (runtimeMode, permission, pattern, requestType) => {
      const eventQueue = createSubscribedEventQueue();
      const runtime = createMockOpenCodeRuntime();
      const client = runtime.runtime.createOpenCodeSdkClient({
        baseUrl: "http://127.0.0.1:4099",
