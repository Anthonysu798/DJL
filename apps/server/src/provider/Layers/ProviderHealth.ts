...
const runOpenCodeCommand = (
  args: ReadonlyArray<string>,
  executable = resolveDjlOpenCodeBinaryPath(),
) => {
  const invocation = buildOpenCodeProcessInvocation(executable, args);
  return runProviderCommand(invocation.command, invocation.args).pipe(
    Effect.flatMap((result) =>
      isWindowsShellCommandMissingResult({ code: result.code, stderr: result.stderr })
        ? Effect.fail(new Error(`spawn ${invocation.command} ENOENT`))
        : Effect.succeed(result),
    ),
  );
};
...
function parseCursorAuthStatusFromOutput(result: CommandResult): {
  ...
  if (result.code === 0) {
    return {
      status: "warning",
      authStatus: "unknown",
      message: "Cursor Agent is installed, but DJL could not verify authentication status.",
    };
  }
  ...
  Effect.gen(function* () {
    ...
    return {
      ...
      ...(hasApiKey
        ...
        : {
            message:
              "Droid CLI is installed. DJL can use the CLI's cached device-pairing login; run `droid` to authenticate locally if needed, or set FACTORY_API_KEY.",
          }),
    } satisfies ServerProviderStatus;
  });
...
export const makeCheckOpenCodeProviderStatus = (
  ...
  Effect.gen(function* () {
    const checkedAt = new Date().toISOString();
    const executable = nonEmptyTrimmed(binaryPath) ?? resolveDjlOpenCodeBinaryPath();
  ...
  Effect.gen(function* () {
    ...
    if (Option.isNone(versionProbe.success)) {
      return {
        ...
        checkedAt,
        message:
          "Pi SDK is bundled, but the CLI health check timed out before DJL could verify the installed version.",
      } satisfies ServerProviderStatus;
    }
    ...
    return {
      ...
      message: configuredAgentDir
        ? `Pi CLI is installed. DJL will use Pi agent dir ${configuredAgentDir}.`
  ...
  Effect.gen(function* () {
    ...
    if (Option.isNone(modelsProbe.success)) {
      return {
        ...
        checkedAt,
        message:
          "Cursor Agent is authenticated, but model discovery timed out before DJL could verify available models.",
      } satisfies ServerProviderStatus;
    }
    ...
    const updateProvider: ProviderHealthShape["updateProvider"] = Effect.fn(
      ...
      if (!isProviderEnabledForSettings(provider, settings)) {
        return yield* new ServerProviderUpdateError({
          provider,
          reason: "Provider is disabled in DJL settings.",
        });
      }
        ...
        const finalProviders = yield* setProviderUpdateState(
          ...
          makeUpdateState({
            ...
            message: stillOutdated
              ? "Update command completed, but DJL still detects an outdated provider version."
              : "Provider updated.",
            output: output ? output.slice(0, UPDATE_OUTPUT_MAX_BYTES) : null,
          }),
        );
