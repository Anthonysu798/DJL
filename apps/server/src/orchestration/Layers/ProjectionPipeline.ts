...
import { createWorkTask } from "../workTaskLifecycle.ts";
import {
      ...
      switch (event.type) {
        case "thread.created":
          ...
          yield* projectionThreadRepository.upsert({
            ...
            latestTurnId: null,
            workTask:
              Option.isSome(project) && project.value.kind === "studio"
                ? createWorkTask(event.payload.threadId, event.payload.createdAt, event.commandId)
                : null,
        ...
        case "thread.session-set": {
          ...
          if (event.payload.session.status !== "running" || turnId === null) {
            if (
              ...
              // A provider can fail validation before it allocates a turn id.
              // The terminal session is authoritative, so remove the request
              // placeholder instead of leaving the renderer on "Thinking".
