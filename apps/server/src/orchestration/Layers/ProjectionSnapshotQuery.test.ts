...
projectionSnapshotLayer("ProjectionSnapshotQuery", (it) => {
  ...
  it.effect("hydrates Work task state in full and shell snapshots", () =>
    Effect.gen(function* () {
      const snapshotQuery = yield* ProjectionSnapshotQuery;
      const sql = yield* SqlClient.SqlClient;
      yield* sql`DELETE FROM projection_projects`;
      ...
      yield* sql`
        INSERT INTO projection_threads (
          ...
          interaction_mode, env_mode, work_task_json, created_at, updated_at, deleted_at
        ) VALUES (
          'work-thread', 'work-project', 'Prepare report',
          '{"provider":"opencode","model":"test"}', 'full-access', 'default', 'local',
          '{"threadId":"work-thread","phase":"review","condition":"active","status":"needs_review","resumePhase":"review","progress":90,"statusReason":"Ready","lastTransitionCommandId":"provider:turn-1","createdAt":"2026-07-13T10:00:00.000Z","updatedAt":"2026-07-13T10:02:00.000Z","completedAt":null}',
      ...
      assert.equal(full.threads[0]?.workTask?.status, "needs_review");
      assert.equal(shell.threads[0]?.workTask?.progress, 90);
    }),
  );
});
