...
it.layer(
  ...
  it.effect("clears a pending turn when provider validation fails before a turn id exists", () =>
    Effect.gen(function* () {
      const eventStore = yield* OrchestrationEventStore;
      const projectionPipeline = yield* OrchestrationProjectionPipeline;
      const sql = yield* SqlClient.SqlClient;
      const threadId = ThreadId.makeUnsafe("thread-start-failure");
      const requestedAt = "2026-02-27T08:00:00.000Z";
      const failedAt = "2026-02-27T08:00:01.000Z";

      yield* eventStore.append({
        type: "thread.turn-start-requested",
        eventId: EventId.makeUnsafe("evt-start-failure-1"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: requestedAt,
        commandId: CommandId.makeUnsafe("cmd-start-failure-1"),
        causationEventId: null,
        correlationId: CorrelationId.makeUnsafe("cmd-start-failure-1"),
        metadata: {},
        payload: {
          threadId,
          messageId: MessageId.makeUnsafe("message-start-failure"),
          runtimeMode: "full-access",
          createdAt: requestedAt,
        },
      });
      ...
      yield* eventStore.append({
        type: "thread.session-set",
        eventId: EventId.makeUnsafe("evt-start-failure-2"),
        aggregateKind: "thread",
        aggregateId: threadId,
        occurredAt: failedAt,
        commandId: CommandId.makeUnsafe("cmd-start-failure-2"),
        causationEventId: null,
        correlationId: CorrelationId.makeUnsafe("cmd-start-failure-2"),
        metadata: {},
        payload: {
          threadId,
          session: {
            ...
            runtimeMode: "full-access",
            activeTurnId: null,
            lastError: "Local model server is not ready.",
            updatedAt: failedAt,
          },
        },
      });
