...
describe("OrchestrationEngine", () => {
  ...
  it("replays persisted events before continuing live from an exclusive cursor", async () => {
    const system = await createOrchestrationSystem();
    const { engine } = system;
    const createdAt = now();
    ...
    await system.run(
      engine.dispatch({
        type: "project.create",
        commandId: CommandId.makeUnsafe("cmd-project-resumable-stream-create"),
        projectId: asProjectId("project-resumable-stream"),
        title: "Resumable Stream Project",
        workspaceRoot: "/tmp/project-resumable-stream",
        defaultModelSelection: {
          provider: "codex",
          model: "gpt-5-codex",
        },
    ...
    await system.run(
      Effect.gen(function* () {
        const eventQueue = yield* Queue.unbounded<OrchestrationEvent>();
        yield* Effect.forkScoped(
          Stream.take(engine.streamDomainEventsFrom(0), 3).pipe(
            Stream.runForEach((event) => Queue.offer(eventQueue, event).pipe(Effect.asVoid)),
          ),
        );

        const replayed = yield* Queue.take(eventQueue);
        sequences.push(replayed.sequence);
        yield* engine.dispatch({
          type: "thread.create",
          commandId: CommandId.makeUnsafe("cmd-resumable-stream-thread-create"),
          threadId: ThreadId.makeUnsafe("thread-resumable-stream"),
          projectId: asProjectId("project-resumable-stream"),
        ...
        yield* engine.dispatch({
          type: "thread.meta.update",
          commandId: CommandId.makeUnsafe("cmd-resumable-stream-thread-update"),
          threadId: ThreadId.makeUnsafe("thread-resumable-stream"),
          title: "resumable-stream-updated",
        });
        sequences.push((yield* Queue.take(eventQueue)).sequence);
        sequences.push((yield* Queue.take(eventQueue)).sequence);
      }).pipe(Effect.scoped),
    );
