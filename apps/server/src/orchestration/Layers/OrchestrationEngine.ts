...
const makeOrchestrationEngine = Effect.gen(function* () {
  ...
  const streamDomainEventsFrom: OrchestrationEngineShape["streamDomainEventsFrom"] = (
    fromSequenceExclusive,
  ) =>
    Stream.unwrap(
      Effect.gen(function* () {
        // Subscribe first so commits that land while the durable replay is
        // being read are retained and cannot fall into a handoff gap.
        const subscription = yield* PubSub.subscribe(eventPubSub);
        const lastDeliveredSequence = yield* Ref.make(fromSequenceExclusive);
        return Stream.concat(
          eventStore.readFromSequence(fromSequenceExclusive),
          Stream.fromSubscription(subscription),
        ).pipe(
          Stream.mapEffect((event) =>
            Ref.modify(lastDeliveredSequence, (lastSequence) =>
              event.sequence > lastSequence
                ? ([event, event.sequence] as const)
                : ([null, lastSequence] as const),
            ),
          ),
          Stream.flatMap((event) => (event === null ? Stream.empty : Stream.succeed(event))),
        );
      }),
    );
