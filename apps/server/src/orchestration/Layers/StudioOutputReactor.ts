...
const make = Effect.gen(function* () {
  ...
  const captureBaselineBeforeTurnUnsafe = Effect.fnUntraced(function* (threadId: ThreadId) {
    ...
    // Managed Work tasks use an isolated per-thread directory. Create it at
    // baseline time so a brand-new task has a valid, empty snapshot before the
    // provider or scoped document tools can write deliverables.
