...
describe("ProviderRuntimeIngestion", () => {
  ...
  it("uses OpenCode's completed text to repair a divergent streamed prefix", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();

    harness.emit({
      type: "content.delta",
      eventId: asEventId("evt-opencode-revised-message-delta"),
      provider: "opencode",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-opencode-revised-message"),
      itemId: asItemId("item-opencode-revised-message"),
      payload: {
        streamKind: "assistant_text",
        delta: "DJL_RECONECT_OK",
      },
    });
    harness.emit({
      type: "item.completed",
      eventId: asEventId("evt-opencode-revised-message-completed"),
      provider: "opencode",
      createdAt: now,
      threadId: asThreadId("thread-1"),
      turnId: asTurnId("turn-opencode-revised-message"),
    ...
    const thread = await waitForThread(harness.engine, (entry) =>
      entry.messages.some(
        (message: ProviderRuntimeTestMessage) =>
          message.id === "assistant:item-opencode-revised-message" && !message.streaming,
      ),
    );
    const message = thread.messages.find(
      (entry: ProviderRuntimeTestMessage) => entry.id === "assistant:item-opencode-revised-message",
    );
    expect(message?.text).toBe("DJL_RECONNECT_OK");
  });
