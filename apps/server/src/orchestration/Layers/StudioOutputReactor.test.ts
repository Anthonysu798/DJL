...
describe("StudioOutputReactor", () => {
  ...
  it("uses the pre-dispatch baseline and captures files when the provider session exits", async () => {
    ...
    const taskWorkspaceRoot = resolveThreadWorkspaceCwd({
      thread: { id: threadId, projectId, envMode: "local", worktreePath: null },
      projects: [{ id: projectId, kind: "studio", workspaceRoot }],
    });
    if (!taskWorkspaceRoot) throw new Error("Expected a managed Work task root.");
    await writeFile(path.join(taskWorkspaceRoot, "report.md"), "finished report");
