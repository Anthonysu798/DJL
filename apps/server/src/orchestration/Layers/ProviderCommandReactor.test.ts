...
import type {
  EnqueueWorkTurnPreparationInput,
  PreparedWorkTurn,
...
describe("ProviderCommandReactor", () => {
  ...
  async function createHarness(input?: {
    ...
    const workPreparationPubSub = Effect.runSync(PubSub.unbounded<PreparedWorkTurn>());
    const enqueueWorkPreparation = vi.fn((_input: EnqueueWorkTurnPreparationInput) => Effect.void);
    const markWorkPreparationDispatched = vi.fn((_id: string, _now: string) => Effect.void);
    ...
    const layer = ProviderCommandReactorLive.pipe(
      ...
      Layer.provideMerge(
        Layer.succeed(WorkPreparationQueue, {
          start: Effect.void,
          enqueue: enqueueWorkPreparation,
          markDispatched: markWorkPreparationDispatched,
          resumeNeedsInput: Effect.void,
          streamCompleted: Stream.fromPubSub(workPreparationPubSub),
          drain: Effect.void,
        }),
      ),
    ...
    return {
      ...
      enqueueWorkPreparation,
      markWorkPreparationDispatched,
      emitPreparedWorkTurn: (job: WorkPreparationJobRecord) =>
        Effect.runPromise(PubSub.publish(workPreparationPubSub, { job }).pipe(Effect.asVoid)),
    };
  }
  ...
  it("routes Work turns through durable preparation and withholds images from nonvision models", async () => {
    const harness = await createHarness({
      projectKind: "studio",
      threadModelSelection: { provider: "opencode", model: "local-test" },
    });
    const now = new Date().toISOString();
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-work-turn-start"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("work-message-1"),
          role: "user",
          text: "Summarize the report",
          attachments: [
            {
              type: "image",
              id: "work-image-1" as never,
              name: "scan.png",
              mimeType: "image/png",
              sizeBytes: 128,
            },
    ...
    if (!queued) throw new Error("Expected Work preparation input");
    await harness.emitPreparedWorkTurn({
      ...
      status: "completed",
      preparedPrompt:
        "<djl_work_task><user_request>Summarize the report</user_request></djl_work_task>",
      error: null,
      attemptCount: 1,
      createdAt: now,
      updatedAt: now,
    ...
    await waitFor(() => harness.sendTurn.mock.calls.length === 1);
    expect(harness.sendTurn.mock.calls[0]?.[0]).toMatchObject({
      input: expect.stringContaining("<djl_work_task>"),
      attachments: [expect.objectContaining({ type: "file", name: "report.pdf" })],
    });
    expect(harness.markWorkPreparationDispatched).toHaveBeenCalledWith(
      "work-prep-1",
      expect.any(String),
    );
  });
  ...
  it("creates a managed Work task folder before first-turn title generation", async () => {
    ...
    const harness = await createHarness({
      ...
      threadModelSelection: { provider: "opencode", model: "deepseek/deepseek-v4-flash" },
    });
    const now = new Date().toISOString();
    let titleGenerationCwd = "";
    harness.generateThreadTitle.mockImplementation((input) => {
      titleGenerationCwd = input.cwd;
      return Effect.sync(() => {
        if (!fs.statSync(input.cwd).isDirectory()) {
          throw new Error("managed Work task folder was not prepared");
        }
        return { title: "Review the launch brief" };
      });
    });
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        ...
        commandId: CommandId.makeUnsafe("cmd-work-title-generic"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        title: "New thread",
      }),
    );
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-work-title-turn-start"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("work-title-message-1"),
          role: "user",
    ...
    await harness.emitPreparedWorkTurn({
      ...
      status: "completed",
      preparedPrompt:
        "<djl_work_task><user_request>Review the launch brief</user_request></djl_work_task>",
      error: null,
      attemptCount: 1,
      createdAt: now,
      updatedAt: now,
    ...
    await waitFor(() => titleGenerationCwd.length > 0 && fs.existsSync(titleGenerationCwd));
    expect(path.dirname(titleGenerationCwd)).toBe(path.join(projectWorkspaceRoot, "Tasks"));
    expect(path.basename(titleGenerationCwd)).toMatch(/^task-[a-f0-9]{24}$/);
    expect(fs.statSync(titleGenerationCwd).isDirectory()).toBe(true);
  });
  ...
  it("clears stale OpenCode resume state and retries the first turn", async () => {
    const harness = await createHarness({
    ...
    harness.sendTurn.mockImplementationOnce(() =>
      Effect.fail(
        new ProviderAdapterRequestError({
          ...
          detail: "Session not found: ses_removed_by_restart",
        }),
      ),
    );
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-turn-start-opencode-stale-resume"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-message-opencode-stale-resume"),
          role: "user",
          text: "start with a fresh native session",
          attachments: [],
        },
  ...
  it("keeps the prompt fallback when title generation leaks tool-call markup", async () => {
    const harness = await createHarness();
    const now = new Date().toISOString();
    harness.generateThreadTitle.mockImplementation(() =>
      Effect.succeed({
        title: '<tool_calls> <tool_call name="Read"> <parameter name="file_path">',
      }),
    );
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.meta.update",
        commandId: CommandId.makeUnsafe("cmd-thread-title-tool-markup"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        title: "New thread",
      }),
    );
    ...
    await Effect.runPromise(
      harness.engine.dispatch({
        type: "thread.turn.start",
        commandId: CommandId.makeUnsafe("cmd-turn-start-tool-markup-title"),
        threadId: ThreadId.makeUnsafe("thread-1"),
        message: {
          messageId: asMessageId("user-message-tool-markup-title-1"),
          role: "user",
          text: "Read these policies and return one cited policy statement",
          attachments: [],
        },
        interactionMode: DEFAULT_PROVIDER_INTERACTION_MODE,
        runtimeMode: "approval-required",
    ...
    await waitFor(async () => {
      ...
      return (
        readModel.threads.find((entry) => entry.id === ThreadId.makeUnsafe("thread-1"))?.title ===
        "Read these policies and return one"
      );
    });
  });
  ...
  it("restarts the provider session when runtime mode changes on the thread or turn request", async () => {
    ...
    await waitFor(async () => {
      ...
      return thread?.runtimeMode === "auto-approval";
    });
    await waitFor(() => harness.startSession.mock.calls.length === 2);
    await Effect.runPromise(
      harness.engine.dispatch({
