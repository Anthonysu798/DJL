...
const make = Effect.gen(function* () {
  ...
  const finalizeAssistantMessage = (input: {
    ...
    Effect.gen(function* () {
      ...
      yield* orchestrationEngine.dispatch({
        ...
        threadId: input.threadId,
        messageId: input.messageId,
        ...(input.completionText !== undefined ? { text: input.completionText } : {}),
        ...(input.turnId ? { turnId: input.turnId } : {}),
        createdAt: input.createdAt,
      });
  ...
  const processRuntimeEvent = (event: ProviderRuntimeEvent) =>
    Effect.gen(function* () {
      ...
      if (assistantCompletion) {
        ...
        yield* finalizeAssistantMessage({
          ...
          ...(assistantCompletion.fallbackText !== undefined && shouldApplyFallbackCompletionText
            ...
            : {}),
          ...(event.provider === "opencode" && assistantCompletion.fallbackText !== undefined
      ...
      const workTransition = thread.workTask ? deriveWorkTaskTransition(event) : null;
      const canApplyWorkTransition =
        workTransition !== null &&
        (!(event.type === "turn.completed" || event.type === "turn.aborted") ||
          shouldApplyThreadLifecycle) &&
        (workTransition.action !== "resolve_input" || thread.workTask?.condition === "needs_input");
      if (workTransition !== null && canApplyWorkTransition) {
        yield* orchestrationEngine
          .dispatch({
            type: "thread.work-task.transition",
            commandId: workTaskProviderCommandId(event, workTransition.action),
            threadId: thread.id,
            action: workTransition.action,
            ...(workTransition.reason !== undefined ? { reason: workTransition.reason } : {}),
            ...(workTransition.progress !== undefined ? { progress: workTransition.progress } : {}),
            createdAt: event.createdAt,
          })
          .pipe(
            Effect.catchCause((cause) =>
              Effect.logWarning("Work task lifecycle transition was rejected", {
                threadId: thread.id,
