...
type PreparedWorkTurnInput = {
  readonly _tag: "prepared-work-turn";
  readonly job: WorkPreparationJobRecord;
};

type ProviderWorkerInput = ProviderIntentEvent | PreparedWorkTurnInput;

type ProviderCommandProcessingEffect = Effect.Effect<void, unknown, Scope.Scope>;

function isPreparedWorkTurnInput(input: ProviderWorkerInput): input is PreparedWorkTurnInput {
  return "_tag" in input && input._tag === "prepared-work-turn";
}

export function attachmentsForPreparedWorkTurn(
  job: WorkPreparationJobRecord,
  ...
  // potentially remote model merely because it was attached to the task.
  return job.attachments.filter(
    (attachment) => attachment.type !== "image" || routing.supportsVision,
  );
}
...
function isStaleOpenCodeResumeError(error: unknown): boolean {
  if (Schema.is(ProviderAdapterRequestError)(error)) {
    return error.provider === "opencode" && isOpenCodeSessionNotFoundDetail(error.detail);
  }
  return isOpenCodeSessionNotFoundDetail(String(error));
}
...
const make = Effect.gen(function* () {
  ...
  const prepareManagedWorkTaskDirectory = Effect.fnUntraced(function* (
    project: OrchestrationProjectShell | undefined,
    cwd: string | undefined,
  ) {
    if (project?.kind !== "studio" || !cwd) {
      return;
    }
  ...
  >();
  const queuedPreparedWorkTurnsByThread = new Map<string, WorkPreparationJobRecord[]>();
        ...
        Effect.mapError(
          (cause) =>
            new ProviderAdapterRequestError({
              provider: threadProvider,
              method: "thread.turn.start",
              detail: `Unable to prepare the managed Work folder: ${String(cause)}`,
            }),
        ),
      );
    }
  ...
  const dispatchTurnForThread = Effect.fnUntraced(function* (input: {
    ...
    if (input.reviewTarget !== undefined) {
      ...
      const sentTurn = yield* sendQueuedProviderTurn(normalizedInput).pipe(
        Effect.catch((error) =>
          Effect.gen(function* () {
            const staleNativeResume =
              (selectedProvider === "claudeAgent" && isStaleClaudeResumeError(error)) ||
              (selectedProvider === "opencode" && isStaleOpenCodeResumeError(error));
            if (!staleNativeResume) {
              return yield* Effect.fail(error);
            }

            // A native server may have been restarted since DJL persisted this thread's
            // cursor. Clear the dead cursor and replay once with DJL transcript context.
            yield* clearStaleProviderResumeState({
              threadId: input.threadId,
              cause: error,
            });
            ...
            yield* Effect.logWarning(
              "provider command reactor retrying turn after stale native resume",
              {
  ...
  const maybeGenerateAndRenameThreadTitleForFirstTurn = Effect.fnUntraced(function* (input: {
    ...
    const nextTitle = yield* prepareManagedWorkTaskDirectory(workspaceProject, cwd).pipe(
      Effect.andThen(textGeneration.generateThreadTitle(titleGenerationInput)),
      Effect.map((generated) => {
        const sanitizedTitle = sanitizeGeneratedThreadTitle(generated.title);
        return isGenericChatThreadTitle(sanitizedTitle) ? fallbackTitle : sanitizedTitle;
      }),
      Effect.catch((error) =>
  ...
  const processTurnStartRequested: (
    ...
    preparedJob?: WorkPreparationJobRecord,
  ) => ProviderCommandProcessingEffect = Effect.fnUntraced(function* (
    event: Extract<ProviderIntentEvent, { type: "thread.turn-start-requested" }>,
    preparedJob?: WorkPreparationJobRecord,
  ) {
    ...
    // A second Work preparation can finish while the previous turn is still
    // running. Retain the prepared prompt itself (not merely the original turn
    // request) and dispatch it after the terminal event.
    if (preparedJob && (yield* hasLiveProviderTurn(event.payload.threadId))) {
      const queued = queuedPreparedWorkTurnsByThread.get(event.payload.threadId) ?? [];
      if (!queued.some((candidate) => candidate.id === preparedJob.id)) {
        queuedPreparedWorkTurnsByThread.set(event.payload.threadId, [...queued, preparedJob]);
      }
      return;
    }

    const key = preparedJob ? `prepared:${preparedJob.id}` : turnStartKeyForEvent(event);
    if (yield* hasHandledTurnStartRecently(key)) {
      return;
    }

    if (preparedJob && !preparedJob.preparedPrompt) {
      return yield* Effect.fail(
        new Error(`Prepared Work job '${preparedJob.id}' has no trusted task brief.`),
      );
    }
    ...
    yield* dispatchTurnForThread({
      ...
      Effect.tap(() =>
        preparedJob
          ? workPreparationQueue.markDispatched(preparedJob.id, new Date().toISOString())
          : Effect.void,
      ),
      Effect.catchCause((cause) =>
        Effect.gen(function* () {
          ...
          yield* drainNextTurnForThread(event.payload.threadId);
        }),
      ),
  ...
  const processPreparedWorkTurn: (
    job: WorkPreparationJobRecord,
  ) => ProviderCommandProcessingEffect = (job) =>
    processTurnStartRequested(
      {
        eventId: EventId.makeUnsafe(job.sourceEventId),
  ...
  const drainNextTurnForThread: (threadId: ThreadId) => ProviderCommandProcessingEffect =
    Effect.fnUntraced(function* (threadId: ThreadId) {
      const preparedQueue = queuedPreparedWorkTurnsByThread.get(threadId);
      const nextPrepared = preparedQueue?.shift();
      if (preparedQueue && preparedQueue.length === 0) {
        queuedPreparedWorkTurnsByThread.delete(threadId);
      }
      if (nextPrepared) {
        yield* processPreparedWorkTurn(nextPrepared);
        return;
      }
  ...
  const processQueueDrainEvent = Effect.fnUntraced(function* (event: ProviderQueueDrainEvent) {
    ...
    yield* drainNextTurnForThread(event.threadId);
  });
  ...
  const processWorkerInputSafely: (
    input: ProviderWorkerInput,
  ) => ProviderCommandProcessingEffect = (input) =>
    isPreparedWorkTurnInput(input)
      ? processPreparedWorkTurn(input.job).pipe(
          Effect.catchCause((cause) => {
            if (Cause.hasInterruptsOnly(cause)) return Effect.failCause(cause);
            return Effect.logWarning(
              "provider command reactor failed to dispatch prepared Work turn",
              {
                jobId: input.job.id,
      ...
      : processDomainEventSafely(input);
  ...
  const worker = yield* makeDrainableWorker(processWorkerInputSafely);
  ...
  const start: ProviderCommandReactorShape["start"] = seedThreadModelSelections.pipe(
    Effect.andThen(
      Effect.all([
        ...
        Stream.runForEach(workPreparationQueue.streamCompleted, ({ job }) =>
          worker.enqueue({ _tag: "prepared-work-turn", job }),
        ).pipe(Effect.forkScoped),
      ]).pipe(Effect.asVoid),
    ),
  );
