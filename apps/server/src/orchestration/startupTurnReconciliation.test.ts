...
describe("planRestartTurnReconciliation", () => {
  ...
  it("cancels an active Work task whose provider turn was orphaned by restart", () => {
    const thread = {
      ...makeThread("stuck-work", {
        session: makeSession("stuck-work", {
          status: "running",
          activeTurnId: TurnId.makeUnsafe("stuck-work-turn"),
        }),
        latestTurn: { state: "running" },
      }),
      workTask: {
    ...
    expect(commands[1]).toEqual({
      ...
      commandId: `restart-reconcile:stuck-work:work-task:${NOW}`,
      threadId: "stuck-work",
      action: "cancel",
  ...
  it("keeps a Work task active when its persisted preparation job can resume", () => {
    const thread = {
      ...makeThread("recovering-work", {
        session: makeSession("recovering-work", {
          status: "starting",
          activeTurnId: null,
        }),
      }),
    ...
    const commands = planRestartTurnReconciliation({
      threads: [thread],
      now: NOW,
      recoveringWorkThreadIds: new Set(["recovering-work"]),
    });

    expect(commands).toHaveLength(1);
    expect(commands[0]).toMatchObject({
      type: "thread.session.set",
      threadId: "recovering-work",
      session: { status: "interrupted", activeTurnId: null },
    });
  });
