...
import { attachmentIdForContentHash, detectUploadedMediaType } from "../streamingAttachmentUpload";
...
type BinaryChatAttachment = Extract<ChatAttachment, { readonly type: "image" | "file" }>;
...
function normalizeStreamingAttachmentReference(input: {
  readonly attachment: StreamingAttachmentReference;
  readonly threadId: string;
  readonly attachmentsDir: string;
  readonly fileSystem: FileSystem.FileSystem;
}): Effect.Effect<ChatAttachment, Error> {
  return Effect.gen(function* () {
    const expectedId = attachmentIdForContentHash(input.threadId, input.attachment.contentHash);
    if (input.attachment.id !== expectedId) {
      return yield* Effect.fail(
        new Error(`Attachment '${input.attachment.name}' is not owned by this task.`),
      );
    }
    const normalized: BinaryChatAttachment = {
      type: input.attachment.type,
      id: input.attachment.id,
      name: input.attachment.name,
      mimeType: input.attachment.mimeType,
      sizeBytes: input.attachment.sizeBytes,
      contentHash: input.attachment.contentHash.toLowerCase(),
      uploadMethod: "stream",
    };
    const attachmentPath = resolveAttachmentPath({
      attachmentsDir: input.attachmentsDir,
    ...
    if (!attachmentPath) {
      return yield* Effect.fail(new Error(`Invalid attachment path for '${normalized.name}'.`));
    }
    if (
      !isSafeRegularAttachmentPath({
        attachmentsDir: input.attachmentsDir,
      ...
      return yield* Effect.fail(
        new Error(`Immutable attachment '${normalized.name}' failed path validation.`),
      );
    }
    const fileInfo = yield* input.fileSystem
      .stat(attachmentPath)
      .pipe(Effect.catch(() => Effect.succeed(null)));
    if (!fileInfo || fileInfo.type !== "File" || Number(fileInfo.size) !== normalized.sizeBytes) {
      return yield* Effect.fail(
        new Error(`Immutable attachment '${normalized.name}' is missing or changed.`),
      );
    }
    const bytes = yield* input.fileSystem
      ...
      .pipe(
        Effect.mapError(
          () => new Error(`Immutable attachment '${normalized.name}' could not be verified.`),
        ),
      );
    const actualHash = createHash("sha256").update(bytes).digest("hex");
    if (actualHash !== input.attachment.contentHash.toLowerCase()) {
      return yield* Effect.fail(
        new Error(`Immutable attachment '${normalized.name}' failed its content hash check.`),
      );
    }
    const detected = yield* Effect.try({
      try: () =>
        detectUploadedMediaType({
          ...
          declaredMimeType: normalized.mimeType,
          header: bytes.subarray(0, 64 * 1024),
        }),
      catch: (cause) =>
        new Error(
          `Immutable attachment '${normalized.name}' failed media validation: ${cause instanceof Error ? cause.message : String(cause)}`,
        ),
    });
    if (detected.type !== normalized.type || detected.mediaType !== normalized.mimeType) {
      return yield* Effect.fail(
        new Error(`Immutable attachment '${normalized.name}' changed media type.`),
      );
    }
    return normalized;
  });
    ...
    const normalizedAttachments = yield* Effect.forEach(
      ...
      (attachment) =>
        Effect.gen(function* () {
          ...
          if (
            "contentHash" in attachment &&
            typeof attachment.contentHash === "string" &&
            "uploadMethod" in attachment &&
            attachment.uploadMethod === "stream"
          ) {
            return yield* normalizeStreamingAttachmentReference({
              attachment: {
          ...
          if (!("dataUrl" in attachment)) {
            return yield* Effect.fail(
              new Error(`Attachment '${attachment.name}' has no upload payload.`),
            );
          }
