...
>;
type WorkTaskTransitionCommand = Extract<
  OrchestrationCommand,
  { readonly type: "thread.work-task.transition" }
>;
type RestartReconciliationCommand =
  | ThreadSessionSetCommand
  | ThreadActivityAppendCommand
  | WorkTaskTransitionCommand;
...
export interface ReconcilableThread {
  ...
  readonly workTask?: Pick<WorkTask, "phase" | "condition"> | null | undefined;
...
export function planRestartTurnReconciliation(input: {
  ...
  readonly now: string;
  readonly recoveringWorkThreadIds?: ReadonlySet<string>;
  ...
  for (const thread of input.threads) {
    ...
    if (
      thread.workTask &&
      !input.recoveringWorkThreadIds?.has(thread.id) &&
      thread.workTask.condition !== "failed" &&
      thread.workTask.condition !== "cancelled" &&
      (thread.workTask.condition === "needs_input" ||
        thread.workTask.phase === "planning" ||
        thread.workTask.phase === "working")
    ) {
      commands.push({
        type: "thread.work-task.transition",
        commandId: CommandId.makeUnsafe(`restart-reconcile:${thread.id}:work-task:${input.now}`),
        threadId: thread.id,
        action: "cancel",
        reason: "The task was interrupted when DJL restarted. Retry to continue.",
        createdAt: input.now,
      });
    }
  }
...
export const reconcileRestartStuckTurns: Effect.Effect<
  void,
  never,
  OrchestrationEngineService | ProjectionSnapshotQuery | WorkPreparationRepository
> = Effect.gen(function* () {
  ...
  const recoveringWorkJobs = yield* workPreparationRepository
    .recover()
    .pipe(
      Effect.catchCause((cause) =>
        Effect.logWarning(
          "restart turn reconciliation continuing without Work preparation recovery context",
          { cause },
        ).pipe(Effect.as([])),
      ),
    );
  const recoveringWorkThreadIds = new Set(recoveringWorkJobs.map((job) => job.threadId as string));
