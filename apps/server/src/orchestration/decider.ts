...
> {
  switch (command.type) {
    ...
    case "thread.message.edit-and-resend": {
      ...
      const editTarget = resolveTailUserMessageEditTarget({
        ...
        activeTurnId: resolveTailReplayTurnId({
          sessionIsRunning: thread.session?.status === "running",
    ...
    case "thread.message.assistant.complete": {
      ...
      return {
        ...
        payload: {
          ...
          messageId: command.messageId,
          role: "assistant",
          text: command.text ?? existingMessage?.text ?? "",
          turnId: resolveStableMessageTurnId({
            existingTurnId: existingMessage?.turnId,
            incomingTurnId: command.turnId,
          }),
    ...
    case "thread.work-task.transition": {
      const thread = yield* requireThread({
        readModel,
        command,
        threadId: command.threadId,
      ...
      if (project?.kind !== "studio") {
        return yield* new OrchestrationCommandInvariantError({
          commandType: command.type,
          detail: "Work lifecycle actions are only available for Work tasks.",
        });
      }
      const currentTask = thread.workTask ?? createWorkTask(thread.id, thread.createdAt, null);
      const task = yield* Effect.try({
        try: () =>
          transitionWorkTask(currentTask, {
          ...
          new OrchestrationCommandInvariantError({
            commandType: command.type,
            detail: cause instanceof Error ? cause.message : "Invalid Work task transition.",
          }),
      });
      return {
        ...withEventBase({
