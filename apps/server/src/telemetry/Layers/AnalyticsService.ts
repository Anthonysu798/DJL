...
// Telemetry is privacy-deny-by-default. Callers may accidentally have rich task objects in
// scope, so only these aggregate/runtime dimensions can leave the process. In particular,
// arbitrary filenames, paths, prompts, document or memory text, and credentials are dropped.
...
export function sanitizeAnalyticsProperties(
  ...
  for (const [key, value] of Object.entries(properties)) {
    if (!ANALYTICS_PROPERTY_ALLOWLIST.has(key)) continue;
    if (typeof value === "boolean") sanitized[key] = value;
    else if (typeof value === "number" && Number.isFinite(value)) sanitized[key] = value;
    else if (typeof value === "string") sanitized[key] = value.slice(0, 128);
  }
