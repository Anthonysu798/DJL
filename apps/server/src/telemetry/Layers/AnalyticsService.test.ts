...
it.layer(NodeServices.layer)("AnalyticsService test", (it) => {
  ...
  it.effect("drops document, prompt, memory, filename, and credential properties", () =>
    Effect.gen(function* () {
      const capturedRequests: Array<RecordedBatchRequest> = [];
      const serverConfigLayer = ServerConfig.layerTest(process.cwd(), {
        prefix: "djl-telemetry-privacy-",
      });
      const telemetryLayer = AnalyticsServiceLayerLive.pipe(Layer.provideMerge(serverConfigLayer));
      const configLayer = ConfigProvider.layer(
        ConfigProvider.fromUnknown({
      ...
      yield* Effect.gen(function* () {
        yield* Layer.launch(batchServerLayer).pipe(Effect.forkScoped);
        const analytics = yield* AnalyticsService;
        yield* analytics.record("work.privacy", {
          provider: "opencode",
          prompt: "Summarize acquisition.pdf",
          filename: "acquisition.pdf",
          memoryText: "Confidential decision",
          credential: "secret-token",
        });
        yield* analytics.flush;
      }).pipe(Effect.provide(runtimeLayer));

      const event = capturedRequests
        .flatMap((request) => request.body?.batch ?? [])
        .find((entry) => entry.event === "work.privacy");
      assert.equal(event?.properties?.provider, "opencode");
      assert.equal(event?.properties?.prompt, undefined);
      assert.equal(event?.properties?.filename, undefined);
      assert.equal(event?.properties?.memoryText, undefined);
      assert.equal(event?.properties?.credential, undefined);
    }),
  );
});
