...
import { WorkPreparationQueue } from "./orchestration/Services/WorkPreparationQueue";
import { WorkPreparationRepository } from "./persistence/Services/WorkPreparationRepository";
...
import { DocumentIntelligence } from "./work/Services/DocumentIntelligence";
import { DocumentRenderer } from "./work/Services/DocumentRenderer";
import { resolveWorkCapabilityFlags } from "./work/workCapabilities";
import { resolveWorkArtifactPath } from "./work/workArtifactPath";
import { previewWorkDocument, toDocumentArtifactPreview } from "./work/workDocumentPreview";
...
function toWsRpcError(
  ...
  commandFailure?: { readonly commandId: CommandId; readonly status: "failed" },
) {
  const error = Schema.is(WsRpcError)(cause)
    ? cause
    : new WsRpcError({
        message:
...
export const makeWsRpcLayer = () =>
  WsRpcGroup.toLayer(
    Effect.gen(function* () {
      ...
      const documentRenderer = yield* DocumentRenderer;
      ...
      const workPreparationRepository = yield* WorkPreparationRepository;
      ...
      const loadServerConfig = Effect.gen(function* () {
        ...
        return {
          ...
          workCapabilities: resolveWorkCapabilityFlags({
            releaseStage: config.workReleaseStage ?? 5,
            localOcrInstallAvailable:
              documentIntelligenceStatus.installAvailable ||
      ...
      return WsRpcGroup.of({
        [ORCHESTRATION_WS_METHODS.dispatchCommand]: (command) =>
          Effect.gen(function* () {
            ...
            Effect.mapError((cause) =>
              toWsRpcError(cause, "Failed to dispatch orchestration command", {
            ...
            projectionReadModelQuery.getShellSnapshot().pipe(
              Effect.map((snapshot) =>
                Stream.concat(
                  ...
                  // Durable replay closes the snapshot-to-live subscription gap.
                  bufferLiveUiStream(
                    orchestrationEngine
                      .streamDomainEventsFrom(snapshot.snapshotSequence)
                  ...
                  Stream.mapError((cause) =>
                    toWsRpcError(cause, "Failed to resume shell event stream"),
                  ),
                ),
              ),
            ...
            projectionReadModelQuery.getThreadDetailSnapshotById(input.threadId).pipe(
              Effect.map((snapshot) =>
                Option.match(snapshot, {
                  ...
                  onSome: (value) =>
                    Stream.concat(
                      Stream.succeed({ kind: "snapshot" as const, snapshot: value }),
                      // Snapshot first, then persisted catch-up, then live events.
                      bufferLiveUiStream(
                        orchestrationEngine
                          .streamDomainEventsFrom(value.snapshotSequence)
                          .pipe(
                      ...
                      Stream.mapError((cause) =>
                        toWsRpcError(cause, "Failed to resume thread event stream"),
                      ),
                    ),
                }),
              ),
        ...
        [WS_METHODS.subscribeOrchestrationDomainEvents]: (input) => {
          const stream =
            input.fromSequenceExclusive === undefined
              ...
              : orchestrationEngine.streamDomainEventsFrom(input.fromSequenceExclusive);
          return bufferLiveUiStream(stream, {
        ...
        [WS_METHODS.workDocumentIntelligenceStatus]: () =>
          config.workReleaseStage !== undefined && config.workReleaseStage < 3
            ? Effect.succeed({
                state: "unavailable" as const,
                installAvailable: false,
                version: null,
                engineVersion: null,
                detail: "Local document intelligence is disabled by the DJL Work rollout policy.",
              })
            : rpcEffect(documentIntelligence.status, "Failed to read document-intelligence status"),
        [WS_METHODS.workDocumentIntelligenceInstall]: () =>
          (config.workReleaseStage ?? 5) < 3
            ? Effect.fail(
                new WsRpcError({ message: "Document intelligence is disabled by policy." }),
              )
            : rpcEffect(
                documentIntelligence.install.pipe(
                  Effect.tap(() => workPreparationQueue.resumeNeedsInput),
                ),
                "Failed to install document intelligence",
              ),
        [WS_METHODS.workDocumentIntelligenceRepair]: () =>
          (config.workReleaseStage ?? 5) < 3
            ...
            : rpcEffect(
                documentIntelligence.repair.pipe(
                  Effect.tap(() => workPreparationQueue.resumeNeedsInput),
                ),
                "Failed to repair document intelligence",
              ),
        [WS_METHODS.workDocumentIntelligenceUninstall]: () =>
          rpcEffect(documentIntelligence.uninstall, "Failed to uninstall document intelligence"),
        [WS_METHODS.workResolveArtifactPath]: (input) =>
          rpcEffect(
            Effect.gen(function* () {
              const context = yield* projectionReadModelQuery.getThreadCheckpointContext(
                input.threadId,
              ...
              if (Option.isNone(context)) {
                return yield* new WsRpcError({ message: "Work task was not found." });
              }
              const resolvedPath = yield* Effect.tryPromise(() =>
                resolveWorkArtifactPath({
                  threadId: input.threadId,
                  projectId: context.value.projectId,
                  projectKind: context.value.projectKind,
                  projectWorkspaceRoot: context.value.workspaceRoot,
                  envMode: context.value.envMode,
                  worktreePath: context.value.worktreePath,
            ...
            "Failed to resolve Work artifact",
          ),
        [WS_METHODS.workPreviewDocument]: (input) =>
          (config.workReleaseStage ?? 5) < 2
            ? Effect.fail(new WsRpcError({ message: "Work document previews are disabled." }))
            : rpcEffect(
                Effect.gen(function* () {
                  const context = yield* projectionReadModelQuery.getThreadCheckpointContext(
                    input.threadId,
                ...
                "Failed to preview Work document",
              ),
        [WS_METHODS.workDocumentRendererStatus]: () =>
          (config.workReleaseStage ?? 5) < 2
            ? Effect.succeed({
                state: "unavailable" as const,
                installAvailable: false,
                ...
                detail: "Native document previews are disabled by the DJL Work rollout policy.",
              })
            : rpcEffect(documentRenderer.status, "Failed to read document-viewer status"),
        [WS_METHODS.workDocumentRendererInstall]: () =>
          rpcEffect(documentRenderer.install, "Failed to install the document viewer"),
        [WS_METHODS.workDocumentRendererRepair]: () =>
          rpcEffect(documentRenderer.repair, "Failed to repair the document viewer"),
        [WS_METHODS.workDocumentRendererUninstall]: () =>
          rpcEffect(documentRenderer.uninstall, "Failed to uninstall the document viewer"),
        [WS_METHODS.workRequestDocumentRender]: (input) =>
          rpcEffect(
            Effect.gen(function* () {
              const context = yield* projectionReadModelQuery.getThreadCheckpointContext(
                input.threadId,
              ...
              return yield* documentRenderer.requestRender({
                threadId: input.threadId,
                projectId: context.value.projectId,
                filePath,
              });
            }),
            "Failed to request document render",
          ),
        [WS_METHODS.workGetDocumentRender]: (input) =>
          rpcEffect(documentRenderer.getRender(input), "Failed to read document render"),
        [WS_METHODS.workCancelDocumentRender]: (input) =>
          rpcEffect(documentRenderer.cancelRender(input), "Failed to cancel document render"),
        [WS_METHODS.subscribeWorkDocumentRenderEvents]: () =>
          bufferLiveUiStream(documentRenderer.events, {
            label: "work.document-renders",
          }),
        [WS_METHODS.localModelsGetSnapshot]: () =>
          rpcEffect(localModels.getSnapshot, "Failed to load local models"),
        [WS_METHODS.localModelsRefresh]: () =>
          rpcEffect(localModels.refresh, "Failed to refresh local models"),
        [WS_METHODS.localModelsInstallRuntime]: (input) =>
          rpcEffect(localModels.installRuntime(input), "Failed to install local model runtime"),
        [WS_METHODS.localModelsStartRuntime]: (input) =>
          rpcEffect(localModels.startRuntime(input), "Failed to start local model runtime"),
        [WS_METHODS.localModelsInstallModel]: (input) =>
          rpcEffect(localModels.installModel(input), "Failed to install local model"),
        [WS_METHODS.localModelsCancelInstall]: (input) =>
          rpcEffect(localModels.cancelInstall(input), "Failed to cancel local model install"),
        [WS_METHODS.localModelsStartSetup]: (input) =>
          rpcEffect(localModels.startSetup(input), "Failed to start local AI setup"),
        [WS_METHODS.localModelsRetrySetup]: (input) =>
        ...
        [WS_METHODS.subscribeLocalModelEvents]: () =>
          Stream.merge(
            Stream.fromEffect(
              localModels.getSnapshot.pipe(
                Effect.map((snapshot) => ({ type: "snapshot.updated" as const, snapshot })),
                Effect.mapError((cause) =>
                  toWsRpcError(cause, "Failed to load local model snapshot"),
                ),
              ),
            ),
        ...
        [WS_METHODS.aiDetectorInstallModel]: (input) =>
          rpcEffect(aiDetector.installModel(input), "Failed to install AI detector model"),
        [WS_METHODS.aiDetectorCancelInstall]: (input) =>
          rpcEffect(aiDetector.cancelInstall(input), "Failed to cancel AI detector install"),
        [WS_METHODS.aiDetectorRemoveModel]: (input) =>
          rpcEffect(aiDetector.removeModel(input), "Failed to remove AI detector model"),
        [WS_METHODS.aiDetectorClearCache]: () =>
          rpcEffect(aiDetector.clearCache, "Failed to clear AI detector cache"),
        [WS_METHODS.subscribeAiDetectorEvents]: () =>
          Stream.merge(
            Stream.fromEffect(
              aiDetector.getState.pipe(
                Effect.map((state) => ({ type: "state.updated" as const, state })),
                Effect.mapError((cause) => toWsRpcError(cause, "Failed to load AI detector state")),
              ),
            ),
            bufferLiveUiStream(aiDetector.events, { label: "aiDetector.events" }),
          ),
        [WS_METHODS.workListPreparedDocuments]: (input) =>
          (config.workReleaseStage ?? 5) < 2
            ...
            : rpcEffect(
                Effect.gen(function* () {
                  ...
                  const artifacts = yield* workPreparationRepository.listRecentArtifactsForThread(
                    input.threadId,
                  );
                  const seenAttachments = new Set<string>();
                  return {
                    artifacts: artifacts.flatMap((artifact) => {
                      if (seenAttachments.has(artifact.attachmentId)) return [];
                      seenAttachments.add(artifact.attachmentId);
                      return [toDocumentArtifactPreview(artifact)];
                    }),
                  };
                }),
                "Failed to list prepared documents",
              ),
        ...
        [WS_METHODS.openCodeListModelProviders]: (input) =>
          rpcEffect(
            providerDiscoveryService.listModelProviders(input).pipe(
              Effect.catchCause((cause) =>
                Effect.uninterruptible(
                  Effect.logWarning("OpenCode model-provider RPC ended unexpectedly", {
        ...
        [WS_METHODS.openCodeRemoveCredential]: (input) =>
          rpcEffect(
