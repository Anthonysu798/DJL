...
describe("resolveThreadWorkspaceCwd", () => {
  ...
  it("resolves an isolated managed task root for a studio-kind thread with no folder", () => {
    expect(
      resolveThreadWorkspaceCwd({
    ...
    ).toMatch(/^\/tmp\/studio-root\/Tasks\/task-[a-f0-9]{24}$/);
  });
    ...
    expect(
      resolveThreadWorkspaceCwd({
        thread: {
          ...
          projectId,
          envMode: "worktree",
          worktreePath: "/tmp/studio-worktree",
        },
        projects: [{ id: projectId, kind: "studio", workspaceRoot: "/tmp/studio-root" }],
      }),
