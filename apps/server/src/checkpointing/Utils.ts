...
export function resolveThreadWorkspaceCwd(input: {
  ...
  if (project?.kind === "studio" && !input.thread.worktreePath && input.thread.id) {
    const taskToken = createHash("sha256").update(input.thread.id).digest("hex").slice(0, 24);
    return path.join(project.workspaceRoot, "Tasks", `task-${taskToken}`);
  }
