...
async function fetchLocalPageTitleResponse(
  ...
  const response = await globalThis.fetch(url, {
    ...
    headers: {
      ...
      "User-Agent": "DJLLocalServerMonitor/1.0",
    },
  });
...
function toServerProcess(
  ...
  return {
    ...
    addresses,
    isStoppable,
    ...(isStoppable ? {} : { stopDisabledReason: "DJL cannot signal this process." }),
  };
}
...
export async function stopLocalServer(
  ...
  if (!target.isStoppable) {
    return {
      pid: input.pid,
      stopped: false,
      message: target.stopDisabledReason ?? "DJL cannot stop this process.",
    };
  }
