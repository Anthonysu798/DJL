...
const runtimeMock = {
  state: {
    ...
    managedRootDirs: [] as Array<string | undefined>,
  ...
  reset() {
    ...
    this.state.managedRootDirs.length = 0;
...
const OpenCodeRuntimeTestDouble: OpenCodeRuntimeShape = {
  startOpenCodeServerProcess: ({ binaryPath, cwd, managedRootDir }) =>
    Effect.gen(function* () {
      ...
      runtimeMock.state.managedRootDirs.push(managedRootDir);
...
it.layer(OpenCodeTextGenerationTestLayer)("OpenCodeTextGenerationServiceLive", (it) => {
  it.effect("reuses a warm server across back-to-back requests and closes it after idling", () =>
    Effect.gen(function* () {
      ...
      expect(runtimeMock.state.startCalls).toEqual([OPENCODE_CLI_SPEC.defaultBinaryPath]);
      ...
      expect(runtimeMock.state.managedRootDirs).toEqual([
        expect.stringMatching(/[\\/]userdata[\\/]opencode$/),
      ]);
  ...
  it.effect("starts a new server after the warm server idles out", () =>
    Effect.gen(function* () {
      ...
      expect(runtimeMock.state.startCalls).toEqual([
        OPENCODE_CLI_SPEC.defaultBinaryPath,
        OPENCODE_CLI_SPEC.defaultBinaryPath,
      ]);
      expect(runtimeMock.state.promptUrls).toEqual([
        "http://127.0.0.1:4301",
