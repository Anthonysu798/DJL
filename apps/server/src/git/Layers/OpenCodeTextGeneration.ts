  ...
  Effect.gen(function* () {
    ...
    const runOpenCodeJson = Effect.fn("runOpenCodeJson")(function* <S extends Schema.Top>(input: {
      ...
      const runAgainstServer = (server: Pick<OpenCodeServerConnection, "url" | "serverPassword">) =>
        Effect.tryPromise({
          try: async () => {
            const connectionPassword = server.serverPassword?.trim() || serverPassword;
            const client = openCodeRuntime.createOpenCodeSdkClient({
              baseUrl: server.url,
              directory: input.cwd,
              ...(connectionPassword.length > 0 ? { serverPassword: connectionPassword } : {}),
              cliSpec: config.cliSpec,
            });
            const sessionCreateInput = {
              title: `DJL ${input.operation}`,
              model: {
