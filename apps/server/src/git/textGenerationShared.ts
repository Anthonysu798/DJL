...
export function buildThreadRecapPrompt(input: {
  ...
  return {
    prompt: [
      "You are writing a compact live recap for DJL's chat side panel.",
...
export function buildAutomationIntentPrompt(input: {
  ...
  return {
    prompt: [
      "You extract structured DJL automation creation intents.",
      ...
      "- DJL automations run a saved prompt on a schedule.",
...
export function buildAutomationCompletionEvaluationPrompt(input: {
  ...
  return {
    prompt: [
      "You evaluate whether a completed DJL heartbeat automation should stop.",
