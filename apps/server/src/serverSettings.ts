...
import { canonicalizeDeprecatedDeepSeekModelSelection } from "./provider/deepSeekCompatibility";
...
function resolveTextGenerationProvider(settings: ServerSettings): ServerSettings {
  const canonicalSettings = canonicalizeTextGenerationSelection(settings);
  const selection = canonicalSettings.textGenerationModelSelection;
...
function canonicalizeTextGenerationSelection(settings: ServerSettings): ServerSettings {
  const selection = settings.textGenerationModelSelection;
  const model = canonicalizeDeprecatedDeepSeekModelSelection(selection.provider, selection.model);
  if (model === selection.model) {
    return settings;
  }
  return {
    ...settings,
    textGenerationModelSelection: { ...selection, model } as ModelSelection,
  };
}
...
const makeServerSettings = Effect.gen(function* () {
  ...
  const start = Effect.gen(function* () {
    ...
    const startup = Effect.gen(function* () {
      ...
      const settings = canonicalizeTextGenerationSelection(loadedSettings);
    ...
    updateSettings: (patch) =>
      writeSemaphore.withPermits(1)(
        Effect.gen(function* () {
          ...
          const next = canonicalizeTextGenerationSelection(
            yield* normalizeSettings(settingsPath, current, patch),
          );
