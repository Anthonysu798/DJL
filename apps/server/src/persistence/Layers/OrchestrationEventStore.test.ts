...
layer("OrchestrationEventStore", (it) => {
  ...
  it.effect("canonicalizes incoming deprecated DeepSeek selections before persisting them", () =>
    Effect.gen(function* () {
      const eventStore = yield* OrchestrationEventStore;
      ...
      const appended = yield* eventStore.append({
        type: "thread.created",
        eventId: EventId.makeUnsafe("evt-deepseek-incoming"),
        aggregateKind: "thread",
        aggregateId: ThreadId.makeUnsafe("thread-deepseek-incoming"),
        occurredAt: now,
        commandId: CommandId.makeUnsafe("cmd-deepseek-incoming"),
        causationEventId: null,
        correlationId: null,
        metadata: {},
        payload: {
          threadId: ThreadId.makeUnsafe("thread-deepseek-incoming"),
          projectId: ProjectId.makeUnsafe("project-deepseek-incoming"),
          title: "DeepSeek Incoming",
          modelSelection: {
            provider: "opencode",
      ...
      assert.deepStrictEqual(
        appended.type === "thread.created" ? appended.payload.modelSelection : null,
