...
function normalizeIncomingEventPayload(event: Omit<OrchestrationEvent, "sequence">): unknown {
  return normalizeLegacyEventRow({ ...event, sequence: 0 } as PersistedEventRow).payload;
}
...
const makeEventStore = Effect.gen(function* () {
  ...
  const append: OrchestrationEventStoreShape["append"] = (event) =>
    appendEventRow({
      ...
      payloadJson: normalizeIncomingEventPayload(event),
