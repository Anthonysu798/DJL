...
import { LEGACY_DJL_HOME_DIR_NAME, resolveDjlHome } from "@synara/shared/djlStoragePaths";
import {
  DEFAULT_PORT,
  deriveServerPaths,
...
import { LocalModelsLive } from "./localModels/LocalModelsLive";
...
import { resolveWorkReleaseStage } from "./work/workCapabilities";
...
const CliEnvConfig = Config.all({
  ...
  djlHome: Config.string("DJL_HOME").pipe(Config.option, Config.map(Option.getOrUndefined)),
  ...
  sharedState: Config.boolean("DJL_SHARED_STATE").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  skipStateLock: Config.boolean("DJL_SKIP_STATE_LOCK").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
  migrationSources: Config.string("DJL_MIGRATION_SOURCES").pipe(
    Config.option,
    Config.map(Option.getOrUndefined),
  ),
});
  ...
  Layer.effect(
    ...
    Effect.gen(function* () {
      ...
      const configuredHome =
        Option.getOrUndefined(input.synaraHome) ?? env.djlHome ?? env.synaraHome;
      const baseDir = yield* resolveBaseDir(configuredHome);
      const userHomeDir = OS.homedir();
      const defaultDjlHome = resolveDjlHome({}, userHomeDir);
      const sharedState = env.sharedState ?? false;
      if (sharedState && env.skipStateLock !== true) {
        yield* Effect.acquireRelease(
          Effect.tryPromise({
            try: () => acquireSharedStateLock(baseDir),
            catch: (cause) => new StartupError({ message: String(cause), cause }),
          }),
          (lock) => Effect.promise(() => lock.release()),
        );
      }
      const requestedMigrationSources = (env.migrationSources ?? "")
        .split(nodePath.delimiter)
        .map((source) => source.trim())
        .filter(
          (source) => source.length > 0 && nodePath.resolve(source) !== nodePath.resolve(baseDir),
        );
      const migrationSources = [
        ...requestedMigrationSources,
        nodePath.join(userHomeDir, LEGACY_DJL_HOME_DIR_NAME),
      ];
      const storageMigrations = yield* Effect.tryPromise({
        try: async () => {
          const results = [];
          for (const [index, legacyBaseDir] of migrationSources.entries()) {
            results.push(
              await migrateLegacyDjlStorage({
                ...
                importLegacyDefault:
                  sharedState ||
                  (baseDir === defaultDjlHome && index === migrationSources.length - 1),
                mergeDevelopmentIntoUserdata: sharedState,
              }),
            );
          }
          return results;
          ...
          new StartupError({ message: "Failed to migrate legacy DJL storage", cause }),
      });
      for (const storageMigration of storageMigrations) {
        if (
          storageMigration.copiedRelativePaths.length > 0 ||
          storageMigration.migratedProviderIds.length > 0 ||
          storageMigration.warnings.length > 0
        ) {
          yield* Effect.logInfo("DJL storage migration completed", storageMigration);
        }
      }
      const derivedPaths = yield* deriveServerPaths(baseDir, devUrl, sharedState);
      ...
      const config: ServerConfigShape = {
        ...
        logProviderEvents,
        logWebSocketEvents,
        workReleaseStage: resolveWorkReleaseStage(process.env.DJL_WORK_RELEASE_STAGE),
...
const LayerLive = (input: CliInput) => {
  ...
  const localModelsLayer = LocalModelsLive;
  const providerLayer = makeServerProviderLayer().pipe(
    // Provider turn dispatch performs the local-runtime preflight, so construct
    // the provider graph with the same local-model service exposed to Server.
    Layer.provideMerge(localModelsLayer),
  );
...
const makeServerProgram = (input: CliInput) =>
  Effect.gen(function* () {
    ...
    yield* Effect.logInfo("DJL running", {
      ...safeConfig,
...
const synaraHomeFlag = Flag.string("home-dir").pipe(
  Flag.withDescription("Base directory for all DJL data (equivalent to DJL_HOME)."),
  Flag.optional,
);
...
export const synaraCli = Command.make("synara", {
  ...
  Command.withDescription("Run the DJL server."),
