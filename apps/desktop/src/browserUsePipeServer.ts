...
const BROWSER_USE_UNIX_PIPE_DIR = "/tmp/codex-browser-use";
const BROWSER_USE_WINDOWS_PIPE_PREFIX = String.raw`\\.\pipe\codex-browser-use-`;
...
export function resolveDefaultBrowserUsePipePath(platform = process.platform): string {
  if (platform === "win32") {
    return `${BROWSER_USE_WINDOWS_PIPE_PREFIX}${BROWSER_USE_PIPE_NAME_PREFIX}-${process.pid}`;
  }
  return Path.join(
    BROWSER_USE_UNIX_PIPE_DIR,
    `${BROWSER_USE_PIPE_NAME_PREFIX}-${process.pid}.sock`,
  );
}

function isDiscoverableBrowserUsePipePath(pipePath: string, platform: NodeJS.Platform): boolean {
  if (platform === "win32") {
    return pipePath.startsWith(BROWSER_USE_WINDOWS_PIPE_PREFIX);
  }
  return Path.dirname(pipePath) === BROWSER_USE_UNIX_PIPE_DIR;
}
...
export function resolveConfiguredBrowserUsePipePath(
  ...
  return configured && isDiscoverableBrowserUsePipePath(configured, platform)
    ? configured
    : resolveDefaultBrowserUsePipePath(platform);
}
...
export class BrowserUsePipeServer {
  ...
  private async createTabForSession(sessionId: string): Promise<{
    ...
    if (!snapshot) {
      throw new Error("No active DJL browser pane available");
    }
