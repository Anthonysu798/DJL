...
import {
  BrowserAnnotationCaptureInput as BrowserAnnotationCaptureInputSchema,
  BrowserAnnotationCommandInput as BrowserAnnotationCommandInputSchema,
...
export { BROWSER_IPC_CHANNELS } from "./browserIpcChannels";

export function sendBrowserAnnotationEvent(
  webContents: WebContents | null | undefined,
  event: BrowserAnnotationEvent,
): void {
  webContents?.send(BROWSER_IPC_CHANNELS.annotationEvent, event);
}
...
export function registerBrowserIpcHandlers(
  ...
  ipcMain.removeHandler(BROWSER_IPC_CHANNELS.annotationCommand);
  ipcMain.handle(BROWSER_IPC_CHANNELS.annotationCommand, async (_event, input: unknown) => {
    const validated = Schema.decodeUnknownSync(BrowserAnnotationCommandInputSchema)(input);
    await browserManager.annotationCommand(validated as BrowserAnnotationCommandInput);
  });

  ipcMain.removeHandler(BROWSER_IPC_CHANNELS.captureAnnotation);
  ipcMain.handle(
    BROWSER_IPC_CHANNELS.captureAnnotation,
    async (_event, input: unknown): Promise<BrowserAnnotationCaptureResult> => {
      const validated = Schema.decodeUnknownSync(BrowserAnnotationCaptureInputSchema)(input);
      return browserManager.captureAnnotation(validated as BrowserAnnotationCaptureInput);
    },
  );
