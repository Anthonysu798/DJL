...
describe("showDesktopConfirmDialog", () => {
  ...
  it("opens a dialog for the focused window and returns true on confirm", async () => {
    ...
    const result = await showDesktopConfirmDialog("Delete worktree?", ownerWindow, (key) =>
      key === "dialog.yes" ? "Oui" : "Non",
    );
  ...
  it("opens an app-level dialog when there is no focused window", async () => {
    ...
    const result = await showDesktopConfirmDialog("Delete worktree?", null, (key) =>
      key === "dialog.yes" ? "Yes" : "No",
    );
