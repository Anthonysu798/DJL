...
import { BrowserAnnotationRuntime } from "./browserAnnotationRuntime";
import { desktopT } from "./desktopI18n";
import {
...
const BROWSER_SESSION_PARTITION = "persist:djl-browser";
...
type BrowserAnnotationListener = (event: BrowserAnnotationEvent) => void;
...
export class DesktopBrowserManager {
  ...
  private readonly annotationListeners = new Set<BrowserAnnotationListener>();
  private readonly annotationRuntimes =
    new BrowserAnnotationRuntimePool<BrowserAnnotationRuntime>();
  private readonly annotationEnabledRuntimeKeys = new Set<string>();
  private readonly browserUseCdpLeaseCountByRuntimeKey = new Map<string, number>();
  ...
  subscribeAnnotations(listener: BrowserAnnotationListener): () => void {
    this.annotationListeners.add(listener);
    return () => this.annotationListeners.delete(listener);
  }

  private emitAnnotation(event: BrowserAnnotationEvent): void {
    for (const listener of this.annotationListeners) listener(event);
  }

  private invalidateAnnotationRuntime(threadId: ThreadId, tabId: string): Promise<void> {
    const key = buildRuntimeKey(threadId, tabId);
    const hadRuntime = Boolean(this.annotationRuntimes.get(key));
    const disposal = this.annotationRuntimes.invalidate(key);
    if (hadRuntime) this.emitAnnotation({ type: "cancelled", threadId, tabId });
    return disposal;
  }
  ...
  async annotationCommand(input: BrowserAnnotationCommandInput): Promise<void> {
    const key = buildRuntimeKey(input.threadId, input.tabId);
    if (input.command.type === "enable" || input.command.type === "disable") {
      const enabled = input.command.type === "enable";
      await transitionAnnotationMode({
        enabled,
        setDesired: (desired) => {
          if (desired) this.annotationEnabledRuntimeKeys.add(key);
          else this.annotationEnabledRuntimeKeys.delete(key);
        },
        command: async () => {
          const runtime = enabled
            ? await this.ensureAnnotationRuntime(input.threadId, input.tabId)
            : this.annotationRuntimes.get(key);
          if (runtime) await runtime.command(input.command);
        },
        invalidate: () => this.annotationRuntimes.invalidate(key),
      });
      return;
    }
    const runtime = await this.ensureAnnotationRuntime(input.threadId, input.tabId);
    await runtime.command(input.command);
  }

  async captureAnnotation(
    input: BrowserAnnotationCaptureInput,
  ): Promise<BrowserAnnotationCaptureResult> {
    const state = this.ensureWorkspace(input.threadId);
    const tab = this.resolveTab(state, input.tabId);
    const live = this.ensureLiveRuntime(input.threadId, tab.id);
    const annotations = await this.ensureAnnotationRuntime(input.threadId, tab.id);
    const captured = await annotations.capture(input, async () =>
      (await live.webContents.capturePage()).toPNG(),
    );
    const { pngBytes, metadata } = captured;
    if (pngBytes.byteLength === 0) throw new Error("Couldn't capture the browser annotation.");
    return {
      name: screenshotFileNameForUrl(tab.lastCommittedUrl ?? tab.url).replace(
        /\.png$/,
        `-annotation-${input.selectionId.slice(0, 12)}.png`,
      ),
      mimeType: "image/png",
      sizeBytes: pngBytes.byteLength,
      bytes: Uint8Array.from(pngBytes),
  ...
  private async ensureAnnotationRuntime(
    threadId: ThreadId,
    tabId: string,
  ): Promise<BrowserAnnotationRuntime> {
    const key = buildRuntimeKey(threadId, tabId);
    return this.annotationRuntimes.ensure(key, () => {
      const live = this.ensureLiveRuntime(threadId, tabId);
      return new BrowserAnnotationRuntime(
        live.webContents,
        threadId,
        tabId,
        (event) => this.emitAnnotation(event),
        () => (this.browserUseCdpLeaseCountByRuntimeKey.get(key) ?? 0) === 0,
        desktopT("browser.restoreCommentingAfterPageChange"),
      );
    });
  }
  ...
  subscribeToCdpEvents(
    ...
    const key = buildRuntimeKey(input.threadId, input.tabId);
    this.browserUseCdpLeaseCountByRuntimeKey.set(
      key,
      (this.browserUseCdpLeaseCountByRuntimeKey.get(key) ?? 0) + 1,
    );
    ...
    return () => {
      ...
      const remaining = (this.browserUseCdpLeaseCountByRuntimeKey.get(key) ?? 1) - 1;
      if (remaining > 0) this.browserUseCdpLeaseCountByRuntimeKey.set(key, remaining);
      else this.browserUseCdpLeaseCountByRuntimeKey.delete(key);
    };
  }
  ...
  private configureRuntimeWebContents(runtime: LiveTabRuntime): void {
    ...
    const annotationKey = buildRuntimeKey(threadId, tabId);
    let annotationNavigationRevision = 0;
    const reconcileAnnotationsAfterNavigation = createAnnotationNavigationReconciler({
      isEnabled: () => this.annotationEnabledRuntimeKeys.has(annotationKey),
      invalidate: () => this.invalidateAnnotationRuntime(threadId, tabId),
      onReady: () => {
        this.emitAnnotation({ type: "runtime-ready", threadId, tabId });
      },
      onError: () => {
        if (!this.annotationEnabledRuntimeKeys.delete(annotationKey)) return;
        this.emitAnnotation({
          type: "runtime-error",
          threadId,
          tabId,
          message: desktopT("browser.restoreCommentingAfterNavigation"),
        });
      },
    });

    const domReady = () => {
      reconcileAnnotationsAfterNavigation(annotationNavigationRevision);
    };
    ...
    const didNavigate = () => {
      annotationNavigationRevision += 1;
      void this.invalidateAnnotationRuntime(threadId, tabId);
      this.queueRuntimeStateSync(threadId, tabId);
    };
    ...
    const didNavigateInPage = (_event: Electron.Event, _url: string, isMainFrame: boolean) => {
      if (isMainFrame) annotationNavigationRevision += 1;
      handleAnnotationInPageNavigation({
        isMainFrame,
        reconcile: () => reconcileAnnotationsAfterNavigation(annotationNavigationRevision),
        syncState: () => this.queueRuntimeStateSync(threadId, tabId),
      });
    };
  ...
  private destroyRuntime(threadId: ThreadId, tabId: string): void {
    const key = buildRuntimeKey(threadId, tabId);
    this.annotationEnabledRuntimeKeys.delete(key);
    this.clearTabSuspendTimer(threadId, tabId);
    this.pendingRuntimeSyncs.delete(key);
    this.runtimeLastActiveAtByKey.delete(key);
    this.browserUseCdpLeaseCountByRuntimeKey.delete(key);
    const runtime = this.runtimes.get(key);
    void this.annotationRuntimes.invalidate(key);
    if (!runtime) {
      return;
    }
