...
export const DJL_STORAGE_SNAPSHOT_FILE_NAME = "djl-storage-origin-v1.json";
...
export function resolveDjlStorageSnapshotPath(userDataPath: string): string {
  return Path.join(userDataPath, DJL_STORAGE_SNAPSHOT_FILE_NAME);
}

export function resolveDjlStorageSnapshotReadPath(userDataPath: string): string {
  const canonicalPath = resolveDjlStorageSnapshotPath(userDataPath);
  if (FS.existsSync(canonicalPath)) return canonicalPath;
  return resolveSynaraStorageSnapshotPath(userDataPath);
}
