...
import { resolveAppDisplayName } from "@synara/shared/branding";
...
export function buildMacLauncherMetadata(input) {
  return {
    launcherVersion: LAUNCHER_VERSION,
    appDisplayName: input.appDisplayName,
    appBundleId: input.appBundleId,
    microphoneUsageDescription: input.microphoneUsageDescription,
    sourceAppBundlePath: input.sourceAppBundlePath,
    sourceAppMtimeMs: input.sourceAppMtimeMs,
    iconSha256: createHash("sha256").update(readFileSync(input.iconPath)).digest("hex"),
  };
}
...
function buildMacLauncher(electronBinaryPath) {
  ...
  const expectedMetadata = buildMacLauncherMetadata({
    appDisplayName: APP_DISPLAY_NAME,
    appBundleId: APP_BUNDLE_ID,
    microphoneUsageDescription: MICROPHONE_USAGE_DESCRIPTION,
    sourceAppBundlePath,
