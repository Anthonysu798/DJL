...
async function runCli({ mainImpl = main, consoleImpl = console, exitImpl = process.exit } = {}) {
  try {
    ...
    consoleImpl.error(message.startsWith("[djl]") ? message : `[djl] ${message}`);
    exitImpl(1);
  }
}
...
async function main({
  ...
  if (command === "up") {
    if (platform === "darwin") {
      consoleImpl.log("[djl] Starting bridge and pairing QR...");
  ...
  if (command === "start") {
    ...
    emitResult({
      ...
      message: "[djl] macOS bridge service is running.",
      jsonOutput,
      consoleImpl,
    });
    return;
  ...
  if (command === "restart") {
    ...
    emitResult({
      ...
      message: "[djl] macOS bridge service restarted.",
      jsonOutput,
      consoleImpl,
    });
    return;
  ...
  if (command === "qr" || command === "pair") {
    ...
    consoleImpl.log("[djl] Refreshing bridge pairing QR...");
    deps.printMacOSBridgePairingQr({
      pairingSession: result.pairingSession,
    });
    return;
  ...
  if (command === "stop") {
    ...
    emitResult({
      ...
      message: "[djl] macOS bridge service stopped.",
      jsonOutput,
      consoleImpl,
    });
    return;
    ...
    try {
      if (platform === "darwin") {
        ...
        emitResult({
          ...
          message:
            "[djl] Stopped the macOS bridge service and cleared the saved pairing state. Run `djl up` to pair again.",
          jsonOutput,
          consoleImpl,
        });
      } else {
        ...
        emitResult({
          ...
          message: "[djl] Cleared the saved pairing state. Run `djl up` to pair again.",
          jsonOutput,
          consoleImpl,
        });
      }
      ...
      consoleImpl.error(
        `[djl] ${(error && error.message) || "Failed to clear the saved pairing state."}`,
      );
      exitImpl(1);
    }
    return;
  ...
  if (command === "resume") {
    try {
      ...
      emitResult({
        ...
        message: `[djl] Opened last active thread: ${state.threadId} (${state.source || "unknown"})`,
        jsonOutput,
        consoleImpl,
      });
    } catch (error) {
      consoleImpl.error(`[djl] ${(error && error.message) || "Failed to reopen the last thread."}`);
      exitImpl(1);
    }
    return;
  }
  ...
  if (command === "watch") {
    try {
      ...
      consoleImpl.error(
        `[djl] ${(error && error.message) || "Failed to watch the thread rollout."}`,
      );
      exitImpl(1);
    }
    return;
  ...
  consoleImpl.error(
    "Usage: djl up | djl run | djl start | djl restart | djl qr | djl pair | djl stop | djl status | " +
      "djl reset-pairing | djl resume | djl watch [threadId] | djl --version | " +
...
function assertMacOSCommand(
  ...
  consoleImpl.error(
    `[djl] \`${name}\` is only available on macOS. Use \`djl up\` or \`djl run\` for the foreground bridge on this OS.`,
  );
  exitImpl(1);
}
