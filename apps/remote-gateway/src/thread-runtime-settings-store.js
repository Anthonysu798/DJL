...
const DEFAULT_STORE_DIR = path.join(os.homedir(), ".djl");

function createThreadRuntimeSettingsStore({
  storeFile = process.env.DJL_THREAD_RUNTIME_STATE_FILE ||
    path.join(
      process.env.DJL_DEVICE_STATE_DIR || DEFAULT_STORE_DIR,
      "thread-runtime-settings.json",
    ),
  fsImpl = fs,
  now = () => Date.now(),
  ...
  function attachToConversation(threadId, conversation) {
    ...
    conversation.djlRuntimeSettings = settings;
    if (settings.model) {
    ...
    conversation.latestCollaborationMode = {
      ...
      settings: {
        ...
        reasoning_effort:
          settings.reasoningEffort || collaborationSettings?.reasoning_effort || null,
      },
    };
    return conversation;
  }
  ...
  function enrichResponse(method, envelope) {
    ...
    if (method === "thread/list") {
      const key = ["data", "items", "threads"].find((candidate) =>
...
function runtimeSettingsFromTurnParams(turnParams, previous = null) {
  ...
  const collaborationSettings =
    params.collaborationMode?.settings || params.collaboration_mode?.settings || {};
  const model =
  ...
  const reasoningEffort =
    ...
    null;
  const serviceTier =
    normalizeString(params.serviceTier) || normalizeString(params.service_tier) || null;
...
function normalizeState(rawState) {
  const rawThreads =
    rawState?.threads && typeof rawState.threads === "object" ? rawState.threads : {};
  const threads = {};
