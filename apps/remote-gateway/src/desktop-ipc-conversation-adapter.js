...
function resolveTurnForConversation({
  ...
  const fallbackTurnId =
    readFallbackTurnId(conversation?.id, fallbackTurnIdsByThreadId) ||
...
function promoteFallbackTurnId(conversation, fallbackTurnIdsByThreadId, explicitTurnId) {
  ...
  const fallbackTurn = conversation.turns.find(
    (candidate) => (readString(candidate?.turnId) || readString(candidate?.id)) === fallbackTurnId,
  );
  if (fallbackTurn) {
    fallbackTurn.id = explicitTurnId;
    fallbackTurn.turnId = explicitTurnId;
    delete fallbackTurn.djlSyntheticTurnId;
  }
  fallbackTurnIdsByThreadId?.delete(conversationId);
}
...
function isOptimisticPendingTurn(turn) {
  return turn?.djlOptimisticPendingTurn === true;
}
...
function createSyntheticTurnId(threadId, now = () => Date.now()) {
  ...
  return `djl-live-turn:${normalizedThreadId}:${now()}:${randomUUID()}`;
}
...
function applyAppServerMessageToConversationState({
  ...
  switch (method) {
    case "thread/started": {
      ...
      conversations.set(
        threadId,
        buildConversationStateFromThread(thread, {
          previous,
          hostId,
          now,
        }),
    ...
    case "thread/name/updated": {
      ...
      const conversation = ensureConversationInMap(conversations, threadId, { hostId, now });
      conversation.title =
        readString(message.params?.threadName) ||
...
function buildConversationStateFromThread(
  thread,
  { previous = null, hostId = LOCAL_HOST_ID, now = () => Date.now() } = {},
) {
  const threadId = readString(thread?.id);
  ...
  const latestModel =
    readString(thread?.model) || readString(thread?.modelProvider) || previous?.latestModel || "";
  ...
  return {
    ...
    unreadMessageCount: Number.isFinite(previous?.unreadMessageCount)
      ? previous.unreadMessageCount
      : 0,
...
function mergeConversationTurnsFromThread(
  threadTurns,
  { previousTurns = [], threadId = "", cwd = "", now = () => Date.now() } = {},
) {
...
function createEmptyConversationState(
  threadId,
  { hostId = LOCAL_HOST_ID, now = () => Date.now(), cwd = "" } = {},
) {
  const timestamp = now();
  return {
    id: threadId,
...
function buildConversationTurn(
  turn,
  { threadId = "", cwd = "", previousTurn = null, now = () => Date.now() } = {},
) {
  const turnId = readString(turn?.id) || readString(turn?.turnId) || readString(turn?.turn_id);
  ...
  const builtTurn = {
    ...
    params,
    turnStartedAtMs:
      timestampSecondsToMs(turn?.startedAt) || previousTurn?.turnStartedAtMs || now(),
...
function sanitizeUserMessageItem(item) {
  ...
  const sanitizedContent = sanitizeUserInputEntries(
...
function normalizeDesktopItemCompatibility(item) {
  ...
  const receiverThreads = Array.isArray(item.receiverThreads) ? item.receiverThreads : [];
...
function adoptInitialPromptUserMessage(turn, item) {
  ...
  const input = item.content.map(userMessageContentFromTurnInput).filter(Boolean);
  if (input.length === 0) {
    return false;
  }
  turn.params = {
...
function upsertTurn(conversation, turn, { now = () => Date.now() } = {}) {
  ...
  const index = conversation.turns.findIndex(
    (candidate) => (readString(candidate?.turnId) || readString(candidate?.id)) === turnId,
  );
...
function ensureTurn(
  conversation,
  turnId,
  { now = () => Date.now(), allowLastTurnFallback = true } = {},
) {
  const normalizedTurnId = readString(turnId);
  if (!normalizedTurnId) {
  ...
  let turn = conversation.turns.find(
    (candidate) =>
      (readString(candidate?.turnId) || readString(candidate?.id)) === normalizedTurnId,
  );
  if (!turn) {
...
function upsertItem(turn, item) {
  ...
  if (isUserMessageItem(sanitizedItem) && !turnHasUserMessageItem(turn)) {
    ...
    if (
      !extractUserText(turn?.params?.input) &&
      adoptInitialPromptUserMessage(turn, sanitizedItem)
    ) {
      return;
    }
  }
...
function upsertRequest(conversation, request) {
  ...
  const index = conversation.requests.findIndex(
    (candidate) => requestIdKey(candidate?.id) === requestId,
  );
...
function applyDeltaNotification(
  conversation,
  method,
  params,
  { fallbackTurnIdsByThreadId = null, allowOptimisticFallback = true, now = () => Date.now() } = {},
) {
  const turn = ensureTurn(
    conversation,
    resolveTurnIdForParams({
...
function normalizeThreadGoal(value, fallbackThreadId = "") {
  ...
  const threadId =
    readString(value.threadId) || readString(value.thread_id) || readString(fallbackThreadId);
