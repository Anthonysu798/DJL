...
// Purpose: Persists the latest active DJL thread so the user can reopen it on the Mac for handoff.
// Layer: CLI helper
...
const STATE_DIR = path.join(os.homedir(), ".djl");
...
function openLastActiveThread({ bundleId = DEFAULT_BUNDLE_ID } = {}) {
  ...
  if (!threadId) {
    throw new Error("No remembered DJL thread found yet.");
  }
