...
// Purpose: Detects, installs, and updates the global Codex CLI with transparent logs for DJL installs and startup flows.
// Layer: CLI helper
...
const SKIP_BOOTSTRAP_ENV_NAME = "DJL_SKIP_CODEX_BOOTSTRAP";
...
function ensureCodexCLI({
  ...
  if (shouldSkipCodexBootstrap(env)) {
    logInfo(logger, `[djl] Skipping Codex CLI bootstrap because ${SKIP_BOOTSTRAP_ENV_NAME}=1.`);
    return {
      status: "skipped",
      versionBefore: null,
      versionAfter: null,
  ...
  logInfo(logger, "[djl] Checking Codex CLI...");
  ...
  if (versionBefore) {
    logInfo(logger, `[djl] Codex CLI found (${versionBefore}).`);
  } else {
    logInfo(logger, "[djl] Codex CLI not found.");
  }
  ...
  if (!npmVersion) {
    logWarn(
      logger,
      "[djl] npm is unavailable, so DJL could not install or update the Codex CLI automatically.",
    );
    return {
      status: "failed",
      versionBefore,
  ...
  logInfo(logger, `[djl] ${actionVerb} Codex CLI via npm (${CODEX_PACKAGE_SPEC})...`);
  ...
  try {
    ...
    logWarn(logger, `[djl] Codex CLI ${versionBefore ? "update" : "install"} failed. ${message}`);
    return {
      status: "failed",
      versionBefore,
      versionAfter: versionBefore,
  ...
  if (!versionAfter) {
    logWarn(
      logger,
      "[djl] Codex CLI bootstrap finished, but `codex --version` is still unavailable in this shell.",
    );
    return {
      status: "failed",
      versionBefore,
  ...
  logInfo(logger, `[djl] Codex CLI ${versionBefore ? "updated" : "installed"} (${versionAfter}).`);
  return {
