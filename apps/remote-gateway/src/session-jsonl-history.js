...
const JSONL_OLDER_HANDOFF_CURSOR = "djl-jsonl-fallback-older-unavailable";
...
function readThreadTurnsListPageFromSessionJsonl(
  ...
  return {
    data: pageTurns,
    nextCursor:
      recent.hasOlderTurns || turns.length > pageTurns.length ? JSONL_OLDER_HANDOFF_CURSOR : null,
...
function readRecentSessionJsonlTurns(
  filePath,
  {
    threadId = "",
    limit = 5,
  ...
  if (!supportsBoundedSessionJsonlReads(fsModule)) {
    ...
    return turns.length > 0
      ? { turns, hasOlderTurns: false, bytesRead: Buffer.byteLength(content, "utf8") }
      : null;
  }
...
function readSessionJsonlMetadataFromFile(
  filePath,
  { fsModule = fs, metadataHeadBytes = DEFAULT_SESSION_JSONL_METADATA_HEAD_BYTES } = {},
) {
  if (!filePath) {
    return { threadId: "", cwd: "" };
  }
...
function readSessionJsonlRange(fileHandle, start, length, fsModule) {
  ...
  const bytesRead =
    safeLength > 0 ? fsModule.readSync(fileHandle, buffer, 0, safeLength, start) : 0;
...
function observedTaskStartedTurnIDs(
  content,
  { sourceByteOffset = 0, sourceLineByteOffsets = null } = {},
) {
  ...
  while (lineStart < raw.length) {
    ...
    const sourceLineNumber = sourceLineByteOffsets?.[lineIndex] ?? fallbackSourceByteOffset;
...
function parseSessionJsonlTurns(
  content,
  {
    threadId = "",
  ...
  while (lineStart < raw.length) {
    ...
    const currentLineStart = lineStart;
    const lineEndWithSeparator = lineEnd < raw.length ? lineEnd + 1 : lineEnd;
    const sourceLineNumber = sourceLineByteOffsets?.[index] ?? fallbackSourceByteOffset;
    fallbackSourceByteOffset += Buffer.byteLength(
      raw.substring(currentLineStart, lineEndWithSeparator),
      "utf8",
    );
...
function isUserHistoryItem(item) {
  return isUserRoleItem(item) || normalizeString(item?.role).toLowerCase() === "user";
}
...
function shouldSkipDuplicateProposedPlanMessage(turn, item) {
  ...
  return turn.items.some(
    (candidate) =>
      normalizeHistoryToken(candidate?.type) === "plan" && candidate?.djlJsonlProgressPlan !== true,
  );
}
...
function applyHistoryAssistantSourceAlias(item, turnId, occurrencesByBaseKey = new Map()) {
  if (
    normalizeHistoryToken(item?.type) !== "message" ||
    normalizeString(item?.role).toLowerCase() !== "assistant"
  ) {
    return item;
  }
  const sourceKey = buildDJLSourceItemKey(turnId, responseItemMessageText(item));
  ...
  if (sourceKey && occurrence === 1) {
    item.djlSourceItemKey = sourceKey;
  }
  return item;
}
