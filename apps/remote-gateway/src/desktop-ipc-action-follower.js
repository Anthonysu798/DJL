...
const DJL_LIVE_OWNER_SOURCE = "desktop-ipc-live-owner";
...
const DESKTOP_TURNS_CURSOR_PREFIX = "djl-desktop-turns:";
...
function buildDesktopTurnsListResult(turns, params = {}) {
  ...
  if (cursor) {
    const parsedCursor = parseDesktopTurnsCursor(cursor, direction, snapshotRevision, orderedTurns);
...
function parseDesktopTurnsCursor(cursor, direction, snapshotRevision, orderedTurns) {
  ...
  if (anchor.startsWith("id:")) {
    ...
    const anchorIndex = orderedTurns.findIndex(
      (turn) =>
...
function createDesktopIpcActionFollower({
  ...
  function oldestEvictableActiveThreadId() {
    for (const threadId of activeThreadIds) {
      if (
        !hasPendingProjectedActions(threadId) &&
    ...
    if (DESKTOP_STATE_READ_METHODS.has(method)) {
      ...
      if (threadId && backgroundOnlyThreadIds.delete(threadId)) {
        ...
        if (rawState) {
          ...
          const hasCanonicalNormalizedHistory =
          ...
          if (hasCanonicalNormalizedHistory) {
            ...
            sendApplicationResponse(
              JSON.stringify({
                ...
                params: {
                  ...
                  djlActionSource: DESKTOP_IPC_ACTION_SOURCE,
                },
              }),
            );
            ...
            emitNormalizedReviewOverlays(threadId, normalizedLiveIndexesByThreadId.get(threadId));
  ...
  function onEnvelope(envelope) {
    ...
    if (isDJLLiveOwnerBroadcast(params)) {
      if (threadId) {
        if (params.djlOwnerReleased === true) {
  ...
  function commitConversationState(
    threadId,
    nextState,
    { isFullSnapshot = false, change = null } = {},
  ) {
    ...
    for (const entry of queue) {
      ...
      if (readString(originalMessage?.method) === "turn/start") {
        if (releasedTurnStart) {
          rejectHeldFollowerRequest(
            originalMessage,
            "Superseded by another held turn/start request.",
          );
          continue;
        }
  ...
  function tryServeDesktopOwnedRead(message) {
    ...
    const threadId = readThreadId(message.params);
    const ownsDesktopCursor =
      method === "thread/turns/list" && isDesktopTurnsCursor(message.params?.cursor);
    if (!threadId) {
      return false;
    }
    if (liveOwnerThreadIds.has(threadId) || isLocallyOwnedThread(threadId)) {
    ...
    if (
      hasNormalizedHistoryOutsideRawTurns(rawState, normalizedLiveIndexesByThreadId.get(threadId))
    ) {
      canonicalHistoryThreadIds.add(threadId);
    }
    ...
    const result =
      method === "thread/turns/list"
        ...
        : {
            ...
            thread,
            ...(method === "thread/resume" ? { djlDesktopIpcMirror: true } : {}),
          };
    if (!result) {
      return ownsDesktopCursor ? rejectDesktopTurnsCursor(message) : false;
    }
    ...
    sendApplicationResponse(
      JSON.stringify({
        ...
        error: {
          code: -32602,
          message:
            "Desktop history changed while paging. Reload this thread to restart history pagination.",
        },
      }),
    );
    return true;
  ...
  function isDesktopLiveTurnStateSnapshotRequest(message) {
    if (
      readString(message?.method) !== "thread/turns/list" ||
      readString(message?.params?.cursor) ||
      message?.params?.djlRequireCanonical === true
    ) {
      return false;
    }
    if (message?.params?.djlTurnStateOnly === true) {
      return true;
    }
    // DJL iPhone 2.1 predates the explicit marker. Its running-state probe
    // has this unique request shape; actual history pages use limits 1 and 5.
    return (
      Number(message?.params?.limit) === 8 &&
  ...
  function hasNormalizedHistoryOutsideRawTurns(rawState, normalizedIndex = null) {
    ...
    const rawTurnIds = new Set(
      (Array.isArray(rawState?.turns) ? rawState.turns : [])
        .map(
          (turn) => readString(turn?.id) || readString(turn?.turnId) || readString(turn?.turn_id),
        )
        .filter(Boolean),
    );
  ...
  function emitDesktopSnapshotLifecycleTransition(threadId, liveState) {
    ...
    const { previousTurnIds: continuityPreviousTurnIds, nextTurnIds: continuityNextTurnIds } =
      matchDesktopTurnIdentityContinuities([...previousById.values()], nextTurns);
    for (const previous of previousById.values()) {
      ...
      sendApplicationResponse(
        JSON.stringify(
          desktopLiveTurnLifecycleNotification("turn/completed", threadId, {
            id: previous.id,
  ...
  function syncProjectedConversationState(threadId, nextState, { isFullSnapshot = false } = {}) {
    ...
    if (
      !canonicalHistoryThreadIds.has(threadId) &&
      hasNormalizedHistoryOutsideRawTurns(nextState, normalizedLiveIndexesByThreadId.get(threadId))
    ) {
      canonicalHistoryThreadIds.add(threadId);
    }
    ...
    for (const notification of output.notifications || []) {
      const preservesTurnIdentity =
        notification.method === "turn/started" &&
        output.turnIdentityContinuityTurnIds?.includes(readString(notification.params?.turnId));
  ...
  function emitNormalizedReviewOverlays(threadId, index) {
    ...
    for (const overlay of overlays) {
      ...
      sendApplicationResponse(
        JSON.stringify({
          method:
            normalizeToken(status) === "inprogress"
          ...
          params: {
            ...
            decisionSource:
              readString(item.decisionSource) || readString(item?.event?.decision_source) || null,
            review: cloneJSON(review),
            action: cloneJSON(item.action),
            djlDesktopMirror: true,
            djlDesktopIpcMirror: true,
            djlGuardianRetrySupported: false,
            djlActionSource: DESKTOP_IPC_ACTION_SOURCE,
          },
        }),
      );
  ...
  function syncBackgroundThreadLifecycle(threadId, _previousState, nextState) {
    ...
    if (previousTurnId && previousTurnId !== nextTurnId) {
      const settledTurn = backgroundRawTurnById(liveState, previousTurnId) || announcedTurn;
      const settledStatus = readString(settledTurn?.status);
      settleAnnouncedBackgroundTurn(
        threadId,
        settledStatus === "failed" || settledStatus === "interrupted" ? settledStatus : "completed",
        settledTurn,
      );
    }
    ...
    if (nextTurnId && nextTurnId !== previousTurnId) {
      sendApplicationResponse(
        JSON.stringify(
          backgroundTurnLifecycleNotification("turn/started", threadId, nextActiveTurn),
        ),
      );
  ...
  function settleAnnouncedBackgroundTurn(threadId, status = "interrupted", turn = null) {
    ...
    sendApplicationResponse(
      JSON.stringify(backgroundTurnLifecycleNotification("turn/completed", threadId, settledTurn)),
    );
    announcedBackgroundTurnsByThreadId.delete(threadId);
    clearBackgroundDisconnectTimer(threadId);
    return true;
  ...
  function submitDesktopActionResponse(route, responseMessage) {
    ...
    ipc
      ...
      .then(() => {
        ...
        sendApplicationResponse(
          JSON.stringify(projectedResolvedNotification(route.threadId, route.requestId)),
        );
      })
      .catch((error) => {
        console.warn(
    ...
    Promise.resolve()
      ...
      .catch((error) => {
        if (
          ...
          console.warn(
            `${logPrefix} desktop IPC baseline recovery failed for ${threadId} (attempt ${recoveryState.attempts}/${MAX_BASELINE_RECOVERY_ATTEMPTS}): ${error.message}`,
          );
        }
  ...
  return {
    ...
    hasLiveThreadState(threadId) {
      ...
      const thread = projectDesktopConversationStateToThread(normalizedThreadId, liveState, {
        now,
      });
...
function createDesktopIpcClient({
  ...
  function ensureConnected() {
    ...
    nextSocket.on("connect", () => {
      isConnecting = false;
      sendRequest("initialize", { clientType: "djl-bridge" })
        .then((result) => {
  ...
  function sendRequest(method, params) {
    ...
    const requestId = `djl-${now().toString(36)}-${Math.random().toString(16).slice(2)}`;
    const envelope = {
      type: "request",
      requestId,
      sourceClientId: method === "initialize" ? "initializing-client" : clientId || "djl-bridge",
  ...
  function sendDiscoveryRequest(request, timeoutMs) {
    ...
    const requestId = `djl-discovery-${now().toString(36)}-${Math.random().toString(16).slice(2)}`;
...
function refreshTouchedNormalizedActiveTurns(index, state, change) {
  ...
  for (const patch of Array.isArray(change?.patches) ? change.patches : []) {
    ...
    if (
      (path[0] === "turnHistory" || path[0] === "turn_history") &&
      path[1] === "history" &&
    ...
    if (
      ...
      const item =
        Number.isInteger(path[5]) && Array.isArray(turn?.items) ? turn.items[path[5]] : null;
      const isStructuralItemPatch = path.length <= 6 || path[6] === "type";
      if (
        turnId &&
        (isStructuralItemPatch || normalizeToken(item?.type) === "automaticapprovalreview")
      ) {
...
function boundedDesktopLiveTurns(
  ...
  if (normalizedIndex) {
    return boundedIndexedDesktopLiveTurns(state, normalizedIndex, nowValue, retainedTurnIds);
  }
...
function boundedIndexedDesktopLiveTurns(state, index, nowValue, retainedTurnIds) {
  ...
  freshActiveTurnIds.sort(
    (left, right) => index.entryIndexByTurnId.get(left) - index.entryIndexByTurnId.get(right),
  );
...
function withStableProjectedTurnId(turn, fullHistoryIndex) {
  return turnIdOf(turn) ? turn : { ...turn, id: `ipc-turn-${fullHistoryIndex}` };
}
...
function desktopLiveTurnLifecycleNotification(method, threadId, turn) {
  const status =
    readString(turn?.status) || (method === "turn/started" ? "inProgress" : "completed");
  return {
    method,
    params: {
      threadId,
      ...
      djlBackgroundDiscovery: method === "turn/started",
      djlActionSource: DESKTOP_IPC_ACTION_SOURCE,
    },
  };
}
...
function normalizeBoundedTurnsForRuntime(turns, state) {
  ...
  return turns.map((turn) => (isActiveRawTurn(turn) ? { ...turn, status: "completed" } : turn));
}
...
function isRawTurnActivityFresh(turn, nowValue) {
  ...
  const activityAt =
    Number.isFinite(durationMs) && durationMs >= 0 ? startedAt + durationMs : startedAt;
...
function compactBackgroundPromptEntries(entries) {
  ...
  return entries.flatMap((entry) => {
    ...
    const text = readString(entry.text) || readString(entry.message) || readString(entry.content);
    return text ? [{ text }] : [];
  });
}
...
function isDJLLiveOwnerBroadcast(params) {
  return readString(params?.djlOwnerSource) === DJL_LIVE_OWNER_SOURCE;
}
...
function isPeerOwnershipSnapshot(params) {
  return !isDJLLiveOwnerBroadcast(params) && normalizeToken(params?.change?.type) === "snapshot";
}

function markDeliveryFailureError(error) {
  error.djlDeliveryFailed = true;
  return error;
}

function isDeliveryFailureError(error) {
  return error?.djlDeliveryFailed === true;
}
