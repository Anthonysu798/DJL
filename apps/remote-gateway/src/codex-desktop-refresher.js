...
class CodexDesktopRefresher {
  ...
  // Keeps one lightweight rollout watcher alive for the current DJL-controlled thread.
...
function readBridgeConfig({
  ...
  const relayUrl = readFirstDefinedEnv(["DJL_RELAY"], defaultRelayUrl, env);
  const configuredPushServiceUrl = readFirstDefinedEnv(["DJL_PUSH_SERVICE_URL"], "", env);
  const defaultPushServiceUrl = sourceCheckout ? "" : privateDefaults.pushServiceUrl;
  const pushServiceUrl =
    configuredPushServiceUrl || defaultPushServiceUrl || derivePushServiceUrl(relayUrl);
  const codexEndpoint = readFirstDefinedEnv(["DJL_CODEX_ENDPOINT"], "", env);
  // This capability is injected only by DJL Electron's main process. It is a
  // loopback URL authenticated with Electron's ephemeral backend token and is
  // never sent to the relay or to the paired phone.
  const electronBackendEndpoint = readFirstDefinedEnv(["DJL_ELECTRON_BACKEND_ENDPOINT"], "", env);
  const refreshCommand = readFirstDefinedEnv(
    ["DJL_REFRESH_COMMAND", "DJL_ON_PHONE_MESSAGE"],
    "",
    env,
  );
  const explicitRefreshEnabled = readOptionalBooleanEnv(["DJL_REFRESH_ENABLED"], env);
  const explicitDesktopIpcEnabled = readOptionalBooleanEnv(["DJL_DESKTOP_IPC_ENABLED"], env);
  const explicitDesktopIpcLiveSyncEnabled = readOptionalBooleanEnv(
    ["DJL_DESKTOP_IPC_LIVE_SYNC"],
    env,
  );
  const explicitRemoteThreadScopeEnabled = readOptionalBooleanEnv(
    ["DJL_REMOTE_THREAD_SCOPE_ENABLED"],
    env,
  );
  const explicitKeepMacAwakeEnabled = readOptionalBooleanEnv(["DJL_KEEP_MAC_AWAKE"], env);
  ...
  // Once opted in, the persisted choice must survive restarts: `djl restart`
  ...
  const persistedRefreshEnabled =
    typeof daemonConfig.refreshEnabled === "boolean" ? daemonConfig.refreshEnabled : null;
  ...
  return {
    ...
    pushPreviewMaxChars: parseIntegerEnv(
      readFirstDefinedEnv(["DJL_PUSH_PREVIEW_MAX_CHARS"], "160", env),
      160,
    ),
    refreshEnabled: explicitRefreshEnabled == null ? defaultRefreshEnabled : explicitRefreshEnabled,
    refreshDebounceMs: parseIntegerEnv(
      readFirstDefinedEnv(["DJL_REFRESH_DEBOUNCE_MS"], String(DEFAULT_DEBOUNCE_MS), env),
      DEFAULT_DEBOUNCE_MS,
    ),
    keepMacAwakeEnabled:
    ...
    desktopIpcEnabled: explicitDesktopIpcEnabled == null ? true : explicitDesktopIpcEnabled,
    desktopIpcSocketPath: readFirstDefinedEnv(["DJL_DESKTOP_IPC_SOCKET"], "", env),
    desktopIpcLiveSyncEnabled:
      explicitDesktopIpcLiveSyncEnabled == null ? true : explicitDesktopIpcLiveSyncEnabled,
    remoteThreadScopeEnabled: explicitRemoteThreadScopeEnabled === true,
    remoteThreadScopeStatePath: readFirstDefinedEnv(["DJL_REMOTE_THREAD_SCOPE_PATH"], "", env),
    desktopIpcSnapshotDebounceMs: parseIntegerEnv(
      readFirstDefinedEnv(["DJL_DESKTOP_IPC_SNAPSHOT_DEBOUNCE_MS"], "75", env),
      75,
    ),
    refreshCommand,
    codexBundleId: readFirstDefinedEnv(["DJL_CODEX_BUNDLE_ID"], DEFAULT_BUNDLE_ID, env),
...
function isSourceCheckout(runtimeRoot, fsImpl) {
  ...
  return (
    path.basename(runtimeRoot) === "djl-bridge" && fsImpl.existsSync(path.join(repoRoot, ".git"))
  );
}
...
function derivePushServiceUrl(relayUrl) {
