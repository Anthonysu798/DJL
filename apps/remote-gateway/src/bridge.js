...
const { createRemoteOwnedThreadScope } = require("./remote-owned-thread-scope");
...
const RELAY_HISTORY_IMAGE_REFERENCE_URL = "djl://history-image-elided";
...
const JSONL_CANONICAL_HANDOFF_CURSOR_PREFIX = "djl-jsonl-handoff-v1:";
...
function createThreadTurnsListFastPageCoordinator({
  ...
  function readHandoffEntry(request) {
    ...
    const token =
      threadTurnsListHandoffDescriptor(cursor)?.token ||
  ...
  async function extendCanonicalResponseThroughAnchor(
    ...
    for (let pageIndex = 0; pageIndex < maxPages && hasRelayCursor(cursor); pageIndex += 1) {
      ...
      for (const turn of nextResult[nextTurnsKey]) {
        ...
        if (
          !turnId ||
          !combinedTurns.some((existing) => turnListTurnIdentifier(existing) === turnId)
        ) {
  ...
  async function resolveCanonicalRequest(
    request,
    fetchCanonical,
    existingEntry = null,
    { alignToHandoffAnchor = false, validateHandoffAnchor = false, handoffAnchorTurnId = "" } = {},
  ) {
    ...
    const canMatchCanonicalAnchor = anchorTurnId && !isSyntheticJsonlHistoryTurnId(anchorTurnId);
    if (entry) {
      const observedOutcomePromise = entry.canonicalOutcomePromise;
      const firstOutcome = await observedOutcomePromise;
      const firstResponseIsUsable =
        firstOutcome.ok && !isEmptyTurnsListResponse(firstOutcome.response);
  ...
  async function resolve(request, { fetchCanonical, readJsonl }) {
    ...
    const isHandoffRequest = cursor === JSONL_OLDER_HANDOFF_CURSOR || Boolean(handoffDescriptor);
    const requiresCanonical = params.djlRequireCanonical === true;
...
function canonicalThreadTurnsListRequest(request) {
  ...
  if (
    params.cursor === JSONL_OLDER_HANDOFF_CURSOR ||
    threadTurnsListHandoffDescriptor(params.cursor)
  ) {
...
function rebindThreadTurnsListResponseId(response, requestId) {
  return response && typeof response === "object" ? { ...response, id: requestId } : response;
}
...
function buildJsonlCanonicalHandoffResponse(response, requestId, token, anchorTurnId = "") {
  ...
  return {
    ...
    result: {
      ...
      djlJsonlFallback: true,
      djlCanonicalHandoff: true,
    },
  };
}
...
function shouldPreferJsonlFirstPage(canonicalResponse, jsonlResponse) {
  ...
  return (
    Boolean(jsonlTurnId) &&
    !canonicalResult[canonicalTurnsKey].some(
      (turn) => turnListTurnIdentifier(turn) === jsonlTurnId,
    ) &&
...
function isCoherentJsonlFirstPageResponse(response) {
  ...
  const status = String(newestTurn?.status || "")
    .replace(/[_-]/g, "")
    .toLowerCase();
  const isExplicitTerminal = new Set([
    "completed",
    "failed",
    "aborted",
    "cancelled",
...
function threadTurnsListResponseContainsAnchor(response, anchorTurnId) {
  ...
  return (
    Boolean(turnsKey) &&
    result[turnsKey].some((turn) => turnListTurnIdentifier(turn) === anchorTurnId)
  );
}
...
function alignThreadTurnsListResponseToAnchor(response, anchorTurnId) {
  ...
  const anchorIndex = result[turnsKey].findIndex(
    (turn) => turnListTurnIdentifier(turn) === anchorTurnId,
  );
  if (anchorIndex < 0) {
    return null;
  }
...
function shouldForwardBridgeManagedInitialize({ codexHandshakeState, usesElectronBackend }) {
  // The Electron adapter is a stateless compatibility layer and deliberately
  // accepts initialize on every phone connection. Forwarding lets its current
  // capabilities reach iOS even though the embedded backend is already warm.
  return usesElectronBackend || codexHandshakeState !== "warm";
}
...
function startBridge({
  ...
  const usesElectronBackend = Boolean(config.electronBackendEndpoint);
  if (usesElectronBackend || process.env.DJL_ELECTRON_BRIDGE_DIAGNOSTICS === "1") {
    console.warn(
      `[djl-electron-bridge] startup transport=${usesElectronBackend ? "electron" : "non-electron"} desktop-ipc=${config.desktopIpcEnabled === false ? "disabled" : "enabled"} refresh=${config.refreshEnabled === true ? "enabled" : "disabled"}`,
    );
  }
  ...
  if (!relayBaseUrl) {
    console.error("[djl] No relay URL configured.");
    console.error("[djl] In a source checkout, run ./run-local-djl.sh or set DJL_RELAY.");
    process.exit(1);
  }
  ...
  try {
    ...
    console.error(
      `[djl] ${(error && error.message) || "Failed to load the saved bridge pairing state."}`,
    );
    process.exit(1);
  }
  ...
  let codexLaunchState = config.codexEndpoint || usesElectronBackend ? "connected" : "starting";
  let codexHandshakeState = config.codexEndpoint || usesElectronBackend ? "warm" : "cold";
  ...
  const remoteThreadScope = createRemoteOwnedThreadScope({
    enabled: config.remoteThreadScopeEnabled === true,
    statePath: config.remoteThreadScopeStatePath || undefined,
  });
  ...
  const secureTransport = createBridgeSecureTransport({
    ...
    // Electron Dev enables this explicitly from main.ts. Do not turn secure
    // transport tracing on for every packaged Electron session.
  ...
  const rolloutLiveMirror =
    !config.codexEndpoint && !usesElectronBackend
      ? createRolloutLiveMirrorController({
          ...
          shouldSuppressThread: (threadId) =>
            shouldSuppressRolloutMirrorForThread(threadId, {
              desktopIpcActionFollower,
              desktopIpcLiveOwner,
            }),
        })
      : null;
  const desktopIpcEnabled = config.desktopIpcEnabled !== false;
  const desktopIpcActionFollower =
    !config.codexEndpoint && !usesElectronBackend && desktopIpcEnabled
      ? createDesktopIpcActionFollower({
          sendApplicationResponse,
  ...
  codex.onError((error) => {
    ...
    if (config.codexEndpoint) {
      console.error(`[djl] Failed to connect to Codex endpoint: ${config.codexEndpoint}`);
    } else if (usesElectronBackend) {
      console.error("[djl] Failed to connect to the DJL Electron embedded backend.");
    } else {
      console.error("[djl] Failed to start `codex app-server`.");
      console.error(`[djl] Launch command: ${codex.describe()}`);
      console.error(
        "[djl] Make sure the Codex CLI is installed, authenticated, and launchable on this OS.",
      );
    }
  ...
  function startRelayWatchdog(trackedSocket) {
    ...
    relayWatchdogTimer = setInterval(() => {
      ...
      if (hasRelayConnectionGoneStale(lastRelayActivityAt)) {
        console.warn("[djl] relay heartbeat stalled; forcing reconnect");
  ...
  function connectRelay() {
    ...
    const nextSocket = new WebSocket(relaySessionUrl, {
      ...
      headers: buildMacConnectionHeaders(deviceState, pairingSession, notificationSecret),
    });
    socket = nextSocket;
  ...
  codex.onMessage((message) => {
    ...
    remoteThreadScope.observeOutbound(parsedMessage);
  ...
  function handleApplicationMessage(rawMessage) {
    ...
    if (remoteThreadScope.shouldRejectInbound(parsedMessage)) {
      sendApplicationResponse(
        JSON.stringify(remoteThreadScope.createRejectedResponse(parsedMessage)),
      );
      return;
    }
  ...
  function observeDesktopIpcLiveOwnerInbound(rawMessage, parsedMessage = null) {
    ...
    if (inboundKey) {
      ...
      evictOldestEntries(
        desktopIpcLiveOwnerObservedInboundKeys,
        FORWARDED_REQUEST_METHODS_MAX_SIZE,
      );
    }
  ...
  function desktopIpcLiveOwnerInboundKey(rawMessage, parsedMessage = null) {
    ...
    const threadId =
      extractThreadId(method, parsed.params) ||
    ...
    (async () => {
      ...
      try {
        const selection = await threadTurnsListFastPageCoordinator.resolve(request, {
          fetchCanonical: (canonicalRequest) =>
            fetchAdaptiveThreadTurnsListForRelay(canonicalRequest, {
        ...
        respondOnce(createJsonRpcErrorResponse(request.id, error, "thread_turns_list_failed"));
      }
    })();
  ...
  function sendBridgeManagedThreadTurnsListResponse(
    request,
    response,
    sendResponse,
  ...
  function maybeBuildJsonlThreadTurnsListFallback(request, response) {
    ...
    if (!threadId || hasRelayCursor(params.cursor) || params.djlRequireCanonical === true) {
      return null;
    }
    ...
    try {
      ...
      console.warn(`[djl] thread/turns/list jsonl fallback failed: ${error.message}`);
      return null;
    }
  }
  ...
  function sanitizeRelayBoundCodexMessage(rawMessage, parsedMessage = null) {
    ...
    let parsed =
      normalizedMessage === rawMessage && parsedMessage
        ? parsedMessage
  ...
  function handleBridgeManagedHandshakeMessage(
    rawMessage,
    sendResponse = sendApplicationResponse,
    parsedMessage = null,
  ) {
    ...
    if (method === "initialize" && parsed.id != null) {
      ...
      if (shouldForwardBridgeManagedInitialize({ codexHandshakeState, usesElectronBackend })) {
        if (!usesElectronBackend) {
  ...
  async function updateBridgePackageAndRestart() {
    return requestManagedDesktopUpdate(onDesktopUpdateRequested);
  }
  ...
  function start() {
    ...
    try {
      ...
      nextChild.on?.("error", (error) => {
        consoleImpl.warn(
          `[djl] Failed to hold the Mac awake while the bridge is active: ${error.message}`,
        );
      });
      nextChild.on?.("exit", () => {
        if (child === nextChild) {
      ...
      consoleImpl.warn(
        `[djl] Failed to start the bridge wake assertion: ${(error && error.message) || "unknown error"}`,
      );
      child = null;
    }
  }
...
async function requestManagedDesktopUpdate(onDesktopUpdateRequested) {
  if (typeof onDesktopUpdateRequested !== "function") {
    const error = new Error(
      "Gateway updates are managed by the DJL desktop app, which is not available.",
    );
    error.errorCode = "desktop_update_unavailable";
    error.userMessage = error.message;
    throw error;
  }
  const result = await onDesktopUpdateRequested();
  return {
    success: true,
...
function buildMacConnectionHeaders(deviceState, pairingSession, notificationSecret) {
  const hostSecret = normalizeNonEmptyString(deviceState?.relaySessionSecret);
  const pushSecret = normalizeNonEmptyString(notificationSecret);
  if (hostSecret.length < 32 || pushSecret.length < 32) {
    throw new Error("DJL relay authentication secrets are unavailable.");
  }
  return {
    "x-role": "mac",
    "x-djl-session-secret": hostSecret,
    "x-djl-notification-secret": pushSecret,
...
function normalizeTurnStartParamsForCodex(params) {
  const normalizedRawMessage = normalizeTurnStartForCodex(
    JSON.stringify({
      method: "turn/start",
      params,
    }),
  ...
  while (combinedTurns.length < requestedLimit) {
    ...
    if (
      ...
      if (pageTurns.length > pageLimit) {
        ...
        if (!completeResponse) {
          throw new Error(
            "thread/turns/list returned an oversized batch without a safe cursor boundary.",
          );
        }
...
function buildCompactedCompleteTurnsListResponse({
  ...
  const response = buildSafeTurnsListResponse(requestId, firstResult, lastResult, turnsKey, turns);
  if (
    measureSanitizedTurnsListResponseBytes(response, sanitizeForRelay, sanitizeContext) <
    payloadSoftLimitBytes
  ) {
  ...
  for (const maxChars of [
    ...
    compacted.result.djlPageCompactedForRelay = true;
    if (
      measureSanitizedTurnsListResponseBytes(compacted, sanitizeForRelay, sanitizeContext) <
      payloadSoftLimitBytes
    ) {
      return compacted;
    }
    ...
    for (const maxChars of [
      ...
      const response = {
        ...
        result: {
          ...
          djlEmergencySingleTurnForRelay: true,
        },
      };
      if (
        measureSanitizedTurnsListResponseBytes(response, sanitizeForRelay, sanitizeContext) <
...
function compactEmergencySingleTurnForRelay(turn, maxChars, maxItems) {
  ...
  safeTurn.items = selectEmergencyHistoryItemsForRelay(items, maxItems).map((item) =>
    compactHistoryItemForRelay(item, maxChars),
  );
  safeTurn.djlEmergencySingleTurnForRelay = true;
  safeTurn.djlPageCompactedForRelay = true;
  return safeTurn;
}
...
function normalizeRelayBoundJsonRpcMessage(
  rawMessage,
  {
...
function sanitizeThreadHistoryImagesForRelay(rawMessage, requestMethod, requestContext = {}) {
  ...
  const didPreTrimTurnWindow =
    Buffer.byteLength(rawMessage, "utf8") > RELAY_THREAD_PAYLOAD_SOFT_LIMIT_BYTES &&
  ...
  return (
    trimThreadPayloadForRelay(parseBridgeJSON(sanitizedPayload), null, trimOptions) ??
    sanitizedPayload
  );
}
...
function readCachedJsonlThreadCwd(cacheKey, rolloutPath) {
  ...
  const ttl = cached.cwd
    ? RELAY_JSONL_THREAD_CWD_CACHE_TTL_MS
    : RELAY_JSONL_THREAD_EMPTY_CWD_CACHE_TTL_MS;
...
function augmentRelayHistoryTurnsWithJsonlArtifacts(
  turns,
  threadId = "",
  ...
  const augmentedTurns = turns.map((turn) => {
    ...
    const merged = mergeRelayHistoryItemsWithJsonlItems(
      ...
      normalizedThreadId,
      {
        includeJsonlItem: includeHistoryItems ? () => true : isJsonlHistoryArtifactItem,
      },
    );
...
function readJsonlArtifactItemsByTurnId(threadId, requestedTurnIds = new Set()) {
  ...
  try {
    ...
    jsonlArtifactItemsCacheByThread.delete(cacheKey);
    console.warn(
      `[djl] history jsonl artifact augmentation failed for ${normalizedThreadId}: ${error.message}`,
    );
  }
...
function readCachedJsonlArtifactItems(cacheKey, threadId, requestedTurnIds = new Set()) {
  ...
  if (stat.mtimeMs !== cached.mtimeMs || stat.size !== cached.size) {
    try {
      ...
      jsonlArtifactItemsCacheByThread.delete(cacheKey);
      console.warn(
        `[djl] history jsonl artifact cache refresh failed for ${threadId}: ${error.message}`,
      );
      return null;
    }
  }
  ...
  const now = Date.now();
  const coversRequestedTurns =
    cached.coversEntireRollout ||
...
function readAndCacheJsonlArtifactItems(
  ...
  try {
    ...
    coveredTurnIds = new Set(
      turns.map((turn) => normalizeNonEmptyString(turn?.id)).filter(Boolean),
    );
...
function mergeRelayHistoryItemsWithJsonlItems(
  existingItems,
  jsonlItems,
  threadId = "",
  ...
  const adoptJsonlSourceAlias = (index, jsonlItem) => {
    const sourceKey = normalizeNonEmptyString(jsonlItem?.djlSourceItemKey);
    const existing = resolvedExistingItems[index];
    if (
      !sourceKey ||
      !existing ||
      typeof existing !== "object" ||
      normalizeNonEmptyString(existing.djlSourceItemKey)
    ) {
      return;
    }
    resolvedExistingItems[index] = { ...existing, djlSourceItemKey: sourceKey };
  ...
  if (matchedAnchorCount === 0) {
    const unanchoredArtifacts = sanitizedJsonlItems.filter(
      (item) => includeJsonlItem(item) && isJsonlHistoryArtifactItem(item),
    );
  ...
  return turnItems.flatMap((item, index) => {
    ...
    if (isProgressPlanItem(item)) {
      return [
        {
          ...item,
          id: normalizeNonEmptyString(item?.id) || `djl-jsonl-progress-plan-${turnId}`,
          djlProgressPlan: true,
          djlJsonlProgressPlan: true,
        },
      ];
    }
      ...
      return [
        {
          ...item,
          id:
            normalizeNonEmptyString(item?.id) || `djl-jsonl-image-view-${turnId}-${imageViewIndex}`,
        },
      ];
    }
    return [item];
...
function isJsonlHistoryArtifactItem(item) {
  const itemType = normalizeHistoryItemToken(item?.type);
  return itemType === "filechange" || itemType === "imageview" || isProgressPlanItem(item);
}

function isProgressPlanItem(item) {
  const itemType = normalizeHistoryItemToken(item?.type);
  return (
    (itemType === "plan" || itemType === "todolist") &&
    (item?.djlJsonlProgressPlan === true || item?.djlProgressPlan === true)
  );
}
...
function areEquivalentRelayHistoryItems(first, second) {
  ...
  if (
    relayHistoryIdentityIsStable(firstIdentity) &&
    relayHistoryIdentityIsStable(secondIdentity) &&
    !(isRelayAssistantHistoryItem(first) && isRelayAssistantHistoryItem(second))
  ) {
    return false;
  }
  if (relayHistoryIdentityIsStable(firstCallId) && relayHistoryIdentityIsStable(secondCallId)) {
    return false;
  }
...
function relayHistoryIdentityIsStable(identity) {
  ...
  return !/^(?:user-message-line|response-item-line|apply-patch-line)-\d+$/.test(
    normalizedIdentity,
  );
}

function relayHistoryItemCallId(item) {
  return normalizeNonEmptyString(item?.call_id) || normalizeNonEmptyString(item?.callId);
}
...
function relayHistoryItemText(item) {
  ...
  if (Array.isArray(item.content)) {
    return item.content.map(relayHistoryItemText).filter(Boolean).join("\n");
  }
...
function sanitizeRelayHistoryTurn(turn, threadId = "") {
  ...
  const turnThreadId =
    ...
    normalizeNonEmptyString(turn.threadId) ||
    normalizeNonEmptyString(turn.thread_id);
  const sanitizedItems = turn.items
    .map((item) => {
...
const LIVE_ITEM_LIFECYCLE_METHODS = new Set(["item/started", "item/updated", "item/completed"]);
...
function sanitizeLiveUserNotification(parsed) {
  ...
  const key =
    typeof parsed?.params?.message === "string"
      ? "message"
      : typeof parsed?.params?.text === "string"
        ? "text"
        : "";
  if (!key) {
    return parsed;
  ...
  return visible === parsed.params[key]
    ? parsed
    : {
        ...parsed,
...
function trimThreadPayloadForRelay(parsed, explicitThread = undefined, options = {}) {
  ...
  if (Buffer.byteLength(encoded, "utf8") <= RELAY_THREAD_PAYLOAD_SOFT_LIMIT_BYTES) {
    ...
    const compactedThread = buildRelayHistoryCompactedThread(
      thread,
      buildRelayCompactedHistoryTurns(
        thread.turns,
        thread.turns,
        preOmittedTurnCount,
        compactionIdSource,
...
function trimTurnsListPayloadForRelay(parsed, turnsKey, originalRawMessage = null) {
  ...
  if (result.djlJsonlFallback === true) {
      ...
      for (const maxChars of [1_000, 0]) {
        ...
        const emergencyPayload = JSON.stringify({
          ...
          result: {
            ...
            djlPageCompactedForRelay: true,
            djlEmergencyJsonlPageForRelay: true,
          },
        });
...
function buildRelayHistoryCompactedThread(thread, turns, omittedTurnCount, keptTurnCount) {
  return {
    ...
    djlHistoryCompacted: omittedTurnCount > 0,
    djlOmittedTurnCount: omittedTurnCount,
    djlKeptTurnCount: keptTurnCount,
  };
}

function buildRelayCompactedHistoryTurns(
  allTurns,
  keptTurns,
  preOmittedTurnCount = 0,
  idSourceOverride = null,
) {
...
function buildRelayHistoryCompactionTurn(omittedTurnCount, keptTurnCount, idSource = {}) {
  ...
  return {
    id: `djl-history-compacted-${baseId}`,
    ...
    status: "completed",
    djlSynthetic: true,
    djlHistoryCompacted: true,
    djlOmittedTurnCount: omittedTurnCount,
    djlKeptTurnCount: keptTurnCount,
    items: [
      {
        id: `djl-history-compacted-item-${baseId}`,
...
function shouldSuppressRolloutMirrorForThread(
  ...
  const followerIsFresh =
    typeof desktopIpcActionFollower?.hasFreshLiveThreadState === "function"
  ...
  const ownerIsFresh =
    typeof desktopIpcLiveOwner?.isFreshThreadOwned === "function"
...
module.exports = {
  buildMacConnectionHeaders,
  requestManagedDesktopUpdate,
  ...
  sanitizeThreadHistoryImagesForRelay,
  shouldForwardBridgeManagedInitialize,
