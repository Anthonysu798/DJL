...
const CHECKPOINT_REFS_PREFIX = "refs/djl/checkpoints";
...
async function captureGitCheckpoint(repoRoot, checkpointRef) {
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "djl-checkpoint-"));
  const tempIndexPath = path.join(tempDir, `index-${process.pid}-${Date.now()}`);
  const env = {
    ...process.env,
    GIT_INDEX_FILE: tempIndexPath,
    GIT_AUTHOR_NAME: "DJL",
    GIT_AUTHOR_EMAIL: "djl@users.noreply.github.com",
    GIT_COMMITTER_NAME: "DJL",
    GIT_COMMITTER_EMAIL: "djl@users.noreply.github.com",
  };
  ...
  try {
    ...
    const message = `djl checkpoint ref=${checkpointRef}`;
...
function validateCheckpointRef(checkpointRef) {
  ...
  if (!normalized.startsWith(`${CHECKPOINT_REFS_PREFIX}/`)) {
    throw workspaceCheckpointError(
      ...
      "Workspace checkpoint refs must stay inside the DJL checkpoint namespace.",
    );
  }
