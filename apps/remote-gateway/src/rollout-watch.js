...
function watchThreadRollout(threadId = "") {
  ...
  console.log(`[djl] Watching thread ${resolvedThreadId}`);
  console.log(`[djl] Rollout file: ${rolloutPath}`);
  console.log("[djl] Waiting for new persisted events... (Ctrl+C to stop)");
...
function resolveThreadId(threadId) {
  ...
  throw new Error("No thread id provided and no remembered DJL thread found.");
}
