...
const RUNTIME_IMAGE_OPENING_TAG =
...
function isContextualUserText(text) {
  ...
  if (
    CONTEXT_MARKER_PAIRS.some(
      ([start, end]) => normalized.startsWith(start) && normalized.endsWith(end),
    )
  ) {
    return true;
  }
  ...
  return (
    trimmed.startsWith(LEGACY_APPLY_PATCH_WARNING_PREFIX) &&
...
function extractVisibleRuntimeEnvelope(text) {
  ...
  const hookPrompt =
    /^<hook_prompt\s+hook_run_id=(?:"[^"]+"|'[^']+')>([\s\S]*?)<\/hook_prompt>$/i.exec(trimmed);
...
function sanitizeUserRoleItem(item) {
  ...
  const hasDirectText = ["text", "message"].some(
    (key) => typeof sanitized[key] === "string" && sanitized[key].trim(),
  );
...
function readInputEntryText(entry) {
  ...
  return content.map(readInputEntryText).filter(Boolean).join("\n");
}
...
function buildDJLSourceItemKey(turnId, text) {
