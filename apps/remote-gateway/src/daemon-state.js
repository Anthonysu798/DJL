...
const DEFAULT_STATE_DIR_NAME = ".djl";
...
// Reuses the existing DJL state root so daemon mode keeps the same local-first storage model.
function resolveDJLStateDir({ env = process.env, osImpl = os } = {}) {
  return (
    normalizeNonEmptyString(env.DJL_DEVICE_STATE_DIR) ||
    path.join(osImpl.homedir(), DEFAULT_STATE_DIR_NAME)
  );
}

function resolveDaemonConfigPath(options = {}) {
  return path.join(resolveDJLStateDir(options), DAEMON_CONFIG_FILE);
}

function resolvePairingSessionPath(options = {}) {
  return path.join(resolveDJLStateDir(options), PAIRING_SESSION_FILE);
}

function resolveBridgeStatusPath(options = {}) {
  return path.join(resolveDJLStateDir(options), BRIDGE_STATUS_FILE);
}

function resolveBridgeLogsDir(options = {}) {
  return path.join(resolveDJLStateDir(options), LOGS_DIR);
}
...
// Captures the last known service heartbeat so `djl status` does not depend on launchctl output alone.
...
function ensureDJLStateDir({ fsImpl = fs, ...options } = {}) {
  fsImpl.mkdirSync(resolveDJLStateDir(options), { recursive: true });
}

function ensureDJLLogsDir({ fsImpl = fs, ...options } = {}) {
