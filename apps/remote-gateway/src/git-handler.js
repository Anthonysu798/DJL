...
async function gitCreatePullRequest(cwd, params, options = {}) {
  ...
  const bodyFile = path.join(
    os.tmpdir(),
    `djl-pr-body-${process.pid}-${Date.now()}-${randomBytes(4).toString("hex")}.md`,
  );
...
async function defaultFeatureBranchName(cwd, params) {
  const prefix = normalizeNonEmptyLine(params.featureBranchPrefix) || "djl/mobile-pr";
...
function firstCommitMessageLine(message) {
  return normalizeNonEmptyLine(message) || "Changes from DJL";
}
...
async function runStructuredCodexJson({
  ...
  const tempDirectory = fs.mkdtempSync(path.join(os.tmpdir(), "djl-git-ai-"));
...
async function stashChangesForWorktreeHandoff(cwd, pathspecArgs = []) {
  const stashLabel = `djl-worktree-handoff-${randomBytes(6).toString("hex")}`;
...
async function applyCopiedLocalChangesToWorktree(cwd, patch) {
  ...
  const patchFilePath = path.join(
    os.tmpdir(),
    `djl-worktree-copy-${randomBytes(6).toString("hex")}.patch`,
  );
