...
const { createElectronAppServerTransport } = require("./electron-app-server-adapter");
...
// Turns common Codex auth/config failures into recovery guidance without handling secrets in DJL.
function formatCodexLaunchFailure({ launchDescription, reason }) {
  ...
  const guidance = [
    ...
    "DJL does not store or forward OpenAI API keys.",
    "Recommended fix: run `codex login` on this Mac, then restart DJL.",
    "If you intentionally use API-key auth, run `printenv OPENAI_API_KEY | codex login --with-api-key` or make that env var available to the DJL daemon yourself.",
  ];
  return `${message}\n${guidance.join("\n")}`;
}
