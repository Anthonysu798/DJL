...
function buildLegacyIOSAppCompatibilityMessage({ bridgeVersion, iosAppVersion } = {}) {
  ...
  return (
    `DJL bridge ${normalizedBridgeVersion} requires DJL iPhone ` +
    `${MINIMUM_SUPPORTED_IOS_APP_VERSION} or later. ` +
    `Update iPhone ${normalizedIOSAppVersion} from the App Store to continue.`
  );
}
...
function buildCachedIOSAppCompatibilityWarning({ bridgeVersion, iosAppVersion } = {}) {
  ...
  return formatNoticeBox({
    title: "!!! WARNING !!!",
    lines: [
      `DJL bridge ${snapshot.bridgeVersion || "latest"} requires DJL iPhone ${snapshot.minimumSupportedIOSAppVersion} or later.`,
      "Update the iPhone app from the App Store to continue.",
    ],
  });
}
