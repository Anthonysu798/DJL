...
function createDesktopConversationProjector({ now = () => Date.now(), maxCacheSize = 64 } = {}) {
  ...
  function project(threadId, rawState, { includeAllActiveTurns = false } = {}) {
    ...
    const requiresSyntheticFullReplace =
      previousProjection &&
    ...
    if (!previousProjection && evictedThreadIds.has(normalizedThreadId)) {
      ...
      notifications = diffProjections(normalizedThreadId, previousProjection, nextProjection);
    }
...
function projectDesktopConversationStateToThread(
  threadId,
  rawState,
  { now = () => Date.now() } = {},
) {
...
function projectConversationState(
  threadId,
  rawState,
  { now = () => Date.now(), turnCache = null, itemCache = null } = {},
) {
  const turns = projectTurns(threadId, rawState, { turnCache, itemCache });
  const activeTurnId = activeTurnIdFromTurns(turns);
  const runtimeSettings = rawState?.djlRuntimeSettings || rawState?.djl_runtime_settings || null;
  const thread = {
    id: threadId,
    sessionId: threadId,
    session_id: threadId,
    ...
    modelProvider:
      readString(rawState?.modelProvider) || readString(rawState?.model_provider) || "",
    model:
      readString(runtimeSettings?.model) ||
    ...
    agentNickname:
      readString(rawState?.agentNickname) || readString(rawState?.agent_nickname) || null,
    ...
    tokenUsage: cloneJSON(
      rawState?.latestTokenUsageInfo ?? rawState?.latest_token_usage_info ?? null,
    ),
    turns,
  };
...
function projectTurnCached(threadId, rawTurn, index, { turnCache = null, itemCache = null } = {}) {
  ...
  const hasStableId = Boolean(
    readString(rawTurn.turnId) || readString(rawTurn.turn_id) || readString(rawTurn.id),
  );
...
function projectTurn(threadId, rawTurn, index, { itemCache = null } = {}) {
  ...
  for (const item of Array.isArray(rawTurn.items) ? rawTurn.items : []) {
    ...
    if (
      ...
      (sameUserInput(item.content, paramsInput) || sameVisibleUserText(item.content, visibleInput))
    ) {
      continue;
    }
...
function bootstrapNotifications(
  ...
  const notifications =
    includeThreadStarted && shouldEmitThreadStarted(projection.thread)
...
function diffTurnLifecycle(threadId, previousProjection, nextProjection) {
  ...
  if (previousActive && !nextActive) {
    const completedTurn =
      findTurn(nextProjection.turns, previousActive) ||
...
function diffTurnItems(threadId, previousProjection, nextProjection) {
  ...
  for (const nextTurn of nextProjection.turns) {
    ...
    const previousItemsById = new Map(
      (previousTurn?.items || []).map((item) => [itemIdOf(item), item]),
    );
...
function diffItem(threadId, turnId, previousItem, nextItem) {
  ...
  if (itemType === "reasoning") {
    const reasoning = reasoningDeltaNotifications(
      threadId,
      turnId,
      itemId,
      previousItem,
  ...
  if (itemType === "commandexecution") {
    ...
    const delta = appendedDelta(
      commandOutput(previousItem),
      commandOutput(nextItem),
      snapshot.commandOutputLen,
    );
    if (delta) {
      return [
        deltaNotification("item/commandExecution/outputDelta", threadId, turnId, itemId, delta),
      ];
    }
    return [itemCompletedNotification(threadId, turnId, nextItem)];
  }
  ...
  if (itemType === "filechange") {
    const delta = appendedDelta(
      fileChangeOutput(previousItem),
      fileChangeOutput(nextItem),
      snapshot.fileOutputLen,
    );
    if (delta) {
  ...
  if (isToolCallItem(nextItem)) {
    const delta = appendedDelta(
      toolCallOutput(previousItem),
      toolCallOutput(nextItem),
      snapshot.toolOutputLen,
    );
    if (delta) {
      return [
        deltaNotification("item/toolCall/outputDelta", threadId, turnId, itemId, delta, {
...
function reasoningDeltaNotifications(threadId, turnId, itemId, previousItem, nextItem, snapshot) {
  ...
  const summaryLens = Array.isArray(snapshot.reasoningSummaryLens)
    ? snapshot.reasoningSummaryLens
    : [];
  const contentLens = Array.isArray(snapshot.reasoningContentLens)
    ? snapshot.reasoningContentLens
    : [];
  ...
  for (let index = 0; index < nextSummary.length; index += 1) {
    ...
    if (delta) {
      notifications.push(
        deltaNotification("item/reasoning/summaryTextDelta", threadId, turnId, itemId, delta, {
          summaryIndex: index,
        }),
      );
    }
  ...
  for (let index = 0; index < nextContent.length; index += 1) {
    ...
    if (delta) {
      notifications.push(
        deltaNotification("item/reasoning/textDelta", threadId, turnId, itemId, delta, {
          contentIndex: index,
        }),
      );
    }
...
function itemStartedNotification(threadId, turnId, item) {
  if (isAutoApprovalReviewItem(item)) {
    return autoApprovalReviewNotification(
...
function appendedDelta(previousText, nextText, snapshotLength) {
  ...
  const previousLength = Number.isInteger(snapshotLength)
    ? snapshotLength
...
function isActiveTurnStatus(value) {
  return (
    normalizeToken(value) === "inprogress" ||
...
function stableTurnItemIDs(turn) {
  return new Set(
    (Array.isArray(turn?.items) ? turn.items : []).flatMap((item) => {
...
function turnPromptSignature(turn) {
  ...
  if (!prompt) {
    const userItem = (Array.isArray(turn?.items) ? turn.items : []).find(
      (item) => normalizeToken(item?.type) === "usermessage",
    );
...
function turnStartIdentity(turn) {
  const value =
    turn?.startedAt ?? turn?.started_at ?? turn?.turnStartedAtMs ?? turn?.turn_started_at_ms;
...
const SKIPPED_ITEM = Symbol("djl-skipped-item");
...
function projectItemForMobile(item, itemType = normalizeToken(item?.type)) {
  ...
  if (itemType === "todolist") {
    projected.djlProgressPlan = true;
  }
  ...
  if (!projected.djlDesktopIpcItemType) {
    projected.djlDesktopIpcItemType = readString(item?.type) || itemType;
  }
  return projected;
}
...
function isToolCallItem(item) {
  const type = normalizeToken(item?.type);
  return (
    isGenericToolCallItemType(type) || type === "collabagenttoolcall" || type === "collabtoolcall"
  );
}
...
function assistantMessageText(item) {
  ...
  return readText(item.text) || readText(item.message) || renderContentText(item.content);
}

function planText(item) {
  return readText(item.text) || renderContentText(item.plan) || renderContentText(item.content);
}
