...
const { createDesktopOwnerIpcClient } = require("./desktop-ipc-owner-transport");
...
function createDesktopIpcLiveOwner({
  ...
  if (
    !enabled ||
    typeof sendCodexRequest !== "function" ||
    typeof sendRawCodexMessage !== "function"
  ) {
    return createDisabledDesktopIpcLiveOwner();
  }
  const sendPhoneNotification =
    typeof sendApplicationResponse === "function" ? sendApplicationResponse : () => {};
  ...
  function markThreadReadByPhone(threadId) {
    ...
    const hadUnread = Boolean(
      conversation && (conversation.hasUnreadTurn || conversation.unreadMessageCount > 0),
    );
    if (hadUnread) {
  ...
  function rememberPendingTurnStart(threadId, params, requestId) {
    ...
    const existingPending = normalizedRequestId
      ? pendingTurnStartEntriesByRequestId.get(normalizedRequestId)
      : null;
  ...
  function insertOptimisticPendingTurn(threadId, entry) {
    ...
    if (
      conversation.turns.some(
        (turn) => (readString(turn?.turnId) || readString(turn?.id)) === optimisticTurnId,
      )
    ) {
  ...
  function ensureOptimisticTurnId(threadId, entry) {
    ...
    entry.optimisticTurnId = `djl-pending-turn:${threadId}:${requestSegment}`;
    return entry.optimisticTurnId;
  }
  ...
  function removeOptimisticPendingTurn(threadId, entry) {
    ...
    const index = conversation.turns.findIndex(
      (turn) =>
        turn?.djlOptimisticPendingTurn &&
  ...
  function claimStartedThreadForPendingLocalStart(message) {
    if (
      readString(message?.method) !== "thread/started" ||
      pendingThreadStartRequestIds.size === 0
    ) {
      return;
    }
  ...
  function removeOwnedThread(
    threadId,
    { broadcastRemoval = false, reason = "", skipArchiveMetadataBroadcast = false } = {},
  ) {
    const normalizedThreadId = readString(threadId);
    if (!normalizedThreadId) {
      return;
    ...
    const droppedQueuedFollowUps =
      (queuedFollowUpsByThreadId.get(normalizedThreadId) || []).length > 0;
  ...
  function broadcastRemovedConversationState(
    threadId,
    { reason = "", skipArchiveMetadataBroadcast = false } = {},
  ) {
    const previousState =
    ...
    ipc.sendBroadcast(THREAD_STREAM_STATE_CHANGED, {
      ...
      djlOwnerSource: DJL_LIVE_OWNER_SOURCE,
  ...
  function seedOwnedConversation(threadId, seed = {}) {
    ...
    const cachedThread = normalizedThreadId
      ? cachedThreadsByThreadId.get(normalizedThreadId)
      : null;
    if (cachedThread) {
  ...
  function hydrateOwnedThreadFromRead(threadId) {
    ...
    const hydration = Promise.resolve()
      ...
      .catch((error) => {
        console.warn(
          `${logPrefix} desktop IPC live owner thread/read hydration failed for ${normalizedThreadId}: ${error?.message || "unknown error"}`,
        );
      })
      .finally(() => {
        pendingThreadHydrationsByThreadId.delete(normalizedThreadId);
  ...
  function isPeerOwnershipBroadcast(params) {
    if (readString(params?.djlOwnerSource) === DJL_LIVE_OWNER_SOURCE) {
      return false;
    }
  ...
  async function handleFollowerRequest(envelope) {
    ...
    switch (method) {
      ...
      case "thread-follower-command-approval-decision":
        return sendServerRequestResponse(conversationId, params.requestId, {
      ...
      case "thread-follower-edit-last-user-turn":
        throw new Error("thread-follower-edit-last-user-turn is not supported by DJL yet.");
      default:
  ...
  async function handleFollowerStartTurn(conversationId, params) {
    const rawTurnStartParams =
      params.turnStartParams || params.turn_start_params || params.turnStart || params;
    const codexParams = mergeFollowerRuntimeOverrides(
      conversationId,
    ...
    const normalizedParams = await Promise.resolve(
      normalizeTurnStartParams(cloneJSON(codexParams)),
    );
    const nextCodexParams =
    ...
    const pendingEntry = rememberPendingTurnStart(conversationId, nextCodexParams, senderRequestId);
    if (pendingEntry) {
  ...
  function mirrorFollowerUserPromptToPhone(threadId, turnStartParams, turnStartResult = null) {
    ...
    const turnId =
      readString(turnStartResult?.turn?.id) ||
    ...
    sendPhoneNotification(
      JSON.stringify({
        ...
        params: {
          ...
          djlActionSource: DJL_LIVE_OWNER_SOURCE,
          createdAt: now(),
        },
      }),
    );
  ...
  async function handleFollowerSteerTurn(conversationId, params) {
    const rawSteerParams = params.turnSteerParams || params.turn_steer_params || params;
    const expectedTurnId =
      readString(rawSteerParams.expectedTurnId) ||
  ...
  function followerApprovalResultForRequest(conversationId, params) {
    ...
    const pendingRequest = (conversations.get(conversationId)?.requests || []).find(
      (request) => requestIdKey(request?.id) === requestId,
    );
    ...
    return {
      permissions:
        grantsRequestedPermissions && isPlainJSONObject(requestedPermissions)
  ...
  function sendServerRequestResponse(conversationId, requestId, result) {
    ...
    const pendingRequest = (conversations.get(conversationId)?.requests || []).find(
      (request) => requestIdKey(request?.id) === normalizedRequestId,
    );
    sendRawCodexMessage(
      JSON.stringify({
  ...
  function applyFollowerThreadSettings(conversationId, threadSettings) {
    ...
    const overrides = followerRuntimeOverridesByThreadId.get(conversationId) || {};
    const model =
      readString(threadSettings.model) ||
    ...
    if (conversation) {
      conversation.latestThreadSettings = {
        ...(conversation.latestThreadSettings &&
        typeof conversation.latestThreadSettings === "object"
          ? conversation.latestThreadSettings
          : {}),
  ...
  function runNextQueuedFollowUp(threadId) {
    ...
    if (
      !Array.isArray(queue) ||
      queue.length === 0 ||
      runningQueuedFollowUpThreadIds.has(threadId)
    ) {
      return;
    }
    const entry = queue[0];
    ...
    const text =
      readString(entry?.context?.text) || readString(entry?.text) || readString(entry?.prompt);
    if (!text) {
      ...
      console.warn(
        `${logPrefix} desktop queued follow-up entry has no extractable text; pausing it for ${threadId}`,
      );
      entry.pausedReason = "djl-unsupported-entry";
      broadcastQueuedFollowUps(threadId);
      return;
    }
    ...
    const startParams = mergeFollowerRuntimeOverrides(
      threadId,
      sanitizeTurnStartParams({
        threadId,
    ...
    Promise.resolve()
      ...
      .catch((error) => {
        console.warn(
          `${logPrefix} desktop queued follow-up failed for ${threadId}: ${error?.message || "unknown error"}`,
        );
      })
      .finally(() => {
  ...
  function commitAcceptedRuntimeSettings(threadId, params, source, turnId) {
    try {
      ...
      console.warn(`[djl] runtime settings persistence failed: ${error.message}`);
      return null;
    }
  }
  ...
  function activeTurnIdForConversation(conversationId) {
    ...
    for (let index = turns.length - 1; index >= 0; index -= 1) {
      ...
      if (
        turnId &&
        (!status || status === "inprogress" || status === "running" || status === "active")
      ) {
        return turnId;
      }
    }
