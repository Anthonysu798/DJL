...
function createDesktopOwnerIpcClient({
  ...
  function ensureConnected() {
    ...
    nextSocket.on("connect", () => {
      isConnecting = false;
      sendRequest("initialize", { clientType: "djl-bridge" }, { initializing: true })
        .then((result) => {
  ...
  function startLocalRouterAfterMissingSocket(reasonCode) {
    ...
    localRouter
      .start({ removeStaleSocket: reasonCode === "ECONNREFUSED" })
      .then(() => {
        if (!shouldReconnect) {
          return;
        }
  ...
  function sendRequest(method, params, { initializing = false } = {}) {
    ...
    const requestId = `djl-owner-${now().toString(36)}-${randomUUID()}`;
    const envelope = {
      type: "request",
      requestId,
      sourceClientId: initializing ? "initializing-client" : clientId || "djl-bridge",
      version: METHOD_VERSION_BY_NAME.get(method) || 1,
      method,
      params: params || {},
    };
    ...
    Promise.resolve()
      ...
      .catch((error) => {
        writeEnvelope({
          ...
          method: envelope.method,
          handledByClientId: clientId,
          error: error?.message || "DJL IPC owner request failed.",
        });
      });
  }
...
function createDesktopIpcRouterServer({
  ...
  function initializeClient(client, envelope) {
    if (!client.id) {
      client.id = `djl-router-${now().toString(36)}-${nextClientSeq}`;
      nextClientSeq += 1;
      clientsById.set(client.id, client);
    }
    client.initialized = true;
    client.type =
      readString(envelope.params?.clientType) || readString(envelope.params?.client_type);
    writeEnvelopeToClient(client, {
      type: "response",
      requestId: envelope.requestId,
      resultType: "success",
      method: "initialize",
      handledByClientId: "djl-ipc-router",
  ...
  async function routeClientRequest(sender, envelope) {
    ...
    const routedRequestId = `djl-routed-${now().toString(36)}-${randomUUID()}`;
  ...
  async function discoverTargetForRequest(sender, request) {
    const candidates = Array.from(clientsById.values()).filter(
      (client) => client.initialized && client !== sender && !client.socket.destroyed,
    );
  ...
  function compareDiscoveryTargets(left, right) {
    const priorityDelta =
      discoveryTargetPriority(left.client) - discoveryTargetPriority(right.client);
  ...
  function discoveryTargetPriority(client) {
    ...
    return normalizeToken(client?.type) === "djlbridge" ? 0 : 1;
  }

  function askClientCanHandle(client, request) {
    const requestId = `djl-router-discovery-${now().toString(36)}-${randomUUID()}`;
