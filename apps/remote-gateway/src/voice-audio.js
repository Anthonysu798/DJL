// FILE: voice-audio.js
// Purpose: Pure WAV and M4A container parsing plus format checks for DJL voice clips.
// Layer: Bridge helper
...
function hasConsistentVoiceWavLayout(wavInfo) {
  return (
    wavInfo.blockAlign === expectedVoiceWavBlockAlign(wavInfo) &&
...
// Parses the MP4/M4A container enough to validate DJL-generated AAC clips
...
function isM4AFileTypeBox(buffer, box) {
  ...
  const brands = [buffer.toString("ascii", box.payloadStart, box.payloadStart + 4)];
  ...
  for (const box of readMp4Boxes(buffer, start, end)) {
    ...
    for (const mediaBox of readMp4Boxes(buffer, box.payloadStart, box.payloadEnd)) {
      if (mediaBox.type === "mdhd") {
        ...
        sampleEntryInfo = readAudioSampleEntryInfo(
          buffer,
...
function readFirstSampleEntry(buffer, start, end) {
  ...
  if (
    ...
    sampleEntry.payloadStart + 28 > sampleEntry.payloadEnd
  ) {
    return null;
  }
  return {
...
function isSupportedM4AAudioTrack(trackInfo) {
  return (
    ...
    (trackInfo.sampleEntryInfo?.channelCount === 1 ||
      trackInfo.sampleEntryInfo?.channelCount === 2) &&
