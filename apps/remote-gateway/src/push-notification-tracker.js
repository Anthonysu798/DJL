...
const DEFAULT_GOAL_PUSH_STATE_PATH = path.join(os.homedir(), ".djl", "goal-push-state.json");
...
function createPushNotificationTracker({
  ...
  async function handleGoalUpdated({ threadId, params }) {
    ...
    if (
      !goalStatusByThreadId.has(resolvedThreadId) &&
      goalStatusByThreadId.size >= MAX_GOAL_STATUS_ENTRIES
    ) {
