...
async function downsampleImageWithSips(
  ...
  const tempDir = await fs.promises.mkdtemp(path.join(os.tmpdir(), "djl-image-preview-"));
...
async function createPatchSandbox(repoRoot, affectedFiles) {
  const sandboxRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "djl-revert-preview-"));
...
async function initializePatchSandboxGitRepo(sandboxRoot) {
  ...
  await git(sandboxRoot, "config", "user.email", "djl@example.local");
  await git(sandboxRoot, "config", "user.name", "DJL");
  await syncPatchSandboxIndex(sandboxRoot);
...
async function createPatchBackup(repoRoot, affectedFiles) {
  const backupRoot = await fs.promises.mkdtemp(path.join(os.tmpdir(), "djl-revert-backup-"));
  const entries = [];
...
async function writeTempPatchFile(patchText) {
  const tempPatchPath = path.join(
    os.tmpdir(),
    `djl-revert-${Date.now()}-${Math.random().toString(16).slice(2)}.patch`,
  );
