...
const { randomBytes, randomUUID, generateKeyPairSync } = require("crypto");
...
function loadOrCreateBridgeDeviceState() {
  ...
  if (fileRecord.state) {
    if (fileRecord.migrated) {
      writeBridgeDeviceState(fileRecord.state);
  ...
  if (fileRecord.error) {
    if (keychainRecord.state) {
      warnOnce(
        "[djl] Recovering the canonical device-state.json from the legacy Keychain pairing mirror.",
      );
  ...
  if (keychainRecord.error) {
    warnOnce(
      "[djl] Ignoring unreadable legacy Keychain pairing mirror; generating a fresh canonical device-state.json.",
    );
...
// Removes the saved bridge identity/trust state so the next `djl up` requires a fresh QR pairing.
...
function createBridgeDeviceState() {
  ...
  return {
    ...
    relaySessionSecret: randomBytes(32).toString("base64url"),
    trustedPhones: {},
...
function readCanonicalFileStateRecord() {
  ...
  try {
    const rawState = JSON.parse(fs.readFileSync(storeFile, "utf8"));
    const state = normalizeBridgeDeviceState(rawState);
    return {
      state,
      error: null,
      migrated: rawState?.relaySessionSecret !== state.relaySessionSecret,
    };
  } catch (error) {
...
function resolveStoreDir() {
  return normalizeNonEmptyString(process.env.DJL_DEVICE_STATE_DIR) || DEFAULT_STORE_DIR;
}

function resolveStoreFile() {
  return (
    normalizeNonEmptyString(process.env.DJL_DEVICE_STATE_FILE) ||
    path.join(resolveStoreDir(), "device-state.json")
  );
}

function resolveKeychainMirrorFile() {
  return normalizeNonEmptyString(process.env.DJL_DEVICE_STATE_KEYCHAIN_MOCK_FILE);
}
...
function reconcileLegacyKeychainMirror(canonicalState, keychainRecord) {
  if (keychainRecord.error) {
    warnOnce(
      "[djl] Ignoring unreadable legacy Keychain pairing mirror; using canonical device-state.json.",
    );
    return;
  }
  ...
  warnOnce(
    "[djl] Canonical bridge pairing state differs from the legacy Keychain mirror; using device-state.json.",
  );
  writeKeychainStateString(JSON.stringify(canonicalState, null, 2));
}
...
function normalizeBridgeDeviceState(rawState) {
  ...
  const storedRelaySessionSecret = normalizeNonEmptyString(rawState?.relaySessionSecret);
  const relaySessionSecret = /^[A-Za-z0-9_-]{43,128}$/.test(storedRelaySessionSecret)
    ? storedRelaySessionSecret
...
function corruptedStateError(source, error) {
  ...
  return new Error(
    `The saved DJL pairing state in ${source} is unreadable. ` +
      "Run `djl reset-pairing` to start fresh." +
      (detail ? ` (${detail})` : ""),
  );
}
