...
function createRolloutLiveMirrorController({
  ...
  function observeInbound(rawMessage, parsedMessage = null) {
    ...
    mirror = createThreadRolloutLiveMirror({
      threadId,
      sendApplicationResponse:
        typeof shouldSuppressThread === "function"
          ? (rawNotification) => {
  ...
  function stop() {
    ...
    if (partialLine) {
      ...
      try {
        ...
        console.warn(
          `${logPrefix} rollout live mirror final flush failed for ${threadId}: ${error.message}`,
        );
      }
    }
    onStop();
...
function bootstrapFromExistingRollout({
  ...
  if (activeRunLines.length === 0 && latestTerminalRun) {
    sendApplicationResponse(
      JSON.stringify(terminalCatchUpNotification(state.threadId, latestTerminalRun)),
    );
    return;
  }
    ...
    sendApplicationResponse(
      JSON.stringify(
        createNotification("turn/activity", {
          ...
          turnId: state.activeTurnId,
          id: state.activeTurnId,
          djlRolloutBootstrapComplete: true,
        }),
      ),
    );
  }
...
function inspectBootstrapRunBoundary(contents) {
  ...
  for (const rawLine of contents.split("\n")) {
    ...
    const userText = isResponseUser
      ...
      : firstNonEmptyString([
          readString(parsed?.payload?.message),
          readString(parsed?.payload?.text),
        ]);
    const isVisibleUser =
      isUser &&
      Boolean(
    ...
    if (taskEventType === "task_started") {
      activeTurnId =
        readString(parsed?.payload?.turn_id) ||
    ...
    if (!activeTurnId) {
      const isNeutral =
        isBootstrapNeutralRecord(parsed, taskEventType) || (isUser && !isVisibleUser);
      if (TERMINAL_TASK_EVENT_TYPES.has(taskEventType)) {
...
function terminalRunFromEvent(entry, fallbackTurnId = "") {
  ...
  const turnId =
    readString(payload.turn_id) || readString(payload.turnId) || readString(fallbackTurnId);
  if (!turnId) {
    return null;
  }
...
function terminalCatchUpNotification(threadId, terminalRun) {
  const params = {
    ...
    djlRolloutTerminalCatchUp: true,
  };
...
function processRolloutLines(
  ...
  state,
  sendApplicationResponse,
  { tagBootstrapReplay = false, nowMs = Date.now() } = {},
) {
  ...
  const emitNotification = (notification) => {
    if (tagBootstrapReplay && notification.params && typeof notification.params === "object") {
      notification.params.djlRolloutBootstrapReplay = true;
    }
    sendApplicationResponse(JSON.stringify(notification));
  };
...
function synthesizeNotificationsFromRolloutEntry(entry, state, { nowMs = Date.now() } = {}) {
  ...
  if (entry?.type === "event_msg") {
    ...
    if (eventType === "thread_goal_updated") {
      ...
      return [
        createNotification("thread/goal/updated", {
          threadId,
    ...
    if (eventType === "thread_goal_cleared") {
      const threadId =
        readString(payload.threadId) || readString(payload.thread_id) || state.threadId;
      return threadId ? [createNotification("thread/goal/cleared", { threadId })] : [];
    }
...
function reasoningNotifications(state, text) {
  ...
  if (summaryEntries) {
    ...
    visibleText = unseenEntries.map((entry) => `**${entry.title}**\n\n<!-- -->`).join("\n\n");
  }
...
function responseItemMessageNotifications(state, entry, payload) {
  ...
  return agentMessageNotifications(state, entry, {
    ...
    turn_id:
      ...
      readString(payload?.internal_chat_message_metadata_passthrough?.turn_id),
    turnId:
      readString(payload?.turnId) ||
      readString(payload?.internal_chat_message_metadata_passthrough?.turnId),
  });
}
...
function userMessageNotifications(
  ...
  entry,
  payload,
  { rawMessage = "", isResponseItem = false } = {},
) {
  ...
  const itemId =
    readString(payload?.id) || readString(payload?.itemId) || readString(payload?.item_id);
  ...
  if (!turnId) {
    ...
    if (
      !state.pendingUserMessages.some(
        (pending) => `${pending.id || ""}:${pending.message}` === pendingKey,
      )
    ) {
...
function userMessageOccurrenceKey(state, turnId, message, { isResponseItem = false } = {}) {
  const baseKey = buildDJLSourceItemKey(turnId, message);
...
function agentMessageNotifications(state, entry, payload) {
  ...
  const occurrence =
    providerItemId && state.pendingEventAgentMessageOccurrencesByBaseKey.has(baseKey)
  ...
  const params = {
    ...
    ...(occurrence === 1 ? { djlSourceItemKey: baseKey } : {}),
    message,
  };
...
function toolStartNotifications(state, payload) {
  ...
  if (readString(toolName).toLowerCase() === "apply_patch") {
    const item = buildApplyPatchFileChangeItem({
      callId,
      patch:
        readString(argumentsObject.patch) ||
        readString(argumentsObject.input) ||
...
function turnFileChangeSnapshotNotifications(state, turnId) {
  ...
  return [
    createNotification("codex/event/patch_apply_end", {
      ...
      success: true,
      changes,
      djlTurnFileChangeSnapshot: true,
    }),
  ];
}
...
function agentMessageDedupeKey(turnId, message) {
  return buildDJLSourceItemKey(turnId, message);
}
