...
const VOICE_UPLOAD_USER_AGENT =
  process.env.DJL_VOICE_UPLOAD_USER_AGENT ||
...
function createVoiceHandler({
  ...
  function preconnectTranscriptionOrigin() {
    ...
    const timeoutID = controller
      ? setTimeout(
          () => controller.abort(new Error("voice preconnect timed out")),
    ...
    Promise.resolve()
      .then(() =>
        fetchImpl(new URL("/", CHATGPT_TRANSCRIPTIONS_URL).toString(), {
          method: "HEAD",
...
async function transcribeVoice(
  ...
  if (!isSupportedVoiceMimeType(mimeType)) {
    throw voiceError(
      ...
      "Only WAV or M4A audio is supported for voice transcription.",
    );
  }
  ...
  if (actualDurationMs > durationMs + MAX_DURATION_DRIFT_MS) {
    throw voiceError(
      "duration_mismatch",
      "The recorded audio duration did not match the voice request.",
    );
  }
...
async function requestTranscription({
  ...
  if (!response.ok) {
    if (response.status === 401 || response.status === 403) {
      throw voiceError("auth_rejected", "Your ChatGPT login has expired. Sign in again.", {
        provider: activeAuthContext.provider,
      });
    }
...
async function readVoiceAuthStatus(sendCodexRequest, { refreshToken = true } = {}) {
  try {
    ...
    console.error("[djl] voice auth: getAuthStatus RPC failed");
    throw voiceError(
      "auth_unavailable",
      "Could not read ChatGPT auth from the Mac runtime. Is the bridge running?",
    );
  }
}
