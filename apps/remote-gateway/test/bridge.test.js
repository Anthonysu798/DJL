...
test("rollout suppression follows the follower's current ownership signal", () => {
  ...
  assert.equal(
    shouldSuppressRolloutMirrorForThread("thread-desktop-owned", { desktopIpcActionFollower }),
    true,
  );
  assert.equal(
    shouldSuppressRolloutMirrorForThread("thread-unowned", { desktopIpcActionFollower }),
    false,
  );
...
test("rollout suppression releases a stale Desktop owner so a growing rollout can mirror", () => {
  ...
  assert.equal(
    shouldSuppressRolloutMirrorForThread("thread-fresh-desktop", {
...
test("Electron bridge forwards every iOS initialize so adapter capabilities reach the phone", () => {
  assert.equal(
    shouldForwardBridgeManagedInitialize({
      codexHandshakeState: "warm",
      usesElectronBackend: true,
    }),
    true,
  );
  assert.equal(
...
test("thread turns-list fast page returns JSONL once and reuses the late canonical page", async () => {
  ...
  const resolveOptions = {
    ...
    readJsonl: async () => ({
      response: {
        ...
        result: {
          data: [
            { id: "turn-jsonl", items: [{ id: "item-jsonl", type: "user_message", role: "user" }] },
          ],
          nextCursor: "djl-jsonl-fallback-older-unavailable",
  ...
  assert.equal(
    ...
    "djl-jsonl-handoff-v1:turn-jsonl:handoff-token",
  );
  assert.equal(fastSelection.response.result.djlCanonicalHandoff, true);
        ...
        response: {
          ...
          result: {
            data: [
              {
                id: "turn-jsonl",
                items: [{ id: "jsonl-item", type: "user_message", role: "user" }],
              },
            ],
            nextCursor: "djl-jsonl-fallback-older-unavailable",
          },
      ...
      fetchCanonical: async () => ({
        ...
        result: {
          data: [
            {
              id: "turn-canonical",
              items: [{ id: "canonical-user", type: "user_message", role: "user" }],
            },
          ],
          nextCursor: null,
        },
...
test("thread turns-list fast page prefers a newer running JSONL turn over stale canonical history", async () => {
      ...
      readJsonl: async () => ({
        response: {
          ...
          result: {
            data: [
              {
                id: "turn-jsonl-running",
                status: "running",
                items: [{ id: "jsonl-running-item", type: "user_message", role: "user" }],
              },
            ],
            nextCursor: "djl-jsonl-fallback-older-unavailable",
          },
  ...
  assert.equal(selection.response.result.djlCanonicalHandoff, true);
});
      ...
      response: {
        ...
        result: {
          data: [
            {
              id: "turn-anchor",
              items: [{ id: "anchor-item", type: "user_message", role: "user" }],
            },
          ],
          nextCursor: "djl-jsonl-fallback-older-unavailable",
        },
    ...
    readJsonl: async () => ({
      response: {
        ...
        result: {
          data: [
            {
              id: "turn-jsonl-anchor",
              items: [{ id: "jsonl-anchor", type: "user_message", role: "user" }],
            },
          ],
          nextCursor: "djl-jsonl-fallback-older-unavailable",
        },
  ...
  const reconcilePromise = coordinator.resolve(
    {
      ...
      params: { ...request.params, djlRequireCanonical: true },
    },
    options,
  );
...
test("canonical reconciliation compacts every turn through the anchor under the relay budget", async () => {
  ...
  assert.equal(reconciled.response.result.djlPageCompactedForRelay, true);
      ...
      response: {
        ...
        result: {
          data: [
            {
              id: "turn-line-2048",
              items: [{ id: "jsonl-synthetic-item", type: "user_message", role: "user" }],
            },
          ],
          nextCursor: "djl-jsonl-fallback-older-unavailable",
        },
...
test("canonical turns-list requests strip bridge-only handoff state", () => {
  const request = {
    ...
    params: {
      ...
      limit: 1,
      cursor: "djl-jsonl-handoff-v1:token",
      djlRequireCanonical: true,
  ...
  assert.equal(request.params.cursor, "djl-jsonl-handoff-v1:token");
});
...
test("newer JSONL turns never displace the canonical cursor anchor at limit one", () => {
  ...
  assert.equal(merged.result.djlJsonlFallback, true);
  assert.equal(merged.result.djlJsonlMergedLatest, true);
});
...
test("normalizeRelayBoundJsonRpcMessage accepts a pre-parsed passthrough envelope", () => {
  ...
  assert.equal(normalizeRelayBoundJsonRpcMessage(raw, { parsedMessage }), raw);
});
...
test("fetchAdaptiveThreadTurnsListForRelay caps initial mobile pages to five turns", async () => {
  ...
  assert.equal(
    response.result.data.some((turn) => turn.id.startsWith("djl-history-compacted-")),
    false,
  );
...
test("fetchAdaptiveThreadTurnsListForRelay returns a compacted single turn when one huge first turn is still too large", async () => {
  ...
  assert.equal(response.result.data[0].djlEmergencySingleTurnForRelay, true);
...
test("sanitizeLiveUserNotification filters fallback context and rewrites visible envelopes", () => {
  ...
  const heartbeat = sanitizeLiveUserNotification({
    ...
    params: {
      threadId: "t",
      message:
        "<heartbeat><automation_id>private</automation_id><instructions>Check CI.</instructions></heartbeat>",
    },
  });
...
test("sanitizeThreadHistoryImagesForRelay sanitizes mixed user content entry-by-entry", () => {
  ...
  assert.deepEqual(sanitized.result.thread.turns[0].items[0].content, [
    ...
    { type: "input_image", url: "djl://history-image-elided" },
  ]);
});
...
test("sanitizeThreadHistoryImagesForRelay restores JSONL context without moving server-only findings", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-jsonl-context-"));
  ...
  assert.deepEqual(
    ...
    ["user_message", "commandExecution", "reviewFinding", "message"],
  );
...
test("sanitizeThreadHistoryImagesForRelay reconciles fallback ids without collapsing repeated occurrences", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-jsonl-fallback-ids-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousCodexHome == null) {
  ...
  fs.mkdirSync(sessionsDir, { recursive: true });
  const rolloutPath = path.join(sessionsDir, `rollout-2026-07-08T20-00-00-${threadId}.jsonl`);
  fs.writeFileSync(
    rolloutPath,
    [
      JSON.stringify({
  ...
  assert.equal(
    items.some((item) => item.id?.startsWith("user-message-line-")),
    false,
  );
  assert.equal(
    items.some((item) => item.id?.startsWith("apply-patch-line-")),
    false,
  );
});
...
test("sanitizeThreadHistoryImagesForRelay folds live-owner assistant ids into JSONL provider rows", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-live-owner-ids-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousCodexHome == null) {
  ...
  assert.equal(
    typeof assistantItems[0].djlSourceItemKey,
    "string",
    "the folded row must adopt the JSONL turn+text source alias",
  );
  assert.equal(assistantItems[0].djlSourceItemKey.startsWith(`${turnId}:`), true);
});

test("sanitizeThreadHistoryImagesForRelay augments app-server history with JSONL fileChange blocks", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-jsonl-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousCodexHome == null) {
  ...
  const interruptedTurn = JSON.parse(
    sanitizeThreadHistoryImagesForRelay(
      JSON.stringify({
  ...
  const invertedServerOrder = JSON.parse(
    sanitizeThreadHistoryImagesForRelay(
      JSON.stringify({
...
test("sanitizeThreadHistoryImagesForRelay caches JSONL artifact scans until the rollout changes", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-jsonl-cache-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
...
test("sanitizeThreadHistoryImagesForRelay lazily restores artifacts for an older cursor turn", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-jsonl-older-artifact-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousCodexHome == null) {
  ...
  const lines = [
    ...
    JSON.stringify({
      type: "event_msg",
      payload: { type: "user_message", message: "Keep the old plan" },
    }),
    JSON.stringify({
      type: "response_item",
      payload: {
  ...
  for (let index = 1; index <= 5; index += 1) {
    lines.push(
      JSON.stringify({
        type: "event_msg",
        payload: { type: "task_started", turn_id: `turn-recent-${index}` },
      }),
      JSON.stringify({
        type: "event_msg",
        payload: { type: "user_message", message: `Recent ${index}` },
      }),
      JSON.stringify({
        type: "response_item",
        payload: {
      ...
      JSON.stringify({
        type: "event_msg",
        payload: { type: "task_complete", turn_id: `turn-recent-${index}` },
      }),
    );
  }
  fs.writeFileSync(
  ...
  assert.equal(sanitized.result.data[0].items[0].djlJsonlProgressPlan, true);
...
test("sanitizeThreadHistoryImagesForRelay restores JSONL update_plan as progress plan history", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-jsonl-plan-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousCodexHome == null) {
  ...
  assert.equal(items[0].djlJsonlProgressPlan, true);
  assert.equal(items[0].djlProgressPlan, true);
...
test("sanitizeThreadHistoryImagesForRelay restores JSONL view_image output previews", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-jsonl-image-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousCodexHome == null) {
  ...
  assert.equal(items[1].djlJsonlToolOutputImage, true);
...
test("sanitizeThreadHistoryImagesForRelay restores JSONL cwd without file changes", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-cwd-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousCodexHome == null) {
...
test("sanitizeThreadHistoryImagesForRelay refreshes JSONL cwd when a newer same-thread rollout appears", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-history-cwd-newer-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousCodexHome == null) {
...
test("sanitizeThreadHistoryImagesForRelay uses CODEX_HOME for generated image fallbacks", (t) => {
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-codex-home-"));
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = codexHome;
  t.after(() => {
    if (previousCodexHome == null) {
...
test("sanitizeThreadHistoryImagesForRelay compacts oversized turns pages", () => {
  ...
  assert.equal(sanitized.result.djlPageCompactedForRelay, true);
  assert.deepEqual(
    sanitized.result.items.map((turn) => turn.id),
    ["turn-1"],
  );
  assert.equal(
    sanitized.result.items.some((turn) => turn.id.startsWith("djl-history-compacted-")),
    false,
  );
  assert.equal(sanitized.result.items[0].djlPageCompactedForRelay, true);
...
test("sanitizeThreadHistoryImagesForRelay preserves oversized turns pages instead of replacing them with a marker", () => {
  ...
  assert.equal(
    sanitized.result.data.some((turn) => turn.id.startsWith("djl-history-compacted-")),
    false,
  );
...
test("sanitizeThreadHistoryImagesForRelay bounds an extreme provisional JSONL turn", () => {
  ...
  const rawMessage = JSON.stringify({
    ...
    result: {
      ...
      nextCursor: "djl-jsonl-handoff-v1:turn-line-4096:extreme-token",
      djlJsonlFallback: true,
  ...
  assert.equal(sanitized.result.djlEmergencyJsonlPageForRelay, true);
  assert.equal(sanitized.result.djlJsonlFallback, true);
  assert.equal(sanitized.result.djlCanonicalHandoff, true);
...
test("sanitizeThreadHistoryImagesForRelay compacts oversized history before the newest turn tail", () => {
  ...
  assert.equal(sanitized.result.thread.djlHistoryCompacted, true);
  assert.equal(sanitized.result.thread.djlOmittedTurnCount, 1);
  assert.equal(sanitized.result.thread.djlKeptTurnCount, 1);
  assert.deepEqual(
    sanitized.result.thread.turns.map((turn) => turn.id),
    ["djl-history-compacted-turn-old", "turn-new"],
  );
  assert.equal(
...
test("sanitizeThreadHistoryImagesForRelay keeps the newest sixteen turns when compacting", () => {
  ...
  assert.equal(sanitized.result.thread.djlHistoryCompacted, true);
  assert.equal(sanitized.result.thread.djlOmittedTurnCount, 29);
  assert.equal(sanitized.result.thread.djlKeptTurnCount, 16);
  assert.deepEqual(
    sanitized.result.thread.turns.map((turn) => turn.id),
    ["djl-history-compacted-turn-1", ...turns.slice(29).map((turn) => turn.id)],
  );
...
test("sanitizeThreadHistoryImagesForRelay compacts oversized raw histories before sanitizing turns", () => {
  ...
  assert.equal(sanitized.result.thread.historyTailTruncatedForRelay, true);
  assert.equal(sanitized.result.thread.djlHistoryCompacted, true);
  assert.equal(sanitized.result.thread.djlOmittedTurnCount, 34);
  assert.equal(sanitized.result.thread.djlKeptTurnCount, 16);
  assert.deepEqual(
    sanitized.result.thread.turns.map((turn) => turn.id),
    ["djl-history-compacted-turn-1", ...turns.slice(34).map((turn) => turn.id)],
  );
