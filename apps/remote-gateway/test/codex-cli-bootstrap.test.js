...
test("ensureCodexCLI installs Codex when it is missing", () => {
  ...
  assert.deepEqual(messages, [
    "[djl] Checking Codex CLI...",
    "[djl] Codex CLI not found.",
    "[djl] Installing Codex CLI via npm (@openai/codex@latest)...",
    "[djl] Codex CLI installed (0.120.0).",
  ]);
});
...
test("ensureCodexCLI updates Codex when it is already installed", () => {
  ...
  assert.deepEqual(messages, [
    "[djl] Checking Codex CLI...",
    "[djl] Codex CLI found (0.118.0).",
    "[djl] Updating Codex CLI via npm (@openai/codex@latest)...",
    "[djl] Codex CLI updated (0.120.0).",
  ]);
});
...
test("shouldSkipCodexBootstrap respects the opt-out env flag", () => {
  assert.equal(shouldSkipCodexBootstrap({ DJL_SKIP_CODEX_BOOTSTRAP: "1" }), true);
  assert.equal(shouldSkipCodexBootstrap({ DJL_SKIP_CODEX_BOOTSTRAP: "true" }), true);
  assert.equal(shouldSkipCodexBootstrap({ DJL_SKIP_CODEX_BOOTSTRAP: "0" }), false);
});
