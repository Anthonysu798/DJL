...
test("desktop-origin active runs replay thinking and exec command activity on resume", async (t) => {
  ...
  assert.equal(outbound[0].params.djlDesktopMirror, true);
  ...
  for (const message of outbound.slice(0, 5)) {
    assert.equal(message.params.djlRolloutBootstrapReplay, true);
  }
  assert.equal(outbound[5].params.djlRolloutBootstrapComplete, true);
  assert.equal(outbound[5].params.turnId, "turn-live");
  assert.equal(outbound[5].params.djlRolloutBootstrapReplay, undefined);
});
...
test("desktop-origin active runs emit activity heartbeat while rollout is quiet", async (t) => {
  ...
  assert.equal(heartbeat.params.djlDesktopMirror, true);
  assert.equal(outbound.at(-1).params.djlRolloutLiveMirror, true);
});
...
test("desktop-origin bootstrap replays the pending user message and final assistant text", async (t) => {
  ...
  assert.equal(outbound[0].params.djlRolloutLiveMirror, true);
  assert.equal(outbound.at(-1).params.djlRolloutBootstrapComplete, true);
...
test("desktop-origin bootstrap emits terminal catch-up for completed runs", async (t) => {
  ...
  assert.equal(outbound[0].params.djlRolloutTerminalCatchUp, true);
  assert.equal(outbound[0].params.djlRolloutBootstrapReplay, undefined);
});
...
test("desktop-origin bootstrap emits terminal catch-up for aborted runs", async (t) => {
  const { homeDir } = createTemporaryRolloutHome({
    ...
    originator: "Codex Desktop",
    source: "desktop",
    lines: [taskStarted("turn-terminal-aborted"), turnAborted("turn-terminal-aborted")],
  });
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = homeDir;
  t.after(() => {
...
test("desktop-origin live tail keeps the active run alive when a sibling turn completes", async (t) => {
  const { homeDir, rolloutPath } = createTemporaryRolloutHome({
    ...
    originator: "Codex Desktop",
    source: "desktop",
    lines: [taskStarted("turn-live-active")],
  });
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = homeDir;
  t.after(() => {
  ...
  const siblingCompleted = outbound.find(
    (message) =>
      message.method === "turn/completed" && message.params.turnId === "turn-older-sibling",
  );
  assert.ok(siblingCompleted, "sibling completion should still be mirrored");

  const laterMessage = outbound.find(
    (message) =>
  ...
  const activeCompleted = outbound.find(
    (message) =>
      message.method === "turn/completed" && message.params.turnId === "turn-live-active",
  );
...
test("desktop-origin mirror stops heartbeating when a crashed run's rollout stays frozen", async (t) => {
  ...
  const { homeDir } = createTemporaryRolloutHome({
    ...
    originator: "Codex Desktop",
    source: "desktop",
    lines: [taskStarted("turn-crash-frozen")],
  });
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = homeDir;
  t.after(() => {
  ...
  await wait(45);
  const heartbeatsBeforeStale = outbound.filter(
    (message) => message.method === "turn/activity",
  ).length;
  assert.ok(heartbeatsBeforeStale >= 1, "heartbeats should flow while inside the stale window");

  await wait(80);
  const heartbeatsAfterStale = outbound.filter(
    (message) => message.method === "turn/activity",
  ).length;
...
test("desktop-origin mirror re-bootstraps after rollout truncation instead of replaying live", async (t) => {
  ...
  fs.writeFileSync(
    rolloutPath,
    [header, taskStarted("turn-truncate-new"), taskComplete("turn-truncate-new"), ""].join("\n"),
  );

  await wait(40);

  const terminalCatchUp = outbound.find(
    (message) =>
      message.method === "turn/completed" && message.params.djlRolloutTerminalCatchUp === true,
  );
  ...
  const untaggedLiveReplay = outbound.find(
    (message) =>
      message.method === "codex/event/agent_message" && !message.params.djlRolloutBootstrapReplay,
  );
  assert.equal(
    untaggedLiveReplay,
    undefined,
    "rewritten contents must not replay as untagged live events",
  );
});
...
test("desktop-origin mirror dedupes the same assistant text across event and response_item shapes", async (t) => {
  const { homeDir, rolloutPath } = createTemporaryRolloutHome({
    ...
    originator: "Codex Desktop",
    source: "desktop",
    lines: [taskStarted("turn-dedupe-shapes")],
  });
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = homeDir;
  t.after(() => {
  ...
  const duplicates = outbound.filter(
    (message) =>
      message.method === "codex/event/agent_message" &&
  ...
  assert.equal(
    duplicates.length,
    1,
    "event_msg and response_item copies of the same text must collapse",
  );
  assert.match(
    duplicates[0].params.djlSourceItemKey,
...
test("desktop-origin mirror keeps repeated identical user steers while deduping their response copies", async (t) => {
  ...
  const userMessages = outbound.filter(
    (message) =>
      message.method === "codex/event/user_message" && message.params.message === "keep going",
  );
...
test("desktop-origin sibling terminal does not hijack a synthetic active turn", async (t) => {
  const { homeDir, rolloutPath } = createTemporaryRolloutHome({
    ...
    originator: "Codex Desktop",
    source: "desktop",
    lines: [taskStartedWithoutTurnId()],
  });
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = homeDir;
  t.after(() => {
  ...
  await wait(20);
  appendRolloutLines(rolloutPath, [taskComplete("turn-parallel-sibling")]);
  await wait(20);
  ...
  assert.equal(
    ...
    undefined,
    "synthetic turn should stay open during grace",
  );

  appendRolloutLines(rolloutPath, [agentMessage("Synthetic run continues", "commentary")]);
  await wait(30);

  const siblingCompleted = outbound.find(
    (message) =>
      message.method === "turn/completed" && message.params.turnId === "turn-parallel-sibling",
  );
  assert.ok(siblingCompleted, "sibling completion should still be mirrored");

  const continuation = outbound.find(
    (message) =>
      message.method === "codex/event/agent_message" &&
...
test("desktop-origin terminal-only real id closes the synthetic active turn", async (t) => {
  ...
  await wait(20);
  appendRolloutLines(rolloutPath, [taskComplete("turn-real-terminal")]);
  await wait(70);

  const realCompleted = outbound.find(
    (message) =>
      message.method === "turn/completed" && message.params.turnId === "turn-real-terminal",
  );
...
test("desktop-origin mirror keeps commentary prose interleaved with tool calls", async (t) => {
  ...
  const agentMessages = outbound.filter(
    (message) => message.method === "codex/event/agent_message",
  );
  assert.deepEqual(
...
test("desktop-origin mirror stays alive on heartbeat-only active runs", async (t) => {
  const { homeDir } = createTemporaryRolloutHome({
    ...
    originator: "Codex Desktop",
    source: "desktop",
    lines: [taskStarted("turn-heartbeat-idle")],
  });
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = homeDir;
  t.after(() => {
  ...
  assert.ok(
    heartbeats.length >= 4,
    `expected heartbeat mirror to stay alive, got ${heartbeats.length}`,
  );
});
...
test("desktop-origin mirror flushes a valid partial EOF line before stopping", async (t) => {
  ...
  assert.ok(
    outbound.some(
      (message) =>
        message.method === "turn/started" && message.params.turnId === "turn-partial-eof",
    ),
  );
});
...
test("desktop-origin update_plan calls mirror as structured activity plan updates", async (t) => {
  ...
  assert.deepEqual(
    outbound.map((message) => message.method),
    ["turn/started", "item/reasoning/textDelta", "turn/plan/updated", "turn/activity"],
  );
  ...
  assert.equal(outbound[2].params.djlDesktopMirror, true);
  assert.equal(
...
test("desktop-origin completed plan items mirror as final plan rows", async (t) => {
  ...
  assert.equal(outbound[2].params.djlRolloutBootstrapComplete, true);
  ...
  assert.equal(outbound[3].params.djlRolloutBootstrapReplay, undefined);
  assert.equal(outbound[4].params.djlRolloutBootstrapReplay, undefined);
});
...
test("desktop-origin task_started without turn_id still mirrors live file changes", async (t) => {
  ...
  assert.equal(outbound[5].params.djlTurnFileChangeSnapshot, true);
...
test("desktop-origin stale runs resume live mirroring when the rollout grows again", async (t) => {
  const { homeDir, rolloutPath } = createTemporaryRolloutHome({
    ...
    originator: "Codex Desktop",
    source: "desktop",
    lines: [userMessage("Long lost run"), taskStarted("turn-stale-resume")],
  });
  const staleDate = new Date(Date.now() - 60 * 60_000);
  fs.utimesSync(rolloutPath, staleDate, staleDate);
  const previousCodexHome = process.env.CODEX_HOME;
  ...
  appendRolloutLines(rolloutPath, [agentMessage("Back from the dead", "final_answer")]);
  await wait(40);

  const agentMessageNotification = outbound.find(
    (message) => message.method === "codex/event/agent_message",
  );
  assert.ok(agentMessageNotification);
  assert.equal(agentMessageNotification.params.turnId, "turn-stale-resume");
  const heartbeat = outbound.find(
    (message) =>
      message.method === "turn/activity" &&
      message.params.turnId === "turn-stale-resume" &&
      message.params.djlRolloutBootstrapComplete !== true,
  );
...
test("desktop-origin live tail closes mirrored turns on turn_aborted", async (t) => {
  const { homeDir, rolloutPath } = createTemporaryRolloutHome({
    ...
    originator: "Codex Desktop",
    source: "desktop",
    lines: [taskStarted("turn-live-abort")],
  });
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = homeDir;
  t.after(() => {
...
test("desktop-origin live tail closes mirrored turns on fatal error", async (t) => {
  const { homeDir, rolloutPath } = createTemporaryRolloutHome({
    ...
    originator: "Codex Desktop",
    source: "desktop",
    lines: [taskStarted("turn-live-error")],
  });
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = homeDir;
  t.after(() => {
...
test("desktop-origin rollouts emit turn-end file-change snapshot after final text", async (t) => {
  ...
  const aggregateIndex = outbound.findIndex(
    (message) =>
      ...
      message.params.djlTurnFileChangeSnapshot === true,
  );
...
test("desktop-origin bootstrap preserves response_item user openers exactly once", async (t) => {
  ...
  assert.deepEqual(
    users.map((message) => message.params.message),
    ["Response opener before start", "Response opener after start"],
  );
  const assistantIndex = outbound.findIndex(
    (message) => message.method === "codex/event/agent_message",
  );
...
test("desktop-origin bootstrap represents an image-only response user opener", async (t) => {
  const { homeDir } = createTemporaryRolloutHome({
    threadId: "thread-response-image-opener",
    originator: "Codex Desktop",
    source: "desktop",
    lines: [
      responseImageUserMessage(),
      taskStarted("turn-response-image-opener"),
      agentMessage("image handled"),
    ],
  });
  const previousCodexHome = process.env.CODEX_HOME;
  process.env.CODEX_HOME = homeDir;
  ...
  controller.observeInbound(
    JSON.stringify({
      method: "thread/resume",
      params: { threadId: "thread-response-image-opener" },
    }),
  );
  await wait(40);
  assert.equal(
    outbound.find((m) => m.method === "codex/event/user_message")?.params.message,
    "Image attachment",
  );
});
...
test("capped bootstrap suppresses orphan deltas and recovers from a new appended boundary", async (t) => {
  ...
  await wait(80);
  assert.equal(
    outbound.some((m) => m.params?.message === "OLD ORPHAN"),
    false,
  );
  ...
  assert.equal(
    outbound.some((m) => m.params?.message === "ignored orphan delta"),
    false,
  );
  appendRolloutLines(rolloutPath, [
    userMessage("Recovered opener"),
  ...
  assert.deepEqual(
    methods.map((m) => m.params.message),
    ["Recovered opener", "Recovered assistant"],
  );
});
