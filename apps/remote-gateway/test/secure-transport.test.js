...
test("pairing QR contains public bootstrap metadata but no relay authentication secret", () => {
  const macIdentity = createOkpKeyPair("ed25519");
  const secureTransport = createTestBridgeSecureTransport({
    sessionId: "session-public-qr",
    relayUrl: "wss://relay.example/relay",
    displayName: "Desk Mac",
    deviceState: {
      macDeviceId: "mac-public-qr",
      macIdentityPrivateKey: macIdentity.privateKey,
      macIdentityPublicKey: macIdentity.publicKey,
      relaySessionSecret: "must-never-enter-the-qr-payload",
      trustedPhones: {},
    },
  });
  ...
  assert.doesNotMatch(JSON.stringify(payload), /relaySessionSecret|must-never-enter/);
});
...
test("secure transport round-trips encrypted payloads after a trusted reconnect handshake", () => {
  ...
  const infoPrefix = `djl-e2ee-v1|session-2|mac-2|phone-2|${serverHello.keyEpoch}`;
...
test("rebinding the relay socket replays bridge output from the last phone ack", () => {
  ...
  const salt = createHash("sha256").update(transcriptBytes).digest();
  const infoPrefix = `djl-e2ee-v1|session-5|mac-5|phone-5|${serverHello.keyEpoch}`;
  const macToPhoneKey = Buffer.from(
    hkdfSync("sha256", sharedSecret, salt, Buffer.from(`${infoPrefix}|macToPhone`, "utf8"), 32),
  );
...
test("same-session relay replays keep live notifications untagged", () => {
  ...
  assert.equal(JSON.parse(livePayload.payloadText).params.djlReplayedEvent, undefined);
  ...
  assert.equal(replayedNotificationPayload.params.djlReplayedEvent, undefined);
...
test("previous-session replayed notifications are tagged as catch-up history", () => {
  ...
  assert.equal(replayedNotificationPayload.params.djlReplayedEvent, true);
  ...
  assert.equal(completionMarkerPayload.method, "djl/bufferedReplay/completed");
  assert.equal(completionMarkerPayload.params.djlBufferedReplayComplete, true);
});
...
test("truncated resume replay discards the partial tail and declares a canonical-history gap", () => {
  ...
  assert.equal(gapMessage.method, "djl/bufferedReplay/gap");
  ...
  assert.equal(JSON.parse(completionPayload.payloadText).params.djlBufferedReplayComplete, true);
});
...
test("resume replay does not advance the replay watermark before a phone ack", () => {
  ...
  const salt = createHash("sha256").update(transcriptBytes).digest();
  const infoPrefix = `djl-e2ee-v1|session-6|mac-6|phone-6|${serverHello.keyEpoch}`;
  const macToPhoneKey = Buffer.from(
    hkdfSync("sha256", sharedSecret, salt, Buffer.from(`${infoPrefix}|macToPhone`, "utf8"), 32),
  );
...
test("resume replay resets overlapping sequence numbers from a previous bridge epoch", () => {
  ...
  assert.equal(resetMessage.method, "djl/bufferedReplay/reset");
...
function buildTranscriptBytes({
  ...
  return Buffer.concat([
    encodeLengthPrefixedUTF8("djl-e2ee-v1"),
