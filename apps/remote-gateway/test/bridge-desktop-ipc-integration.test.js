...
test("bridge forwards desktop IPC actions to the phone and routes replies back to Codex Desktop", async (t) => {
  const { tempDir, socketPath: ipcSocketPath } = createIpcTestSocket("djl-bridge-ipc-");
...
test("bridge recovers desktop IPC state when the first live update is patch-only", async (t) => {
  const { tempDir, socketPath: ipcSocketPath } = createIpcTestSocket("djl-bridge-ipc-recovery-");
  const relayServer = new WebSocket.Server({ port: 0 });
  const relayMessages = [];
  let relaySocket = null;
  let ipcServerSocket = null;
...
test("bridge forwards live desktop assistant deltas to the phone", async (t) => {
  const { tempDir, socketPath: ipcSocketPath } = createIpcTestSocket("djl-bridge-ipc-delta-");
  const relayServer = new WebSocket.Server({ port: 0 });
  const relayMessages = [];
  let relaySocket = null;
  let ipcServerSocket = null;
...
test("bridge serves Desktop-owned thread history from cached IPC state", async (t) => {
  const { tempDir, socketPath: ipcSocketPath } = createIpcTestSocket("djl-bridge-ipc-read-");
  const relayServer = new WebSocket.Server({ port: 0 });
  const relayMessages = [];
  let relaySocket = null;
  let ipcServerSocket = null;
...
test("bridge maps Desktop IPC archive broadcasts to phone notifications", async (t) => {
  const { tempDir, socketPath: ipcSocketPath } = createIpcTestSocket("djl-bridge-ipc-archive-");
  const relayServer = new WebSocket.Server({ port: 0 });
  const relayMessages = [];
  let relaySocket = null;
  let ipcServerSocket = null;
  ...
  assert.equal(archiveMessage.params.djlDesktopIpcMirror, true);
  ...
  assert.equal(unarchiveMessage.params.djlDesktopIpcMirror, true);
});
...
test("bridge observes held desktop IPC turns only after local fallback", async (t) => {
  ...
  assert.equal(
    fakeCodex.sent.some((message) => message.id === "held-turn-start"),
    false,
  );

  followerOptions.forwardToLocalCodex(heldTurnStart);
  await waitFor(() => liveOwnerInbound.some((message) => message.id === "held-turn-start"));
  assert.equal(liveOwnerInbound.filter((message) => message.id === "held-turn-start").length, 1);
  assert.equal(fakeCodex.sent.filter((message) => message.id === "held-turn-start").length, 1);
});

test("bridge isolation mode never connects to the global Codex Desktop IPC bus", async (t) => {
  const relayServer = new WebSocket.Server({ port: 0 });
  const scopeDir = fs.mkdtempSync(path.join(os.tmpdir(), "djl-owned-thread-integration-"));
  const relayMessages = [];
  let relaySocket = null;
  let bridge = null;
  let fakeCodex = null;
  let followerCreateCount = 0;
  let liveOwnerCreateCount = 0;
  ...
  const { startBridge } = loadBridgeWithTestDoubles({
    ...
    desktopIpcLiveOwnerModule: {
      createDesktopIpcLiveOwner() {
        liveOwnerCreateCount += 1;
        return { observeInbound() {}, observeOutbound() {}, stopAll() {} };
      },
    },
  });
  ...
  bridge = startBridge({
    ...
    config: {
      ...
      desktopIpcLiveSyncEnabled: false,
      remoteThreadScopeEnabled: true,
      remoteThreadScopeStatePath: path.join(scopeDir, "owned-threads.json"),
    },
  });
  ...
  relaySocket.send(
    JSON.stringify({
      id: "isolated-thread-list",
      method: "thread/list",
      params: { limit: 20 },
    }),
  );

  await waitFor(() => fakeCodex.sent.some((message) => message.id === "isolated-thread-list"));
  fakeCodex.emitMessage({
    id: "isolated-thread-list",
    result: {
      data: [
        { id: "thread-global-codex", cwd: "/repo" },
        { id: "thread-djl", cwd: "/repo" },
      ],
      nextCursor: null,
    },
  });
  const filteredList = await waitForMessage(
    relayMessages,
    (message) => message.id === "isolated-thread-list",
  );
  assert.deepEqual(
    filteredList.result.data.map((thread) => thread.id),
    ["thread-djl"],
  );

  relaySocket.send(
    JSON.stringify({
      id: "read-global-codex",
      method: "thread/read",
      params: { threadId: "thread-global-codex" },
    }),
  );
  const rejectedRead = await waitForMessage(
    relayMessages,
    (message) => message.id === "read-global-codex",
  );
  assert.equal(rejectedRead.error.data.errorCode, "thread_not_owned_by_djl");
  assert.equal(
    fakeCodex.sent.some((message) => message.id === "read-global-codex"),
    false,
  );
  assert.equal(followerCreateCount, 0);
  assert.equal(liveOwnerCreateCount, 0);
});
  ...
  return {
    loadOrCreateBridgeDeviceState() {
      return {
        ...
        relaySessionSecret: "test-relay-session-secret-that-is-long-enough",
        trustedPhones: {},
      };
    },
