...
const { buildConversationStatePatches } = require("../src/desktop-ipc-state-patches");
        ...
        items: [
          {
            id: "ctx-agents",
            type: "userMessage",
            content: [{ type: "input_text", text: agentsText }],
          },
...
test("conversation adapter supplies receiverThreads required by Desktop collab rendering", () => {
  ...
  assert.deepEqual(state.turns[0].items[0].receiverThreads, [
...
test("conversation adapter streams fileChange output deltas into fileChange items", () => {
  ...
  const turn = conversations
    .get("thread-file-change")
    .turns.find((candidate) => candidate.turnId === "turn-file-change");
...
test("conversation adapter keeps turnless fileChange events off optimistic pending turns", () => {
  ...
  const optimisticTurnId = `djl-pending-turn:${threadId}:request-2`;
  const conversations = new Map();
...
test("conversation adapter keeps the prompt in params and drops the echoed userMessage item", () => {
  ...
  assert.deepEqual(conversations.get("thread-canonical-user").turns[0].params.input, [
...
test("conversation adapter dedupes the echoed prompt after fallback turn id promotion", () => {
  ...
  assert.deepEqual(fallbackTurn.params.input, [
    { type: "input_text", text: "prompt before promotion" },
  ]);
    ...
    [
      ...
      [
        ...
        },
        {
          params: {
            threadId: "thread-fifo",
            input: [{ type: "input_text", text: "second prompt" }],
          },
        },
      ],
    ],
...
test("conversation adapter keeps a stable fallback turn until a real turn id arrives", () => {
  ...
  assert.match(syntheticTurnId, /^djl-live-turn:thread-turnless:/);
