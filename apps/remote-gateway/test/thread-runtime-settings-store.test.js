...
test("persists accepted per-thread runtime settings and explicit normal speed", (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "djl-runtime-settings-"));
  ...
  const normal = firstStore.commit(
    "thread-1",
    {
      model: "gpt-5.5",
      effort: "medium",
...
test("enriches thread list and read responses without exposing a separate side channel", (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "djl-runtime-enrich-"));
  const storeFile = path.join(directory, "settings.json");
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
...
test("ignores Desktop-origin runtime settings and legacy persisted records", (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "djl-runtime-phone-authority-"));
  const storeFile = path.join(directory, "settings.json");
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  ...
  assert.equal(
    store.commit(
      ...
      { source: "desktop", turnId: "turn-new-desktop" },
    ),
    null,
  );
