...
// Purpose: Verifies DJL-owned Codex streams are exposed to Desktop/VSCode through the local IPC bus.
// Layer: Unit test
...
test("conversation adapter ignores empty plan updates but keeps explanation-only updates", () => {
  ...
  assert.equal(conversations.get(threadId).turns[0].items[0].djlProgressPlan, true);
});
...
test("conversation adapter mirrors phone auto-approval review lifecycle into Desktop state", () => {
  ...
  assert.equal(items[0].djlGuardianRetrySupported, false);
});

test("live owner broadcasts DJL-owned thread snapshots over Desktop IPC", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-");
  const frames = [];
    ...
    attachFrameReader(socket, (frame) => {
      ...
      if (frame.method === "initialize") {
        writeFrame(socket, {
          ...
          result: { clientId: "djl-owner-test" },
        });
      }
    });
  });
  ...
  assert.equal(broadcast.params.djlOwnerSource, "desktop-ipc-live-owner");
...
test("live owner broadcasts patches after the first owned thread snapshot", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-patches-");
  const frames = [];
  let serverSocket = null;
  ...
  const turnPatchBroadcast = await waitForMessage(
    ...
    (frame) =>
      ...
      frame.params.change.patches.some(
        (patch) =>
          patch.op === "add" && JSON.stringify(patch.path) === JSON.stringify(["turns", 0]),
      ),
  );
...
test("live owner starts a local IPC router when no Codex IPC socket exists", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-router-");
  const codexRequests = [];
  const desktopFrames = [];
  ...
  assert.deepEqual(
    codexRequests.filter((request) => request.method === "turn/start"),
    [
      {
        method: "turn/start",
        params: {
...
test("live owner seeds existing thread snapshots from thread reads before ownership", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-existing-");
  const frames = [];
  let serverSocket = null;
...
test("live owner does not rewind live state from a stale cached thread on later turns", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-stale-cache-");
  let serverSocket = null;
...
test("live owner keeps phone turn input in snapshots when turn/started has no items", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-user-input-");
  const frames = [];
  let serverSocket = null;
...
test("live owner broadcasts phone image turns before turn/started arrives", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-image-pending-");
  const frames = [];
  let serverSocket = null;
  ...
  assert.match(pendingTurn.turnId, /^djl-pending-turn:/);
  assert.equal(pendingTurn.djlOptimisticPendingTurn, true);
  ...
  await waitFor(
    () => owner._debugSnapshot("thread-image-pending")?.turns?.[0]?.turnId === "turn-image-pending",
  );
  const promotedState = owner._debugSnapshot("thread-image-pending");
  assert.equal(promotedState.turns.length, 1);
  assert.equal(promotedState.turns[0].djlOptimisticPendingTurn, undefined);
...
test("live owner promotes rapid pending turns in FIFO order", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-pending-fifo-");
  let serverSocket = null;
  ...
  await waitFor(
    () => owner._debugSnapshot("thread-pending-fifo")?.turns?.[0]?.turnId === "turn-fifo-1",
  );
  let turns = owner._debugSnapshot("thread-pending-fifo").turns;
  assert.equal(turns.length, 2);
  assert.equal(turns[0].params.input[0].text, "first");
  assert.match(turns[1].turnId, /^djl-pending-turn:/);
  ...
  await waitFor(
    () => owner._debugSnapshot("thread-pending-fifo")?.turns?.[1]?.turnId === "turn-fifo-2",
  );
  turns = owner._debugSnapshot("thread-pending-fifo").turns;
  assert.deepEqual(
    turns.map((turn) => turn.turnId),
    ["turn-fifo-1", "turn-fifo-2"],
  );
  assert.deepEqual(
...
test("live owner promotes rapid turn-less pending starts in FIFO order", async (t) => {
  ...
  await waitFor(() => {
    ...
    return (
      turns.length === 2 &&
      turns.every((turn) => !turn.djlOptimisticPendingTurn) &&
      turns[0].params.input[0].text === "first turnless" &&
      turns[1].params.input[0].text === "second turnless"
    );
  });
  ...
  assert.deepEqual(
    turns.map((turn) => turn.params.input[0].text),
    ["first turnless", "second turnless"],
  );
});

test("live owner router keeps same-id requests from different clients separate", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-routed-ids-");
  ...
  const routedRequests = handlerFrames.filter(
    (frame) => frame.type === "request" && frame.method === "desktop-owned-action",
  );
  assert.equal(routedRequests.length, 2);
  assert.notEqual(routedRequests[0].requestId, routedRequests[1].requestId);
});

test("live owner router prefers DJL handler when multiple clients can handle", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-router-priority-");
  let desktopSocket = null;
  let djlSocket = null;
  ...
  const djl = await connectHandler("djl-priority-init", "djl-bridge", "djl");
  djlSocket = djl.socket;
  ...
  assert.deepEqual(response.result, { handledBy: "djl" });
  assert.equal(
    desktop.frames.some(
      (frame) => frame.type === "request" && frame.method === "desktop-owned-action",
    ),
  ...
  assert.equal(
    djl.frames.some((frame) => frame.type === "request" && frame.method === "desktop-owned-action"),
    true,
  );
});

test("live owner hydrates existing threads before first mobile-owned snapshot", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-hydrate-");
  const frames = [];
  const codexRequests = [];
  let serverSocket = null;
...
test("live owner hydrates unknown threads before prompt-less owner actions publish", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-hydrate-compact-");
  const frames = [];
  let serverSocket = null;
  let resolveThreadRead = null;
...
test("live owner does not publish partial existing-thread snapshots when hydration fails", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-hydrate-fail-");
  const frames = [];
  let serverSocket = null;
...
test("live owner retries failed initial hydration without another owner event", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-hydrate-retry-");
  const frames = [];
  let serverSocket = null;
  let readAttempts = 0;
...
test("live owner releases ownership after bounded initial hydration failures", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-hydrate-bounded-");
  const frames = [];
  let serverSocket = null;
  let readAttempts = 0;
...
test("live owner handles discovery and start-turn follower requests for owned threads", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-follower-");
  const codexRequests = [];
  ...
  const unsupportedDiscoveryResponse = await waitForFrame(
    serverSocket,
    (frame) =>
      frame.type === "client-discovery-response" && frame.requestId === "discovery-unsupported",
  );
  ...
  assert.equal(appNotifications[0].params.djlDesktopMirror, true);
  assert.equal(appNotifications[0].params.djlDesktopIpcMirror, true);
});

test("live owner dedupes held phone turn starts routed back through follower IPC", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-held-dedupe-");
  let serverSocket = null;
  ...
  assert.equal(
    secondTurn.items.some((item) => item.type === "userMessage"),
    false,
  );
});

test("live owner broadcasts a removed snapshot before archive cleanup", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-archive-remove-");
  const frames = [];
  let serverSocket = null;
  ...
  const removed = await waitForMessage(
    ...
    (frame) =>
      ...
      frame.method === "thread-stream-state-changed" &&
      frame.params?.conversationId === "thread-archive-remove" &&
      frame.params?.djlOwnerReleased === true,
  );
  assert.equal(removed.params.djlOwnerSource, "desktop-ipc-live-owner");
  ...
  assert.equal(removed.params.change.conversationState.djlRemoved, true);
...
test("live owner broadcasts phone archive requests for unowned threads over Desktop IPC", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-archive-unowned-");
  const frames = [];
  let serverSocket = null;
...
test("live owner broadcasts phone unarchive requests over Desktop IPC", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-unarchive-");
  const frames = [];
  let serverSocket = null;
...
test("live owner flushes only the final archive metadata state after IPC connects", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-archive-order-");
  const frames = [];
  let serverSocket = null;
  ...
  await wait(25);
  const archiveBroadcasts = frames.filter(
    (frame) =>
      frame.type === "broadcast" &&
      frame.method === "thread-archived" &&
      frame.params?.conversationId === "thread-archive-order",
...
test("live owner yields ownership when a peer archives an owned thread", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-peer-archive-");
  const frames = [];
  let serverSocket = null;
...
test("live owner normalizes follower start-turn params before app-server requests", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-normalize-");
  const codexRequests = [];
  let serverSocket = null;
...
test("live owner applies Desktop runtime overrides to later follower turn starts", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-runtime-overrides-");
  const codexRequests = [];
  let serverSocket = null;
...
test("live owner serves follower load-complete-history with a fresh snapshot revision", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-load-history-");
  const frames = [];
  let serverSocket = null;
...
test("live owner applies Desktop thread settings and broadcasts phone read state", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-settings-read-");
  const frames = [];
  const codexRequests = [];
  let serverSocket = null;
...
test("live owner runs Desktop queued follow-ups between turns", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-queue-");
  const frames = [];
  const codexRequests = [];
  let serverSocket = null;
  ...
  assert.deepEqual(queuedStart.params.input, [
    { type: "text", text: "queued follow-up from desktop" },
  ]);
...
test("live owner pauses undecodable queued entries and halts the queue after interrupts", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-queue-guards-");
  const frames = [];
  const codexRequests = [];
  let serverSocket = null;
  ...
  const pausedBroadcast = await waitForMessage(
    ...
    (frame) =>
      frame.type === "broadcast" &&
      frame.method === "thread-queued-followups-changed" &&
      frame.params?.messages?.[0]?.pausedReason === "djl-unsupported-entry",
  );
...
test("live owner dedupes read-state broadcasts for already-clean threads", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-read-dedupe-");
  const frames = [];
  let serverSocket = null;
...
test("live owner normalizes image_url input entries for Desktop snapshots", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-image-");
  const frames = [];
  let serverSocket = null;
...
test("live owner announces phone threads to the Desktop sidebar once", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-sidebar-");
  const frames = [];
  let serverSocket = null;
  ...
  const announcementCount = () =>
    frames.filter((frame) => frame.type === "broadcast" && frame.method === "thread-unarchived")
      .length;
...
test("live owner keeps ownership when peer sends non-owner patch broadcasts", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-peer-patch-");
  const codexRequests = [];
  let serverSocket = null;
...
test("live owner yields ownership when a peer sends an untagged snapshot", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-peer-snapshot-");
  const codexRequests = [];
  let serverSocket = null;
...
test("live owner keeps mid-run threads when a peer sends an idle snapshot", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-idle-claim-");
  let serverSocket = null;
...
test("live owner releases idle threads on unsubscribe but keeps running ones", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-unsub-");
  let serverSocket = null;
...
test("live owner drops cached conversation state when yielding to a peer owner", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-yield-state-");
  const frames = [];
  let serverSocket = null;
...
test("live owner routes follower approval responses back to app-server", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-approval-");
...
test("live owner replies to follower approvals with the original app-server id", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-approval-id-");
  const rawCodexMessages = [];
  let serverSocket = null;
...
test("live owner pairs notification-only thread starts by requested cwd", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-start-cwd-");
  const frames = [];
  let serverSocket = null;
  ...
  assert.equal(
    frames.some(
      (frame) => frame.type === "broadcast" && frame.params?.conversationId === "thread-other-cwd",
    ),
    false,
  );
...
test("live owner converts desktop permission approvals into grant payloads", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-live-owner-permissions-");
  const rawCodexMessages = [];
  let serverSocket = null;
