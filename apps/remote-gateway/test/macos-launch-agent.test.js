...
test("buildLaunchAgentPlist points launchd at run-service with djl state paths", () => {
  const plist = buildLaunchAgentPlist({
    homeDir: "/Users/tester",
    pathEnv: "/usr/local/bin:/usr/bin",
    stateDir: "/Users/tester/.djl",
    stdoutLogPath: "/Users/tester/.djl/logs/bridge.stdout.log",
    stderrLogPath: "/Users/tester/.djl/logs/bridge.stderr.log",
    nodePath: "/usr/local/bin/node",
    cliPath: "/tmp/djl/bin/djl.js",
  });

  assert.match(plist, /<string>com\.djl\.bridge<\/string>/);
  assert.match(plist, /<string>run-service<\/string>/);
  assert.match(
    plist,
    /<key>KeepAlive<\/key>\s*<dict>\s*<key>SuccessfulExit<\/key>\s*<false\/>\s*<\/dict>/,
  );
  assert.match(plist, /<key>DJL_DEVICE_STATE_DIR<\/key>/);
});
...
test("resolveLaunchAgentPlistPath writes into the user's LaunchAgents folder", () => {
  assert.equal(
    ...
    path.join("/Users/tester", "Library", "LaunchAgents", "com.djl.bridge.plist"),
  );
});
  ...
  withTempDaemonEnv(() => {
    ...
    stopMacOSBridgeService({
      ...
      execFileSyncImpl(command, args) {
        ...
        return "/usr/local/bin/node /usr/local/bin/djl run-service";
      },
      processImpl: {
        pid: 9999,
        kill(pid, signal) {
    ...
    assert.deepEqual(calls, [
      [
        ...
        [
          "bootout",
          `gui/${TEST_UID}`,
          path.join(process.env.HOME, "Library", "LaunchAgents", "com.djl.bridge.plist"),
        ],
      ],
      ["launchctl", ["bootout", `gui/${TEST_UID}/com.djl.bridge`]],
    ]);
  });
});
...
test("startMacOSBridgeService kickstarts the launch agent after bootstrap", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    const env = {
      ...process.env,
      HOME: rootDir,
      DJL_DEVICE_STATE_DIR: rootDir,
      DJL_RELAY: "ws://127.0.0.1:9000/relay",
      UID: String(TEST_UID),
    };
    ...
    assert.deepEqual(
      ...
      [
        [
          ...
          "bootout",
          `gui/${TEST_UID}`,
          path.join(rootDir, "Library", "LaunchAgents", "com.djl.bridge.plist"),
        ],
        ["launchctl", "bootout", `gui/${TEST_UID}/com.djl.bridge`, undefined],
        [
          "launchctl",
          "bootstrap",
          `gui/${TEST_UID}`,
        ...
        ["launchctl", "kickstart", "-k", `gui/${TEST_UID}/com.djl.bridge`],
      ],
    );
...
test("restartMacOSBridgeService kickstarts an existing LaunchAgent without relay config", async () => {
  await withTempDaemonEnv(async ({ rootDir }) => {
    ...
    const plistPath = path.join(rootDir, "Library", "LaunchAgents", "com.djl.bridge.plist");
    ...
    assert.deepEqual(calls, [["launchctl", ["kickstart", "-k", `gui/${TEST_UID}/com.djl.bridge`]]]);
  });
});
...
test("getMacOSBridgeServiceStatus reports launchd + runtime metadata together", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    const plistPath = path.join(rootDir, "LaunchAgents", "com.djl.bridge.plist");
    fs.mkdirSync(path.dirname(plistPath), { recursive: true });
    fs.writeFileSync(plistPath, "plist");

    const status = getMacOSBridgeServiceStatus({
      platform: "darwin",
      env: { HOME: rootDir, DJL_DEVICE_STATE_DIR: rootDir, UID: String(TEST_UID) },
      execFileSyncImpl() {
        return "pid = 55";
      },
    });
...
function withTempDaemonEnv(run) {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "djl-launch-agent-"));
  process.env.DJL_DEVICE_STATE_DIR = rootDir;
