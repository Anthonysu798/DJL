...
test("daemon-state stores config, pairing payloads, and status under the djl state dir", () => {
  withTempDaemonEnv(({ rootDir }) => {
    ...
    assert.equal(resolveDJLStateDir(), rootDir);
...
function withTempDaemonEnv(run) {
  ...
  const rootDir = fs.mkdtempSync(path.join(os.tmpdir(), "djl-daemon-state-"));
