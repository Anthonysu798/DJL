...
test("buildIOSAppCompatibilitySnapshot blocks iPhone 1.x on bridge 2.0.0", () => {
  ...
  assert.equal(snapshot.legacyBridgeVersion, null);
  assert.equal(snapshot.downgradeCommand, null);
  assert.match(snapshot.message, /requires DJL iPhone 2\.0 or later/i);
  assert.doesNotMatch(snapshot.message, /install|npm|downgrade/i);
});
...
test("buildCachedIOSAppCompatibilityWarning warns when the last seen iPhone app is 1.x", () => {
  ...
  assert.match(warning, /requires DJL iPhone 2\.0 or later/i);
  assert.match(warning, /Update the iPhone app from the App Store to continue/i);
  assert.doesNotMatch(warning, /npm|install -g|downgrade/i);
});
