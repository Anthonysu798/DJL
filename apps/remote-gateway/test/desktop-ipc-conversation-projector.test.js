...
test("desktop conversation projector bootstraps active Desktop turns for mobile", () => {
  ...
  assert.equal(output.notifications[1].params.djlDesktopMirror, true);
  assert.equal(output.notifications[1].params.djlDesktopIpcMirror, true);
});
...
test("desktop conversation projector pushes newer runtime settings to an already-open phone thread", () => {
  ...
  const runtimeUpdate = output.notifications.find(
...
test("desktop conversation projector marks Litter todo-list items as progress plans", () => {
  ...
  assert.equal(thread.turns[0].items[0].djlProgressPlan, true);
  ...
  assert.equal(
    lifecycleItems.every((item) => item.djlProgressPlan === true),
    true,
  );
});
...
test("desktop conversation projector normalizes Desktop tool aliases for mobile lifecycle", () => {
  ...
  assert.deepEqual(
    startedItems.map((item) => item.djlDesktopIpcItemType),
...
test("desktop conversation projector emits generic tool output deltas from Desktop aliases", () => {
  ...
  assert.equal(output.notifications[0].params.item.djlDesktopIpcItemType, "dynamicToolCall");
});
...
test("desktop conversation projector hides injected context from user bubbles", () => {
  ...
  const userStarts = output.notifications.filter(
    (notification) =>
      notification.method === "item/started" && notification.params.item.type === "userMessage",
  );
  assert.equal(userStarts.length, 1);
  assert.deepEqual(userStarts[0].params.item.content, [{ type: "text", text: "fix the bug" }]);
});
...
test("desktop conversation projector hides native goal continuation context", () => {
  ...
  assert.deepEqual(
    output.notifications
      .filter((entry) => entry.method === "item/started")
      .map((entry) => entry.params.itemId),
    ["assistant-goal"],
  );
});
...
test("desktop conversation projector mirrors token usage updates", () => {
  ...
  assert.equal(output.notifications[0].params.djlDesktopMirror, true);
});
...
test("projects Desktop conversation state into thread/read backfill shape", () => {
  ...
  assert.deepEqual(
    thread.turns[0].items.slice(2).map((item) => item.djlDesktopIpcItemType),
...
test("desktop conversation projector live-syncs goal metadata without transcript rows", () => {
  ...
  assert.deepEqual(
    bootstrap.notifications.map((entry) => entry.method),
    ["thread/goal/updated"],
  );
  ...
  assert.deepEqual(
    progress.notifications.map((entry) => entry.method),
    ["thread/goal/updated"],
  );
  ...
  assert.deepEqual(
    completed.notifications.map((entry) => entry.method),
    ["thread/goal/updated"],
  );
  ...
  assert.deepEqual(
    cleared.notifications.map((entry) => entry.method),
    ["thread/goal/cleared"],
  );
...
test("desktop conversation projector re-emits guardian reviews as autoApprovalReview notifications", () => {
  ...
  assert.equal(started.params.djlDesktopIpcMirror, true);
...
test("projects Desktop guardian reviews into thread/read backfill items", () => {
  ...
  assert.equal(item.djlDesktopIpcMirror, true);
});
