...
function makeTempRepo() {
  const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "djl-git-handler-"));
  git(repoDir, "init", "-b", "main");
  git(repoDir, "config", "core.autocrlf", "false");
  git(repoDir, "config", "user.name", "DJL Tests");
  git(repoDir, "config", "user.email", "tests@example.com");
  fs.writeFileSync(path.join(repoDir, "README.md"), "# Test\n");
  fs.mkdirSync(path.join(repoDir, "djl-bridge", "src"), { recursive: true });
  fs.writeFileSync(
    path.join(repoDir, "djl-bridge", "src", "index.js"),
    "export const ready = true;\n",
  );
  git(repoDir, "add", "README.md");
  git(repoDir, "add", "djl-bridge/src/index.js");
...
function makeBareRemote() {
  return fs.mkdtempSync(path.join(os.tmpdir(), "djl-git-handler-remote-"));
}
...
test("gitStatus reports non-repository directories without failing", async () => {
  const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "djl-git-handler-nonrepo-"));
...
test("gitInit creates a main unborn branch without committing files", async () => {
  const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "djl-git-handler-init-"));
...
test("gitBranchesWithStatus returns explicit non-repository state", async () => {
  const projectDir = fs.mkdtempSync(path.join(os.tmpdir(), "djl-git-handler-branches-nonrepo-"));
...
test("gitBranches scopes worktree paths to the current project subdirectory", async () => {
  const repoDir = makeTempRepo();
  const projectDir = path.join(repoDir, "djl-bridge");
  const siblingWorktree = path.join(path.dirname(repoDir), `${path.basename(repoDir)}-wt-feature`);
  ...
  try {
    ...
    assert.equal(
      result.worktreePathByBranch["feature/clean-switch"],
      canonicalPath(path.join(siblingWorktree, "djl-bridge")),
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(siblingWorktree, { recursive: true, force: true });
...
test("gitBranches scopes local checkout path for subdirectory worktrees", async () => {
  const repoDir = makeTempRepo();
  const localProjectDir = path.join(repoDir, "djl-bridge");
  const siblingWorktree = path.join(path.dirname(repoDir), `${path.basename(repoDir)}-wt-feature`);
  const siblingProjectDir = path.join(siblingWorktree, "djl-bridge");
...
test("gitCheckout surfaces a specific error when the requested branch does not exist locally", async () => {
  ...
  try {
    await assert.rejects(
      __test.gitCheckout(repoDir, { branch: "djl/missing" }),
      (error) =>
        error?.errorCode === "branch_not_found" &&
        error?.userMessage === "Branch 'djl/missing' does not exist locally.",
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitCreateBranch normalizes bare names into djl/* and checks out the new branch", async () => {
  const repoDir = makeTempRepo();

  try {
    const result = await __test.gitCreateBranch(repoDir, { name: "new-branch" });

    assert.equal(result.branch, "djl/new-branch");
    assert.equal(result.status?.branch, "djl/new-branch");
    assert.equal(git(repoDir, "rev-parse", "--abbrev-ref", "HEAD"), "djl/new-branch");
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
});

test("normalizeCreatedBranchName avoids double-prefixing djl branches", () => {
  assert.equal(__test.normalizeCreatedBranchName("feature/foo"), "djl/feature/foo");
  assert.equal(__test.normalizeCreatedBranchName("djl/feature/foo"), "djl/feature/foo");
  assert.equal(__test.normalizeCreatedBranchName("my new branch"), "djl/my-new-branch");
  assert.equal(__test.normalizeCreatedBranchName("feature / login page"), "djl/feature/login-page");
  ...
  try {
    await assert.rejects(
      ...
      (error) =>
        ...
        error?.userMessage === "Branch 'djl/feature..oops' is not a valid Git branch name.",
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitStatus fetches current upstream before computing update availability", async () => {
  const repoDir = makeTempRepo();
  const remoteDir = makeBareRemote();
  const cloneDir = fs.mkdtempSync(path.join(os.tmpdir(), "djl-git-handler-clone-"));
  ...
  try {
    ...
    git(cloneDir, "config", "user.name", "DJL Tests");
...
test("gitCreateBranch rejects duplicate branch names with a specific error", async () => {
  ...
  try {
    git(repoDir, "branch", "djl/already-there");

    await assert.rejects(
      __test.gitCreateBranch(repoDir, { name: "already-there" }),
      (error) =>
        error?.errorCode === "branch_exists" &&
        error?.userMessage === "Branch 'djl/already-there' already exists.",
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitBranches hides remote-only branches from the local selector list", async () => {
  ...
  try {
    pushRemoteOnlyBranch(repoDir, remoteDir, "djl/remote-only");

    const result = await __test.gitBranches(repoDir);

    assert.ok(!result.branches.includes("djl/remote-only"));
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
  }
...
test("gitBranches preserves the repo default branch even when it is not checked out locally", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "djl/topic");
    ...
    assert.ok(result.branches.includes("djl/topic"));
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
  }
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) =>
        error?.errorCode === "branch_exists" &&
        error?.userMessage ===
          "Branch 'djl/remote-only' already exists on origin. Check it out locally instead of creating a new branch.",
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
...
test("gitStatus marks a branch as published when origin has it even without local upstream", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "djl/published-no-upstream");
...
test("gitGeneratePullRequestDraft summarizes branch changes against the default branch", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "djl/pr-draft");
    fs.writeFileSync(path.join(repoDir, "README.md"), "# Test\n\nbranch change\n");
    git(repoDir, "add", "README.md");
    ...
    assert.match(capturedInvocation?.prompt || "", /Current branch: djl\/pr-draft/);
...
test("gitGenerateCommitMessage supports repositories without an initial commit", async () => {
  const repoDir = fs.mkdtempSync(path.join(os.tmpdir(), "djl-git-handler-unborn-"));
  let capturedInvocation = null;
...
test("gitCreatePullRequest creates a real GitHub PR through gh", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "djl/create-pr");
    fs.writeFileSync(path.join(repoDir, "README.md"), "# Test\n\nreal pr\n");
    git(repoDir, "add", "README.md");
    git(repoDir, "commit", "-m", "Prepare PR branch");
    git(repoDir, "push", "-u", "origin", "djl/create-pr");
    ...
    assert.ok(
      ghCalls.some((call) => call.join(" ").includes("pr create --base main --head djl/create-pr")),
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
    fs.rmSync(remoteDir, { recursive: true, force: true });
...
test("gitRunStackedAction commits pushes and creates a PR", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "djl/stacked-pr");
    fs.writeFileSync(path.join(repoDir, "README.md"), "# Test\n\nstacked action\n");
            ...
            ? JSON.stringify([
                {
                  ...
                  baseRefName: "main",
                  headRefName: "djl/mobile-pr-20260502000000",
                  state: "open",
                },
              ])
            : "[]",
...
test("gitCreatePullRequest returns the URL printed by gh when list lags behind creation", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "djl/stdout-pr-url");
    fs.writeFileSync(path.join(repoDir, "README.md"), "# Test\n\nstdout url\n");
    git(repoDir, "add", "README.md");
    git(repoDir, "commit", "-m", "Prepare stdout URL PR");
    git(repoDir, "push", "-u", "origin", "djl/stdout-pr-url");
...
test("gitCreatePullRequest preserves nothing-to-compare errors", async () => {
  ...
  try {
    ...
    git(repoDir, "remote", "add", "origin", remoteDir);
    git(repoDir, "push", "-u", "origin", "main");
    git(repoDir, "checkout", "-b", "djl/no-pr-diff");
    git(repoDir, "push", "-u", "origin", "djl/no-pr-diff");
...
test("gitCreateWorktree creates a managed worktree under CODEX_HOME/worktrees", async () => {
  ...
  try {
    ...
    assert.equal(result.branch, "djl/new-worktree");
    assert.equal(result.alreadyExisted, false);
    assert.ok(result.worktreePath.startsWith(managedWorktreesRoot));
    assert.equal(path.basename(result.worktreePath), "djl-bridge");
    assert.equal(git(result.worktreePath, "rev-parse", "--abbrev-ref", "HEAD"), "djl/new-worktree");
...
test("gitCreateWorktree reuses an existing worktree for the same djl branch", async () => {
  const repoDir = makeTempRepo();
  const projectDir = path.join(repoDir, "djl-bridge");
  const codexHome = fs.mkdtempSync(path.join(os.tmpdir(), "djl-codex-home-"));
  const previousCodexHome = process.env.CODEX_HOME;
  const siblingWorktree = path.join(
    ...
    `${path.basename(repoDir)}-wt-djl-existing`,
  );

  process.env.CODEX_HOME = codexHome;

  try {
    git(repoDir, "branch", "djl/existing");
    git(repoDir, "worktree", "add", siblingWorktree, "djl/existing");
    ...
    assert.equal(result.branch, "djl/existing");
    assert.equal(result.alreadyExisted, true);
    assert.equal(result.worktreePath, canonicalPath(path.join(siblingWorktree, "djl-bridge")));
  } finally {
    if (previousCodexHome === undefined) {
      delete process.env.CODEX_HOME;
    } else {
  ...
  try {
    ...
    await assert.rejects(
      ...
      (error) =>
        error?.errorCode === "branch_exists" &&
        error?.userMessage ===
          "Branch 'djl/already-there' already exists locally. Choose another name or open that branch instead.",
    );
  } finally {
    fs.rmSync(repoDir, { recursive: true, force: true });
  }
...
test("gitCreateWorktree carries tracked and untracked changes into the new worktree and cleans local", async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(repoDir, "djl-bridge", "scratch.txt"), "carry me\n");
    ...
    assert.equal(git(repoDir, "status", "--short"), "");
    assert.equal(fs.existsSync(path.join(repoDir, "djl-bridge", "scratch.txt")), false);
...
test("gitCreateWorktree can copy tracked and untracked changes into the new worktree without cleaning local", async () => {
  ...
  try {
    ...
    fs.writeFileSync(path.join(repoDir, "djl-bridge", "scratch.txt"), "keep me too\n");
    ...
    assert.match(git(repoDir, "status", "--short"), /djl-bridge\/src\/index\.js/);
    assert.equal(
      fs.readFileSync(path.join(repoDir, "djl-bridge", "scratch.txt"), "utf8"),
      "keep me too\n",
    );
