...
test("desktop turns/list returns newest-first pages without reversing reopen history", () => {
  ...
  assert.deepEqual(
    pageAfterStreamingGrowth.data.map((turn) => turn.id),
    ["turn-1"],
  );
  ...
  assert.deepEqual(
    pageAfterNonUserItemGrowth.data.map((turn) => turn.id),
    ["turn-1"],
  );
...
test("desktop turns/list private cursors never fall through to app-server", (t) => {
  ...
  const follower = createDesktopIpcActionFollower({
    socketPath: path.join(os.tmpdir(), `missing-djl-${randomUUID()}.sock`),
  ...
  const handled = follower.observeInbound(
    JSON.stringify({
      id: "private-cursor-read",
      method: "thread/turns/list",
      params: {
        threadId: "thread-with-expired-desktop-page",
        cursor: "djl-desktop-turns:desc:id:missing-turn",
      },
    }),
  );
  ...
  assert.deepEqual(actions, [
    {
      ...
      params: {
        ...
        turnId: "turn-1",
        itemId: "item-1",
        djlActionSource: "desktop-ipc-action-follower",
        djlDesktopMirror: true,
        djlDesktopIpcMirror: true,
        questions: [
          {
            id: "q1",
            header: "Mode",
...
test("projects command, file, and permission approvals while ignoring completed requests", () => {
  ...
  assert.equal(actions[3].params.djlActionSource, "desktop-ipc-action-follower");
});
...
test("desktop IPC follower backs off baseline recovery instead of hot-looping", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-recovery-backoff-");
...
test("desktop IPC follower announces thread replacement when synthetic turn ids become real", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-follower-full-replace-");
  let serverSocket = null;
  ...
  assert.equal(replaced.params.djlDesktopMirror, true);
  assert.equal(replaced.params.djlDesktopIpcMirror, true);
  ...
  const followUpTurnStarted = outbound
    .slice(replacedIndex + 1)
    .find((message) => message.method === "turn/started");
  assert.equal(followUpTurnStarted.params.turnId, "turn-real-id");
  assert.equal(followUpTurnStarted.params.djlTurnIdentityContinuity, true);
  assert.equal(
    outbound
      .slice(canonicalReplacementStartIndex)
      .some(
        (message) => message.method === "turn/completed" && message.params?.turnId === "ipc-turn-0",
      ),
    false,
  ...
  await waitFor(() =>
    outbound
      .slice(distinctReplacementStartIndex)
      .some(
        (message) =>
          message.method === "turn/started" &&
          message.params?.threadId === distinctThreadId &&
  ...
  const distinctTurnStarted = outbound
    .slice(distinctReplacementStartIndex)
    .find(
      (message) =>
        message.method === "turn/started" &&
  ...
  assert.notEqual(distinctTurnStarted.params.djlTurnIdentityContinuity, true);
});
...
test("desktop IPC follower projects first add patch-only action updates without a baseline read", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-recovery-");
  let baselineReads = 0;
  let serverSocket = null;
...
test("desktop IPC follower uses baseline recovery for patch-only updates that need existing state", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-replace-recovery-");
  let baselineReads = 0;
  let serverSocket = null;
...
test("desktop IPC follower does not issue baseline reads just because a chat opens", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-lazy-recovery-");
  let baselineReads = 0;
  let serverSocket = null;
...
test("desktop IPC follower waits for a usable snapshot when a first patch needs missing state", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-wait-snapshot-");
  let serverSocket = null;
...
test("desktop IPC follower does not block add patch-only actions on a failing baseline reader", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-recovery-fallback-");
  let serverSocket = null;
...
test("desktop IPC follower answers client discovery requests as a passive client", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-discovery-");
  const serverFrames = [];
  let serverSocket = null;
...
test("desktop IPC follower forwards pending actions and routes iOS replies back to the Mac", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-follower-");
  const serverFrames = [];
  let serverSocket = null;
...
test("desktop IPC follower keeps projected actions pending across IPC disconnects", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-follower-disconnect-action-");
  let serverSocket = null;
...
test("desktop IPC follower routes phone turns to Desktop-owned threads", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-follower-turn-start-");
  const serverFrames = [];
  let serverSocket = null;
  ...
  await waitFor(() =>
    serverFrames.find((frame) => frame.method === "thread-follower-update-thread-settings"),
  );
  await waitFor(() => serverFrames.find((frame) => frame.method === "thread-follower-start-turn"));
  const settingsFrame = serverFrames.find(
    (frame) => frame.method === "thread-follower-update-thread-settings",
  );
  const turnStartFrame = serverFrames.find(
  ...
  assert.deepEqual(
    outbound.find((message) => message.id === "phone-turn-start-1"),
    {
      id: "phone-turn-start-1",
      result: { turn: { id: "turn-from-phone" } },
    },
  ...
  for (const request of routedRequests) {
    const handledRoute = follower.observeInbound(
      JSON.stringify({
        id: request.id,
    ...
    assert.deepEqual(
      outbound.find((message) => message.id === request.id),
      {
        id: request.id,
        result: { turn: { id: "turn-from-phone" } },
      },
  ...
  for (const mutation of unsupportedMutations) {
    ...
    assert.equal(handled, true);
    assert.deepEqual(
      outbound.find((message) => message.id === mutation.id),
      {
        id: mutation.id,
        error: {
          code: -32004,
...
test("desktop IPC follower falls back locally when no Desktop client can handle the request", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-follower-local-fallback-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
  ...
  assert.equal(
    outbound.some((message) => message.id === "phone-turn-start-route-fallback"),
    false,
  );

  const handledAgain = follower.observeInbound(
    JSON.stringify({
...
test("desktop IPC follower falls back locally when Desktop settings sync times out before turn delivery", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-follower-settings-timeout-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
...
test("desktop IPC follower does not rerun ambiguous Desktop failures locally", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-follower-ambiguous-error-");
  const localForwards = [];
  let serverSocket = null;
  ...
  const handledTimeout = follower.observeInbound(
    JSON.stringify({
  ...
  await waitFor(
    () => outbound.some((message) => message.id === "phone-turn-start-desktop-timeout"),
    1_000,
  );
  const timeoutResponse = outbound.find(
    (message) => message.id === "phone-turn-start-desktop-timeout",
  );
...
test("desktop IPC follower mirrors live assistant text growth from desktop state", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-assistant-delta-");
  let serverSocket = null;
  ...
  assert.equal(deltaMessage.params.djlDesktopMirror, true);
  assert.equal(deltaMessage.params.djlDesktopIpcMirror, true);
});

test("desktop IPC follower discovers running sidebar threads before the phone opens them", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-background-running-");
  let serverSocket = null;
  ...
  await waitFor(() =>
    outbound.some(
      (message) =>
        message.method === "turn/started" && message.params?.threadId === "thread-running-unopened",
    ),
  );
  await waitFor(() =>
    outbound.some(
  ...
  const idleOpenHandled = follower.observeInbound(
    JSON.stringify({
  ...
  await waitFor(() =>
    outbound.some(
      (message) =>
        message.method === "turn/started" && message.params?.threadId === "thread-idle-unopened",
    ),
  );
  writeFrame(serverSocket, {
    type: "broadcast",
  ...
  await waitFor(() =>
    outbound.some(
      (message) =>
        message.method === "turn/completed" && message.params?.threadId === "thread-idle-unopened",
    ),
  );
  ...
  const readHandled = follower.observeInbound(
    JSON.stringify({
  ...
  assert.equal(
    outbound.find((message) => message.id === "open-running-thread")?.result?.thread?.turns?.[0]
      ?.items?.[0]?.text,
    "Working still",
  );
...
test("desktop IPC follower settles an announced background turn after reconnect", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-background-reconnect-");
  let serverSocket = null;
  const server = net.createServer((socket) => {
    serverSocket = socket;
    attachFrameReader(socket, (frame) => {
  ...
  writeFrame(
    serverSocket,
    backgroundConversationSnapshot("thread-background-reconnect", "inProgress", {
  ...
  writeFrame(
    serverSocket,
    backgroundConversationSnapshot("thread-background-reconnect", "completed", {
      turnId: "turn-background-reconnect",
    }),
  );
  ...
  await wait(275);
  const completions = outbound.filter(
    (message) =>
      message.method === "turn/completed" &&
      message.params?.threadId === "thread-background-reconnect",
  );
...
test("desktop IPC follower interrupts announced background turns when Litter stays disconnected", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-background-disconnect-");
  let serverSocket = null;
  const server = net.createServer((socket) => {
    serverSocket = socket;
    attachFrameReader(socket, (frame) => {
  ...
  writeFrame(
    serverSocket,
    backgroundConversationSnapshot("thread-background-disconnect", "inProgress", {
...
test("desktop IPC follower settles a running background thread before archive", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-background-archive-");
  let serverSocket = null;
  const server = net.createServer((socket) => {
    serverSocket = socket;
    attachFrameReader(socket, (frame) => {
  ...
  writeFrame(
    serverSocket,
    backgroundConversationSnapshot("thread-background-archive", "inProgress", {
  ...
  const completionIndex = outbound.findIndex(
    (message) =>
      message.method === "turn/completed" &&
...
test("desktop IPC follower does not evict an announced background turn", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-background-lru-");
  let serverSocket = null;
  const server = net.createServer((socket) => {
    serverSocket = socket;
    attachFrameReader(socket, (frame) => {
  ...
  writeFrame(
    serverSocket,
    backgroundConversationSnapshot("thread-background-lru-protected", "inProgress", {
  ...
  for (let index = 0; index < 512; index += 1) {
    writeFrame(
      serverSocket,
      backgroundConversationSnapshot(`thread-background-lru-idle-${index}`, "completed", {
...
test("desktop IPC background recovery stays lifecycle-only until open", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-background-recovery-");
  let serverSocket = null;
  const baseline = {
    turns: [
      {
  ...
  writeFrame(
    serverSocket,
    backgroundConversationSnapshot("thread-background-recovery", "inProgress", { items: [] }),
  );
  await waitFor(() => outbound.some((message) => message.method === "turn/started"));
  ...
  const handledResume = follower.observeInbound(
    JSON.stringify({
  ...
  assert.equal(resume.result.djlDesktopIpcMirror, true);
});

test("desktop IPC follower normalizes phone turn starts before Desktop follower requests", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-follower-normalize-");
  const serverFrames = [];
  let serverSocket = null;
...
test("desktop IPC follower releases desktop state when the live owner claims a thread", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-owner-release-");
  const serverFrames = [];
  let serverSocket = null;
  ...
  writeFrame(serverSocket, {
    type: "broadcast",
    method: "thread-stream-state-changed",
    sourceClientId: "djl-owner",
    version: 6,
    params: {
      conversationId: "thread-released",
      djlOwnerSource: "desktop-ipc-live-owner",
      change: {
        type: "snapshot",
        conversationState: { turns: [], requests: [] },
      },
...
test("desktop IPC follower keeps held turns queued across a transient IPC disconnect", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-hold-disconnect-");
  const serverFrames = [];
  const localForwards = [];
  const outbound = [];
  let serverSocket = null;
  ...
  await waitFor(
    () => serverFrames.find((frame) => frame.method === "thread-follower-start-turn"),
    2_000,
  );
  await waitFor(
    () => outbound.find((message) => message.id === "phone-turn-start-hold-disconnect"),
    1_000,
  );
  assert.deepEqual(
    outbound.find((message) => message.id === "phone-turn-start-hold-disconnect"),
...
test("desktop IPC follower keeps live owner routing guard across IPC disconnects", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-owner-disconnect-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
...
test("desktop IPC follower accepts peer ownership snapshots before a phone resume", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-peer-snapshot-before-resume-");
  const serverFrames = [];
  let serverSocket = null;
  ...
  await waitFor(
    () => outbound.find((message) => message.id === "phone-turn-start-peer-before-resume"),
    1_000,
  );
  assert.deepEqual(
    outbound.find((message) => message.id === "phone-turn-start-peer-before-resume"),
...
test("desktop IPC follower ignores peer patches while the live owner owns a thread", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-ignore-peer-patch-");
  const serverFrames = [];
  let serverSocket = null;
...
test("desktop IPC follower ignores Desktop echoes for locally owned threads", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-local-owner-echo-");
  let serverSocket = null;
...
test("desktop IPC follower holds quick phone turns until the desktop snapshot arrives", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-hold-turn-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
  ...
  assert.deepEqual(
    outbound.find((message) => message.id === "phone-turn-start-held"),
    {
      id: "phone-turn-start-held",
      result: { turn: { id: "turn-held" } },
    },
...
test("desktop IPC follower routes held phone turns once discovery confirms desktop ownership", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-probe-owned-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
  ...
  assert.deepEqual(
    outbound.find((message) => message.id === "phone-turn-start-probe"),
    {
      id: "phone-turn-start-probe",
      result: { turn: { id: "turn-probe-owned" } },
    },
...
test("desktop IPC follower coalesces duplicate held turn starts for a thread", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-held-turn-coalesce-");
  const serverFrames = [];
  let serverSocket = null;
  ...
  await waitFor(
    () => outbound.find((message) => message.id === "phone-turn-start-coalesce-old"),
    1_000,
  );
  assert.equal(
    outbound.find((message) => message.id === "phone-turn-start-coalesce-old").error?.code,
    -32000,
  );
  await waitFor(
    () => outbound.find((message) => message.id === "phone-turn-start-coalesce-new"),
    1_000,
  );
  assert.deepEqual(
    outbound.find((message) => message.id === "phone-turn-start-coalesce-new"),
  ...
  const routedStarts = serverFrames.filter(
    (frame) => frame.method === "thread-follower-start-turn",
  );
...
test("desktop IPC follower retries held ownership probes after IPC connects", async (t) => {
  ...
  assert.equal(handled, true);
  assert.equal(
    writtenFrames.some((frame) => frame.type === "client-discovery-request"),
    false,
  );
  ...
  await waitFor(
    () => outbound.find((message) => message.id === "phone-turn-start-connect-probe"),
    1_000,
  );
  assert.deepEqual(
    outbound.find((message) => message.id === "phone-turn-start-connect-probe"),
...
test("desktop IPC follower ignores stale positive discovery after a held turn already expired", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-probe-expired-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
  ...
  const firstHandled = follower.observeInbound(
    JSON.stringify({
  ...
  await waitFor(
    () => localForwards.some((message) => message.id === "phone-turn-start-expired-probe"),
    1_000,
  );
  ...
  const secondHandled = follower.observeInbound(
    JSON.stringify({
...
test("desktop IPC follower ignores stale positive discovery after live owner claims a thread", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-probe-stale-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
  ...
  await waitFor(
    () => localForwards.some((message) => message.id === "phone-turn-start-stale-probe"),
    1_000,
  );
  writeFrame(serverSocket, {
    type: "client-discovery-response",
...
test("desktop IPC follower cancels held turns when the live owner removes a thread", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-probe-removed-");
  const serverFrames = [];
  const localForwards = [];
  const outbound = [];
  let serverSocket = null;
  ...
  writeFrame(serverSocket, {
    ...
    params: {
      ...
      djlOwnerSource: "desktop-ipc-live-owner",
      djlOwnerReleased: true,
      change: {
        type: "snapshot",
        conversationState: { djlRemoved: true, turns: [], requests: [] },
      },
    },
  });
...
test("desktop IPC follower keeps held phone turns queued when discovery denies ownership", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-probe-denied-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
  ...
  await waitFor(
    () => serverFrames.some((frame) => frame.type === "client-discovery-request"),
    1_000,
  );
  await wait(30);
  assert.deepEqual(localForwards, []);
...
test("desktop IPC follower forwards held phone turns to local codex when no snapshot arrives", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-hold-timeout-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
...
test("desktop IPC follower ignores DJL-owned live owner broadcasts", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-owner-echo-");
  let serverSocket = null;
...
test("desktop IPC follower yields stale active ownership and cleanly replaces it from a fresh snapshot", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-stale-active-read-");
  let serverSocket = null;
  ...
  const freshServed = follower.observeInbound(
    JSON.stringify({
      id: "read-fresh",
      method: "thread/read",
      params: { threadId: "thread-stale-active" },
  ...
  assert.equal(
    outbound.some((message) => message.id === "read-fresh"),
    true,
  );
  ...
  const staleServed = follower.observeInbound(
    JSON.stringify({
      id: "read-stale",
      method: "thread/read",
      params: { threadId: "thread-stale-active" },
  ...
  assert.equal(
    outbound.some((message) => message.id === "read-stale"),
    false,
  );
  ...
  await waitFor(() => {
    ...
    return (
      messages.some((message) => message.method === "thread/replaced") &&
      messages.some((message) => message.method === "thread/started")
    );
  });
  const freshEpochMessages = outbound.slice(freshEpochStartIndex);
  const replacementAnnouncementIndex = freshEpochMessages.findIndex(
    (message) => message.method === "thread/replaced",
  );
  const replacementBootstrapIndex = freshEpochMessages.findIndex(
  ...
  const replacementBootstrap = freshEpochMessages.find(
    (message) => message.method === "thread/started",
  );
  assert.equal(replacementBootstrap.params.threadId, "thread-stale-active");
  assert.equal(replacementBootstrap.params.djlDesktopMirror, true);
  assert.deepEqual(
  ...
  fakeNow += 60_000;
  const idleServed = follower.observeInbound(
    JSON.stringify({
      id: "read-idle",
      method: "thread/read",
      params: { threadId: "thread-stale-active" },
...
test("desktop IPC follower yields normalized history reads while keeping its bounded live tail", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-normalized-history-yield-");
  let serverSocket = null;
  ...
  const stateProbeHandled = follower.observeInbound(
    JSON.stringify({
  ...
  assert.equal(stateProbe?.result?.djlDesktopLiveState, true);
  ...
  assert.equal(
    outbound.some(
      (message) =>
        message.method === "turn/started" && message.params?.turnId === "turn-stale-running",
    ),
    false,
  );
  outbound.length = 0;
  ...
  await wait(50);
  outbound.length = 0;
  const blankHandled = follower.observeInbound(
    JSON.stringify({
      id: "blank-turns",
      method: "thread/turns/list",
  ...
  assert.deepEqual(outbound.find((message) => message.id === "blank-turns")?.result?.data, []);
});

test("desktop IPC follower coalesces stale normalized snapshot bursts before publishing lifecycle", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-snapshot-coalesce-");
  const outbound = [];
  let serverSocket = null;
  ...
  assert.equal(
    outbound.some((message) => message.id === "provisional-explicit-state"),
    false,
  );
  ...
  await wait(50);
  assert.equal(
    outbound.some((message) => message.method?.startsWith("turn/")),
    false,
  );

  await waitFor(() =>
    outbound.some(
      (message) => message.method === "turn/started" && message.params?.turnId === "turn-new",
    ),
  );
  assert.notEqual(
    outbound.find(
      (message) => message.method === "turn/started" && message.params?.turnId === "turn-new",
    )?.params?.djlTurnIdentityContinuity,
    true,
    "a distinct replacement turn must advance the phone run generation",
  );
  assert.equal(
    outbound.some(
      (message) => message.method === "turn/started" && message.params?.turnId === "turn-old",
    ),
    false,
  );
  assert.equal(outbound.filter((message) => message.method === "thread/replaced").length, 1);
  outbound.length = 0;
...
test("desktop IPC follower completes either parallel active turn across normalized snapshots", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-parallel-normalized-");
  const outbound = [];
  let serverSocket = null;
  const nowValue = Date.now();
...
test("desktop IPC follower forces canonical repair after a normalized reconnect snapshot", async (t) => {
  const { socketPath, state } = await startInitializedIpcTestServer(
    t,
    "djl-ipc-normalized-reconnect-repair-",
  );
  const outbound = [];
  const follower = createDesktopIpcActionFollower({
    socketPath,
...
test("desktop IPC follower bootstraps a normalized active turn when the phone opens it", async (t) => {
  const { socketPath, state } = await startInitializedIpcTestServer(
    t,
    "djl-ipc-background-canonical-repair-",
  );
  const outbound = [];
  const follower = createDesktopIpcActionFollower({
    socketPath,
...
test("desktop IPC follower preserves promoted synthetic-turn identity from prompt and start", async (t) => {
  ...
  await waitFor(() =>
    outbound.some(
      (message) =>
        message.method === "turn/started" && message.params?.turnId === "turn-promoted-canonical",
    ),
  );
  const canonicalStart = outbound.find(
    (message) =>
      message.method === "turn/started" && message.params?.turnId === "turn-promoted-canonical",
  );
  assert.equal(canonicalStart.params.djlTurnIdentityContinuity, true);
  assert.equal(
    outbound.some(
      (message) =>
        message.method === "turn/started" &&
...
test("desktop IPC follower reuses its normalized history index for live content patches", async (t) => {
  const { socketPath, state } = await startInitializedIpcTestServer(
    t,
    "djl-ipc-normalized-index-reuse-",
  );
  const outbound = [];
  let indexRebuilds = 0;
  const nowValue = Date.now();
  ...
  await waitFor(
    () =>
      outbound.filter((message) => message.method === "item/agentMessage/delta").length ===
      contentPatchCount,
  );
    ...
    params: {
      ...
      change: {
        ...
        patches: [
          {
            op: "replace",
            path: ["turnHistory", "history", "entitiesByKey", "turn:turn-index-live", "status"],
            value: "completed",
          },
  ...
  assert.equal(
    indexRebuilds,
    1,
    "status patches must update the active set without a full rebuild",
  );
  outbound.length = 0;
    ...
    params: {
      ...
      change: {
        ...
        patches: [
          {
            op: "add",
            path: ["turnHistory", "history", "entitiesByKey", "turn:turn-index-next"],
            value: {
...
test("desktop IPC follower incrementally mirrors normalized guardian review updates", async (t) => {
  const { socketPath, state } = await startInitializedIpcTestServer(
    t,
    "djl-ipc-normalized-guardian-review-",
  );
  const outbound = [];
  let indexRebuilds = 0;
  ...
  await waitFor(() =>
    outbound.some((message) => message.method === "item/autoApprovalReview/started"),
  );
  assert.equal(indexRebuilds, 1);
  outbound.length = 0;
  ...
  await waitFor(() =>
    outbound.some((message) => message.method === "item/autoApprovalReview/completed"),
  );
    ...
    params: {
      ...
      change: {
        ...
        patches: [
          {
            op: "replace",
            path: ["turnHistory", "history", "entitiesByKey", turnKey, "items", 1, "text"],
            value: "AB",
          },
        ],
      },
...
test("desktop IPC follower delivers normalized guardian reviews when an idle background thread is opened", async (t) => {
  const { socketPath, state } = await startInitializedIpcTestServer(t, "djl-ipc-bg-open-review-");
  const outbound = [];
  ...
  const reviewNotification = outbound.find(
    (message) => message.method === "item/autoApprovalReview/completed",
  );
  ...
  assert.equal(reviewNotification.params.djlGuardianRetrySupported, false);
  assert.equal(
    outbound.some((message) => message.method === "thread/replaced"),
    true,
...
test("desktop IPC follower keeps phone interest in a thread across a Desktop disconnect", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-active-thread-disconnect-");
  const serverFrames = [];
  const localForwards = [];
  let serverSocket = null;
  ...
  const heldBeforeDisconnect = follower.observeInbound(
    JSON.stringify({
  ...
  const handledAfterDisconnect = follower.observeInbound(
    JSON.stringify({
...
test("desktop IPC follower caps activeThreadIds so a marathon connection cannot grow it forever", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-active-thread-cap-");
  let serverSocket = null;
  ...
  const heldOldest = follower.observeInbound(
    JSON.stringify({
  ...
  assert.equal(
    heldOldest,
    false,
    "the oldest thread id should have been evicted once the cap was exceeded",
  );

  const newestThreadId = `thread-cap-${totalThreads - 1}`;
  const heldNewest = follower.observeInbound(
    JSON.stringify({
  ...
  assert.equal(
    heldNewest,
    true,
    "the most recently observed thread id should still be treated as active",
  );
});

test("desktop IPC follower protects pending prompts from active-thread LRU eviction", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-active-thread-pending-cap-");
  const serverFrames = [];
  const outbound = [];
  let serverSocket = null;
  ...
  await waitFor(() =>
    serverFrames.find((frame) => frame.method === "thread-follower-submit-user-input"),
  );
  const replyFrame = serverFrames.find(
    (frame) => frame.method === "thread-follower-submit-user-input",
  );
...
test("desktop IPC follower refreshes active-thread recency so re-read threads survive eviction", async (t) => {
  const { tempDir, socketPath } = createIpcTestSocket("djl-ipc-active-thread-lru-");
  let serverSocket = null;
  ...
  const heldRefreshed = follower.observeInbound(
    JSON.stringify({
  ...
  assert.equal(
    heldRefreshed,
    true,
    "a re-read thread must have its recency refreshed and survive eviction",
  );

  const heldEvicted = follower.observeInbound(
    JSON.stringify({
...
function backgroundConversationSnapshot(threadId, status, { turnId = null, items = [] } = {}) {
