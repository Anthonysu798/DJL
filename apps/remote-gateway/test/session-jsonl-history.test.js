...
test("readThreadTurnsListPageFromSessionJsonl builds a recent turns page from rollout JSONL", () => {
  ...
  assert.equal(page.djlJsonlFallback, true);
  ...
  assert.match(
    page.data[0].items[1].djlSourceItemKey,
...
test("bounded JSONL history reads a complete tail turn without loading the full file", (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "djl-jsonl-tail-"));
  ...
  const readPage = () =>
    readThreadTurnsListPageFromSessionJsonl(filePath, {
      threadId,
      limit: 1,
      maxLimit: 1,
      fsModule: boundedFs,
  ...
  assert.equal(firstPage.nextCursor, "djl-jsonl-fallback-older-unavailable");
...
test("synthetic JSONL ids stay stable when an append crosses the tail-window boundary", (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "djl-jsonl-tail-boundary-"));
  const filePath = path.join(directory, "rollout.jsonl");
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
...
test("bounded JSONL history rejects a tail that does not contain a complete turn start", (t) => {
  const directory = fs.mkdtempSync(path.join(os.tmpdir(), "djl-jsonl-partial-"));
  const filePath = path.join(directory, "rollout.jsonl");
  t.after(() => fs.rmSync(directory, { recursive: true, force: true }));
  fs.writeFileSync(
    filePath,
...
test("parseSessionJsonlTurns keeps the active turn when a parallel sibling turn completes", () => {
  ...
  const syntheticRunningTurns = turns.filter(
    (turn) => turn.id.startsWith("turn-line-") && turn.status === "running",
  );
...
test("parseSessionJsonlTurns drops expanded skill context user items", () => {
  ...
  assert.equal(
    turns[0].items.some((item) => item.role === "assistant"),
    true,
  );
});
...
test("readThreadTurnsListPageFromSessionJsonl caps fallback pages to five turns", () => {
  ...
  assert.equal(page.nextCursor, "djl-jsonl-fallback-older-unavailable");
});
...
test("parseSessionJsonlTurns restores view_image tool output as imageView without inline data", () => {
  ...
  assert.equal(turns[0].items[1].djlJsonlToolOutputImage, true);
});
