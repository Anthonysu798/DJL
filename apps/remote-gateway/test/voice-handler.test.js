...
test("voice/transcribe returns transcribed text without exposing auth tokens", async () => {
  ...
  assert.match(
    logText,
    /voice transcribe request received durationMs=1200 actualDurationMs=\d+ audioBytes=46/,
  );
  assert.match(
    logText,
    /voice transcribe auth selected attempt=1 source=mac_runtime method=chatgpt provider=chatgpt/,
  );
  assert.match(
    logText,
    /voice transcribe provider status attempt=1 provider=chatgpt status=200 ok=true/,
  );
...
test("voice/transcribe logs provider failures without raw provider details", async () => {
  ...
  assert.doesNotMatch(
    ...
    /Bearer|provider leaked|chatgpt-provider-failure/,
  );
  assert.match(
    logText,
    /voice transcribe request received durationMs=300 actualDurationMs=\d+ audioBytes=46/,
  );
  assert.match(
    logText,
    /voice transcribe auth selected attempt=1 source=mac_runtime method=chatgpt provider=chatgpt/,
  ...
  assert.match(
    logText,
    /voice transcribe provider status attempt=1 provider=chatgpt status=500 ok=false/,
  );
...
test("voice/transcribe does not fall back to OPENAI_API_KEY when ChatGPT auth is rejected", async () => {
  ...
  assert.equal(
    fetchCalls.some((call) => call.url.includes("api.openai.com")),
    false,
  );
...
test("voice/transcribe rejects malformed or non-WAV audio before contacting the provider", async () => {
  const cases = [
    ...
    },
    {
      ...
      audioBase64: Buffer.from("hello from djl").toString("base64"),
...
test("voice/prewarm preconnects to the provider and pre-loads auth", async () => {
  ...
  assert.deepEqual(responses, [
    { id: "voice-prewarm-1", result: { ok: true, formats: ["wav", "m4a"] } },
  ]);
  assert.equal(authRequestCount, 1);
  assert.equal(fetchCalls.length, 1);
...
test("voice/transcribe accepts mono AAC clips whose sample entry reports 2 channels", async () => {
  ...
  // so DJL-generated clips from AVAudioFile must not be rejected.
  const responses = [];
  const handler = createVoiceHandler({
    sendCodexRequest: async () => ({
      authMethod: "chatgpt",
