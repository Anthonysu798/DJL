...
import {
  readOpenCodeModelCatalogCache,
  readOpenCodeProviderCatalogCache,
  writeOpenCodeModelCatalogCache,
  writeOpenCodeProviderCatalogCache,
} from "./providerCatalogBrowserCache";
...
export const providerDiscoveryQueryKeys = {
  ...
  openCodeModelProviders: (_cwd?: string | null) =>
    ["provider-discovery", "opencode-model-providers"] as const,
};

export function openCodeModelProvidersQueryOptions(_cwd?: string | null) {
  const cached = readOpenCodeProviderCatalogCache();
  return queryOptions({
    queryKey: providerDiscoveryQueryKeys.openCodeModelProviders(),
    queryFn: async (): Promise<OpenCodeListModelProvidersResult> => {
      const result = await ensureNativeApi().provider.listModelProviders({});
      writeOpenCodeProviderCatalogCache(result);
      return result;
    },
    initialData: cached?.data,
    ...(cached ? { initialDataUpdatedAt: cached.updatedAt } : {}),
    staleTime: 30_000,
...
export function providerModelsQueryOptions(input: {
  ...
  const discoveryCwd = input.provider === "opencode" ? null : (input.cwd ?? null);
  const cached = input.provider === "opencode" ? readOpenCodeModelCatalogCache() : null;
  return queryOptions({
    ...
    queryFn: async () => {
      ...
      if (input.provider === "opencode") {
        writeOpenCodeModelCatalogCache(result);
      }
      return result;
    },
    initialData: cached?.data,
    ...(cached ? { initialDataUpdatedAt: cached.updatedAt } : {}),
    enabled: input.enabled ?? true,
    // The server returns DJL's last-known-good OpenCode catalog immediately and
    // refreshes it in the background. Keep failures bounded instead of extending
    // an unavailable first-run runtime into a long retry loop.
    retry: input.provider === "opencode" ? 2 : input.provider === "cursor" ? 1 : 3,
    ...(input.provider === "opencode"
      ? { retryDelay: (attemptIndex: number) => Math.min(500 * 2 ** attemptIndex, 1_000) }
      : {}),
    staleTime: 60_000,
