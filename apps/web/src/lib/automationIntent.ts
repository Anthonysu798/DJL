...
const intentCopy = (key: string, defaultValue: string, values?: Record<string, unknown>) =>
  translateRendererCopy(`work:automations.intent.${key}`, defaultValue, values);
...
export function formatAutomationIntentCadence(
  ...
  if (schedule.type === "interval") {
    ...
    const [amount, unit] =
      seconds % 86_400 === 0
        ? [seconds / 86_400, copy("units.dayCompact", "d")]
        : seconds % 3_600 === 0
          ? [seconds / 3_600, copy("units.hourCompact", "h")]
          : seconds % 60 === 0
            ? [seconds / 60, copy("units.minuteCompact", "m")]
            : [seconds, copy("units.secondCompact", "s")];
    return copy("everyCompact", "Every {{amount}}{{unit}}", { amount, unit });
  }
  if (schedule.type === "once") {
    const date = new Date(schedule.runAt);
    const formatted = Number.isNaN(date.getTime())
      ? schedule.runAt
      : formatLocaleDateTime(date, locale, { dateStyle: "medium", timeStyle: "short" });
    return copy("onceAt", "Once at {{date}}", { date: formatted });
  }
  if (schedule.type === "cron") {
    return copy("cron", "Cron {{expression}}", { expression: schedule.expression });
  }
  if (schedule.type === "daily") {
    return copy("dailyAt", "Daily at {{time}}", { time: schedule.timeOfDay });
  }
  if (schedule.type === "weekdays") {
    return copy("weekdaysAt", "Weekdays at {{time}}", { time: schedule.timeOfDay });
  }
  if (schedule.type === "weekly") {
    return copy("weeklyAt", "Weekly at {{time}}", { time: schedule.timeOfDay });
  }
  return copy("manual", "Manual");
}
...
function parseCronSchedule(searchText: string): ParsedSchedule | null {
  ...
  // Time-zone discovery only; no user-facing date or time is formatted here.
...
function parseDailySchedule(searchText: string): ParsedSchedule | null {
  ...
  if (timedDailyMatch) {
    ...
    return timeOfDay ? { schedule: { type: "daily", timeOfDay } } : null;
  }
...
function parseWeekdaysSchedule(searchText: string): ParsedSchedule | null {
  ...
  if (timedWeekdaysMatch) {
    ...
    return timeOfDay ? { schedule: { type: "weekdays", timeOfDay } } : null;
  }
...
function sentenceCase(value: string): string {
  ...
  if (!trimmed) {
    return intentCopy("chatAutomation", "Chat automation");
  }
