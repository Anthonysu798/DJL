...
describe("studioProjects", () => {
  ...
  it("preserves the folder-conflict summary and exact server detail for the route", async () => {
    const rawDetail =
      "Orchestration command invariant failed (project.create): Project 'project-visible' already uses workspace root '/Users/tester/Documents/Synara/Studio'.";
    nativeApiMock.dispatchError = new Error(rawDetail);
    nativeApiMock.shellSnapshotProjects = [
      {
        id: "project-visible",
        kind: "project",
    ...
    await expect(
      ...
      error: {
        summary:
          'Work can\'t use "/Users/tester/Documents/Synara/Studio" because another project already uses that folder. Remove or move that project, then retry.',
        detail: rawDetail,
      },
    });
  });
