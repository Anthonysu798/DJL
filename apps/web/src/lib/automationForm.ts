...
import { rendererLocale, translateRendererCopy } from "../i18n";

const automationCopy = (key: string, defaultValue: string, values?: Record<string, unknown>) =>
...
export const SCHEDULE_KIND_OPTIONS: readonly { value: ScheduleKind; readonly label: string }[] = [
  {
    value: "manual",
    get label() {
      return automationCopy("scheduleKinds.manual", "Manual");
    },
  },
  {
    value: "once",
    get label() {
      return automationCopy("scheduleKinds.once", "Once");
    },
  },
  {
    value: "hourly",
    get label() {
      return automationCopy("scheduleKinds.hourly", "Hourly");
    },
  },
  {
    value: "daily",
    get label() {
      return automationCopy("scheduleKinds.daily", "Daily");
    },
  },
  {
    value: "weekdays",
    get label() {
      return automationCopy("scheduleKinds.weekdays", "Weekdays");
    },
  },
  {
    value: "weekly",
    get label() {
      return automationCopy("scheduleKinds.weekly", "Weekly");
    },
  },
  {
    value: "custom",
    get label() {
      return automationCopy("scheduleKinds.custom", "Custom");
    },
  },
  {
    value: "cron",
    get label() {
      return automationCopy("scheduleKinds.cron", "Cron");
    },
  },
];
...
function localTimezone(): string {
  // Time-zone discovery only: this does not format user-facing copy and therefore
  // intentionally bypasses the selected-locale formatter layer.
...
export function formatDateTime(
  ...
  if (!value) return automationCopy("schedule.notScheduled", "Not scheduled");
  const date = new Date(value);
...
function formatIntervalSchedule(seconds: number): string {
  return seconds % 60 === 0
    ? automationCopy("interval.everyMinutes", "Every {{count}} min", { count: seconds / 60 })
    : automationCopy("interval.everySeconds", "Every {{count}} sec", { count: seconds });
}

function formatIntervalCadence(seconds: number): string {
  if (seconds === 3600) return automationCopy("schedule.hourly", "Hourly");
  if (seconds % 3600 === 0) {
    return automationCopy("intent.everyCompact", "Every {{amount}}{{unit}}", {
      amount: seconds / 3600,
      unit: automationCopy("intent.units.hourCompact", "h"),
    });
  }
  if (seconds % 60 === 0) {
    return automationCopy("intent.everyCompact", "Every {{amount}}{{unit}}", {
      amount: seconds / 60,
      unit: automationCopy("intent.units.minuteCompact", "m"),
    });
  }
  return automationCopy("intent.everyCompact", "Every {{amount}}{{unit}}", {
    amount: seconds,
    unit: automationCopy("intent.units.secondCompact", "s"),
  });
}
...
export function formatSchedule(
  ...
  switch (schedule.type) {
    case "manual":
      return automationCopy("schedule.manual", "Manual");
    case "once":
      return automationCopy("scheduleDisplay.once", "Once {{date}}", {
        date: formatDateTime(schedule.runAt, locale, timestampFormat),
      });
    case "interval":
      return formatIntervalSchedule(schedule.everySeconds);
    case "daily":
      return automationCopy("scheduleDisplay.daily", "Daily {{time}}{{timezone}}", {
        time: formatClockTime(schedule.timeOfDay, locale, timestampFormat),
        timezone: timezoneSuffix(schedule),
      });
    case "weekdays":
      return automationCopy("scheduleDisplay.weekdays", "Weekdays {{time}}{{timezone}}", {
        time: formatClockTime(schedule.timeOfDay, locale, timestampFormat),
        timezone: timezoneSuffix(schedule),
      });
    case "weekly":
      return automationCopy("scheduleDisplay.weekly", "Weekly {{day}} {{time}}{{timezone}}", {
        day: weekdayLabel(schedule.dayOfWeek),
        time: formatClockTime(schedule.timeOfDay, locale, timestampFormat),
        timezone: timezoneSuffix(schedule),
      });
    case "cron":
      return automationCopy("scheduleDisplay.cron", "Cron {{expression}} {{timezone}}", {
...
/** Formats a stored 24-hour wall-clock value without applying a time-zone shift. */
export function formatClockTime(
  ...
  if (hour > 23 || minute > 59) return timeOfDay;
  return formatLocaleDateTime(Date.UTC(2026, 0, 1, hour, minute), locale, {
    ...getTimestampFormatOptions(timestampFormat, false),
    timeZone: "UTC",
  });
}
...
export function formatCadence(
  ...
  switch (schedule.type) {
    case "manual":
      return automationCopy("schedule.manual", "Manual");
    case "once":
      return formatDateTime(schedule.runAt, locale, timestampFormat);
    case "interval":
      return formatIntervalCadence(schedule.everySeconds);
    case "daily":
      return automationCopy("intent.dailyAt", "Daily at {{time}}", {
        time: formatClockTime(schedule.timeOfDay, locale, timestampFormat),
      });
    case "weekdays":
      return automationCopy("intent.weekdaysAt", "Weekdays at {{time}}", {
        time: formatClockTime(schedule.timeOfDay, locale, timestampFormat),
      });
    case "weekly":
      return automationCopy("intent.weeklyOnAt", "{{day}} at {{time}}", {
        day: weekdayLabel(schedule.dayOfWeek),
        time: formatClockTime(schedule.timeOfDay, locale, timestampFormat),
      });
    case "cron":
      return automationCopy("intent.cron", "Cron {{expression}}", {
        expression: schedule.expression,
      });
  }
}
...
export function weekdayLabel(value: number): string {
  const key = ["sun", "mon", "tue", "wed", "thu", "fri", "sat"][value] ?? "sun";
