...
import { ContainerChatFailureError } from "./containerChatFailure";
          ...
          throw new ContainerChatFailureError(
            {
              summary: translateRendererCopy(
                "work:studio.errors.folderConflict",
                'Work can\'t use "{{path}}" because another project already uses that folder. Remove or move that project, then retry.',
                { path: workspaceRoot },
              ),
              detail: message,
            },
