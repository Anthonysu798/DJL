...
async function dispatchKanbanDraftThreadOnce(
  ...
  const composerBrowserFindings = draftComposerState?.browserFindings ?? [];
  const sendableTerminalContexts = filterTerminalContextsWithText(
    draftComposerState?.terminalContexts ?? [],
  );
  const serializedFindingContext = buildKanbanBrowserFindingDispatch({
    prompt,
    messageText: appendFileCommentsToPrompt(
      appendTerminalContextsToPrompt(
    ...
    findings: composerBrowserFindings,
  });
  const titleSeed =
    serializedFindingContext.titleSeed ||
    (composerImages[0]
      ? translateRendererCopy("work:kanban.dispatch.imageTitle", "Image: {{name}}", {
          name: composerImages[0].name,
        })
      : "") ||
    (composerFiles[0]
      ? translateRendererCopy("work:kanban.dispatch.fileTitle", "File: {{name}}", {
          name: composerFiles[0].name,
        })
      : "") ||
    (composerAssistantSelections.length > 0
      ? translateRendererCopy(
          "work:kanban.dispatch.referencedSelection",
          "Referenced assistant selection",
        )
      : "") ||
    (sendableTerminalContexts.length > 0
      ? translateRendererCopy("work:kanban.dispatch.attachedTerminal", "Attached terminal context")
      : "") ||
    (composerFileComments.length > 0
      ? formatFileCommentTitleSeed(composerFileComments.length)
      : "") ||
    translateRendererCopy("work:kanban.actions.newTask", "New task");
  ...
  const messageText = serializedFindingContext.messageText;
    ...
    return {
      ...
      message:
        error instanceof Error
          ? error.message
          : translateRendererCopy(
              "work:kanban.dispatch.sendDraftedPromptFailed",
              "Could not send the drafted prompt.",
            ),
    };
  }
