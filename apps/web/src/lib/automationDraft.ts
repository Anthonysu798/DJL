...
const warningCopy = (key: string, defaultValue: string): string =>
  translateRendererCopy(`work:automations.warnings.${key}`, defaultValue);
...
export function buildAutomationDraftWarnings(input: {
  ...
  if (input.hasEphemeralContext) {
    warnings.push({
      ...
      title: warningCopy("context.title", "Composer context is not persisted"),
      detail: warningCopy(
        "context.detail",
  ...
  if (input.schedule.type === "manual") {
    warnings.push({
      id: "missing-schedule",
      title: warningCopy("schedule.title", "Schedule needs review"),
      detail: warningCopy(
        "schedule.detail",
  ...
  if (input.schedule.type === "interval" && input.schedule.everySeconds < 60) {
    warnings.push({
      ...
      title: warningCopy("fastLoop.title", "Fast recurring loop"),
      detail: warningCopy(
        "fastLoop.detail",
  ...
  if (input.runtimeMode === "full-access") {
    warnings.push({
      id: "full-access",
      title: warningCopy("fullAccess.title", "Full access"),
      detail: warningCopy(
        "fullAccess.detail",
    ...
    warnings.push({
      ...
      title:
        input.worktreeMode === "auto"
          ? warningCopy("localCheckout.autoTitle", "Auto fallback may use local checkout")
          : warningCopy("localCheckout.title", "Local checkout"),
      detail:
        input.worktreeMode === "auto"
          ? warningCopy(
              "localCheckout.autoDetail",
              "If DJL cannot create a worktree, runs may fall back to editing the active project checkout.",
            )
          : warningCopy(
              "localCheckout.detail",
  ...
  if (
    ...
    warnings.push({
      id: "worktree-cleanup",
      title: warningCopy("cleanup.title", "Worktree cleanup"),
      detail: warningCopy(
        "cleanup.detail",
  ...
  if (
    ...
    warnings.push({
      ...
      title: warningCopy("generated.title", "Review generated fields"),
      detail: warningCopy(
        "generated.detail",
        "DJL was not fully confident about the parsed automation fields.",
      ),
      requiresAcknowledgement: false,
    });
  }
  if (containsAutomationSkillReference(input.prompt)) {
    warnings.push({
      id: "skill-reference",
      title: warningCopy("skill.title", "Skill reference kept in prompt"),
      detail: warningCopy(
        "skill.detail",
