...
type RateLimitCopyKey = keyof typeof englishCatalog.settings.usage.rateLimits;

function rateLimitText(
  key: RateLimitCopyKey,
  values: Record<string, string | number> = {},
  t?: TFunction,
): string {
  const defaultValue = englishCatalog.settings.usage.rateLimits[key];
  if (t) {
    return t(`usage.rateLimits.${key}`, { ns: "settings", defaultValue, ...values });
  }
...
export function formatRateLimitRemainingPercent(
  remainingPercent: number | undefined,
  locale?: string,
): string {
  if (remainingPercent === undefined) return "—";
  return formatLocaleNumber(Math.min(100, Math.max(0, remainingPercent)) / 100, locale ?? "en", {
    style: "percent",
...
export function formatRateLimitResetCountdown(resetsAt: string, t?: TFunction): string {
  const resetMs = Date.parse(resetsAt);
  ...
  if (diffMs <= 0) {
    return rateLimitText("resetsSoon", {}, t);
  }
  ...
  if (days > 0) {
    return rateLimitText("resetsInDaysHours", { days, hours }, t);
  }
  if (hours > 0) {
    return rateLimitText("resetsInHoursMinutes", { hours, minutes }, t);
  }
  if (minutes > 0) {
    return rateLimitText("resetsInMinutes", { minutes }, t);
  }
  return rateLimitText("resetsSoon", {}, t);
}

export function formatRateLimitResetTime(
  resetsAt: string,
  locale = "en",
  ...
  if (diffMs > 0 && diffMs < 24 * 60 * 60 * 1000) {
    return formatLocaleDateTime(resetMs, locale, getTimestampFormatOptions(timestampFormat, false));
  }

  return formatLocaleDateTime(resetMs, locale, {
    day: "numeric",
    month: "short",
  });
}

export function formatRateLimitWindowLabel(label: string, t?: TFunction): string {
  if (label === "Weekly") return rateLimitText("weekly", {}, t);
  if (label === "Current") return rateLimitText("current", {}, t);
  if (label === "5h") return rateLimitText("fiveHours", {}, t);
  return label;
}
