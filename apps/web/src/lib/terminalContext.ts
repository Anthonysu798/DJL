...
import type { BrowserFindingPromptEntry } from "@synara/contracts";
...
export interface DisplayedUserMessageState {
  ...
  browserFindings?: BrowserFindingPromptEntry[];
}
...
const TRAILING_SERIALIZED_COMPOSER_BLOCK_PATTERNS = [
  /\n*(<browser_findings>[\s\S]*?<\/browser_findings>)\s*$/u,
  /\n*(<pasted_text>\n[\s\S]*?\n<\/pasted_text>)\s*$/u,
  /\n*(<file_comments>\n[\s\S]*?\n<\/file_comments>)\s*$/u,
...
export function deriveDisplayedUserMessageState(
  ...
  const extractedBrowserFindings = parseBrowserFindingsBlock(prompt);
  const extractedPastedTexts = extractTrailingPastedTexts(extractedBrowserFindings.visibleText);
  ...
  return {
    ...
    ...(extractedBrowserFindings.findings.length > 0
      ? { browserFindings: extractedBrowserFindings.findings }
      : {}),
  };
}
