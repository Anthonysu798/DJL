...
const t = ((key: string, values?: Record<string, unknown>) => {
  const messages: Record<string, string> = {
    ...
    "terminalClose.confirmNamed": `Close terminal "${values?.title}"?`,
    "terminalClose.confirm": "Close this terminal?",
    "terminalClose.historyAndThread":
      "This permanently clears the terminal history for this tab and deletes the empty terminal thread.",
    "terminalClose.history": "This permanently clears the terminal history for this tab.",
  };
  return messages[key] ?? key;
}) as Parameters<typeof buildTerminalCloseConfirmationMessage>[0]["t"];
  ...
  it("uses the caller translation function while preserving the terminal title", () => {
    ...
    expect(
      buildTerminalCloseConfirmationMessage({
        ...
        t: keyT,
      } as Parameters<typeof buildTerminalCloseConfirmationMessage>[0]),
    ).toBe("terminalClose.confirmNamed:Deploy shell\nterminalClose.history:");
  });
