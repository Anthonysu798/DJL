...
export const projectQueryKeys = {
  ...
  workDocumentPreview: (threadId: ThreadId | null, path: string | null) =>
    ["work", "document-preview", threadId, path] as const,
  workDocumentRendererStatus: ["work", "document-renderer", "status"] as const,
  workDocumentRenderRequest: (threadId: ThreadId | null, path: string | null) =>
    ["work", "document-render", "request", threadId, path] as const,
  workDocumentRender: (threadId: ThreadId | null, renderId: string | null) =>
    ["work", "document-render", threadId, renderId] as const,
...
export function workDocumentPreviewQueryOptions(input: {
  threadId: ThreadId | null;
  path: string | null;
  enabled?: boolean;
}) {
  return queryOptions<WorkPreviewDocumentResult>({
    queryKey: projectQueryKeys.workDocumentPreview(input.threadId, input.path),
    queryFn: async () => {
      const api = ensureNativeApi();
      if (!input.threadId || !input.path) {
        throw new Error("Work document preview is unavailable.");
      }
      return api.work.previewDocument({ threadId: input.threadId, path: input.path });
    },
    enabled: (input.enabled ?? true) && input.threadId !== null && input.path !== null,
    staleTime: DEFAULT_READ_FILE_STALE_TIME,
  });
}

export function workDocumentRendererStatusQueryOptions(input?: { enabled?: boolean }) {
  return queryOptions<DocumentRendererStatus>({
    queryKey: projectQueryKeys.workDocumentRendererStatus,
    queryFn: () => ensureNativeApi().work.documentRenderer.status(),
    enabled: input?.enabled ?? true,
    staleTime: 10_000,
  });
}

export function workDocumentRenderRequestQueryOptions(input: {
  threadId: ThreadId | null;
  path: string | null;
  enabled?: boolean;
}) {
  return queryOptions<WorkRequestDocumentRenderResult>({
    queryKey: projectQueryKeys.workDocumentRenderRequest(input.threadId, input.path),
    queryFn: () => {
      if (!input.threadId || !input.path) throw new Error("Document render is unavailable.");
      return ensureNativeApi().work.requestDocumentRender({
...
export function workDocumentRenderQueryOptions(input: {
  threadId: ThreadId | null;
  renderId: string | null;
  enabled?: boolean;
}) {
  return queryOptions<WorkGetDocumentRenderResult>({
    queryKey: projectQueryKeys.workDocumentRender(input.threadId, input.renderId),
    queryFn: () => {
      if (!input.threadId || !input.renderId) throw new Error("Document render is unavailable.");
      return ensureNativeApi().work.getDocumentRender({
        threadId: input.threadId,
        renderId: input.renderId,
      });
    },
    enabled: (input.enabled ?? true) && input.threadId !== null && input.renderId !== null,
    refetchInterval: (query) =>
      query.state.data?.state === "queued" || query.state.data?.state === "rendering" ? 350 : false,
    retry: 1,
  });
}
