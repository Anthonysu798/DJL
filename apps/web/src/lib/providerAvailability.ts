...
export function normalizeProviderStatusForLocalConfig(input: {
  ...
  return {
    ...
    available: true,
    status: "warning",
    message: translateRendererCopy(
      "common:hardening.customBinary",
      "{{provider}} uses a custom local binary path in this app. Availability will be confirmed when you start a session.",
