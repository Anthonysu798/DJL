...
export function formatLineRangeLabel(
  ...
  return endLine !== startLine
    ? t("references.location.lines", { start: startLine, end: endLine })
    : t("references.location.line", { number: startLine });
}
...
export function formatSelectionLabel(
  ...
  if (typeof startColumn !== "number" || typeof endColumn !== "number") {
    return formatLineRangeLabel(reference.startLine, endLine, t);
  }
  if (reference.startLine === endLine) {
    const columns = startColumn === endColumn ? `${startColumn}` : `${startColumn}-${endColumn}`;
    return t("references.location.lineColumns", { line: reference.startLine, columns });
  }
  return t("references.location.lineColumnRange", {
...
export function formatChatFileReference(
  ...
  const label = formatSelectionLabel(reference, t);
  if (label) {
    return `${token} (${label})`;
  }
...
export function buildWhyChangedPrompt(path: string, t: TFunction<"workspace">): string {
  return t("references.prompts.whyChanged", { reference: formatComposerMentionToken(path) });
}
...
export function buildWhyLinesPrompt(
  reference: ChatFileReference,
  t: TFunction<"workspace">,
): string {
  const token = formatComposerMentionToken(reference.path);
  if (typeof reference.startLine !== "number") {
    return t("references.prompts.whyFile", { reference: token });
  }
  const endLine = reference.endLine ?? reference.startLine;
  return t("references.prompts.whyLines", {
    location: formatLineRangeLabel(reference.startLine, endLine, t),
    reference: token,
  });
}
...
export function buildDiffSelectionReference(
  path: string,
  snippet: string,
  t: TFunction<"workspace">,
): string {
  return formatChatFileReference({ path, snippet }, t);
}
...
export function appendChatFileReference(
  threadId: ThreadId,
  reference: ChatFileReference,
  t: TFunction<"workspace">,
): void {
  appendComposerPromptText(threadId, formatChatFileReference(reference, t));
}
