...
// Confirm them with the authoritative full snapshot before route guards replace a
// remembered thread URL. Projection repair is an explicit maintenance operation;
// startup recovery must not block navigation on it when the valid history is empty.
export async function refreshEmptyRouteRestoreSnapshot(
  ...
  if (shellSnapshotHasProjectsOrThreads(shellSnapshot)) {
    ...
    // Project-only shell snapshots do not prove route recovery is done; confirm
    // the thread history with the authoritative full snapshot below.
  }
  ...
  return readModelHasThreads(readModel);
}
