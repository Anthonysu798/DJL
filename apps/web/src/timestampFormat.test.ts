...
describe("getTimestampFormatOptions", () => {
  ...
  it("formats with the selected app locale while retaining the hour-cycle override", () => {
    const value = "2026-07-15T13:05:00.000Z";
    const english = formatShortTimestamp(value, "12-hour", "en", "UTC");
    const french = formatShortTimestamp(value, "24-hour", "fr", "UTC");

    expect(english).toMatch(/1:05/);
    expect(english).toMatch(/PM/i);
    expect(french).toBe("13:05");
  });
});
