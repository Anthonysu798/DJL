...
const recentViewCopy = (key: string, defaultValue: string) =>
  translateRendererCopy(`shell:recentViews.${key}`, defaultValue);
  ...
  return input.recentViews.map((view) => {
    ...
    switch (view.kind) {
      case "thread": {
        ...
        const title = normalizeOptionalId(thread?.title) ?? recentViewCopy("newChat", "New chat");
        const subtitleParts = [
          projectName ?? recentViewCopy("chat", "Chat"),
          base.isTerminal ? recentViewCopy("terminal", "Terminal") : recentViewCopy("chat", "Chat"),
          base.isSplit ? recentViewCopy("split", "Split") : null,
      ...
      case "workspace":
        return {
          ...
          icon: { kind: "workspace" },
          title:
            workspaceNameById.get(view.workspaceId) ?? recentViewCopy("workspace", "Workspace"),
          subtitle: recentViewCopy("terminalWorkspace", "Terminal workspace"),
        };
      case "settings":
        return {
          ...base,
          icon: { kind: "settings" },
          title: recentViewCopy("settings", "Settings"),
          subtitle: view.section
            ? recentViewCopy(
                `settingsSections.${SETTINGS_LABELS[view.section] ?? "unknown"}`,
                view.section,
              )
            : recentViewCopy("appSettings", "App settings"),
        };
      case "plugins":
        return {
          ...base,
          icon: { kind: "plugins" },
          title: recentViewCopy("plugins", "Plugins"),
          subtitle: recentViewCopy("pluginsSubtitle", "Extensions and integrations"),
        };
    }
  });
}
