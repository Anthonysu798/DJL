...
function threadShellsEqual(left: ThreadShell | undefined, right: ThreadShell): boolean {
  return (
    ...
    deepEqualJson(left.workTask ?? null, right.workTask ?? null) &&
...
function normalizeThreadFromReadModel(
  ...
  const workTask =
    previous?.workTask && incoming.workTask && deepEqualJson(previous.workTask, incoming.workTask)
      ? previous.workTask
      : (incoming.workTask ?? null);
  ...
  if (
    ...
    deepEqualJson(previous.workTask ?? null, workTask) &&
...
function applyOrchestrationEvent(
  ...
  switch (event.type) {
        ...
        (thread) => {
          ...
          const syntheticResolvedActivity = {
            ...
            summary: translateRendererCopy(
              "common:hardening.userInputSubmitted",
    ...
    case "thread.work-task-transitioned":
      return applyThreadUpdate(
        state,
        event.payload.threadId,
        (thread) => {
          if (deepEqualJson(thread.workTask ?? null, event.payload.task)) return thread;
          return {
            ...thread,
            workTask: event.payload.task,
            updatedAt: event.payload.task.updatedAt,
          };
        },
        { ...options, recomputeSummarySignals: false, updateSidebarSummary: true },
      );
