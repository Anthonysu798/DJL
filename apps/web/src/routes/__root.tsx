...
import { isThreadDetailEventForThread } from "@synara/shared/orchestrationThreadEvents";
...
import { LocalModelSetupCoordinator } from "../localModelSetupCoordinator";
import {
        ...
        <div className="flex h-screen flex-col bg-background text-foreground">
          <div className="flex flex-1 items-center justify-center">
            <p className="text-sm text-muted-foreground">
              {t("startup.connecting", { app: APP_DISPLAY_NAME })}
            </p>
          </div>
        </div>
  ...
  const updateAll = useCallback(
    async (providers: ReadonlyArray<ServerProviderStatus>) => {
      ...
      const toastId =
        ...
        toastManager.add({
          type: "loading",
          title: t("providerUpdates.updating"),
          description:
            providers.length === 1
              ? t("providerUpdates.updatingNamed", {
        ...
        for (const provider of providers) {
          try {
            ...
            if (updateState?.status === "failed" || updateState?.status === "unchanged") {
              failures.push({
                provider,
                reason: updateState.message ?? t("providerUpdates.errors.commandIncomplete"),
              });
            } else if (refreshed?.versionAdvisory?.status === "behind_latest") {
              failures.push({
                provider,
                reason: t("providerUpdates.errors.stillOutdated"),
              });
            }
          } catch (error) {
            failures.push({
              provider,
              reason:
                error instanceof Error ? error.message : t("providerUpdates.errors.requestFailed"),
            });
          }
        }
      } catch (error) {
          ...
          failures.push({
            ...
            reason:
              error instanceof Error
                ? error.message
                : t("providerUpdates.errors.requestStartFailed"),
          });
        }
      } finally {
        ...
        toastManager.update(toastId, {
          ...
          title:
            failures.length === providers.length
              ? t("providerUpdates.failedAll")
              : t("providerUpdates.failedSome"),
          description:
            manualCommands.length > 0
              ? `${failureLines}\n\n${t("providerUpdates.manualCommands", { count: manualCommands.length })}`
              : failureLines,
          data: {
      ...
      toastManager.update(toastId, {
        ...
        title:
          providers.length === 1
            ? t("providerUpdates.updatedNamed", {
                provider: PROVIDER_DISPLAY_NAMES[providers[0]!.provider],
              })
  ...
  useEffect(() => {
    ...
    const title =
      outdatedProviders.length === 1
        ? t("providerUpdates.availableNamed", { provider: providerName })
        : t("providerUpdates.availableCount", { count: outdatedProviders.length });
    const description =
      outdatedProviders.length === 1
        ? t("providerUpdates.availableNamedDescription", { provider: providerName })
        : t("providerUpdates.availableAdditional", {
    ...
    toastId = toastManager.add({
      ...
      timeout: 0,
      actionProps: {
        children: t("providerUpdates.review"),
        onClick: () => {
          if (activeToastRef.current?.toastId === toastId) {
            toastManager.close(toastId);
            activeToastRef.current = null;
  ...
  return (
    <div className="relative flex min-h-screen items-center justify-center overflow-hidden bg-background px-4 py-10 text-foreground sm:px-6">
      ...
      <section className="relative w-full max-w-xl rounded-2xl border border-border/80 bg-card/90 p-6 shadow-2xl shadow-black/20 backdrop-blur-md sm:p-8">
        ...
        <h1 className="mt-3 text-2xl font-semibold sm:text-3xl">{t("error.title")}</h1>
        <p className="mt-2 text-sm leading-relaxed text-muted-foreground">{t("error.summary")}</p>
...
function EventRouter() {
  ...
  useEffect(() => {
    ...
    const loadShellSnapshotOnce = async (force = false) => {
      const snapshot = await api.orchestration.getShellSnapshot();
      if (!force && !shouldApplyBootstrapShellSnapshot(snapshot)) {
        return;
      }
      ...
      if (event.type === "activity") {
        ...
        if (event.cliKind || currentCliKind !== null) {
          terminalStore.setTerminalMetadata(terminalThreadId, event.terminalId, {
            ...
            label: event.cliKind
              ? defaultTerminalTitleForCliKind(event.cliKind)
              : t("terminal.title"),
          });
        }
      }
    ...
    const unsubTransportState = addWsTransportStateListener(
      (state) => {
        if (state !== "open") return;
        void (async () => {
          // Queries can exhaust their bounded retries while the packaged backend
          // is still starting. Revalidate them as soon as the socket is usable
          // instead of relying on a later tab/window focus change.
          void queryClient.invalidateQueries({ queryKey: providerDiscoveryQueryKeys.all });
          await ensureScopedSubscriptions();
          if (disposed) return;
          await loadShellSnapshotOnce(true);
        })().catch(() => undefined);
      },
      { replayCurrent: true },
    );
    ...
    const unsubServerConfigUpdated = onServerConfigUpdated((payload) => {
      ...
      toastManager.add({
        type: "warning",
        title: t("keybindings.invalid"),
        description: issue.message,
        actionProps: {
          children: t("keybindings.openFile"),
          onClick: () => {
            void queryClient
              ...
              .catch((error) => {
                toastManager.add({
                  ...
                  description:
                    error instanceof Error ? error.message : t("keybindings.openFailedFallback"),
                });
              });
          },
        },
