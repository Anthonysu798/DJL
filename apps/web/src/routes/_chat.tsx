    ...
    return onServerMaintenanceUpdated((event) => {
      ...
      if (state === "started") {
        toastIdRef.current = toastManager.add({
          type: "loading",
          title: t("maintenance.threadRetention.running"),
          description: t("maintenance.threadRetention.preparing"),
          timeout: 0,
        ...
        toastManager.update(toastId, {
          ...
          description:
            totalCount && totalCount > 0
              ? t("maintenance.threadRetention.progressOf", {
                  count: deletedCount ?? 0,
                  total: totalCount,
                })
              : t("maintenance.threadRetention.progress", { count: deletedCount ?? 0 }),
          timeout: 0,
          data: { allowCrossThreadVisibility: true },
        });
        return;
      ...
      if (state === "failed") {
        ...
        if (toastId) {
          toastManager.update(toastId, {
            type: "warning",
            title: t("maintenance.threadRetention.paused"),
            description: error ?? t("maintenance.threadRetention.retryLater"),
            timeout: 6000,
            data: { allowCrossThreadVisibility: true },
          });
          return;
      ...
      toastManager.update(toastId, {
        type: "success",
        title: t("maintenance.threadRetention.complete"),
        description:
          deletedCount && deletedCount > 0
            ? t("maintenance.threadRetention.completedCount", { count: deletedCount })
            : t("maintenance.threadRetention.none"),
        timeout: 3500,
        data: { allowCrossThreadVisibility: true },
      });
    });
