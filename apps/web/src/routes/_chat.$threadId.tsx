...
} from "../lib/browserPanelCloseGuard";
import {
...
const DiffLoadingFallback = (props: { mode: DiffPanelMode; hideHeader?: boolean }) => {
  ...
  return (
    ...
    >
      <DiffPanelLoadingState label={t("panels.loadingDiff")} />
    </DiffPanelShell>
  );
};
...
function SplitPaneEmbeddedPanel(props: {
  ...
  threadId: ThreadIdType | null;
  onClosePanel: () => void;
  onRegisterBrowserCloseRequest: (requestClose: (onClosed?: () => void) => void) => () => void;
  panelState: Pick<SplitViewPanePanelState, "panel" | "diffTurnId" | "diffFilePath">;
  isFocused: boolean;
  onUpdatePanelState: (
    patch: Partial<Pick<SplitViewPanePanelState, "panel" | "diffTurnId" | "diffFilePath">>,
  ...
  return (
    ...
    >
      ...
      />
      {props.panel === "browser" ? (
        <Suspense fallback={<PanelStateMessage>{t("panels.loadingBrowser")}</PanelStateMessage>}>
          <BrowserPanel
            mode="sidebar"
            threadId={props.threadId}
            onClosePanel={props.onClosePanel}
            onRegisterCloseRequest={props.onRegisterBrowserCloseRequest}
          />
        </Suspense>
      ) : (
        <LazyDiffPanel
  ...
  return (
    ...
    >
      ...
      <SplitPaneEmbeddedPanel
        ...
        threadId={props.threadId}
        onClosePanel={props.onClosePanel}
        onRegisterBrowserCloseRequest={props.onRegisterBrowserCloseRequest}
        panelState={props.panelState}
        isFocused={props.isFocused}
        onUpdatePanelState={props.onUpdatePanelState}
      />
...
function SplitChatSurface(props: { splitViewId: SplitViewId; routeThreadId: ThreadIdType }) {
  ...
  const browserPaneCloseRequestsRef = useRef(new Map<PaneId, (onClosed?: () => void) => void>());
  ...
  const togglePanePanel = useCallback(
    (paneId: PaneId, panel: ChatRightPanel) => {
      ...
      if (panel === "browser") {
        toggleBrowserPanelWithGuard({
          browserOpen: leaf.panel.panel === "browser",
          requestClose: browserPaneCloseRequestsRef.current.get(paneId),
          toggle: applyToggle,
        });
        return;
      }
  ...
  const openPaneTurnDiff = useCallback(
    (paneId: PaneId, turnId: TurnId, filePath?: string) => {
      const leaf = activeSplitView ? findLeafPaneById(activeSplitView.root, paneId) : null;
      replaceBrowserPanelWithGuard({
        browserOpen: leaf?.panel.panel === "browser",
        requestClose: browserPaneCloseRequestsRef.current.get(paneId),
        replace: () =>
  ...
  const renderLeaf = ({ leaf }: { leaf: LeafPane }): ReactNode => {
    ...
    return (
      <SplitPaneSurface
        ...
        onRegisterBrowserCloseRequest={(requestClose) => {
          browserPaneCloseRequestsRef.current.set(leaf.id, requestClose);
          return () => {
            if (browserPaneCloseRequestsRef.current.get(leaf.id) === requestClose) {
              browserPaneCloseRequestsRef.current.delete(leaf.id);
            }
          };
        }}
      ...
      >
        <DialogPopup className="max-w-lg">
          <DialogHeader className="items-center text-center">
            <DialogTitle>{t("split.chooseChat")}</DialogTitle>
...
function RightDockPanePlaceholder(props: { kind: RightDockPaneKind }) {
  const { t } = useTranslation("chat");
  const { labelKey } = getRightDockPaneMeta(props.kind);
  return <PanelStateMessage>{t("panels.comingSoon", { panel: t(labelKey) })}</PanelStateMessage>;
}
...
function SingleChatSurface(props: {
  ...
  const browserPaneCloseRequestsRef = useRef(new Map<string, (onClosed?: () => void) => void>());
  ...
  const handleToggleDiff = useCallback(() => {
    ...
    replaceBrowserPanelWithGuard({
      browserOpen: activePane?.kind === "browser",
      requestClose:
        activePane?.kind === "browser"
          ? browserPaneCloseRequestsRef.current.get(activePane.id)
          : undefined,
      replace: () => toggleSingletonPane(props.threadId, { kind: "diff" }),
    });
  }, [activePane, props.threadId, requestImmediateDockHydration, toggleSingletonPane]);
    ...
    toggleBrowserPanelWithGuard({
      ...
      requestClose:
        activePane?.kind === "browser"
          ? browserPaneCloseRequestsRef.current.get(activePane.id)
          : undefined,
      toggle: () => toggleSingletonPane(props.threadId, { kind: "browser" }),
    });
  }, [activePane, props.threadId, requestImmediateDockHydration, toggleSingletonPane]);
  ...
  const handleOpenTurnDiff = useCallback(
    ...
    [activePane, openPane, props.threadId, requestImmediateDockHydration],
  );
  ...
  const handleReferenceInChat = useCallback(
    (reference: ChatFileReference) => {
      appendChatFileReference(props.threadId, reference, workspaceT);
    },
    [props.threadId, workspaceT],
  );
  const handleAskWhyInChat = useCallback(
    (reference: ChatFileReference) => {
      appendComposerPromptText(props.threadId, buildWhyLinesPrompt(reference, workspaceT));
    },
    [props.threadId, workspaceT],
  );
    ...
    (projectId: ProjectId) => {
      void openEditorProject(projectId).catch((error: unknown) => {
        toastManager.add({
          ...
          description: error instanceof Error ? error.message : t("errors.openProjectFallback"),
        });
      });
    },
    [openEditorProject, t],
  ...
  const paneLabelOverrides = useMemo(() => {
    ...
    for (const pane of dockState.panes) {
      if (pane.kind === "sidechat" && pane.threadId) {
        overrides[pane.id] = titleByThreadId?.get(pane.threadId) || t("panels.names.side");
      } else if (pane.kind === "file" && pane.filePath) {
        overrides[pane.id] = basenameOfPath(pane.filePath);
      }
    }
    return overrides;
  }, [threadSummaries, dockState.panes, t]);
  ...
  const handleAddDockPane = useCallback(
    (kind: RightDockPaneKind) => {
      ...
      if (kind === "sidechat") {
        ...
        if (!createSidechat) {
          toastManager.add({
            type: "warning",
            title: t("errors.sideUnavailable"),
            description: t("errors.sideUnavailableDescription"),
          });
          return;
        }
        void createSidechat().catch((error) => {
          toastManager.add({
            type: "error",
            title: t("errors.startSide"),
            description: error instanceof Error ? error.message : t("errors.startSideFallback"),
          });
        });
        return;
      }
    ...
    [openPane, props.threadId, requestImmediateDockHydration, t],
  );
    ...
    (
      ...
      switch (pane.kind) {
        case "browser":
          return (
            <Suspense
              fallback={<PanelStateMessage>{t("panels.loadingBrowser")}</PanelStateMessage>}
            >
              <BrowserPanel
                mode="sidebar"
                threadId={props.threadId}
                onClosePanel={() => closePane(props.threadId, pane.id)}
                onRegisterCloseRequest={(requestClose) => {
                  browserPaneCloseRequestsRef.current.set(pane.id, requestClose);
                  return () => {
                    if (browserPaneCloseRequestsRef.current.get(pane.id) === requestClose) {
                      browserPaneCloseRequestsRef.current.delete(pane.id);
                    }
                  };
                }}
        ...
        case "terminal":
          if (context.runtimeMode === "preview") {
            return <PanelStateMessage>{t("panels.sleepingTerminal")}</PanelStateMessage>;
          }
        ...
        case "git":
          return (
            <Suspense fallback={<PanelStateMessage>{t("panels.loadingGit")}</PanelStateMessage>}>
              <GitPanel
                hostThreadId={props.threadId}
                projectId={props.projectId}
        ...
        case "explorer":
          return (
            <Suspense
              fallback={<PanelStateMessage>{t("panels.loadingExplorer")}</PanelStateMessage>}
            >
              <DockExplorerPane
                threadId={props.threadId}
        ...
        case "file":
          return (
            <Suspense fallback={<PanelStateMessage>{t("panels.loadingFile")}</PanelStateMessage>}>
              <DockFilePane
                threadId={props.threadId}
                workspaceRoot={workspaceRoot}
    ...
    <WorkspaceFileOpenerContext.Provider value={dockFileOpener}>
      ...
      >
        ...
        >
          <RouteInsetSurface surfaceClassName={CHAT_BACKGROUND_CLASS_NAME}>
            <DeferredChatView
              ...
              viewModeAction={{
                label: t("environment.editor.view"),
                active: false,
        ...
        <RightDock
          ...
          onClosePane={(paneId) => {
            const requestClose = browserPaneCloseRequestsRef.current.get(paneId);
            if (requestClose) {
              requestClose();
              return;
            }
