...
import {
  ...
  automationMutationErrorContent,
...
describe("automation shared route helpers", () => {
  ...
  it("keeps custom max-iteration caps visible in picker options", () => {
    ...
    expect(maxIterationOptions("1")[0]).toEqual({ value: "1", label: "1 run" });
    expect(maxIterationOptions("2")[0]).toEqual({ value: "2", label: "2 runs" });
  });

  it("formats dates with the selected renderer locale", () => {
    const value = "2026-07-14T15:30:00.000Z";
    ...
    expect(formatDateTime(value, "fr")).toBe(expected);
  });

  it("honors explicit 12/24-hour settings for one-time schedules", () => {
    const once = { type: "once", runAt: "2026-07-15T18:05:00.000Z" } as const;
    const expected12 = formatLocaleDateTime(once.runAt, "en", {
      month: "short",
      day: "numeric",
      ...getTimestampFormatOptions("12-hour", false),
    });
    const expected24 = formatLocaleDateTime(once.runAt, "en", {
      month: "short",
      day: "numeric",
      ...getTimestampFormatOptions("24-hour", false),
    });
    expect(expected12).not.toBe(expected24);
    expect(formatDateTime(once.runAt, "en", "12-hour")).toBe(expected12);
    expect(formatDateTime(once.runAt, "en", "24-hour")).toBe(expected24);
    expect(formatSchedule(once, "en", "12-hour")).toContain(expected12);
    expect(formatSchedule(once, "en", "24-hour")).toContain(expected24);
  });

  it("formats stored daily and weekly wall-clock times without timezone shifting", () => {
    expect(formatClockTime("00:00", "en", "12-hour")).toBe("12:00 AM");
    expect(formatClockTime("12:00", "en", "12-hour")).toBe("12:00 PM");
    expect(formatClockTime("18:05", "en", "12-hour")).toBe("6:05 PM");
    expect(formatClockTime("00:00", "en", "24-hour")).toBe("00:00");
    expect(formatClockTime("12:00", "en", "24-hour")).toBe("12:00");
    expect(formatClockTime("18:05", "en", "24-hour")).toBe("18:05");

    const daily = { type: "daily", timeOfDay: "18:05", timezone: "America/St_Johns" } as const;
    const weekly = {
      type: "weekly",
      dayOfWeek: 3,
      timeOfDay: "18:05",
    ...
    expect(formatCadence(daily, "en", "12-hour")).toContain("6:05 PM");
    expect(formatCadence(daily, "en", "24-hour")).toContain("18:05");
    expect(formatSchedule(weekly, "en", "12-hour")).toContain("6:05 PM");
    expect(formatSchedule(weekly, "en", "24-hour")).toContain("18:05");
    expect(formatSchedule(weekly, "en", "12-hour")).toContain("Asia/Kathmandu");
    expect(formatClockTime("25:00", "en", "12-hour")).toBe("25:00");
  });

  it("localizes compact relative-time words and units", () => {
    const now = Date.now();
    expect(formatRelativeTime(new Date(now - 10_000).toISOString(), "en")).toBe("now");
    expect(formatRelativeTime(new Date(now - 3 * 60_000).toISOString(), "en")).toBe("3 min. ago");
    expect(formatRelativeTime(new Date(now - 3 * 60_000).toISOString(), "fr")).not.toBe(
      "3 min. ago",
    );
  });

  it("keeps exact automation mutation errors as detail under an action summary", () => {
    const rawDetail = "provider rejected run 7 at /tmp/worktree";
    expect(
      automationMutationErrorContent("runNow", new Error(rawDetail), (key) =>
        key === "automations.errors.runNow" ? "Couldn’t run the automation." : key,
      ),
    ).toEqual({ title: "Couldn’t run the automation.", description: rawDetail });
  });
