...
const indexCopy = (key: string, defaultValue: string, values?: Record<string, unknown>) =>
...
function triageRunLabel(run: AutomationRun): string {
  if (run.status === "succeeded" && run.result?.unread)
    return indexCopy("list.newResult", "New result");
  ...
  return (
    ...
    >
      ...
      {onDelete ? (
        <button
          type="button"
          aria-label={t("automations.actions.delete", { ns: "work" })}
...
function rowMeta(
  ...
  if (!definition.enabled) {
    return automationLifecycleState(definition) === "done"
      ? indexCopy("lifecycle.done", "Done")
      : indexCopy("lifecycle.paused", "Paused");
  }
  return formatCadence(definition.schedule, locale, timestampFormat);
}
...
function AutomationsRouteView() {
  ...
  const projectName = (definition: AutomationDefinition) =>
    ...
    t("automations.detail.unknownProject", { ns: "work" });
  ...
  const sourceSuffix = (definition: AutomationDefinition) => {
    ...
    return sourceThread
      ? t("automations.list.fromThread", {
          ns: "work",
          thread: resolveThreadPickerTitle(sourceThread.title),
        })
      : "";
  };
  ...
  const subtitle = (definition: AutomationDefinition) => {
    ...
    if (definition.mode === "heartbeat") {
      ...
      return t("automations.list.heartbeatTarget", { ns: "work", target, suffix });
    }
  ...
  const renderTriageRow = (run: AutomationRun) => {
    ...
    const target = definition
      ? subtitle(definition)
      : t("automations.list.savedRun", { ns: "work" });
    return (
      <AutomationListRow
        key={run.id}
        ...
        title={definition?.name ?? t("automations.list.automationRun", { ns: "work" })}
      ...
      <section className="flex flex-col gap-0.5">
        <div className="flex items-center justify-between gap-3 px-2 pb-1">
          <h2 className="text-sm font-medium text-foreground">
            {t("automations.list.needsReview", { ns: "work" })}
          </h2>
          <div className="flex items-center gap-0.5 rounded-md bg-[var(--color-background-elevated-secondary)] p-0.5 text-xs">
            {(["unread", "all"] as const).map((value) => (
              ...
              >
                {value === "unread"
                  ? t("automations.list.unreadCount", { ns: "work", count: unreadTriageCount })
                  : t("automations.list.allCount", { ns: "work", count: allTriageRuns.length })}
              </button>
            ))}
          </div>
        </div>
        {triageRuns.length === 0 ? (
          <div className="px-2 py-4 text-xs text-muted-foreground">
            {t("automations.list.noUnread", { ns: "work" })}
          </div>
        ) : (
          ...
          <div className="mx-auto flex w-full max-w-3xl flex-col gap-6 px-6 pb-12 pt-8">
            ...
            {isLoading ? (
              <div className="py-16 text-center text-sm text-muted-foreground">
                {t("automations.list.loading", { ns: "work" })}
              </div>
            ) : data.definitions.length === 0 ? (
              <div className="flex flex-col items-center gap-1 py-16 text-center">
                <p className="text-sm font-medium text-foreground">
                  {t("automations.list.emptyTitle", { ns: "work" })}
                </p>
                <p className="max-w-xs text-xs text-muted-foreground">
                  {t("automations.list.emptyDescription", { ns: "work" })}
                </p>
              </div>
            ) : (
              <div className="flex flex-col gap-6">
                {renderTriage()}
                {renderSection(t("automations.list.current", { ns: "work" }), active)}
                {renderSection(t("automations.lifecycle.paused", { ns: "work" }), inactive)}
              </div>
            )}
          </div>
        </main>
