...
describe("composerDraftStore browser findings", () => {
  ...
  it("atomically removes a finding and its linked image", () => {
    const store = useComposerDraftStore.getState();
    const image = makeImage({ id: "finding-image", previewUrl: "data:image/png;base64,AA==" });
    store.addImage(findingThreadId, image);
    expect(store.addBrowserFinding(findingThreadId, makeFinding(image.id))).toBe(true);
    store.removeBrowserFinding(findingThreadId, `finding-${image.id}`);
    const draft = useComposerDraftStore.getState().draftsByThreadId[findingThreadId];
    expect(draft?.browserFindings ?? []).toEqual([]);
    expect(draft?.images ?? []).toEqual([]);
  });

  it("cascades direct image removal and preserves findings through persistence", () => {
    const store = useComposerDraftStore.getState();
    const image = makeImage({
      id: "persisted-finding-image",
      previewUrl: "data:image/png;base64,AA==",
    });
    store.addImage(findingThreadId, image);
    store.syncPersistedAttachments(findingThreadId, [
      {
        id: image.id,
        name: image.name,
    ...
    const persisted = options.partialize?.(useComposerDraftStore.getState()) as unknown;
    const merged = options.merge?.(
      persisted,
      useComposerDraftStore.getInitialState(),
    ) as ReturnType<typeof useComposerDraftStore.getState>;
    expect(merged.draftsByThreadId[findingThreadId]?.browserFindings).toHaveLength(1);
    store.removeImage(findingThreadId, image.id);
    expect(
      useComposerDraftStore.getState().draftsByThreadId[findingThreadId]?.browserFindings ?? [],
    ).toEqual([]);
  });

  it("persists, hydrates, and restores findings from prompt history", () => {
    const store = useComposerDraftStore.getState();
    const image = makeImage({
      id: "history-finding-image",
      previewUrl: "data:image/png;base64,AA==",
    });
    store.addImage(findingThreadId, image);
    store.syncPersistedAttachments(findingThreadId, [
    ...
    store.setPromptHistorySavedDraft(
      findingThreadId,
      captureComposerPromptHistorySavedDraft({ threadId: findingThreadId, draft, prompt: "" }),
    );
    const options = useComposerDraftStore.persist.getOptions();
    const persisted = options.partialize?.(useComposerDraftStore.getState()) as unknown;
    const merged = options.merge?.(
    ...
    useComposerDraftStore.setState(merged, true);
    useComposerDraftStore.getState().restorePromptHistorySavedDraft(findingThreadId);
    expect(
      useComposerDraftStore.getState().draftsByThreadId[findingThreadId]?.browserFindings,
    ).toHaveLength(1);
  });

  it("filters orphan findings from persisted drafts and queued retries", () => {
    const options = useComposerDraftStore.persist.getOptions();
    const orphan = makeFinding("missing-image");
    const merged = options.merge?.(
      {
        state: {
          draftsByThreadId: {
            [findingThreadId]: {
              browserFindings: [orphan],
              queuedTurns: [{ ...makeQueuedChatTurn("orphan-queue"), browserFindings: [orphan] }],
            },
          },
        },
        version: 6,
    ...
    expect(merged.draftsByThreadId[findingThreadId]?.browserFindings ?? []).toEqual([]);
    const queued = merged.draftsByThreadId[findingThreadId]?.queuedTurns[0];
    expect(queued?.kind === "chat" ? (queued.browserFindings ?? []) : []).toEqual([]);
  });

  it("hydrates valid queued findings and preserves them when a failed turn is reinserted", () => {
    const store = useComposerDraftStore.getState();
    const image = makeImage({
      id: "queued-finding-image",
      previewUrl: "data:image/png;base64,AA==",
    });
    const finding = makeFinding(image.id);
    const queued = {
      ...makeQueuedChatTurn("queued-finding", image),
      browserFindings: [finding],
    };
    store.enqueueQueuedTurn(findingThreadId, queued);
    const options = useComposerDraftStore.persist.getOptions();
    const persisted = options.partialize?.(useComposerDraftStore.getState()) as unknown;
    const merged = options.merge?.(
      persisted,
    ...
    const hydrated = merged.draftsByThreadId[findingThreadId]?.queuedTurns[0];
    expect(hydrated?.kind === "chat" ? hydrated.browserFindings : []).toEqual([finding]);
    expect(hydrated?.kind === "chat" ? hydrated.images.map((entry) => entry.id) : []).toEqual([
      image.id,
    ]);

    useComposerDraftStore.setState(merged, true);
    useComposerDraftStore.getState().removeQueuedTurn(findingThreadId, queued.id);
    useComposerDraftStore.getState().insertQueuedTurn(findingThreadId, queued, 0);
    const retried =
      useComposerDraftStore.getState().draftsByThreadId[findingThreadId]?.queuedTurns[0];
    expect(retried?.kind === "chat" ? retried.browserFindings : []).toEqual([finding]);
    expect(retried?.kind === "chat" ? retried.images[0]?.id : null).toBe(image.id);
  });

  it("transfers and clears findings with their linked images", () => {
    const targetThreadId = ThreadId.makeUnsafe("thread-browser-findings-target");
    const store = useComposerDraftStore.getState();
    store.clearDraftThread(targetThreadId);
    const image = makeImage({ id: "transfer-finding-image", previewUrl: "blob:transfer-finding" });
    store.addImage(findingThreadId, image);
    expect(store.addBrowserFinding(findingThreadId, makeFinding(image.id))).toBe(true);
    store.copyTransferableComposerState(findingThreadId, targetThreadId);
    expect(
      useComposerDraftStore.getState().draftsByThreadId[targetThreadId]?.browserFindings,
    ).toHaveLength(1);
    store.clearComposerContent(targetThreadId);
    const cleared = useComposerDraftStore.getState().draftsByThreadId[targetThreadId];
    expect(cleared?.browserFindings ?? []).toEqual([]);
    expect(cleared?.images ?? []).toEqual([]);
  });

  it("rejects empty edits and enforces the aggregate attachment limit", () => {
    const store = useComposerDraftStore.getState();
    const image = makeImage({ id: "limited-finding-image", previewUrl: "blob:limited-finding" });
    store.addImage(findingThreadId, image);
    expect(store.addBrowserFinding(findingThreadId, makeFinding(image.id))).toBe(true);
    store.updateBrowserFindingComment(findingThreadId, `finding-${image.id}`, "   ");
    expect(
      useComposerDraftStore.getState().draftsByThreadId[findingThreadId]?.browserFindings[0]
        ?.comment,
    ).toBe("Tighten this spacing");

    const files = Array.from({ length: 20 }, (_, index) =>
      makeFile({ id: `limit-file-${index}`, name: `limit-${index}.txt` }),
    );
    useComposerDraftStore.setState((state) => ({
      draftsByThreadId: {
        ...state.draftsByThreadId,
        [findingThreadId]: {
          ...state.draftsByThreadId[findingThreadId]!,
          files,
        },
      },
    }));
    expect(
      useComposerDraftStore
        .getState()
        .addBrowserFinding(findingThreadId, { ...makeFinding(image.id), id: "over-limit" }),
    ).toBe(false);
  });
});
...
describe("composerDraftStore modelSelection", () => {
  ...
  it("falls back deterministically when an OpenCode credential removal omits the persisted model", () => {
    const state = deriveEffectiveComposerModelState({
      draft: {
        modelSelectionByProvider: {},
        activeProvider: "opencode",
...
describe("composerDraftStore queued follow-ups", () => {
  ...
  it("persists queued turns that use accept-edits", () => {
    const store = useComposerDraftStore.getState();
    store.enqueueQueuedTurn(threadId, {
      ...makeQueuedTurn("queued-accept-edits"),
    ...
    expect(mergedState.draftsByThreadId[threadId]?.queuedTurns[0]?.runtimeMode).toBe(
      "accept-edits",
    );
  });
...
describe("composerDraftStore runtime and interaction settings", () => {
  ...
  it("persists and hydrates the accept-edits runtime mode", () => {
    const store = useComposerDraftStore.getState();
    store.setRuntimeMode(threadId, "accept-edits");
    ...
    expect(mergedState.draftsByThreadId[threadId]?.runtimeMode).toBe("accept-edits");
  });

  it("persists and hydrates the auto-approval runtime mode", () => {
    const store = useComposerDraftStore.getState();
    store.setRuntimeMode(threadId, "auto-approval" as never);
    ...
    expect(mergedState.draftsByThreadId[threadId]?.runtimeMode).toBe("auto-approval");
  });
