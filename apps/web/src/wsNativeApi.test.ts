...
describe("wsNativeApi", () => {
  ...
  it("forwards local model requests and events", async () => {
    ...
    const api = createWsNativeApi();
    const listener = vi.fn();
    const unsubscribe = api.localModels.onEvent(listener);

    await api.localModels.getSnapshot();
    await api.localModels.installRuntime({ runtime: "ollama" });
    await api.localModels.installModel({ runtime: "ollama", modelId: "granite4.1:3b" });
    await api.localModels.startSetup({ runtime: "ollama" });
    await api.localModels.retrySetup({ jobId: "setup-1" });
    await api.localModels.cancelSetup({ jobId: "setup-1" });
    emitPush(WS_CHANNELS.localModelEvent, { type: "snapshot.updated", snapshot });

    expect(requestMock).toHaveBeenNthCalledWith(1, WS_METHODS.localModelsGetSnapshot, undefined, {
      timeoutMs: 8_000,
    });
    expect(requestMock).toHaveBeenNthCalledWith(2, WS_METHODS.localModelsInstallRuntime, {
      runtime: "ollama",
    });
    expect(requestMock).toHaveBeenNthCalledWith(3, WS_METHODS.localModelsInstallModel, {
      runtime: "ollama",
      modelId: "granite4.1:3b",
    });
    expect(requestMock).toHaveBeenNthCalledWith(4, WS_METHODS.localModelsStartSetup, {
      runtime: "ollama",
    });
    expect(requestMock).toHaveBeenNthCalledWith(5, WS_METHODS.localModelsRetrySetup, {
      jobId: "setup-1",
    });
    expect(requestMock).toHaveBeenNthCalledWith(6, WS_METHODS.localModelsCancelSetup, {
      jobId: "setup-1",
    });
    expect(listener).toHaveBeenCalledWith({ type: "snapshot.updated", snapshot });
