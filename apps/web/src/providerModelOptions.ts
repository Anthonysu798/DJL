  ...
  for (const dynamicModel of input.dynamicModels) {
    ...
    normalizedDynamicOptions.push({
      ...
      ...(dynamicModel.upstreamProviderName?.trim()
        ...
        : {}),
      ...(dynamicModel.processingLocality
        ? { processingLocality: dynamicModel.processingLocality }
        : {}),
      ...(typeof dynamicModel.supportsVision === "boolean"
        ? { supportsVision: dynamicModel.supportsVision }
        : {}),
...
export function groupProviderModelOptionsWithFavorites(input: {
  ...
  return [
    {
      key: "__favorites__",
      label: input.favoriteLabel ?? translateRendererCopy("chat:models.favourites", "Favourites"),
