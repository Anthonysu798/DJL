...
import {
  AppLocalePreference,
  type AppLocalePreference as AppLocalePreferenceType,
...
export const DEFAULT_APP_LANGUAGE: AppLocalePreferenceType = "system";
...
export const AppSettingsSchema = Schema.Struct({
  language: Schema.Unknown.pipe(
    Schema.decodeTo(AppLocalePreference, {
      decode: SchemaGetter.transform(normalizeAppLocalePreference),
      encode: SchemaGetter.transform((value) => value),
    }),
    withDefaults(() => DEFAULT_APP_LANGUAGE),
  ),
  ...
  kimiRegion: Schema.Literals(["existing", "global", "mainland-cn"]).pipe(
...
function appSettingsPatchToServerSettingsPatch(patch: Partial<AppSettings>): ServerSettingsPatch {
  ...
  if (
    hasOwn(patch, "kimiBinaryPath") ||
    hasOwn(patch, "customKimiModels") ||
    hasOwn(patch, "kimiRegion")
  ) {
    providers.kimi = {
      ...(hasOwn(patch, "kimiRegion") ? { region: patch.kimiRegion ?? "existing" } : {}),
