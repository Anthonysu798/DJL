...
const workDocumentRenderEventListeners = new Set<(payload: DocumentRenderEvent) => void>();
...
export function createWsNativeApi(): NativeApi {
  ...
  transport.subscribe(WS_CHANNELS.workDocumentRenderEvent, (message) => {
    const payload = message.data;
    for (const listener of workDocumentRenderEventListeners) {
      try {
        listener(payload);
      } catch {
        // Swallow listener errors
  ...
  transport.subscribe(WS_CHANNELS.localModelEvent, (message) => {
    const payload = message.data;
    for (const listener of localModelEventListeners) {
      try {
        listener(payload);
      } catch {
        // Swallow listener errors
  ...
  transport.subscribe(WS_CHANNELS.aiDetectorEvent, (message) => {
    ...
    for (const listener of aiDetectorEventListeners) {
      try {
        ...
        // Listener failures must not interrupt transport projection.
      }
    }
  });
  ...
  const api: NativeApi = {
    ...
    localModels: {
      getSnapshot: () =>
        transport.request(WS_METHODS.localModelsGetSnapshot, undefined, { timeoutMs: 8_000 }),
      refresh: () =>
        transport.request(WS_METHODS.localModelsRefresh, undefined, { timeoutMs: 10_000 }),
      installRuntime: (input) => transport.request(WS_METHODS.localModelsInstallRuntime, input),
      startRuntime: (input) => transport.request(WS_METHODS.localModelsStartRuntime, input),
      installModel: (input) => transport.request(WS_METHODS.localModelsInstallModel, input),
      cancelInstall: (input) => transport.request(WS_METHODS.localModelsCancelInstall, input),
    ...
    aiDetector: {
      ...
      installModel: (input) =>
        transport.request(WS_METHODS.aiDetectorInstallModel, input, { timeoutMs: null }),
      cancelInstall: (input) => transport.request(WS_METHODS.aiDetectorCancelInstall, input),
      removeModel: (input) => transport.request(WS_METHODS.aiDetectorRemoveModel, input),
      clearCache: () => transport.request(WS_METHODS.aiDetectorClearCache),
      onEvent: (callback) => {
        aiDetectorEventListeners.add(callback);
        return () => aiDetectorEventListeners.delete(callback);
      },
    },
    work: {
      listPreparedDocuments: (input) =>
        transport.request(WS_METHODS.workListPreparedDocuments, input),
      previewDocument: (input) => transport.request(WS_METHODS.workPreviewDocument, input),
      resolveArtifactPath: (input) => transport.request(WS_METHODS.workResolveArtifactPath, input),
      documentIntelligence: {
        status: () => transport.request(WS_METHODS.workDocumentIntelligenceStatus),
        install: () => transport.request(WS_METHODS.workDocumentIntelligenceInstall),
        repair: () => transport.request(WS_METHODS.workDocumentIntelligenceRepair),
        uninstall: () => transport.request(WS_METHODS.workDocumentIntelligenceUninstall),
      },
      documentRenderer: {
        status: () => transport.request(WS_METHODS.workDocumentRendererStatus),
        install: () => transport.request(WS_METHODS.workDocumentRendererInstall),
        repair: () => transport.request(WS_METHODS.workDocumentRendererRepair),
        uninstall: () => transport.request(WS_METHODS.workDocumentRendererUninstall),
        onEvent: (callback) => {
          workDocumentRenderEventListeners.add(callback);
          return () => workDocumentRenderEventListeners.delete(callback);
        },
      },
      requestDocumentRender: (input) =>
        transport.request(WS_METHODS.workRequestDocumentRender, input),
      getDocumentRender: (input) => transport.request(WS_METHODS.workGetDocumentRender, input),
      cancelDocumentRender: (input) =>
        transport.request(WS_METHODS.workCancelDocumentRender, input),
    },
    shell: {
      ...
      openPath: async (path) => {
        if (window.desktopBridge?.openPath) {
          await window.desktopBridge.openPath(path);
          return;
        }
    ...
    provider: {
      ...
      listModels: (input) =>
        transport.request(WS_METHODS.providerListModels, input, {
          timeoutMs: input.provider === "opencode" ? 25_000 : 15_000,
        }),
      listAgents: (input) => transport.request(WS_METHODS.providerListAgents, input),
      listModelProviders: (input) =>
        transport.request(WS_METHODS.openCodeListModelProviders, input, { timeoutMs: 25_000 }),
      setApiKey: (input) => transport.request(WS_METHODS.openCodeSetApiKey, input),
      removeCredential: (input) => transport.request(WS_METHODS.openCodeRemoveCredential, input),
    },
    orchestration: {
    ...
    browser: {
      ...
      annotationCommand: async (input) => {
        if (window.desktopBridge) {
          await window.desktopBridge.browser.annotationCommand(input);
          return;
        }
        throw new Error("Browser annotations require the desktop app.");
      },
      captureAnnotation: async (input) => {
        if (window.desktopBridge) {
          return window.desktopBridge.browser.captureAnnotation(input);
        }
        throw new Error("Browser annotations require the desktop app.");
      },
      ...
      onAnnotationEvent: (callback) => {
        if (window.desktopBridge) {
          return window.desktopBridge.browser.onAnnotationEvent(callback);
        }
        return () => {};
      },
    },
