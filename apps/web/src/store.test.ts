...
describe("store pure functions", () => {
  ...
  it("projects Work task transitions into thread and sidebar state", () => {
    ...
    const next = applyOrchestrationEvents(
      ...
      [
        makeDomainEvent("thread.work-task-transitioned", {
          threadId,
          action: "start_work",
          task: {
            threadId,
            ...
            resumePhase: "working",
            progress: 35,
            statusReason: "Preparing the documents",
            lastTransitionCommandId: CommandId.makeUnsafe("work:event-tool-1:start_work"),
            createdAt: "2026-02-27T00:00:00.000Z",
            updatedAt: "2026-02-27T00:01:00.000Z",
            completedAt: null,
          },
    ...
    expect(next.threads[0]?.workTask).toMatchObject({
      status: "working",
      progress: 35,
      statusReason: "Preparing the documents",
    });
    expect(next.sidebarThreadSummaryById[threadId]?.workTask).toBe(next.threads[0]?.workTask);
  });
