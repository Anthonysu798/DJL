...
import { Cause, Data, Effect, Exit, Layer, ManagedRuntime, Option, Scope, Stream } from "effect";
...
export class WsTransport {
  ...
  constructor(url?: string) {
    ...
    const session = this.createSession(++this.sessionVersion);
  ...
  async request<T = unknown>(
    ...
    return (await this.runtime.runPromise(requestEffect)) as T;
  }
  ...
  dispose(): Promise<void> {
    ...
    this.disposed = true;
    // Invalidate every active stream before cancellation so their interruption
    // callbacks cannot start a replacement transport during shutdown.
    ...
    this.disposePromise = runtime
      .runPromise(Scope.close(clientScope, Exit.void))
      .catch(() => undefined)
      // React cleanup and stream interruption callbacks can enqueue one final
      // request. Let that queue drain before releasing the Effect runtime.
      .then(() => new Promise<void>((resolve) => globalThis.setTimeout(resolve, 50)))
      .then(() => runtime.dispose().catch(() => undefined));
    return this.disposePromise;
  }

  private createSession(sessionVersion: number) {
    ...
    // A transport can be disposed before its first consumer attaches while a
    // connection is still being established. Keep the rejected promise usable
    // by callers, but mark it handled immediately so teardown cannot surface a
    // late `ManagedRuntime disposed` unhandled rejection.
  ...
  private reconnect(): Promise<RpcClientInstance> {
    ...
    // Bump the generation before cancelling sibling streams. Their exits belong
    // to the old socket and must not recursively enqueue more reconnects.
    const reconnectSessionVersion = ++this.sessionVersion;
    ...
    this.reconnectPromise = this.openReconnectSession(reconnectSessionVersion).finally(() => {
  ...
  private async openReconnectSession(sessionVersion: number): Promise<RpcClientInstance> {
    while (!this.disposed && sessionVersion === this.sessionVersion) {
      const delayMs = Math.min(500 * 2 ** this.reconnectFailures, 5_000);
      this.reconnectFailures += 1;
      await new Promise((resolve) => window.setTimeout(resolve, delayMs));

      if (this.disposed || sessionVersion !== this.sessionVersion) {
        break;
      }

      this.setState("connecting");
      const session = this.createSession(sessionVersion);
      this.runtime = session.runtime;
      this.clientScope = session.clientScope;
      this.clientPromise = session.clientPromise;
    ...
    throw new Error("Transport reconnect superseded");
  }
  ...
  private startChannelStream(channel: WsPushChannel): void {
    void this.getClient()
      .then((client) => {
        ...
        } else if (channel === WS_CHANNELS.workDocumentRenderEvent) {
          this.startStream(
            "work.documentRenders",
            client[WS_METHODS.subscribeWorkDocumentRenderEvents]({}),
            (event: DocumentRenderEvent) => this.emit(WS_CHANNELS.workDocumentRenderEvent, event),
            restartChannel,
          );
        } else if (channel === WS_CHANNELS.localModelEvent) {
          this.startStream(
            "localModels.events",
            client[WS_METHODS.subscribeLocalModelEvents]({}),
            (event: LocalModelEvent) => this.emit(WS_CHANNELS.localModelEvent, event),
            restartChannel,
          );
        } else if (channel === WS_CHANNELS.aiDetectorEvent) {
          this.startStream(
            "aiDetector.events",
            client[WS_METHODS.subscribeAiDetectorEvents]({}),
            (event: AiDetectorEvent) => this.emit(WS_CHANNELS.aiDetectorEvent, event),
            restartChannel,
          );
  ...
  private stopChannelStream(channel: WsPushChannel): void {
    if (isServerLifecyclePushChannel(channel)) {
      ...
      this.stopStream("work.documentRenders");
  ...
  private startStream<T>(
    ...
    options: { readonly reconnectOnCleanEnd?: boolean } = {},
  ): void {
    ...
    let synchronousExit: Exit.Exit<void, WsTransportRpcError> | undefined;
    const handleExit = (exit: Exit.Exit<void, WsTransportRpcError>, cleanup: () => void) => {
      // A stream may be intentionally replaced before its cancellation callback
      // runs (thread subscriptions do this during route/snapshot hydration). Only
      // the currently registered stream instance may affect transport lifecycle.
      // Key-only stop markers are insufficient because the replacement reuses the
      // same key while the old stream's interrupted exit is still queued.
      if (this.streamCleanups.get(key) !== cleanup) return;
      this.streamCleanups.delete(key);
      if (streamSessionVersion !== this.sessionVersion) {
        return;
      }
      if (this.disposed) {
        return;
      ...
      if (restart) {
        // Draft thread routes subscribe before the server-side thread exists.
        // That RPC intentionally completes without a snapshot; the shell stream
        // requests it again as soon as thread creation is projected. Treating
        // this expected clean end as a socket failure destroys unrelated RPCs,
        // including the first send request for the draft.
        if (!Exit.isFailure(exit) && options.reconnectOnCleanEnd === false) {
          return;
        }
    ...
    cancel = this.runtime.runCallback(
      Stream.runForEach(runnableStream, (event) => Effect.sync(() => listener(event))),
      {
        onExit: (exit) => {
          if (!cancel) {
            synchronousExit = exit;
            return;
          }
          handleExit(exit, cancel);
        },
      },
    );
    this.streamCleanups.set(key, cancel);
    if (synchronousExit) {
      handleExit(synchronousExit, cancel);
    }
  }
