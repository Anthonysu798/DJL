...
import { showDesktopThreadNotification } from "./taskCompletion";
...
describe("collectInputNeededThreadCandidates", () => {
  it("preserves approval detail through collection, localized copy, and the desktop bridge", async () => {
    ...
    const next = [
      makeThread({
        activities: [
          {
            id: EventId.makeUnsafe("activity-approval-detail"),
            tone: "approval",
            kind: "approval.requested",
            summary: "Command approval requested",
            payload: {
              requestId: "approval-request-detail",
              requestKind: "command",
              detail: "bun run focused -- --raw-provider-arg",
            },
            turnId: TurnId.makeUnsafe("turn-1"),
    ...
    const [candidate] = collectInputNeededThreadCandidates(previous, next);
    expect(candidate?.summary).toBe("bun run focused -- --raw-provider-arg");
    const copy = buildInputNeededCopy(candidate!, i18n.t);
    expect(copy).toEqual({
      title: "Approbation requise",
      body: "Polish notifications : bun run focused -- --raw-provider-arg",
    });
    const sent: unknown[] = [];
