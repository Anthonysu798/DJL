...
function statusPill(
  ...
  switch (status) {
    case "needs-auth":
      return {
        label: t("usage.status.notSignedIn", { ns: "settings" }),
        className: "bg-amber-500/12 text-amber-600 dark:text-amber-400",
      };
    case "unsupported":
      return {
        label: t("usage.status.unsupported", { ns: "settings" }),
        className: "bg-muted text-muted-foreground",
      };
    case "error":
      return {
        label: t("usage.status.unavailable", { ns: "settings" }),
      ...
      <div className="space-y-3.5 p-4">
        ...
        {status === "ok" && hasUsage ? (
          ...
          <p className="text-xs leading-relaxed text-muted-foreground">
            {status === "ok"
              ? t("usage.noData")
              : t("usage.unavailableWithDetail", {
                  detail: snapshot.detail ?? providerUsageNeedsAuthDetail(provider),
                })}
          </p>
        )}
      </div>
...
export function ProviderUsageSettingsPanel() {
  ...
  return (
    <section className={SETTINGS_PANEL_SECTION_CLASS_NAME}>
      <div className="flex items-center justify-between gap-2">
        <h2 className={SETTINGS_SECTION_LABEL_CLASS_NAME}>{t("usage.title")}</h2>
        <Button
          size="xs"
          variant="outline"
          className="shrink-0"
      ...
      {showInitialLoading ? (
        <SettingsCard>
          <div className="px-4 py-3.5 text-xs text-muted-foreground">{t("usage.loading")}</div>
        </SettingsCard>
      ) : (
