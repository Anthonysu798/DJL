...
export function skillOriginInfo(scope: string | undefined, t?: TFunction): SkillOriginInfo {
  const text = (key: "shared" | "project" | "personal") =>
    t?.(`skills.origins.${key}`, { ns: "settings" }) ?? englishCatalog.settings.skills.origins[key];
  switch (scope) {
    case "synara":
      return { label: "DJL", provider: null };
    case "codex":
    ...
    case "agents":
      return { label: text("shared"), provider: null };
    case "project":
      return { label: text("project"), provider: null };
    default:
      return { label: scope ?? text("personal"), provider: null };
  }
}
...
function sectionTitle(section: string, t?: TFunction): string {
  if (section === SHARED_SKILLS_SECTION) {
    return (
      t?.("skills.sections.shared", { ns: "settings" }) ??
      englishCatalog.settings.skills.sections.shared
    );
  }
  const origin = skillOriginInfo(section, t).label;
  return (
    t?.("skills.sections.from", { ns: "settings", origin }) ??
    englishCatalog.settings.skills.sections.from.replace("{{origin}}", origin)
  );
}
...
export function buildSettingsSkillGroups(
  ...
  for (const skill of skills) {
    ...
    const source: SettingsSkillSource = {
      skill,
      origin,
      originInfo: skillOriginInfo(origin, t),
    };
  ...
  return [...groups.entries()]
    .map(([key, unsortedSources]): SettingsSkillGroup | null => {
      ...
      const description =
        ...
        englishCatalog.settings.skills.noDescription;
      return {
        key,
...
export function buildSettingsSkillSections(
  ...
  for (const group of buildSettingsSkillGroups(skills, t)) {
