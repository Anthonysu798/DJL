    ...
    <div className="space-y-4">
      <div className="relative w-full">
        <Input
          ...
          variant="soft"
          nativeInput
          placeholder={settingsT("shortcuts.searchPlaceholder")}
          value={query}
          aria-label={settingsT("shortcuts.searchAriaLabel")}
      ...
      {filteredSections.length > 0 ? (
        ...
        >
          <div className="flex items-center justify-between gap-4 px-3 py-2 text-[11px] font-medium text-muted-foreground">
            <span>{settingsT("shortcuts.commandColumn")}</span>
            <span>{settingsT("shortcuts.keybindingColumn")}</span>
          </div>
        ...
        >
          {settingsT("shortcuts.empty", { query })}
        </div>
      )}
    </div>
  );
