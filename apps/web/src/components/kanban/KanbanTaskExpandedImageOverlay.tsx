...
export function KanbanTaskExpandedImageOverlay({
  ...
  return (
    <div
      ...
      role="dialog"
      aria-modal="true"
      aria-label={t("kanban.imagePreview.dialogLabel")}
    >
      <button
        type="button"
        className="absolute inset-0 z-0 cursor-zoom-out"
        aria-label={t("kanban.imagePreview.close")}
        onClick={onClose}
      />
      {expandedImage.images.length > 1 ? (
        <Button
          ...
          aria-label={t("kanban.imagePreview.previous")}
      ...
      {expandedImage.images.length > 1 ? (
        <Button
          ...
          aria-label={t("kanban.imagePreview.next")}
          onClick={() => onNavigate(1)}
        >
