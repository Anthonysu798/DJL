...
describe("command-center status derivation", () => {
  it("prioritizes failures, waiting requests, review, running, then done", () => {
    const failed = buildKanbanBoard(
      makeBoardInput({
        threads: [
          makeSidebarThreadSummary({
            latestTurn: makeLatestTurn({ state: "error" }),
            session: makeSession({ status: "error", lastError: "spawn codex ENOENT" }),
            hasPendingUserInput: true,
          }),
        ],
      }),
    ).projects[0]!.done[0]!;
    const waiting = buildKanbanBoard(
      makeBoardInput({
        threads: [
          makeSidebarThreadSummary({
            latestTurn: makeLatestTurn({ state: "running", completedAt: null }),
    ...
    ).projects[0]!.inProgress[0]!;
    const review = buildKanbanBoard(
      makeBoardInput({
        threads: [
          makeSidebarThreadSummary({
            latestTurn: makeLatestTurn(),
    ...
    const running = buildKanbanBoard(
      makeBoardInput({ threads: [makeSidebarThreadSummary({ hasLiveTailWork: true })] }),
    ).projects[0]!.inProgress[0]!;
    const done = buildKanbanBoard(
      makeBoardInput({ threads: [makeSidebarThreadSummary({ latestTurn: makeLatestTurn() })] }),
    ).projects[0]!.done[0]!;

    expect(deriveKanbanCommandStatus(failed)).toBe("failed");
    expect(deriveKanbanCommandStatus(waiting)).toBe("waiting");
    expect(deriveKanbanCommandStatus(review)).toBe("reviewReady");
    expect(deriveKanbanCommandStatus(running)).toBe("running");
    expect(deriveKanbanCommandStatus(done)).toBe("done");
  });

  it("builds priority-ordered lanes across projects with counts and recency ordering", () => {
    const firstProjectId = ProjectId.makeUnsafe("project-1");
    const secondProjectId = ProjectId.makeUnsafe("project-2");
    const board = buildKanbanBoard(
      makeBoardInput({
        projects: [
          { id: firstProjectId, kind: "project", name: "Alpha" },
          { id: secondProjectId, kind: "project", name: "Beta" },
        ],
        threads: [
          makeSidebarThreadSummary({
            id: ThreadId.makeUnsafe("waiting-old"),
          ...
          makeSidebarThreadSummary({
            id: ThreadId.makeUnsafe("waiting-new"),
        ...
        composerDraftByThreadId: {
          "draft-1": { prompt: "Ship it", hasAttachments: false, provider: "codex" },
        },
      }),
    );

    const commandCenter = buildKanbanCommandCenter(board);
    expect(commandCenter.lanes.map((lane) => lane.status)).toEqual([
      "waiting",
      "failed",
      "running",
      "reviewReady",
    ...
    expect(commandCenter.lanes[0]?.items.map((item) => item.card.threadId)).toEqual([
      "waiting-new",
      "waiting-old",
    ]);
    expect(commandCenter.lanes[0]?.items.map((item) => item.projectName)).toEqual([
      "Beta",
      "Alpha",
    ]);
    expect(commandCenter.draftCount).toBe(1);
    expect(commandCenter.totalCount).toBe(2);
  });
});
