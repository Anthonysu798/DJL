  ...
  useEffect(() => {
    ...
    for (const [threadId, entry] of entries) {
      ...
      if (outcome === "failed") {
        toastManager.add({
          type: "error",
          title: t("kanban.dispatch.taskDidNotStart"),
          description:
            thread.session?.lastError ?? t("kanban.dispatch.movedBack", { title: entry.title }),
        });
      }
    }
    ...
    const intervalId = window.setInterval(() => {
      ...
      for (const [threadId, entry] of expired) {
        ...
        toastManager.add({
          type: "error",
          title: t("kanban.dispatch.taskDidNotStart"),
          description: t("kanban.dispatch.movedBack", { title: entry.title }),
        });
      }
