  ...
  const handleDispatchDrop = useCallback(
    async (card: KanbanCard) => {
      ...
      if (result.kind === "dispatched") {
        toastManager.add({
          type: "success",
          title: t("kanban.dispatch.draftSent"),
      ...
      if (result.kind === "open-thread") {
        const description =
          result.reason === "empty"
            ? t("kanban.dispatch.empty")
            : result.reason === "worktree-pending"
              ? t("kanban.dispatch.worktreePending")
              : t("kanban.dispatch.continueInChat");
        toastManager.add({
          type: "info",
          title: t("kanban.dispatch.finishInChat"),
          description,
        });
        onOpenCard(card);
        return;
      ...
      if (result.kind === "unavailable") {
        toastManager.add({
          type: "error",
          title: t("kanban.errors.notConnected"),
          description: t("kanban.dispatch.reconnect"),
        });
        return;
      }
      toastManager.add({
        type: "error",
        title: t("kanban.dispatch.failed"),
  ...
  const handleDragEnd = useCallback(
    (event: DragEndEvent) => {
      ...
      if (targetColumn === "done") {
        toastManager.add({
          type: "info",
          title: t("kanban.derivedDone.title"),
          description: t("kanban.derivedDone.description"),
        });
      }
    },
