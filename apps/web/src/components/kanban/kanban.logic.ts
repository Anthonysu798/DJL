...
function kanbanFallbackDraftTitle(): string {
  return translateRendererCopy("work:kanban.card.newThread", "New thread");
}
...
/** Attention-oriented status used by the all-project command center. */
export type KanbanCommandStatus = "waiting" | "failed" | "running" | "reviewReady" | "done";

export const KANBAN_COMMAND_STATUS_ORDER: readonly KanbanCommandStatus[] = [
  "waiting",
  "failed",
  "running",
  "reviewReady",
...
/** Home/Work pulse leads with live activity before attention and history. */
export const KANBAN_COMMAND_SUMMARY_STATUS_ORDER: readonly KanbanCommandStatus[] = [
  "running",
  "waiting",
  "failed",
  "reviewReady",
...
export interface KanbanCommandCenterItem {
  card: KanbanCard;
  projectId: ProjectId;
  projectName: string;
...
export interface KanbanCommandCenterLane {
  status: KanbanCommandStatus;
  items: KanbanCommandCenterItem[];
}

export interface KanbanCommandCenter {
  lanes: KanbanCommandCenterLane[];
  draftCount: number;
  totalCount: number;
}
    ...
    title:
      composerDraft.prompt.length > 0
        ...
        : composerDraft.hasAttachments
          ? translateRendererCopy("work:kanban.card.attachedReferences", "Attached references")
          : kanbanFallbackDraftTitle(),
...
function forceOptimisticInProgressCard(
  ...
  return {
    ...
    title:
      card.title === kanbanFallbackDraftTitle() && entry.title.length > 0
        ? entry.title
        : card.title,
    draftPrompt: "",
...
/**
 * Maps workflow columns onto the user-facing command-center states. Failure wins
 * over stale pending requests because a dead provider cannot accept an answer;
 * actionable requests then win over general running work, followed by review.
 */
export function deriveKanbanCommandStatus(card: KanbanCard): KanbanCommandStatus | null {
  if (card.column === "draft") {
    return null;
  }
  if (card.isOptimisticDispatch) {
    return "running";
  }
  const thread = card.thread;
  if (!thread) {
    return card.column === "inProgress" ? "running" : "done";
  }
  if (
    thread.workTask?.status === "failed" ||
  ...
  if (
    thread.workTask?.status === "needs_input" ||
    ((thread.hasPendingApprovals || thread.hasPendingUserInput) && canAnswer)
  ) {
    return "waiting";
  }
  if (thread.workTask?.status === "needs_review" || thread.hasActionableProposedPlan) {
    return "reviewReady";
  }
  if (card.column === "inProgress") {
    return "running";
...
/** Builds the cross-project priority lanes without introducing another client store. */
export function buildKanbanCommandCenter(board: KanbanBoard): KanbanCommandCenter {
  const itemsByStatus = new Map<KanbanCommandStatus, KanbanCommandCenterItem[]>(
    KANBAN_COMMAND_STATUS_ORDER.map((status) => [status, []]),
  );
  let draftCount = 0;

  for (const project of board.projects) {
    draftCount += project.draft.length;
    for (const card of [...project.inProgress, ...project.done]) {
      const status = deriveKanbanCommandStatus(card);
      if (!status) {
        continue;
      }
      itemsByStatus.get(status)?.push({
        card,
        projectId: project.projectId,
        projectName: project.projectName,
        status,
        technicalDetail:
          card.thread?.session?.lastError?.trim() ||
          (status === "failed" ? card.thread?.workTask?.statusReason?.trim() : null) ||
          null,
      });
    }
  }

  const lanes = KANBAN_COMMAND_STATUS_ORDER.map((status) => ({
    status,
    items: (itemsByStatus.get(status) ?? []).toSorted((left, right) =>
      compareByRecencyDesc(left.card, right.card),
    ),
  }));
  return {
    lanes,
