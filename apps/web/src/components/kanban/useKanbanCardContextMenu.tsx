  ...
  const archiveCardThread = useCallback(
    async (threadId: ThreadId) => {
      ...
      if (isThreadRunningTurn(thread)) {
        toastManager.add({
          type: "error",
          title: t("kanban.contextMenu.cannotArchive", { ns: "work" }),
          description: t("kanban.contextMenu.stopBeforeArchive", { ns: "work" }),
        });
        return;
      }
    ...
    async (card: KanbanCard) => {
      ...
      const shouldDeleteWorktree =
        ...
        (await api.dialogs.confirm(
          [
            t("kanban.contextMenu.onlyWorktree", { ns: "work" }),
            displayWorktreePath ?? orphanedWorktreePath,
            "",
            t("kanban.contextMenu.deleteWorktree", { ns: "work" }),
          ].join("\n"),
        ));
      ...
      try {
        ...
        toastManager.add({
          type: "error",
          title: t("kanban.contextMenu.worktreeRemovalFailed", { ns: "work" }),
          description: t("kanban.contextMenu.worktreeRemovalDetail", {
            ns: "work",
            path: displayWorktreePath ?? orphanedWorktreePath,
            detail:
              error instanceof Error ? error.message : t("kanban.errors.unknown", { ns: "work" }),
          }),
        });
      }
    },
      ...
      void (async () => {
        const clicked = await api.contextMenu.show(
          [
            ...(isThreadActionCard
              ? [
                  { id: "rename", label: t("kanban.contextMenu.rename", { ns: "work" }) },
                  {
                    id: "toggle-pin",
                    label: t(
                      card.thread?.isPinned ? "kanban.contextMenu.unpin" : "kanban.contextMenu.pin",
                      { ns: "work" },
                    ),
                  },
                ]
            ...
            ...(workspacePath
              ? [
                  {
                    id: "copy-path",
                    label: t("kanban.contextMenu.copyPath", { ns: "work" }),
            ...
            ...(isThreadBacked
              ? [
                  {
                    id: "copy-thread-id",
                    label: t("kanban.contextMenu.copyThreadId", { ns: "work" }),
                  },
                ]
              : []),
            ...(isThreadActionCard
              ...
              : []),
            {
              ...
              label: deletesOnlyDraft
                ? t("kanban.contextMenu.deleteDraft", { ns: "work" })
        ...
        if (clicked === "toggle-pin" && isThreadActionCard && card.thread) {
          ...
          void setThreadPinned(card.threadId, next).catch(() => {
            toastManager.add({
              type: "error",
              title: t(next ? "kanban.contextMenu.pinFailed" : "kanban.contextMenu.unpinFailed", {
                ns: "work",
              }),
            });
          });
        ...
        if (clicked === "archive") {
          ...
          if (settings.confirmThreadArchive) {
            const confirmed = await api.dialogs.confirm(
              [
                t("kanban.contextMenu.archiveConfirm", { ns: "work", title: card.title }),
                t("kanban.contextMenu.archiveDescription", { ns: "work" }),
              ].join("\n"),
            );
            if (!confirmed) return;
          }
        ...
        if (settings.confirmThreadDelete) {
          const confirmed = await api.dialogs.confirm(
            deletesOnlyDraft
              ? t("kanban.contextMenu.deleteDraftConfirm", { ns: "work" })
              : [
                  t("kanban.contextMenu.deleteThreadConfirm", {
                    ns: "work",
                    title: card.title,
                  }),
                  t("kanban.contextMenu.deleteThreadDescription", { ns: "work" }),
                ].join("\n"),
          );
          if (!confirmed) return;
        }
      ...
      <RenameThreadDialog
        ...
        onSave={async (newTitle) => {
          ...
          if (outcome === "unavailable") {
            toastManager.add({
              type: "error",
              title: t("kanban.errors.notConnected", { ns: "work" }),
              description: t("kanban.contextMenu.reconnectRename", { ns: "work" }),
            });
            return;
          }
