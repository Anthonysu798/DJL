...
export function deriveAgentActivityTimelineState(
  ...
  reasoningTraceLabel: string,
  formatUpdateCount: (count: number, preview: string | null) => string,
  ...
  const flushReasoningEntries = () => {
    ...
    const displayPreview =
      updateCount > 1 ? formatUpdateCount(updateCount, latestPreview) : latestPreview;
    const displayEntry: WorkLogEntry = {
      ...latest,
      id: groupId,
      label: reasoningTraceLabel,
      toolTitle: reasoningTraceLabel,
      tone: "tool",
