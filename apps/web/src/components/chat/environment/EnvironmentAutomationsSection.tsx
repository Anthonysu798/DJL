...
export function EnvironmentAutomationsSection({
  ...
  return (
    <div className="flex flex-col gap-0.5">
      <EnvironmentSectionLabel>{t("environment.automations")}</EnvironmentSectionLabel>
      {automations.map(({ definition }) => {
        const cadence = definition.enabled
          ? formatCadence(definition.schedule, locale, timestampFormat)
          : t("environment.paused");
        return (
          <EnvironmentRow
            key={definition.id}
            icon={
            ...
            aria-label={t("environment.editAutomation", { name: definition.name })}
