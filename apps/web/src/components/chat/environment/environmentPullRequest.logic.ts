...
export function summarizePullRequestChecks(
  ...
  if (failing > 0) {
    return { labelKey: "environment.pullRequest.failingChecks", count: failing, tone: "failure" };
  }
  const cancelled = checks.filter((check) => check.status === "cancelled").length;
  if (cancelled > 0) {
    return {
      labelKey: "environment.pullRequest.cancelledChecks",
      count: cancelled,
      tone: "failure",
    };
  }
  ...
  if (pending > 0) {
    return { labelKey: "environment.pullRequest.pendingChecks", count: pending, tone: "pending" };
  }
  if (checks.length === 0) {
    return { labelKey: "environment.pullRequest.noChecks", tone: "none" };
  }
  const successful = checks.filter((check) => check.status === "success").length;
  if (successful === 0) {
    return { labelKey: "environment.pullRequest.noRequiredChecks", tone: "none" };
  }
  return { labelKey: "environment.pullRequest.allChecksPassed", tone: "success" };
}

export const PULL_REQUEST_CHECK_STATUS_KEYS: Record<GitPullRequestCheck["status"], string> = {
  pending: "environment.pullRequest.status.running",
  success: "environment.pullRequest.status.succeeded",
  failure: "environment.pullRequest.status.failed",
  skipped: "environment.pullRequest.status.skipped",
  neutral: "environment.pullRequest.status.neutral",
  cancelled: "environment.pullRequest.status.cancelled",
};
...
export function summarizePullRequestComments(count: number, truncated = false) {
  if (count === 0) {
    return {
      key: truncated
        ? "environment.pullRequest.commentsMayExist"
        : "environment.pullRequest.noComments",
      count,
    };
  }
  return {
    key: truncated
      ? "environment.pullRequest.commentCountTruncated"
      : "environment.pullRequest.commentCount",
    count,
  };
}
...
export function describePullRequestComment(
  ...
  const first = lines[0];
  if (!first) {
    return { title: emptyCommentLabel, snippet: null };
  }
...
export function buildFixReviewCommentsPrompt(input: {
  ...
  copy: {
    commentHeading: (comment: GitPullRequestComment) => string;
    intro: (prNumber: number, prUrl: string) => string;
    warning: string;
    more: (prUrl: string) => string;
  };
}): string {
  ...
  const items = included.map((comment, index) => {
    ...
    return `${index + 1}. ${input.copy.commentHeading(comment)}:\n> ${body.replace(/\n/g, "\n> ")}`;
  });
  const hasMore = input.commentsTruncated === true || input.comments.length > included.length;
  const footer = hasMore ? [input.copy.more(input.prUrl)] : [];
  return [
    input.copy.intro(input.prNumber, input.prUrl),
    input.copy.warning,
    ...items,
    ...footer,
  ].join("\n\n");
