...
export function EnvironmentPinnedSection({
  ...
  return (
    <EnvironmentCollapsibleSection label={t("environment.pinned.title")}>
  ...
  return (
    <li className="group/pin flex items-center gap-1.5 rounded-lg px-2 py-1 hover:bg-[var(--color-background-elevated-secondary)]">
      <Checkbox
        ...
        aria-label={pin.done ? t("environment.items.markNotDone") : t("environment.items.markDone")}
      />
      {editing ? (
        <input
          ref={inputRef}
          ...
          placeholder={available ? "" : t("environment.items.label")}
        ...
        <button
          ...
          aria-label={
            available ? t("environment.pinned.jumpAria") : t("environment.pinned.unavailableAria")
          }
          title={available ? t("environment.pinned.jumpHint") : t("environment.pinned.renameHint")}
          className={cn(
