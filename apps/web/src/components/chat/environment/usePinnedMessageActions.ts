...
export function usePinnedMessageActions({
  ...
  const handlePinnedMessageDispatchError = useCallback(
    (error: unknown) => {
      toastManager.add({
        type: "error",
        title: t("environment.pinned.updateFailed"),
        description:
          error instanceof Error ? error.message : t("environment.pinned.updateFailedFallback"),
      });
    },
    [t],
  );
  ...
  const handleThreadNotesDispatchError = useCallback(
    (error: unknown) => {
      toastManager.add({
        type: "error",
        title: t("environment.notes.saveFailed"),
        description:
          error instanceof Error ? error.message : t("environment.notes.saveFailedFallback"),
      });
    },
    [t],
  );
  ...
  const handleTogglePinMessage = useCallback(
    (messageId: MessageId) => {
      ...
      if (pins.length >= PINNED_MESSAGES_MAX_COUNT) {
        toastManager.add({
          type: "warning",
          title: t("environment.pinned.limitReached"),
          description: t("environment.pinned.limitDescription", {
    ...
    [handlePinnedMessageDispatchError, t],
  );
