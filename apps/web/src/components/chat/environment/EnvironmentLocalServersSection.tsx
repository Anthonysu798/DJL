...
function LocalServersRefreshButton({
  ...
  return (
    <MenuItem
      ...
      disabled={refreshing}
      onClick={onRefresh}
      aria-label={t("environment.localServers.refresh")}
      title={t("environment.localServers.refreshShort")}
...
function LocalServerRow({
  ...
  const stopHint = server.isStoppable
    ? t("environment.localServers.stop", { name: primaryLabel })
    ...
    <Menu>
      <MenuTrigger render={<button type="button" className={ENVIRONMENT_ROW_CLASS_NAME} />}>
        <EnvironmentRowBody
          ...
          label={t("environment.localServers.title")}
          trailing={trailing}
        />
      </MenuTrigger>
      <ComposerPickerMenuPopup align="start" side="bottom" className="w-72 min-w-72">
        <div className="flex items-center justify-between gap-2 pb-0.5 pl-2 pr-3 pt-px">
          <span className="truncate text-[length:var(--app-font-size-ui-xs,10px)] font-normal text-muted-foreground/50">
            {localServersQuery.isLoading
              ...
              : t("environment.localServers.runningCount", { count: serverCount })}
          </span>
        ...
        {localServersQuery.isLoading ? (
          <LocalServersPlaceholder
            ...
            title={t("environment.localServers.scanning")}
          />
        ) : localServersQuery.isError ? (
          <LocalServersPlaceholder
            icon={<GlobeIcon className="size-4" />}
            title={t("environment.localServers.scanFailed")}
            subtitle={
              localServersQuery.error instanceof Error
                ? localServersQuery.error.message
                : t("environment.localServers.scanFailedDescription")
            }
          />
        ) : serverCount === 0 ? (
          <LocalServersPlaceholder
            icon={<GlobeIcon className="size-4" />}
            title={t("environment.localServers.none")}
            subtitle={t("environment.localServers.noneDescription")}
          />
        ) : (
