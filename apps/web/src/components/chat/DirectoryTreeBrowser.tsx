...
export const DirectoryTreeBrowser = memo(function DirectoryTreeBrowser({
  ...
  const resolvedEmptyLabel = emptyLabel ?? t("files.noFolders");
  const resolvedUnavailableLabel = unavailableLabel ?? t("files.homeUnavailable");
  const resolvedLoadingLabel = loadingLabel ?? t("files.loadingFolders");
  ...
  const loadDirectory = useCallback(
    async (relativePath = "") => {
      ...
      try {
        ...
        setErrorMessage(error instanceof Error ? error.message : t("files.loadFoldersError"));
      } finally {
    ...
    [entriesByParent, includeFiles, rootPath, t],
  );
      ...
      entries.flatMap((entry) => {
        ...
        return [
          ...
          >
            <button
              type="button"
              aria-label={t(expanded ? "files.collapseFolder" : "files.expandFolder", {
                name: entry.name,
              })}
              className={cn(
