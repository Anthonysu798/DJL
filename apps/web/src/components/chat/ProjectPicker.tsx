...
interface ActiveFolderOption {
  ...
  origin: "developer-project" | "folder";
}

export function projectPickerCopy(purpose: "chat" | "work", translate: (key: string) => string) {
  return purpose === "work"
    ? {
        trigger: translate("projectPicker.work.trigger"),
        search: translate("projectPicker.work.search"),
        activeGroup: translate("projectPicker.work.activeGroup"),
        chooseFolder: translate("projectPicker.work.chooseFolder"),
        managed: translate("projectPicker.work.managed"),
        managedDetail: translate("projectPicker.work.managedDetail"),
      }
    : {
        trigger: translate("projectPicker.chat.trigger"),
        search: translate("projectPicker.chat.search"),
        activeGroup: translate("projectPicker.chat.activeGroup"),
        chooseFolder: translate("projectPicker.chat.chooseFolder"),
        managed: translate("projectPicker.chat.managed"),
        managedDetail: "",
      };
}
...
export const ProjectPicker = memo(function ProjectPicker({
  ...
  const copy = projectPickerCopy(purpose, t);
  ...
  const handleResetToHome = useCallback(() => {
    try {
      void Promise.resolve(onResetToHome?.())
        ...
        .catch((error) => {
          setErrorMessage(error instanceof Error ? error.message : t("projectPicker.updateFailed"));
        });
    } catch (error) {
      setErrorMessage(error instanceof Error ? error.message : t("projectPicker.updateFailed"));
    }
  ...
  const shouldShowResetToHome = purpose === "work" || showResetToHome || isProjectSelectionMode;
          ...
          <div className="min-w-0 flex-1">
            ...
            {purpose === "work" ? (
              <div className="truncate text-[11px] text-muted-foreground/60">
                {t(
                  folder.origin === "developer-project"
                    ? "projectPicker.developerProject"
                    : "projectPicker.folder",
                )}
              </div>
            ) : null}
          </div>
      ...
      <ComboboxPopup align={align} side={side} className="p-0">
              ...
              {shouldShowResetToHome ? (
                ...
                >
                  ...
                  <span className="min-w-0">
                    <span className="block truncate">{copy.managed}</span>
                    {copy.managedDetail ? (
        ...
        >
          ...
          <ComboboxList className="max-h-64">
            ...
            {!isProjectSelectionMode && filteredActiveFolderOptions.length > 0 ? (
              <ComboboxGroup>
                <ComboboxGroupLabel>{copy.activeGroup}</ComboboxGroupLabel>
                {filteredActiveFolderOptions.map((folder, index) =>
                  renderActiveFolderOption(folder, index),
                )}
              </ComboboxGroup>
