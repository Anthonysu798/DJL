...
import { ChatHeaderButton, ChatHeaderIconButton, SurfaceTabChip } from "./chatHeaderControls";
...
import { RightDockPaneMenuItems, type RightDockPaneMenuItem } from "./RightDockPaneMenuItems";
    ...
    <Menu modal={false}>
      ...
      <ComposerPickerMenuPopup align="start" side="bottom" sideOffset={6} className="w-72 min-w-72">
        {historyThreads.length === 0 ? (
          <MenuItem disabled>{t("header.noChats")}</MenuItem>
        ) : (
            ...
            >
              ...
              {thread.id === props.activeThreadId ? (
                ...
                <span className="shrink-0 text-[10px] text-muted-foreground tabular-nums">
                  {formatRelativeTime(thread.updatedAt ?? thread.createdAt, locale)}
                </span>
              )}
            </MenuItem>
          ))
        ...
        <div className="ml-auto flex min-w-0 items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
          {chatTabs.map((thread, index) => (
            <SurfaceTabChip
              ...
              title={thread.title}
              label={t("header.chatNumber", { number: index + 1 })}
              ...
              closeLabel={t("header.closeNamed", { name: thread.title })}
          ...
          {terminalTabVisible ? (
            <SurfaceTabChip
              ...
              closeLabel={t("header.closeTerminal")}
...
export function resolveHeaderPanelMenuItems(input: {
  browserOpen: boolean;
  diffOpen: boolean;
  diffDisabledReason: string | null;
  diffTotals?: RepoDiffTotals;
  ...
  isElectron: boolean;
  isGitRepo: boolean;
  gitRequiredDetail: string;
  desktopRequiredDetail: string;
}): RightDockPaneMenuItem[] {
  const diffDisabled = !input.isGitRepo || (input.diffDisabledReason !== null && !input.diffOpen);
  const diffDetail = !input.isGitRepo
    ? input.gitRequiredDetail
    : input.diffDisabledReason && !input.diffOpen
      ? input.diffDisabledReason
      : input.diffTotals?.hasChanges
        ? `+${input.diffTotals.additions} −${input.diffTotals.deletions}`
        : (input.diffToggleShortcutLabel ?? undefined);

  return [
    {
      kind: "browser",
      active: input.browserOpen,
      disabled: !input.isElectron,
      ...(!input.isElectron ? { detail: input.desktopRequiredDetail } : {}),
    },
    {
      kind: "diff",
      active: input.diffOpen,
...
export function ChatHeaderPanelLauncher(props: {
  browserOpen: boolean;
    ...
    <Menu modal={false}>
      <MenuTrigger
        render={
          <ChatHeaderIconButton
            type="button"
            label={t("header.panelsMenu")}
            title={t("header.panelsMenu")}
          />
        }
      >
      ...
      <ComposerPickerMenuPopup align="end" side="bottom" className="w-64 min-w-64">
        <RightDockPaneMenuItems
          items={resolveHeaderPanelMenuItems({
            browserOpen: props.browserOpen,
            diffOpen: props.diffOpen,
            diffDisabledReason: props.diffDisabledReason,
            diffTotals: props.diffTotals,
            diffToggleShortcutLabel: props.diffToggleShortcutLabel,
            isElectron: props.desktopAvailable,
            isGitRepo: props.isGitRepo,
            gitRequiredDetail: t("header.gitRepositoryRequired"),
            desktopRequiredDetail: t("header.desktopAppRequired"),
          })}
          onSelect={(kind) => {
            if (kind === "browser") {
              props.onOpenBrowser();
              ...
              >
                ...
                {showSidechatTitleChip && onCloseThreadPane ? (
                  <IconButton
                    variant="chrome"
                    size="icon-xs"
                    label={t("header.closeSelectedSide")}
                    tooltip={t("header.closeSelectedSide")}
                    tooltipSide="bottom"
              ...
              <TooltipTrigger
                render={
                  ...
                  >
                    ...
                    {!compact ? (
                      <span className="truncate font-normal">{t("header.handoff")}</span>
                    ) : null}
                  </MenuTrigger>
                }
              />
            ...
            <ComposerPickerMenuPopup align="end" side="bottom" className="w-48 min-w-48">
              {handoffActionTargetProviders.map((provider) => (
                <MenuItem key={provider} onClick={() => onCreateHandoff(provider)}>
                  ...
                  <span>
                    {t("header.handoffTo", { provider: PROVIDER_DISPLAY_NAMES[provider] })}
                  </span>
                </MenuItem>
              ))}
            </ComposerPickerMenuPopup>
