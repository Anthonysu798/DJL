...
import { CHAT_FILE_REFERENCE_DRAG_TYPE, type ChatFileReference } from "~/lib/chatReferences";
...
export function setFileReferenceDragData(dataTransfer: DataTransfer, path: string): void {
  ...
  dataTransfer.setData(CHAT_FILE_REFERENCE_DRAG_TYPE, formatComposerMentionToken(path));
...
function ExplorerLoadingRows(props: { depth: number }) {
  ...
  return (
    <div
      ...
      role="status"
      aria-label={t("files.loadingDirectory")}
    >
  ...
  if (query.error) {
    return (
      <p className="px-3 py-2 text-[11px] text-destructive/80">
        {query.error instanceof Error ? query.error.message : t("files.loadDirectoryError")}
      </p>
    );
  }
...
function useTreeEntryContextMenu(
  ...
  return useCallback(
    (entry: ProjectFileSystemEntry, position: { x: number; y: number }) => {
      void showFileReferenceContextMenu({ path: entry.path, position, onReferenceInChat, t });
    },
    [onReferenceInChat, t],
  );
}
