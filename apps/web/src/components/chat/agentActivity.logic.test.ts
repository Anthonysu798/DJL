...
import {
  deriveAgentActivityTimelineState as deriveAgentActivityTimelineStateBase,
...
const deriveAgentActivityTimelineState = (entries: ReadonlyArray<WorkLogEntry>) =>
  deriveAgentActivityTimelineStateBase(entries, "Reasoning trace", (count, preview) =>
    preview ? `${count} updates - ${preview}` : `${count} updates`,
  );
