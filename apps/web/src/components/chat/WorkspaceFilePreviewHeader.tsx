...
interface WorkspaceFilePreviewHeaderProps {
  ...
  truncated?: boolean;
  /** Pin the primary Open action to a document-appropriate OS handler. */
...
const MARKDOWN_VIEW_SEGMENTS = [
  {
    rendered: false,
    labelKey: "files.source",
    titleKey: "files.sourceDescription",
    Icon: FileIcon,
  },
  {
    rendered: true,
    labelKey: "files.preview",
    titleKey: "files.previewDescription",
    Icon: EyeIcon,
  },
] as const;
    ...
    >
      ...
      <div className="flex shrink-0 items-center gap-1.5">
        {props.isMarkdown ? (
          <div
            role="radiogroup"
            aria-label={t("files.markdownView")}
            ...
            {MARKDOWN_VIEW_SEGMENTS.map((segment) => {
              ...
              return (
                ...
                >
                  ...
                  <span className="sr-only @sm/header-actions:not-sr-only">
                    {t(segment.labelKey)}
                  </span>
                </button>
              );
        ...
        {hasChatActions ? (
          <Menu>
            <MenuTrigger
              render={<ChatHeaderIconButton label={t("files.moreActions")} tone="plain" />}
            >
              <EllipsisIcon aria-hidden="true" className="size-3.5" />
            </MenuTrigger>
            <ComposerPickerMenuPopup align="end" side="bottom" className="w-52 min-w-52">
              {onReferenceInChat ? (
                <MenuItem onClick={referenceWholeFile}>{t("files.referenceInChat")}</MenuItem>
              ) : null}
              {onAskWhyInChat ? (
                <MenuItem onClick={askWhyWholeFile}>{t("files.askWhy")}</MenuItem>
              ) : null}
        ...
        <OpenInPicker
          ...
          {...(props.defaultEditor ? { defaultEditor: props.defaultEditor } : {})}
        />
      </div>
    </div>
  );
