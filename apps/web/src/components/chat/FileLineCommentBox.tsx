      ...
      <div className="flex items-center justify-between gap-2">
        <span className="inline-flex items-center gap-2 text-[13px] font-semibold text-[var(--color-text-foreground)]">
          <span className="editor-file-viewer__comment-badge">
            <DjlLogo className="size-3 text-[var(--color-text-foreground-secondary)]" />
          </span>
          {t("fileComment.title", { ns: "chat" })}
        </span>
        <span className="text-[12px] text-muted-foreground">
          {t("fileComment.target", { ns: "chat", line: props.lineLabel })}
        </span>
      </div>
      <textarea
        ref={textareaRef}
        ...
        placeholder={t("fileComment.placeholder", { ns: "chat" })}
        rows={2}
