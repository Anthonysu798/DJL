...
import { MessageId, type BrowserFindingDraft } from "@synara/contracts";
...
describe("ChatTranscriptPane", () => {
  ...
  it("renders a linked annotated screenshot only through the findings chip", async () => {
    const finding: BrowserFindingDraft = {
      version: 1,
      id: "finding-transcript",
      imageId: "image-transcript",
      screenshotName: "annotated-transcript.png",
      markerNumber: 1,
      comment: "Adjust this frame",
    ...
    const screen = await render(
      <ChatTranscriptPane
        activeThreadId="thread-browser-finding"
        activeTurnInProgress={false}
        activeTurnStartedAt={null}
        chatFontSizePx={15}
        emptyStateProjectName={undefined}
        ...
        timelineEntries={[
          {
            id: "user-browser-finding",
            kind: "message",
            createdAt: "2026-07-13T00:00:00.000Z",
            message: {
              id: MessageId.makeUnsafe("user-browser-finding"),
              role: "user",
              text: appendBrowserFindingsBlock("", [finding]),
              attachments: [
                {
                  type: "image",
                  id: "image-transcript",
                  name: "annotated-transcript.png",
                  mimeType: "image/png",
                  sizeBytes: 100,
    ...
    try {
      await expect.element(page.getByRole("button", { name: "1 browser finding" })).toBeVisible();
      expect(screen.container.querySelector('img[alt="annotated-transcript.png"]')).toBeNull();
    } finally {
      await screen.unmount();
    }
  });
