...
// Wrangler writes Durable Object and cache data beneath `.wrangler/` while the
// local relay is running. Those files are runtime state, never agent-authored
// work, and historical checkpoint summaries may still contain them even after
// the directory is ignored by Git.
function isGeneratedRuntimeStatePath(path: string): boolean {
  return /(?:^|\/)\.wrangler(?:\/|$)/.test(path);
}
...
> = {
  automation: { Icon: ClockIcon, labelKey: "transcript.sentViaAutomation" },
  steer: { Icon: SteerIcon, labelKey: "transcript.steering" },
};
    ...
    <div className="w-fit max-w-full rounded-xl border border-[color:var(--color-border-light)] bg-[var(--color-background-elevated-primary)] px-3.5 py-3 font-system-ui shadow-xs">
      <div className="flex items-center gap-2">
        ...
        <span className="shimmer text-[13px] font-medium text-[var(--color-text-foreground-secondary)]">
          {t("transcript.preparingWorktree")}
        </span>
      </div>
        ...
        {steps.map((step, index) => {
          ...
          return (
            <li key={step.id} className="relative flex items-center gap-2.5 py-[3px]">
              ...
              >
                {step.label}
                {step.status === "error" ? t("timeline.failedSuffix") : ""}
              </span>
            </li>
          );
        })}
    ...
    >
              ...
              {showOverflowToggle && (
                <div className="mt-1.5 flex items-center justify-start gap-2 px-0.5">
                  ...
                  >
                    {isExpanded
                      ? t("timeline.showLess")
                      : t("timeline.showMoreCount", { count: hiddenCount })}
                  </button>
                </div>
              )}
            </div>
      ...
      {row.kind === "message" &&
        ...
        (() => {
          ...
          const renderedBrowserFindings = displayedUserMessage.browserFindings ?? [];
          const linkedBrowserFindingScreenshotNames = new Set(
            renderedBrowserFindings.map((finding) => finding.screenshotName),
          );
          const visibleUserImages = userImages.filter(
            (image) => !linkedBrowserFindingScreenshotNames.has(image.name),
          );
          ...
          const hasLeadingMedia = hasLeadingUserMedia({
            imageCount: visibleUserImages.length,
          ...
          return (
            <div className="flex w-full justify-end">
              ...
              >
                ...
                {renderedBrowserFindings.length > 0 && (
                  <div className="mb-1 self-end">
                    <BrowserFindingsSummaryChip
                      findings={renderedBrowserFindings}
                      images={userImages}
                      onExpandImage={onImageExpand}
                    />
                  </div>
                )}
                ...
                {isEditingThisMessage ? (
                  ...
                  >
                    ...
                    {userMessagePreview.collapsible && (
                      ...
                      >
                        {userMessageExpanded ? t("timeline.showLess") : t("timeline.showMore")}
                      </button>
                    )}
                  </div>
                ) : null}
                {!isEditingThisMessage && (
                  ...
                  >
                    <p className={cn("tabular-nums", MESSAGE_HOVER_REVEAL_CLASS_NAME)}>
                      {formatShortTimestamp(row.message.createdAt, timestampFormat, locale)}
                    </p>
                    <div className="flex items-center gap-2">
                      ...
                      {showEditUserMessage && (
                        <MessageActionButton
                          label={t("transcript.editMessage")}
                          tooltip={t("transcript.editAndResend")}
                      ...
                      {canRevertAgentWork ? (
                        <MessageActionButton
                          label={t("transcript.revertMessage")}
                          tooltip={t("transcript.revertMessage")}
        ...
        (() => {
          ...
          const inlineEditedFilesFromTurnSummary =
            ...
            (turnSummary?.files.length ?? 0) > 0
              ? turnSummary!.files.filter((file) => !isGeneratedRuntimeStatePath(file.path))
              : [];
              ...
              <div className="group min-w-0 py-0.5">
                  ...
                  >
                    {showPinToggle ? (
                      ...
                      <MessageActionButton
                        ...
                        tooltip={t(
                          messagePinned ? "timeline.unpinFromPanel" : "timeline.pinToPanel",
                        )}
                ...
                {(() => {
                  ...
                  const checkpointFiles = turnSummary.files.filter(
                    (file) => !isGeneratedRuntimeStatePath(file.path),
                  );
                  ...
                  const editedFilesLabel = t("transcript.editedFileCount", {
                      ...
                      >
                        ...
                        <div className="flex shrink-0 items-center gap-2">
                          ...
                          <button
                            ...
                            aria-label={
                              fileChangesExpanded
                                ? t("timeline.collapseChangedFiles")
                                : t("timeline.expandChangedFiles")
                            }
                            onClick={(event) => {
  ...
  if (!hasRenderableTranscriptContent && !isWorking) {
    ...
    return (
      <div className="flex h-full items-center justify-center">
        <p className="text-sm text-muted-foreground/30">{t("transcript.startConversation")}</p>
      </div>
    );
  }
...
const UserImageAttachmentThumbnail = memo(function UserImageAttachmentThumbnail(props: {
  ...
  return (
    <button
      ...
      aria-label={t("attachments.previewNamed", { name: props.image.name })}
  ...
  return (
    ...
    >
      <textarea
        ...
        rows={1}
        aria-label={t("transcript.editMessage", { ns: "chat" })}
...
function commandTooltipContent(command: string, displayText: string, t: (key: string) => string) {
  return (
    <div className="max-w-96 whitespace-pre-wrap leading-tight">
      <div className="space-y-2">
        <div className="space-y-0.5">
          <div className="text-muted-foreground/70">{t("transcript.summary")}</div>
          <div>{displayText}</div>
        </div>
        <div className="space-y-0.5">
          <div className="text-muted-foreground/70">{t("transcript.rawCall")}</div>
            ...
            >
              ...
              {hiddenSubagentCount > 0 ? (
                <div className="pl-4 text-[10px] text-muted-foreground/46">
                  {t("transcript.moreCount", { count: hiddenSubagentCount })}
                </div>
              ) : null}
            </div>
          ) : null}
