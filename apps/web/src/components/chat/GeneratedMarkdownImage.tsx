...
export function GeneratedMarkdownImage(props: GeneratedMarkdownImageProps) {
  ...
  const accessibleName = alt?.trim() || t("image.generated", { ns: "chat" });
  const downloadImage = useLocalImageDownloadClick({
    downloadUrl,
    downloadName,
    errorTitle: t("image.downloadError", { ns: "chat" }),
  });
  ...
  if (status === "error") {
    return (
      <LocalImageErrorCard
        ...
        downloadAriaLabel={t("image.download", { ns: "chat" })}
  ...
  return (
    <span className="chat-generated-image" data-status={status}>
      <button
        ...
        onClick={expandImage}
        aria-label={t("image.expand", { ns: "chat" })}
      >
        {status === "loading" ? (
        ...
        <span className="chat-generated-image__overlay" aria-hidden="true">
          <span className="chat-generated-image__overlay-pill chat-generated-image__overlay-pill--expand">
            ...
            <span>{t("image.expandAction", { ns: "chat" })}</span>
          </span>
        </span>
      </button>
      <a
        ...
        aria-label={t("image.download", { ns: "chat" })}
        title={t("actions.download", { ns: "common" })}
      >
        <DownloadIcon className="size-3.5" aria-hidden="true" />
        <span>{t("actions.download", { ns: "common" })}</span>
      </a>
    </span>
  );
}
