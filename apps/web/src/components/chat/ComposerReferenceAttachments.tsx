...
interface ComposerReferenceAttachmentsProps {
  ...
  browserFindings?: ReadonlyArray<BrowserFindingDraft>;
  ...
  onEditBrowserFindingComment?: (findingId: string, comment: string) => void;
  onRemoveBrowserFinding?: (findingId: string) => void;
}
...
export function ComposerReferenceAttachments({
  ...
  onRemoveFile,
  onRemoveImage,
  onEditBrowserFindingComment,
  onRemoveBrowserFinding,
  ...
  return (
    <div className="-mx-1.5 -mt-1 mb-2 flex flex-wrap items-start gap-1.5">
      ...
      <BrowserFindingsSummaryChip
        ...
        {...(onEditBrowserFindingComment ? { onEditComment: onEditBrowserFindingComment } : {})}
        {...(onRemoveBrowserFinding ? { onRemove: onRemoveBrowserFinding } : {})}
      />
      ...
      {images
        .filter((image) => !browserFindings.some((finding) => finding.imageId === image.id))
        .map((image) => (
