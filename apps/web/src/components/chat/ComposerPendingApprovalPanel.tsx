...
type ApprovalAction = {
  ...
  key: "approveOnce" | "alwaysAllow" | "decline" | "cancelTurn";
...
const KIND_PROMPT_KEY: Record<
  PendingApproval["requestKind"],
  "command" | "fileRead" | "fileChange" | "tool"
> = {
  command: "command",
  "file-read": "fileRead",
    ...
    >
      <div className="flex items-start justify-between gap-3">
        <p className="min-w-0 text-[13px] font-medium leading-snug text-foreground/90">
          {t(`approval.prompt.${KIND_PROMPT_KEY[approval.requestKind]}`)}
          {parsed.tool ? (
      ...
      <div className="mt-2.5 space-y-0.5">
        {APPROVAL_ACTIONS.map((action, index) => (
          <ComposerChoiceRow
            key={action.decision}
            shortcut={index + 1}
            label={t(`approval.actions.${action.key}.label`)}
            description={t(`approval.actions.${action.key}.description`)}
            tone={action.tone}
...
function ApprovalDetail({ parsed }: { parsed: ParsedApproval }) {
  ...
  return <p className="mt-2 text-[12px] text-muted-foreground/65">{t("approval.review")}</p>;
}
