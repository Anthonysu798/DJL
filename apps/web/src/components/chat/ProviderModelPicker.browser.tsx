...
describe("ProviderModelPicker", () => {
  ...
  it("shows a Settings action when no authenticated models are available", async () => {
    const mounted = await mountPicker({
      provider: "opencode",
      model: "openai/gpt-5",
      lockedProvider: null,
      modelOptionsByProvider: { ...MODEL_OPTIONS_BY_PROVIDER, opencode: [] },
    });

    try {
      const configureTrigger = page.getByRole("button", { name: "Configure a model" });
      await expect.element(configureTrigger).toBeInTheDocument();
      await configureTrigger.click();

      await vi.waitFor(() => {
        const text = document.body.textContent ?? "";
        expect(text).toContain("Configure a model");
      });
    } finally {
      await mounted.cleanup();
    }
