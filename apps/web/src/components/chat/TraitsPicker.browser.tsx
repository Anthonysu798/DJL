...
async function mountOpenCodePicker(props?: {
  ...
  const screen = await render(
    <OpenCodeTraitsPickerHarness
      ...
      {...(props?.runtimeAgents ? { runtimeAgents: props.runtimeAgents } : {})}
...
describe("TraitsPicker (OpenCode)", () => {
  ...
  it("does not render an agent-only Build control while OpenCode agent discovery is pending", async () => {
    await using mounted = await mountOpenCodePicker({
      model: "openrouter/gpt-oss-120b:free",
    });

    await vi.waitFor(() => {
      expect(mounted.host.textContent ?? "").not.toContain("Build");
      expect(mounted.host.querySelector("button")).toBeNull();
    });
  });

  it("does not render OpenCode's built-in Build and Plan agents in the traits picker", async () => {
    await using mounted = await mountOpenCodePicker({
      model: "openrouter/gpt-oss-120b:free",
      runtimeAgents: [
