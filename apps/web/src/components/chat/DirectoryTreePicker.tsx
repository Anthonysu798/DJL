    ...
    <Popover open={open} onOpenChange={setOpen}>
      ...
      <PopoverPopup align="start" className="w-[min(32rem,calc(100vw-2rem))] p-0">
        <div className="border-b border-border/60 px-4 py-3">
          <p className="text-sm font-medium text-foreground">{t("files.startFromFolder")}</p>
          <p className="mt-1 truncate text-xs text-muted-foreground/60">
            {rootPath ?? t("files.noHome")}
          </p>
        </div>
        <DirectoryTreeBrowser
          rootPath={rootPath}
          emptyLabel={emptyLabel ?? t("files.noFolders")}
          unavailableLabel={t("files.homeUnavailable")}
          loadingLabel={includeFiles ? t("files.loadingEntries") : t("files.loadingFolders")}
