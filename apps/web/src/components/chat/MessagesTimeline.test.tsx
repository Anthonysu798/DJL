...
describe("MessagesTimeline", () => {
  ...
  it("folds work log summaries above the next assistant message footer", async () => {
    ...
    expect(markup).toContain(formatShortTimestamp("2026-03-17T19:12:29.000Z", "locale", "en"));
    expect(markup).toContain("Worked for 1.0s");
    expect(markup).not.toContain("data-scroll-anchor-ignore");
    expect(markup).not.toContain(
      `${formatShortTimestamp("2026-03-17T19:12:29.000Z", "locale", "en")} • 1.0s`,
    );
    expect(markup).not.toContain("Work log");
  });
  ...
  it("hides generated Wrangler runtime state from persisted change cards", async () => {
    const { MessagesTimeline } = await import("./MessagesTimeline");
    const assistantMessageId = MessageId.makeUnsafe("message-assistant-wrangler-state");
    const markup = renderToStaticMarkup(
      <MessagesTimeline
        hasMessages
        isWorking={false}
        ...
        timelineEntries={[
          {
            id: "entry-assistant-wrangler-state",
            kind: "message",
            createdAt: "2026-03-17T19:12:29.000Z",
            message: {
              id: assistantMessageId,
        ...
        turnDiffSummaryByAssistantMessageId={
          new Map([
            [
              assistantMessageId,
              {
                turnId: TurnId.makeUnsafe("turn-wrangler-state-1"),
                checkpointTurnCount: 1,
                checkpointTurnCounts: [1],
                checkpointRef: CheckpointRef.makeUnsafe("refs/synara/checkpoints/thread/turn/1"),
                status: "ready",
                ...
                files: [
                  {
                    path: "apps/remote-relay/.wrangler/state/v3/do/djl-remote-relay-DJLRelaySession/session.sqlite",
                    additions: 1,
                    deletions: 0,
                  },
                ],
    ...
    expect(markup).not.toContain(".wrangler/state");
  });
});
