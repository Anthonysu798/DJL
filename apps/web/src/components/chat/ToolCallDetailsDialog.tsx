          ...
          <div className="flex min-w-0 items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              <DialogTitle className="truncate text-base">
                {details?.title ?? t("tool.title")}
              </DialogTitle>
              <DialogDescription>
                {details?.kind === "file-change"
                  ? t("tool.editDescription")
                  : t("tool.commandDescription")}
              </DialogDescription>
            </div>
          </div>
        </DialogHeader>
  ...
  return (
    <>
      ...
      {details.files?.length ? (
        <ToolDetailSection title={t("tool.sections.files")}>
      ...
      {details.diff ? (
        <ToolDetailSection title={t("tool.sections.diff")}>
          <DiffCodeBlock>{details.diff}</DiffCodeBlock>
        </ToolDetailSection>
      ) : null}

      {details.edits?.length ? (
        <ToolDetailSection title={t("tool.sections.edits")}>
              ...
              >
                ...
                <div className="grid gap-0 md:grid-cols-2">
                  {edit.oldText !== undefined ? (
                    <TextChangeBlock title={t("tool.sections.before")} tone="remove">
                      {edit.oldText}
                    </TextChangeBlock>
                  ) : null}
                  {edit.newText !== undefined ? (
                    <TextChangeBlock title={t("tool.sections.after")} tone="add">
                      {edit.newText}
                    </TextChangeBlock>
                  ) : null}
                </div>
      ...
      {details.content ? (
        <ToolDetailSection title={t("tool.sections.writtenContent")}>
...
function ToolOutputSection({ output }: { output: WorkLogToolOutputDetails }) {
  const { t } = useTranslation("chat");
  return (
    <ToolDetailSection title={t("tool.sections.output")}>
      <div className="space-y-3">
        {output.output ? (
          <MarkdownToolCodeBlock language="text">{output.output}</MarkdownToolCodeBlock>
        ) : null}
        {output.stdout ? (
          <LabeledCodeBlock title={t("tool.sections.stdout")} tone="output">
            {output.stdout}
          </LabeledCodeBlock>
        ) : null}
        {output.stderr ? (
          <LabeledCodeBlock title={t("tool.sections.stderr")} tone="error">
            {output.stderr}
          </LabeledCodeBlock>
        ) : null}
