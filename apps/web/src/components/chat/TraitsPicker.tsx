...
function isBuiltInModeAgent(provider: ProviderKind, agentName: string): boolean {
  const defaultAgent = defaultAgentForProvider(provider);
  return agentName === defaultAgent || agentName === "plan";
}
...
function getAgentOptions(
  ...
  const discoveredAgents = runtimeAgents ?? [];
  const defaultAgent = defaultAgentForProvider(provider);
  if (!defaultAgent) return [];

  // DJL's Build/Plan interaction mode owns the provider's built-in default and
  // plan agents. Only surface this provider-level picker when additional custom
  // agents exist, keeping the mode toggle in the composer extras menu.
  const customAgents = discoveredAgents.filter(
    (agent) => !isBuiltInModeAgent(provider, agent.name),
  );
  if (customAgents.length === 0) {
    return [];
  }

  const discoveredDefaultAgent = discoveredAgents.find((agent) => agent.name === defaultAgent);
  return [
    discoveredDefaultAgent ?? {
      name: defaultAgent,
      displayName: defaultAgent === "build" ? "Build" : "Code",
    },
    ...customAgents,
  ];
}
...
function getSelectedAgentValue(
  ...
  return selectedAgent && selectedAgent.length > 0 && !isBuiltInModeAgent(provider, selectedAgent)
    ? selectedAgent
    : defaultAgent;
}
...
export function resolveTraitsTriggerSummary(options: {
  ...
  const primaryLabel = ultrathinkPromptControlled
    ? translate("traits.ultrathink")
    : effortLabel
      ? effortLabel
      : thinkingEnabled !== null
        ? translate("traits.thinkingState", {
            state: translate(thinkingEnabled ? "status.on" : "status.off"),
          })
        : isFastOnlyControl
          ? fastModeEnabled
            ? translate("traits.fast")
            : translate("traits.default")
          : null;
  ...
  const summaryText = [
    ...
    showsFastBadge ? translate("traits.fast") : null,
    contextWindowLabel,
  ]
      ...
      <MenuRadioGroup value={value} onValueChange={onValueChange}>
        {options.map((option) => {
          const item = (
            ...
            >
              {option.label}
              {option.isDefault ? t("traits.defaultSuffix") : ""}
            </MenuRadioItem>
          );
...
export const TraitsMenuContent = memo(function TraitsMenuContentImpl({
  ...
  return (
    <>
      {thinkingEnabled !== null ? (
        <TraitRadioSection
          label={t("traits.thinking")}
          value={thinkingEnabled ? "on" : "off"}
          options={[
            { value: "on", label: t("traits.onDefault") },
      ...
      {contextWindowOptions.length > 1 ? (
        <>
          ...
          <TraitRadioSection
            label={contextWindowDescriptor?.label ?? t("context.title")}
      ...
      {effortLevels.length > 0 ? (
        <>
          ...
          <TraitRadioSection
            label={t(
              provider === "kilo" || provider === "opencode" ? "traits.variant" : "traits.effort",
            )}
            note={
              ultrathinkPromptControlled ? (
                <div className="px-2 pb-1.5 text-muted-foreground/80 text-xs">
                  {t("traits.removeUltrathink")}
                </div>
              ) : undefined
            }
            value={effort ?? ""}
      ...
      {includeFastMode && supportsFastModeControl ? (
        <>
          ...
          <TraitRadioSection
            label={t("traits.speed")}
            value={fastModeEnabled ? "on" : "off"}
            options={[
              { value: "off", label: t("traits.default") },
              { value: "on", label: t("traits.fast") },
            ]}
      ...
      {hasAgentControls ? (
        <>
          ...
          <TraitRadioSection
            label={t(provider === "kilo" ? "traits.mode" : "traits.agent")}
  ...
  const triggerContent = hideLabel ? (
    ...
    <span className="flex min-w-0 w-full items-center gap-2 overflow-hidden">
      ...
      <span className="min-w-0 flex flex-1 items-center gap-1.5 truncate">
        {visiblePrimaryTriggerLabel ? (
          ...
          <span className="truncate">{t("traits.options")}</span>
        )}
        {showsFastBadge ? (
          <>
    ...
    <>
      ...
      <span className="inline-flex items-center gap-1.5">
        <span>{visiblePrimaryTriggerLabel ?? t("traits.options")}</span>
        {showsFastBadge ? (
          <>
