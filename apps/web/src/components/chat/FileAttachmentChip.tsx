...
function fileAttachmentDetail(
  file: ChatFileAttachment,
  unknownType: string,
  locale: string,
): string {
  const mimeType = file.mimeType.trim() || unknownType;
  return `${mimeType} - ${formatBytes(file.sizeBytes, locale)}`;
}
    ...
    >
      ...
      {onRemove ? (
        <AttachmentRemoveButton
          ...
          label={t("attachments.removeNamed", { name: file.name })}
...
export function FileAttachmentChip({
  ...
  const { t, i18n } = useTranslation("chat");
  const detail = fileAttachmentDetail(
    file,
    t("attachments.unknownType"),
  ...
  const trigger =
    variant === "card" ? (
      <AttachmentCard
        ...
        removeLabel={t("attachments.removeNamed", { name: file.name })}
      />
    ) : (
