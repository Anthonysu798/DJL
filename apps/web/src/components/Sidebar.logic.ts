...
function createThreadStatusDismissalKey(
  label: Extract<ThreadStatusPill["kind"], "Pending Approval" | "Awaiting Input" | "Plan Ready">,
...
export function resolveThreadStatusPill(input: {
  ...
  if (hasPendingApprovals) {
    ...
    return {
      ...
      label: translateRendererCopy("shell:sidebar.status.pendingApproval", "Pending Approval"),
  ...
  if (hasPendingUserInput) {
    ...
    return {
      kind: "Awaiting Input",
      label: translateRendererCopy("shell:sidebar.status.awaitingInput", "Awaiting Input"),
  ...
  if (thread.hasLiveTailWork) {
    return {
      kind: "Working",
      label: translateRendererCopy("shell:sidebar.status.working", "Working"),
  ...
  if (thread.session?.status === "connecting") {
    return {
      kind: "Connecting",
      label: translateRendererCopy("shell:sidebar.status.connecting", "Connecting"),
      colorClass: "text-sky-600 dark:text-sky-300/80",
      dotClass: "bg-sky-500 dark:bg-sky-300/80",
      pulse: true,
      dismissible: false,
  ...
  if (hasPlanReadyPrompt) {
    ...
    return {
      kind: "Plan Ready",
      label: translateRendererCopy("shell:sidebar.status.planReady", "Plan Ready"),
...
export function resolvePrStatePresentation(pr: {
  ...
  if (pr.state === "open") {
    if (pr.mergeability === "conflicting") {
      return {
        label: translateRendererCopy("shell:sidebar.pr.conflicts", "PR has conflicts"),
        colorClass: "text-amber-600 dark:text-amber-300/90",
    ...
    if (pr.isDraft === true) {
      return {
        label: translateRendererCopy("shell:sidebar.pr.draft", "PR draft"),
    ...
    return {
      label: translateRendererCopy("shell:sidebar.pr.open", "PR open"),
  ...
  if (pr.state === "closed") {
    return {
      label: translateRendererCopy("shell:sidebar.pr.closed", "PR closed"),
      colorClass: "text-zinc-500 dark:text-zinc-400/80",
      iconKind: "pull-request",
    };
  }
  return {
    label: translateRendererCopy("shell:sidebar.pr.merged", "PR merged"),
