  ...
  return (
    ...
    >
      <canvas
        ...
        aria-label={t("preview.page", { number: pageNumber })}
      />
      <div
        ref={textLayerRef}
      ...
      {error ? (
        <div className="pdf-viewer-page__error" role="alert">
          <span>{t(error.summaryKey, { number: pageNumber })}</span>
          <span className="pdf-viewer-page__error-detail">{error.detail}</span>
        </div>
      ) : null}
    </div>
  );
...
function PdfLinkAnchor({
  ...
  return (
    <button
      ...
      className="pdf-viewer-page__link"
      style={style}
      aria-label={t("preview.goToPage", { number: link.targetPageNumber })}
      onClick={() => {
