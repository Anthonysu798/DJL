      ...
      <div className="flex h-12 shrink-0 items-center justify-between border-b border-border/60 px-3">
        <div className="flex items-center gap-2">
          ...
          {activeTaskList ? (
            <span className="text-[11px] text-muted-foreground/60">
              {formatTimestamp(activeTaskList.createdAt, timestampFormat, locale)}
            </span>
          ) : null}
        </div>
        <div className="flex items-center gap-1">
          ...
          <Button
            ...
            variant="ghost"
            onClick={onClose}
            aria-label={t("composer.plan.hideSidebar", { name: t("composer.plan.label") })}
            ...
            <div className="space-y-2">
              ...
              >
                ...
                <span className="text-[10px] font-semibold text-muted-foreground/40 group-hover:text-muted-foreground/60">
                  {planTitle ?? t("composer.plan.details")}
                </span>
              </button>
              {proposedPlanExpanded ? (
          ...
          {!activeTaskList && !planMarkdown ? (
            <div className="flex flex-col items-center justify-center py-12 text-center">
              <p className="text-[13px] text-muted-foreground/40">
                {t("composer.plan.emptyTitle")}
              </p>
              <p className="mt-1 text-[11px] text-muted-foreground/30">
                {t("composer.plan.emptyDescription")}
              </p>
            </div>
          ) : null}
        </div>
