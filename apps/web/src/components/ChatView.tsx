...
import {
  ...
  hasFileUndoSettled,
  normalizeComposerBrowserFindings,
...
function buildQueuedComposerPreviewText(input: {
  ...
  if (input.browserFindingCount > 0) {
    return input.translate("references.browserFindingCount", {
      count: input.browserFindingCount,
    });
  }
  const firstImage = input.images[0];
  if (firstImage) {
    return input.translate("queue.imageNamed", { name: firstImage.name });
  }
  const firstFile = input.files[0];
  if (firstFile) {
    return input.translate("queue.fileNamed", { name: firstFile.name });
  }
  ...
  const pastedTitle = formatPastedTextTitleSeed(input.pastedTexts, input.translate);
  if (pastedTitle) {
    return pastedTitle;
  }
  return input.translate("queue.followUp");
}
...
function formatPastedTextTitleSeed(
  ...
  return pastedTexts.length === 1
    ? pastedTextTitle(firstPastedText.text, translate("attachments.pasted.title"))
    : translate("attachments.pasted.count", { count: pastedTexts.length });
}
...
export default function ChatView({
  ...
  const composerBrowserFindings = normalizeComposerBrowserFindings(composerDraft.browserFindings);
  ...
  const addComposerDraftBrowserFinding = useComposerDraftStore((store) => store.addBrowserFinding);
  const removeComposerDraftImage = useComposerDraftStore((store) => store.removeImage);
  const updateComposerDraftBrowserFindingComment = useComposerDraftStore(
    (store) => store.updateBrowserFindingComment,
  );
  const removeComposerDraftBrowserFinding = useComposerDraftStore(
    (store) => store.removeBrowserFinding,
  );
  ...
  const claudeDynamicAgentsQuery = useQuery(
    providerAgentsQueryOptions({ provider: "claudeAgent", enabled: false }),
  );
  const codexDynamicAgentsQuery = useQuery(
    providerAgentsQueryOptions({ provider: "codex", enabled: false }),
  );
  ...
  const agentActivityTimelineState = useMemo(
    () =>
      deriveAgentActivityTimelineState(
        workLogEntries,
        t("activity.reasoningTrace"),
        (count, preview) =>
          preview
            ? t("activity.updateCountWithPreview", { count, preview })
            : t("activity.updateCount", { count }),
      ),
    [t, workLogEntries],
  );
  ...
  const planSidebarLabel = sidebarProposedPlan
    ? t("composer.plan.details")
    : t("composer.plan.tasks");
  const planSidebarToggleLabel = planSidebarOpen
    ? t("composer.plan.hideNamed", { name: planSidebarLabel })
    : planSidebarLabel;
  const planSidebarToggleTitle = planSidebarOpen
    ? t("composer.plan.hideSidebar", { name: planSidebarLabel })
    : t("composer.plan.showSidebar", { name: planSidebarLabel });
  ...
  const handleCopyProjectInstructionsToNotes = useCallback(() => {
    ...
    void handleNotesChange(activeThreadId, nextNotes)
      .then(() => {
        toastManager.add({
          type: "success",
          title: t("environment.projectInstructions.added"),
        });
      })
      .catch(() => {
  ...
  const handleRemoveThreadMarker = useCallback(
    (markerId: ThreadMarkerId) => {
      ...
      void dispatchThreadMarkerRemove(activeThreadId, markerId).catch((error) => {
        ...
        toastManager.add({
          type: "error",
          title: t("environment.markers.removeFailed"),
        });
      });
    },
    [activeThreadId, t],
  ...
  const handleToggleThreadMarkerDone = useCallback(
    (markerId: ThreadMarkerId) => {
      ...
      void dispatchThreadMarkerDoneSet(activeThreadId, markerId, !marker.done).catch((error) => {
        ...
        toastManager.add({
          type: "error",
          title: t("environment.markers.updateFailed"),
        });
      });
    },
  ...
  const handleRenameThreadMarker = useCallback(
    (markerId: ThreadMarkerId, label: string | null) => {
      ...
      void dispatchThreadMarkerLabelSet(activeThreadId, markerId, label).catch((error) => {
        ...
        toastManager.add({
          type: "error",
          title: t("environment.markers.renameFailed"),
        });
      });
    },
    [activeThreadId, t],
      ...
      return [
        {
          ...
          label: t("commandMenu.fork.newWorktree"),
          description: t("commandMenu.fork.newWorktreeDescription"),
        },
        {
          id: "fork-target:local",
          type: "fork-target" as const,
          target: "local" as const,
          label: t("commandMenu.fork.local"),
          description:
            activeThread?.worktreePath || activeThread?.envMode === "worktree"
              ? t("commandMenu.fork.localWorktreeDescription")
              : t("commandMenu.fork.localThreadDescription"),
        },
      ];
    }
      ...
      return [
        {
          ...
          label: t("commandMenu.review.changes"),
          description: t("commandMenu.review.changesDescription"),
        },
        {
          id: "review-target:base-branch",
          type: "review-target" as const,
          target: "base-branch" as const,
          label: t("commandMenu.review.base"),
          description: t("commandMenu.review.baseDescription"),
        },
      ];
    }
  ...
  const handoffActionLabel = activeThread ? t("handoff.actionExisting") : t("handoff.actionCreate");
  ...
  }, [navigate, onOpenBrowserPanel, resolvedBrowserOpen, threadId]);
  const openBrowserUrl = useCallback(
    (url: string) => {
      ...
      void api?.browser.open({ threadId, initialUrl: url }).catch((error) => {
        toastManager.add({
          type: "error",
          title: t("errors.openRepository"),
          description: error instanceof Error ? error.message : t("errors.openRepositoryFallback"),
        });
      });
      if (onOpenBrowserUrl) {
      ...
      try {
        ...
        setThreadError(
          ...
          error instanceof Error
            ? error.message
            : t("errors.runScriptFailed", { name: script.name }),
        );
        if (options?.throwOnError) {
          throw error instanceof Error
            ? error
            : new Error(t("errors.runScriptFailed", { name: script.name }));
        }
        return null;
      }
    },
  ...
  const deleteProjectScript = useCallback(
    async (scriptId: string) => {
      ...
      try {
        ...
        toastManager.add({
          type: "success",
          title: t("projectScripts.deleted", { name: deletedName ?? t("common:unknown") }),
        });
      } catch (error) {
        toastManager.add({
          type: "error",
          title: t("projectScripts.deleteFailed"),
        ...
        if (api) {
          void api.orchestration
            ...
            .catch((error) => {
              toastManager.add({
                type: "error",
                title: t("errors.updateAccessMode"),
                description: error instanceof Error ? error.message : t("errors.generic"),
              });
            });
        }
  ...
  const createMarkerFromPendingSelection = useCallback(
    (style: ThreadMarkerStyle, color: ThreadMarkerColor) => {
      ...
      if (!message) {
        toastManager.add({
          type: "warning",
          title: t("environment.markers.messageNotFound"),
        });
        return;
      }
      ...
      if (!range) {
        toastManager.add({
          type: "warning",
          title: t("environment.markers.selectUnique"),
          description: t("environment.markers.selectUniqueDescription"),
        });
        return;
      }
      dismissTranscriptSelectionAction();
      ...
      void dispatchThreadMarkerAdd({
        ...
        toastManager.add({
          type: "error",
          title: t("environment.markers.createFailed"),
        });
      });
    },
    [
  ...
  const startComposerVoiceRecording = useCallback(async () => {
    ...
    if (pendingUserInputs.length > 0) {
      toastManager.add({
        type: "error",
        title: t("voice.answerQuestionsFirst"),
      });
      return;
    }
    ...
    try {
      ...
      const description =
        error instanceof Error
          ...
          : t("voice.transcriptionFailedFallback");
      ...
      toastManager.add({
        type: "error",
        title: authExpired ? t("voice.signInAgain") : t("voice.transcriptionFailed"),
        description: authExpired ? t("voice.sessionRejected") : description,
        ...(authExpired
          ? {
              actionProps: {
                children: t("voice.refreshStatus"),
                onClick: () => {
                  void refreshProviderStatuses();
                },
              },
  ...
  const addComposerImages = useCallback(
    (files: readonly File[]) => {
      ...
      if (pendingUserInputs.length > 0) {
        toastManager.add({
          type: "error",
          title: t("attachments.answerBeforeImages"),
        });
        return;
      }
  ...
  const addComposerFiles = useCallback(
    (files: readonly File[]) => {
      ...
      if (pendingUserInputs.length > 0) {
        toastManager.add({
          type: "error",
          title: t("attachments.answerBeforeFiles"),
        });
        return;
      }
  ...
  const addComposerAttachments = useCallback(
    (files: readonly File[]) => {
      const partitioned = partitionComposerAttachmentFiles(files);
      if (partitioned.images.length > 0) {
        addComposerImages(partitioned.images);
      }
      if (partitioned.files.length > 0) {
        addComposerFiles(partitioned.files);
      }
    },
  ...
  const onRevertToTurnCount = useCallback(
    async (turnCount: number) => {
      ...
      if (hasLiveTurn || isSendBusy || isConnecting) {
        setThreadError(activeThread.id, t("errors.interruptBeforeRevert"));
        return;
      }
      const confirmed = await api.dialogs.confirm(
        t("confirmations.revertCheckpoint", { count: turnCount }),
      );
      if (!confirmed) {
        return;
      }
      ...
      try {
        ...
        setThreadError(
          activeThread.id,
          err instanceof Error ? err.message : t("errors.revertThreadFailed"),
        );
      }
      setIsRevertingCheckpoint(false);
    },
  ...
  const onUndoTurnFiles = useCallback(
    async (turnCounts: readonly number[]) => {
      ...
      if (hasLiveTurn || isSendBusy || isConnecting) {
        setThreadError(activeThread.id, t("errors.interruptBeforeUndo"));
        return;
      }
      const confirmed = await api.dialogs.confirm(t("confirmations.undoFileChanges"));
      if (!confirmed) return;
      ...
      try {
        ...
        setThreadError(
          activeThread.id,
          err instanceof Error ? err.message : t("errors.undoFilesFailed"),
        );
      }
    },
    [activeThread, hasLiveTurn, isConnecting, isRevertingCheckpoint, isSendBusy, setThreadError, t],
    ...
    async (targetProvider: ProviderKind) => {
      ...
      try {
        ...
        toastManager.add({
          ...
          description: error instanceof Error ? error.message : t("errors.createHandoffFallback"),
        });
      }
    },
    ...
    async (input: {
      ...
      try {
            ...
            try {
              await api.orchestration.dispatchCommand({
                ...
                activity: {
                  ...
                  summary: t("automation.createdSummary", {
                    name: definition.name,
                    cadence: formatCadence(definition.schedule, locale, timestampFormat),
                  }),
                  payload: {
                    ...
                    cadenceLabel: formatCadence(definition.schedule, locale, timestampFormat),
              ...
              toastManager.add({
                type: "warning",
                title: t("automation.threadNoteFailed"),
                description: t("automation.threadNoteFailedDescription"),
              });
            }
          })();
        }
        ...
        toastManager.add({
          ...
          description: `${definition.name} - ${formatCadence(definition.schedule, locale, timestampFormat)}`,
        });
        return true;
      } catch (error) {
        toastManager.add({
          ...
          description:
            error instanceof Error ? error.message : t("automation.createFailedFallback"),
        });
        return false;
      } finally {
  ...
  const ensureAutomationTargetThread = useCallback(
    async (input: {
      ...
      if (!api || !activeProject || !activeThread) {
        toastManager.add({
          type: "warning",
          title: t("automation.chatRequired"),
          description: t("automation.chatRequiredDescription"),
        });
        return null;
      }
      if (isServerThread) {
      ...
      try {
        ...
        if (result === "unavailable") {
          toastManager.add({
            type: "error",
            title: t("automation.createChatFailed"),
            description: t("automation.createChatFailedDescription"),
          });
          return null;
        }
        ...
        toastManager.add({
          ...
          title: t("automation.createChatFailed"),
          description:
            error instanceof Error ? error.message : t("automation.createChatFailedDescription"),
        });
        return null;
      }
    },
    ...
    async (input: {
      ...
      try {
        ...
        toastManager.add({
          ...
          description: `${updated.name} - ${formatCadence(updated.schedule, locale, timestampFormat)}`,
        });
        return true;
      } catch {
        return false;
  ...
  const onSend = async (
    ...
    const browserFindingsForSend = queuedChatTurn?.browserFindings ?? composerBrowserFindings;
      ...
      if (automationRequest.type !== "normal-chat") {
        if (automationRequest.type === "needs-clarification") {
          ...
          if (!hasPromptOnlySendableContent || hasLiveTurn) {
            toastManager.add({
              type: "warning",
              title: t("automation.needsDetail"),
              description: automationRequest.reason ?? t("automation.needsDetailFallback"),
            });
            return true;
          }
    ...
    const browserPromptAttachment: BrowserPromptAttachmentResolution =
      browserFindingsForSend.length > 0
        ? { requested: false, image: null }
    ...
    if (browserPromptAttachment.image) {
      ...
      if (nextAttachmentCount <= PROVIDER_SEND_TURN_MAX_ATTACHMENTS) {
        ...
        toastManager.add({
          type: "warning",
          title: t("references.limit", { count: PROVIDER_SEND_TURN_MAX_ATTACHMENTS }),
          description: t("references.browserScreenshotSkipped"),
        });
      }
    } else if (browserPromptAttachment.requested) {
      const description =
        browserPromptAttachment.reason === "no-open-browser"
          ? t("references.browserOpenFirst")
          : browserPromptAttachment.reason === "no-active-tab"
            ? t("references.browserNoActiveTab")
            : browserPromptAttachment.reason === "attachment-too-large"
              ? t("references.browserTooLarge", { limit: IMAGE_SIZE_LIMIT_LABEL })
              : t("references.browserAttachFailed");
      toastManager.add({
        type: "warning",
        title: t("references.browserAttachFailedTitle"),
        description,
      });
    }
    ...
    if (hasLiveTurn && dispatchMode === "queue" && queuedChatTurn === null) {
      ...
      enqueueQueuedComposerTurn(activeThread.id, {
        ...
        previewText: buildQueuedComposerPreviewText({
          ...
          browserFindingCount: browserFindingsForSend.length,
          translate: t,
        }),
        prompt: promptForSend,
        images: queuedImagesForPersistence,
        browserFindings: browserFindingsForSend,
        files: composerFilesForSend,
        assistantSelections: composerAssistantSelectionsForSend,
        fileComments: composerFileCommentsForSend,
        terminalContexts: sendableComposerTerminalContexts,
    ...
    if (!titleSeed) {
      if (browserFindingsForSend.length > 0) {
        titleSeed = `Browser finding${browserFindingsForSend.length === 1 ? "" : "s"}`;
      } else if (firstComposerImageNameForTitle) {
        ...
        titleSeed =
          formatPastedTextTitleSeed(sendableComposerPastedTexts, t) ?? GENERIC_CHAT_THREAD_TITLE;
      } else {
    ...
    if (isFirstMessage && isContainerLandingProject && firstSendTarget.kind !== "current") {
      if (firstSendTarget.kind === "create-project") {
        ...
        try {
          ...
          const description =
            error instanceof Error ? error.message : t("errors.createProjectFailed");
    ...
    if (shouldCreateWorktree && !nextThreadBranch) {
      setStoreThreadError(threadIdForSend, t("errors.selectBaseBranch"));
      return false;
    }
    ...
    const messageTextForSend = appendBrowserFindingsBlock(
      appendPastedTextsToPrompt(
        appendFileCommentsToPrompt(
          appendTerminalContextsToPrompt(
    ...
    await (async () => {
      ...
      // Work uploads begin only after a local draft has been promoted to a durable
      // server task. This lets the upload endpoint enforce thread ownership and
      // avoids buffering large Office files as data URLs in the renderer.
      const turnAttachments = await buildUploadComposerAttachments({
        images: composerImagesSnapshot,
        files: composerFilesSnapshot,
        assistantSelections: composerAssistantSelectionsSnapshot,
        ...(targetProjectKindForSend === "studio"
          ? { streaming: { threadId: ThreadId.makeUnsafe(threadIdForSend) } }
          : {}),
      });
      ...
      if (
        ...
        for (const finding of browserFindingsForSend) {
          addComposerDraftBrowserFinding(threadIdForSend, finding);
        }
      ...
      setThreadError(
        threadIdForSend,
        err instanceof Error ? err.message : t("errors.sendMessageFailed"),
      );
    });
    ...
    async (requestId: ApprovalRequestId, decision: ProviderApprovalDecision) => {
      ...
      await api.orchestration
        ...
        .catch((err: unknown) => {
          setStoreThreadError(
            activeThreadId,
            err instanceof Error ? err.message : t("errors.submitApprovalFailed"),
          );
        });
      setRespondingRequestIds((existing) => existing.filter((id) => id !== requestId));
    },
    [activeThreadId, runtimeMode, setComposerDraftRuntimeMode, setStoreThreadError, t],
  );
    ...
    async (requestId: ApprovalRequestId, answers: ProviderUserInputAnswers) => {
      ...
      await api.orchestration
        ...
        .catch((err: unknown) => {
          setStoreThreadError(
            activeThreadId,
            err instanceof Error ? err.message : t("errors.submitUserInputFailed"),
          );
        });
  ...
  async function onSubmitPlanFollowUp({
    ...
    try {
      ...
      setThreadError(
        threadIdForSend,
        err instanceof Error ? err.message : t("errors.sendPlanFollowUpFailed"),
      );
      sendInFlightRef.current = false;
      resetLocalDispatch();
      return false;
  ...
  const onEditUserMessage = useCallback(
    async (messageId: MessageId, text: string): Promise<boolean> => {
      ...
      const editTarget = resolveTailUserMessageEditTarget({
        ...
        messageId,
        activeTurnId: resolveTailReplayTurnId({
          sessionIsRunning: activeThread.session?.orchestrationStatus === "running",
          sessionActiveTurnId: activeThread.session?.activeTurnId,
          latestTurn: activeThread.latestTurn,
        }),
      });
      if (!editTarget.editable) {
        setThreadError(activeThread.id, t("errors.editLatestOnly"));
        return false;
      }
      ...
      if (isSendBusy || isConnecting || sendInFlightRef.current) {
        setThreadError(activeThread.id, t("errors.waitBeforeEdit"));
        return false;
      }
      ...
      try {
        ...
        setThreadError(
          activeThread.id,
          err instanceof Error ? err.message : t("errors.editMessageFailed"),
        );
        return false;
      } finally {
        setIsRevertingCheckpoint(false);
  ...
  const onImplementPlanInNewThread = useCallback(async () => {
    ...
    await api.orchestration
      ...
      .catch(async (err) => {
        ...
        toastManager.add({
          type: "error",
          title: t("errors.startImplementationThread"),
          description: err instanceof Error ? err.message : t("errors.createThreadFallback"),
        });
      })
      .then(finish, finish);
  }, [
  ...
  const handleSelectWorkProjectForEmptyDraft = useCallback(
    (projectId: ProjectId) => {
      const project = useStore
        .getState()
        .projects.find((candidate) => candidate.id === projectId && candidate.kind === "project");
      ...
      handleSelectWorkspaceRoot(project.cwd);
    },
    [handleSelectWorkspaceRoot],
  );

  const handleSelectWorkFolderFromPicker = useCallback(
    (workspaceRoot: string) => {
      handleSelectWorkspaceRoot(workspaceRoot);
    },
    [handleSelectWorkspaceRoot],
  );
  ...
  const {
    ...
    handleClearConversation: async () => {
      if (!activeProject) {
        toastManager.add({
          type: "warning",
          title: t("errors.clearUnavailable"),
          description: t("errors.clearUnavailableDescription"),
        });
        return;
      }
      ...
      >
        ...
        {isElectron && (
          ...
          >
            ...
            <span className="text-xs text-muted-foreground/50">{t("empty.noActiveThread")}</span>
          </div>
        )}
        <div className="flex flex-1 items-center justify-center">
          <div className="text-center">
            <p className="text-sm">{t("empty.selectOrCreate")}</p>
          </div>
        </div>
      </div>
    );
  ...
  const handleRenameActiveThread = async (newTitle: string) => {
    const outcome = await dispatchThreadRename({
      ...
      toastManager.add({
        type: "error",
        title: t("errors.renameThreadFailed"),
        description: error instanceof Error ? error.message : t("errors.generic"),
      });
      throw error;
    });
    ...
    if (outcome === "empty") {
      toastManager.add({
        type: "warning",
        title: t("errors.threadTitleEmpty"),
      });
      return;
    }
    ...
    >
      {isEmptyChatLanding ? (
        <ProjectPicker
          align="start"
          side="top"
          purpose={isStudioContainer ? "work" : "chat"}
          ...
          onSelectProject={
            isStudioContainer
              ? handleSelectWorkProjectForEmptyDraft
              : handleSelectProjectForEmptyDraft
          }
          onSelectWorkspaceRoot={handleSelectWorkspaceRoot}
          onCreateProjectFromPath={
            isStudioContainer ? handleSelectWorkFolderFromPicker : handleCreateProjectFromPickerPath
          }
      ...
      {showEmptyLandingBranchToolbar ? (
        <Button
          ...
          title={isThreadTemporary ? t("temporary.activeHint") : t("temporary.inactiveHint")}
          aria-label={t("a11y.temporary")}
          className={cn(
        ...
        >
          ...
          <span className="sr-only sm:not-sr-only">{t("temporary.label")}</span>
        </Button>
      ) : null}
    </div>
  ) : null;
                ...
                >
                    ...
                    <div className={COMPOSER_COMMAND_MENU_FLOATING_WRAPPER_CLASS_NAME}>
                      {isLocalFolderBrowserOpen ? (
                        <ComposerLocalDirectoryMenu
                          ...
                          rootLabel={
                            localFolderBrowseRootPath ?? t("composer.localFoldersUnavailable")
                          }
                  ...
                  {!isComposerApprovalState &&
                    ...
                    (composerAssistantSelections.length > 0 ||
                      ...
                      composerBrowserFindings.length > 0 ||
                      ...
                      <ComposerReferenceAttachments
                        ...
                        onEditBrowserFindingComment={(findingId, comment) =>
                          updateComposerDraftBrowserFindingComment(threadId, findingId, comment)
                        }
                        onRemoveBrowserFinding={(findingId) =>
                          removeComposerDraftBrowserFinding(threadId, findingId)
                        }
                      />
                    )}
                  ...
                  >
                    ...
                    >
                      ...
                      {activePendingProgress ? (
                        ...
                        >
                          {activePendingIsResponding
                            ? t("composer.actions.submitting")
                            : activePendingProgress.isLastQuestion
                              ? t("composer.actions.submitAnswers")
                              : t("composer.actions.nextQuestion")}
                        </Button>
                      ) : phase === "running" ? (
                        <Button
                          type="button"
                          ...
                          aria-label={t("composer.actions.stop")}
                          title={t("composer.actions.stopHint")}
                        >
                          <span
                            aria-hidden="true"
                        ...
                        showPlanFollowUpPrompt ? (
                          prompt.trim().length > 0 ? (
                            ...
                            >
                              {isConnecting || isSendBusy
                                ? t("composer.actions.sending")
                                : t("composer.actions.refine")}
                            </Button>
                          ) : (
                            <div className="flex items-center">
                              ...
                              <Menu>
                                <MenuTrigger
                                  render={
                                    <Button
                                      ...
                                      aria-label={t("composer.actions.implementationActions")}
                                      disabled={isSendBusy || isConnecting}
                                    />
                                  }
                                >
                                ...
                                <MenuPopup align="end" side="top">
                                  ...
                                  >
                                    {t("composer.actions.implementNewThread")}
                                  </MenuItem>
                                </MenuPopup>
                              </Menu>
                            </div>
                          ...
                          <>
                            ...
                            <Button
                              ...
                              aria-label={
                                isConnecting
                                  ? t("composer.actions.connecting")
                                  : isVoiceTranscribing
                                    ? t("composer.actions.transcribing")
                                    : isPreparingWorktree
                                      ? t("composer.actions.preparingWorktree")
                                      : isSendBusy
    ...
    >
      ...
      >
        <ChatHeader
          ...
          showPanelLauncher={!isEditorRail}
