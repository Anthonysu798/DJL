...
import type { ContainerChatFailure } from "../lib/containerChatFailure";
...
export type ChatRouteStartMode = "fresh" | "restore";
...
export function RestoreOrCreateChatRoute({
  ...
  const [chatFailure, setChatFailure] = useState<ContainerChatFailure | null>(null);
  ...
  return (
    <SplashScreen
      {...(chatFailure?.summary !== undefined ? { errorMessage: chatFailure.summary } : {})}
      {...(chatFailure?.detail !== undefined ? { errorDetail: chatFailure.detail } : {})}
      onRetry={chatFailure ? () => setAttempt((value) => value + 1) : null}
    />
  );
}
