...
export async function requestBrowserPanelClose(
...
export async function restoreOrDisableBrowserAnnotation(
  cancelSelection: () => Promise<void>,
  disableRuntime: () => Promise<void>,
): Promise<"cancelled" | "disabled" | "failed"> {
  try {
...
export function clearMatchingBrowserError(
  currentError: string | null,
  resolvedError: string,
): string | null {
  return currentError === resolvedError ? null : currentError;
}

export function shouldPreserveBrowserCommentingDuringTransitionFailure(input: {
  desiredEnabled: boolean;
  activeTabIsLoading: boolean;
  activeTabAttached: boolean;
}): boolean {
  return input.desiredEnabled && input.activeTabAttached && input.activeTabIsLoading;
}
  ...
  if (query.length > 0) {
    const directTitle = directTarget.startsWith(BROWSER_SEARCH_URL_PREFIX)
      ? translateRendererCopy(
          "workspace:browser.suggestions.search",
          'Search the web for "{{query}}"',
          {
            query: input.query.trim(),
          },
        )
      : translateRendererCopy("workspace:browser.suggestions.open", "Open {{target}}", {
          target: directTarget,
        });
...
export function resolveBrowserChromeStatus(input: {
  ...
  if (!input.hasActiveTab) {
    return {
      tone: "default",
      label: input.workspaceReady
        ? (input.labels?.noTabs ??
          translateRendererCopy("workspace:browser.status.noTabs", "No tabs open"))
        : (input.labels?.starting ??
          translateRendererCopy("workspace:browser.status.starting", "Starting browser...")),
    };
  }
  ...
  if (input.activeTabStatus === "suspended") {
    return {
      tone: "default",
      label:
        input.labels?.restoring ??
        translateRendererCopy("workspace:browser.status.restoring", "Restoring tab..."),
    };
  }
