...
export function PullRequestThreadDialog({
  ...
  const validationMessage = !referenceDirty
    ...
    : reference.trim().length === 0
      ? t("pullRequest.validation.required")
      : parsedReference === null
        ? t("pullRequest.validation.invalid")
        : null;
  const operationError =
    resolvedPullRequest === null && resolvePullRequestQuery.isError
      ? { summary: t("pullRequest.errors.resolve"), detail: resolvePullRequestQuery.error }
      : preparePullRequestThreadMutation.error
        ? {
            summary: t("pullRequest.errors.prepare"),
            detail: preparePullRequestThreadMutation.error,
          }
        : null;
  ...
  return (
    ...
    >
      <DialogPopup className="max-w-xl">
        <DialogHeader>
          <DialogTitle>{t("pullRequest.title")}</DialogTitle>
          <DialogDescription>{t("pullRequest.description")}</DialogDescription>
        </DialogHeader>
        <DialogPanel className="space-y-4">
            ...
            <div className="rounded-xl border border-border/70 bg-muted/24 p-3">
              <div className="flex items-start justify-between gap-3">
                ...
                <span className={cn("shrink-0 text-xs capitalize", statusTone)}>
                  {t(`pullRequest.status.${resolvedPullRequest.state}`)}
                </span>
              </div>
            </div>
          ) : null}
          ...
          {validationMessage ? (
            ...
            <div className="space-y-1 text-destructive text-xs">
              <p>{operationError.summary}</p>
              {operationError.detail instanceof Error ? (
                <pre className="whitespace-pre-wrap text-[10px]">
                  {operationError.detail.message}
                </pre>
              ) : null}
            </div>
          ) : null}
