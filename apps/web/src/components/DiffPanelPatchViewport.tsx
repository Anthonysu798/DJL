    ...
    if (props.error && !props.renderablePatch) {
      return (
        <div className={viewportClassName}>
          ...
          >
            <div className="space-y-1 text-left text-red-500/80">
              <p className="text-[11px] font-medium">{t("diff.errors.load")}</p>
              <pre className="whitespace-pre-wrap text-[10px]">{props.error}</pre>
            </div>
          </PanelStateMessage>
        </div>
      );
