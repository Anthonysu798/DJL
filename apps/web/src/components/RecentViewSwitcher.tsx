...
export function RecentViewSwitcher(props: {
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-[90] flex items-start justify-center pt-[14vh]">
      <div
        role="listbox"
        aria-label={t("recentViews.ariaLabel")}
      ...
      >
            ...
            return (
              ...
              >
                ...
                <div className="flex shrink-0 items-center gap-1.5 text-muted-foreground">
                  {entry.isSplit ? (
                    <PanelLeftIcon className="size-3.5" aria-label={t("recentViews.splitView")} />
                  ) : null}
                  {entry.isPinned ? (
                    <PinFilledIcon className="size-3.5" aria-label={t("recentViews.pinned")} />
                  ) : null}
                </div>
              </div>
            );
        ...
        <div className="flex items-center justify-between gap-3 border-t border-border/60 px-3 py-2 text-[11px] text-muted-foreground">
          <span className="shrink-0">
            {t("recentViews.count", { count: props.entries.length })}
          </span>
