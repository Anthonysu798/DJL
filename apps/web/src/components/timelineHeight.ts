...
const USER_BROWSER_FINDINGS_CHIP_HEIGHT_PX = 36;
...
export function estimateTimelineMessageHeight(
  ...
  if (message.role === "user") {
    ...
    const browserFindingScreenshotNames = new Set(
      (displayedUserMessage.browserFindings ?? []).map((finding) => finding.screenshotName),
    );
    const imageAttachmentCount =
      message.attachments?.filter(
        (attachment) =>
          attachment.type === "image" &&
          (!attachment.name || !browserFindingScreenshotNames.has(attachment.name)),
      ).length ?? 0;
    const browserFindingsHeight =
      (displayedUserMessage.browserFindings?.length ?? 0) > 0
        ? USER_BROWSER_FINDINGS_CHIP_HEIGHT_PX
        : 0;
