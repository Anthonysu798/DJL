...
const SIDEBAR_VIEW_LABEL_KEYS: Record<SidebarView, string> = {
  ...
  workspace: "sidebar.views.workspace",
};
...
function ProjectRunIndicatorDot({ className }: { className?: string }) {
  ...
  return (
    <span
      aria-hidden="true"
      title={t("sidebar.projectMenu.devRunning")}
      className={cn(
...
function resolveThreadRowMetaChips(input: {
      ...
      threadAutomations.length === 1
        ? `${firstAutomation.name} · ${
            firstAutomation.enabled
              ? formatCadence(firstAutomation.schedule, input.locale, input.timestampFormat)
              : input.translate("sidebar.status.paused")
          }`
        : input.translate("sidebar.automationCount", { count: threadAutomations.length });
    chips.push({
      id: "automation",
      tooltip,
      icon: (
  ...
  if (input.thread.forkSourceThreadId && !isSidechatThread) {
    chips.push({
      id: "fork",
      tooltip: input.translate("sidebar.forkedThread"),
      icon: (
        <SidebarGlyph
          icon={GoRepoForked}
          variant="meta"
...
function ThreadTerminalIndicator({
  terminalStatus,
  terminalCount,
}: {
  ...
  const badgeTooltip =
    terminalCount > 1
      ? t("sidebar.terminalOpenCount", { count: terminalCount })
      : terminalStatus
        ? t(terminalStatus.labelKey)
        : t("sidebar.terminalOpen");
      ...
      <TooltipTrigger
        render={
          <span
            ...
            className="sidebar-icon-chip inline-flex size-3 min-w-3 shrink-0 items-center justify-center rounded-full px-px"
          >
            {terminalCount > 1 ? (
              <span
                className={cn(
...
interface TerminalStatusIndicator {
  kind: "attention" | "complete" | "running";
  labelKey: string;
  colorClass: string;
  pulse: boolean;
}
...
function terminalStatusFromThreadState(input: {
  ...
  if (terminalAttentionStates.includes("attention")) {
    return {
      kind: "attention",
      labelKey: "sidebar.status.terminalInputNeeded",
  ...
  if ((input.runningTerminalIds?.length ?? 0) > 0) {
    return {
      kind: "running",
      labelKey: "sidebar.status.terminalProcessRunning",
  ...
  if (terminalAttentionStates.includes("review")) {
    return {
      kind: "complete",
      labelKey: "sidebar.status.terminalTaskCompleted",
  ...
  return (
    <Menu>
      <SidebarIconButton
        render={<MenuTrigger />}
        icon={IoFilter}
        label={t("sidebar.sortProjects")}
        tooltip={t("sidebar.sortProjects")}
        tooltipSide="right"
      />
      <MenuPopup
        align="end"
...
function ThreadSortMenuItems({
  ...
  return (
    ...
    >
      {(
        Object.entries(SIDEBAR_THREAD_SORT_LABEL_KEYS) as Array<[SidebarThreadSortOrder, string]>
      ).map(([value, labelKey]) => (
        <MenuRadioItem key={value} value={value} className="min-h-7 py-1 sm:text-xs">
          {t(labelKey)}
        </MenuRadioItem>
  ...
  return (
    <Menu>
      <SidebarIconButton
        render={<MenuTrigger />}
        icon={IoFilter}
        label={t("sidebar.sortChats")}
        tooltip={t("sidebar.sortChats")}
        tooltipSide="top"
      />
      <MenuPopup
        align="end"
      ...
      <div className="sidebar-segmented-picker relative isolate inline-flex w-full rounded-lg p-0.5">
        ...
        {views.map((view) => {
          ...
          return (
            ...
            >
              {t(SIDEBAR_VIEW_LABEL_KEYS[view])}
            </button>
          );
        })}
      </div>
...
export default function Sidebar() {
    ...
    (threadId: ThreadId) => {
      ...
      void setThreadPinned(threadId, !isPinned).catch((error) => {
        ...
        toastManager.add({
          type: "error",
          title: isPinned
            ? t("sidebar.threadActions.unpinFailed")
            : t("sidebar.threadActions.pinFailed"),
        });
      });
    },
  ...
  const openPrLink = useCallback(
    (event: MouseEvent<HTMLElement>, prUrl: string) => {
      ...
      if (!api) {
        toastManager.add({
          type: "error",
          title: t("sidebar.errors.linkUnavailable"),
        });
        return;
      }
      ...
      void api.shell.openExternal(prUrl).catch((error) => {
        toastManager.add({
          type: "error",
          title: t("sidebar.errors.openPr"),
          description: error instanceof Error ? error.message : t("sidebar.errors.generic"),
        });
      });
    },
    [t],
    ...
    async (projectId: ProjectId, isPinned: boolean) => {
      ...
      if (isPinned) {
        ...
        if (!accepted) {
          ...
          toastManager.add({
            type: "warning",
            title: t("sidebar.projectActions.pinLimit"),
            description: t("sidebar.projectActions.pinLimitDescription", {
              count: MAX_PINNED_PROJECTS,
            }),
          });
          return;
        }
    ...
    (projectId: ProjectId) => {
      ...
      void setProjectPinned(projectId, !isPinned).catch((error) => {
        ...
        toastManager.add({
          type: "error",
          title: isPinned
            ? t("sidebar.projectActions.unpinFailed")
            : t("sidebar.projectActions.pinFailed"),
  ...
  const addProjectFromPath = useCallback(
    async (rawCwd: string, options: { createIfMissing?: boolean } = {}) => {
      ...
      try {
        ...
        const description =
          error instanceof Error ? error.message : t("sidebar.errors.addProjectFallback");
        setIsAddingProject(false);
    ...
    try {
      ...
      if (pickedPath) {
        ...
        await addProjectFromPath(pickedPath).catch((error: unknown) => {
          ...
          toastManager.add({
            type: "error",
            title: t("sidebar.errors.addProject"),
            description,
          });
        });
      }
      ...
      const description =
        error instanceof Error ? error.message : t("sidebar.errors.openFolderPickerFallback");
      setAddProjectError(description);
      toastManager.add({
        type: "error",
        title: t("sidebar.errors.openFolderPicker"),
        description,
      });
      setIsPickingFolder(false);
    }
  }, [isPickingFolder, addProjectFromPath, t]);
  ...
  const handleImportThread = useCallback(
    async (provider: ImportProviderKind, externalId: string) => {
      ...
      const title = t(
        provider === "codex"
          ? "sidebar.palette.importedThreadTitle"
          : "sidebar.palette.importedSessionTitle",
        { provider: providerName, suffix: suffix ? ` ${suffix}` : "" },
      );
  ...
  const commitRename = useCallback(
    async (threadId: ThreadId, newTitle: string, originalTitle: string) => {
      const outcome = await dispatchThreadRename({
        ...
        toastManager.add({
          type: "error",
          title: t("sidebar.errors.renameThread"),
          description: error instanceof Error ? error.message : t("sidebar.errors.generic"),
        });
        return null;
      });
      ...
      if (outcome === "empty") {
        toastManager.add({
          type: "warning",
          title: t("sidebar.errors.threadTitleEmpty"),
        });
      }
    },
    [t],
    ...
    async (
      ...
      const shouldDeleteWorktree =
        ...
        (await api.dialogs.confirm(
          t("sidebar.confirm.deleteWorktree", {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : t("sidebar.errors.removeWorktreeFallback");
        ...
        toastManager.add({
          type: "error",
          title: t("sidebar.errors.worktreeRemovalFailed"),
          description: t("sidebar.errors.worktreeRemovalFailedDescription", {
            path: displayWorktreePath ?? orphanedWorktreePath,
            detail: message,
          }),
        });
  ...
  const handoffThread = useCallback(
    async (thread: Thread, targetProvider: ProviderKind) => {
      try {
        ...
        toastManager.add({
          type: "error",
          title: t("sidebar.errors.createHandoff"),
          description:
            error instanceof Error ? error.message : t("sidebar.errors.createHandoffFallback"),
        });
      }
    },
  ...
  const confirmAndDeleteThread = useCallback(
    async (threadId: ThreadId) => {
      ...
      if (appSettings.confirmThreadDelete) {
        const api = readNativeApi();
        const confirmationMessage = t("sidebar.confirm.deleteThread", {
          title: thread.title,
        });
        const confirmed = api
    ...
    [appSettings.confirmThreadDelete, deleteThread, sidebarThreadSummaryById, t],
  );
  ...
  const archiveThread = useCallback(
    async (threadId: ThreadId): Promise<boolean> => {
      ...
      if (isThreadRunningTurn(thread)) {
        toastManager.add({
          type: "error",
          title: t("sidebar.archive.cannot"),
          description: t("sidebar.archive.stopRunning"),
        });
        return false;
      }
    ...
    [appSettings.sidebarThreadSortOrder, handleNewChat, navigate, routeThreadId, sidebarThreads, t],
  );
  ...
  const restoreArchivedThreadFromToast = useCallback(
    async (input: { threadId: ThreadId; returnToThreadOnUndo: boolean }): Promise<boolean> => {
      ...
      try {
        ...
        if (!currentThread) {
          toastManager.add({
            type: "error",
            title: t("sidebar.archive.restoreFailed"),
            description: t("sidebar.archive.threadMissing"),
          });
          return false;
        }
        try {
        ...
        toastManager.add({
          ...
          title: t("sidebar.archive.restoreFailed"),
          description:
            error instanceof Error ? error.message : t("sidebar.archive.restoreFallback"),
        });
        return false;
      } finally {
    ...
    [navigate, t, unarchiveArchivedThread],
  );
  ...
  const archiveThreadWithUndo = useCallback(
    async (threadId: ThreadId) => {
      try {
        ...
        toastManager.add({
          type: "error",
          title: t("sidebar.archive.failed"),
          description: error instanceof Error ? error.message : t("sidebar.archive.failedFallback"),
        });
      }
    },
    [archiveThread, routeThreadId, showArchiveUndoToast, t],
  );
  ...
  const confirmAndArchiveThread = useCallback(
    async (threadId: ThreadId) => {
      ...
      if (appSettings.confirmThreadArchive) {
        const api = readNativeApi();
        const confirmationMessage = t("sidebar.confirm.archiveThread", {
          title: thread.title,
        });
        const confirmed = api
          ? await api.dialogs.confirm(confirmationMessage)
    ...
    [appSettings.confirmThreadArchive, archiveThreadWithUndo, sidebarThreadSummaryById, t],
  );
  ...
  const archiveAllThreadsInProject = useCallback(
    async (projectId: ProjectId): Promise<void> => {
      ...
      if (projectThreads.length === 0) {
        toastManager.add({
          type: "info",
          title: t("sidebar.archive.nothing"),
          description: t("sidebar.archive.noThreads", { project: project.name }),
        });
        return;
      }
      ...
      if (archivableThreads.length === 0) {
        toastManager.add({
          type: "error",
          title: t("sidebar.archive.cannotThreads"),
          description: t("sidebar.archive.allRunning", { count: runningCount }),
        });
        return;
      }
      ...
      const archiveLines = [
        t("sidebar.archive.confirmProject", {
          count: archivableThreads.length,
          project: project.name,
        }),
        t("sidebar.archive.hiddenDescription"),
      ];
      if (runningCount > 0) {
        archiveLines.push("", t("sidebar.archive.runningSkipped", { count: runningCount }));
      }
      ...
      if (archivedCount > 0) {
        const skippedDescription =
          runningCount > 0 ? t("sidebar.archive.skipped", { count: runningCount }) : "";
        toastManager.add({
          type: failureCount > 0 ? "warning" : "success",
          title: t("sidebar.archive.archivedCount", { count: archivedCount }),
          description:
            failureCount > 0
              ? `${t("sidebar.archive.failedResult", { count: failureCount })}${skippedDescription ? ` ${skippedDescription}` : ""}`
              : runningCount > 0
                ? skippedDescription
                : t("sidebar.projectCleared", { project: project.name }),
        });
      } else if (failureCount > 0) {
        toastManager.add({
          type: "error",
          title: t("sidebar.archive.failedThreads"),
          description: t("sidebar.archive.failedCount", {
            count: failureCount,
            project: project.name,
          }),
        });
    ...
    [archiveThread, projectById, removeFromSelection, sidebarThreads, t],
  );
  ...
  const deleteProjectThreads = useCallback(
    async (
      ...
      if (projectThreads.length === 0) {
        if (options?.showEmptyToast ?? true) {
          toastManager.add({
            type: "info",
            title: t("sidebar.delete.nothing"),
            description: t("sidebar.delete.noThreads", { project: project.name }),
          });
        }
        return {
          deletedCount: 0,
      ...
      const deleteConfirmationMessage =
        options?.confirmMessage === undefined
          ? t("sidebar.delete.confirmProject", {
      ...
      if (options?.showResultToast ?? true) {
        if (deletedCount > 0) {
          toastManager.add({
            type: failureCount > 0 ? "warning" : "success",
            title: t("sidebar.delete.deletedCount", { count: deletedCount }),
            description:
              failureCount > 0
                ? t("sidebar.delete.failedResult", { count: failureCount })
                : t("sidebar.projectCleared", { project: project.name }),
          });
        } else if (failureCount > 0) {
          toastManager.add({
            type: "error",
            title: t("sidebar.delete.failedThreads"),
            description: t("sidebar.delete.failedCount", {
              count: failureCount,
              project: project.name,
            }),
          });
    ...
    [deleteThread, projectById, removeFromSelection, sidebarThreads, t],
  );
  ...
  const handleThreadContextMenu = useCallback(
    async (
      ...
      const handoffItems = handoffTargets.map((provider, index) => ({
        id: `handoff:${provider}`,
        label: t("sidebar.menu.handoffTo", { provider: PROVIDER_DISPLAY_NAMES[provider] }),
      ...
      const clicked = await api.contextMenu.show(
        [
          ...
          ...(threadStatus?.dismissible
            ? [{ id: "clear-notification", label: t("sidebar.menu.clearNotification") }]
            : []),
          { id: "mark-unread", label: t("sidebar.menu.markUnread") },
          ...handoffItems,
          { id: "copy-path", label: t("sidebar.menu.copyPath"), separatorBefore: true },
          ...(threadWorkspacePath
            ? [{ id: "open-path-in-terminal", label: t("sidebar.menu.openPathInTerminal") }]
            : []),
          { id: "copy-thread-id", label: t("sidebar.menu.copyThreadId") },
          ...(options?.extraItems ?? []),
          { id: "archive", label: t("common:actions.archive"), separatorBefore: true },
          { id: "delete", label: t("common:actions.delete"), destructive: true },
        ],
        position,
      );
      ...
      if (clicked === "copy-path") {
        if (!threadWorkspacePath) {
          toastManager.add({
            type: "error",
            title: t("sidebar.errors.pathUnavailable"),
            description: t("sidebar.errors.pathCopyUnavailable"),
          });
          return;
        }
      ...
      if (clicked === "open-path-in-terminal") {
        if (!threadWorkspacePath) {
          toastManager.add({
            type: "error",
            title: t("sidebar.errors.pathUnavailable"),
            description: t("sidebar.errors.pathOpenUnavailable"),
          });
          return;
        }
        ...
        try {
          ...
          toastManager.add({
            type: "error",
            title: t("sidebar.errors.openTerminal"),
            description:
              error instanceof Error ? error.message : t("sidebar.errors.openTerminalFallback"),
          });
        }
        return;
      }
    ...
    async (position: { x: number; y: number }) => {
      ...
      const clicked = await api.contextMenu.show(
        [
          ...
          { id: "archive", label: t("sidebar.menu.archiveCount", { count }) },
      ...
      if (clicked === "archive") {
        if (appSettings.confirmThreadArchive) {
          const confirmed = await api.dialogs.confirm(
            t("sidebar.archive.confirmSelected", { count }),
          );
          if (!confirmed) return;
        }
      ...
      if (appSettings.confirmThreadDelete) {
        const confirmed = await api.dialogs.confirm(t("sidebar.delete.confirmSelected", { count }));
        if (!confirmed) return;
      }
  ...
  const handleStartProjectRun = useCallback(
    async (projectId: ProjectId, commandOverride?: string) => {
      ...
      try {
        ...
        toastManager.add({
          type: "error",
          title: t("sidebar.run.failed", { project: project.name }),
          description: error instanceof Error ? error.message : t("sidebar.run.failedFallback"),
        });
      }
    },
    [
  ...
  const handleStopProjectRun = useCallback(
    async (projectId: ProjectId) => {
      ...
      try {
        ...
        toastManager.add({
          type: "error",
          title: t("sidebar.run.stopFailed"),
          description: error instanceof Error ? error.message : t("sidebar.run.stopFailedFallback"),
        });
      } finally {
        void queryClient.invalidateQueries({ queryKey: serverQueryKeys.localServers() });
      }
    },
    [queryClient, storeRemoveProjectRun, t],
  );

  const handleOpenProjectRunServer = useCallback(
    async (projectId: ProjectId) => {
      const api = readNativeApi();
      ...
      try {
        ...
        toastManager.add({
          type: "error",
          title: t("sidebar.run.openFailed", { server: localServerAddressLabel(server) }),
          description: error instanceof Error ? error.message : t("sidebar.run.openFailedFallback"),
        });
      }
    },
    [t],
    ...
    async (projectId: ProjectId, clicked: ProjectContextMenuId) => {
      ...
      if (clicked === "open-in-finder") {
        try {
          ...
          toastManager.add({
            type: "error",
            title: t("sidebar.errors.openFinder"),
            description:
              error instanceof Error ? error.message : t("sidebar.errors.openFinderFallback"),
          });
        }
        return;
      }
      ...
      const confirmed = await api.dialogs.confirm(
        projectThreads.length > 0
          ? t("sidebar.projectActions.confirmRemoveWithThreads", {
              project: project.name,
              count: projectThreads.length,
            })
          : t("sidebar.projectActions.confirmRemove", { project: project.name }),
      );
      if (!confirmed) return;
      ...
      try {
        ...
        if (deletionResult.failureCount > 0) {
          toastManager.add({
            type: "error",
            title: t("sidebar.projectActions.removeFailed", { project: project.name }),
            description: t("sidebar.delete.failedCount", {
        ...
        toastManager.add({
          type: "success",
          title: t("sidebar.projectActions.removed", { project: project.name }),
          description:
            deletionResult.deletedCount > 0
              ? t("sidebar.projectActions.removedWithThreads", {
                  count: deletionResult.deletedCount,
                })
              : t("sidebar.projectActions.removedDescription"),
        });
      } catch (error) {
        const message =
          error instanceof Error ? error.message : t("sidebar.projectActions.removeFallback");
  ...
  function renderThreadArchiveAction(
    ...
    return (
      <SidebarIconButton
        ...
        label={t("sidebar.archiveThread", { ns: "shell" })}
        title={t("sidebar.archiveThread", { ns: "shell" })}
          ...
          >
            ...
            <div className="flex min-w-0 flex-1 items-center gap-1.5 text-left">
              ...
              {!isSubagentThread && threadStatus?.label === "Pending Approval" ? (
                <span
                  aria-label={t("sidebar.pendingApproval", { ns: "shell" })}
            ...
            <div className="ml-auto flex shrink-0 items-center gap-1.5 pr-1">
              {canToggleSubagents ? (
                <button
                  ...
                  aria-label={t(
                    isExpanded ? "sidebar.subagents.collapse" : "sidebar.subagents.expand",
                    { countLabel: childCountLabel },
                  )}
              ...
              {showCompactMeta && isTemporaryThread && !thread.sidechatSourceThreadId ? (
                <Tooltip>
                  ...
                  <TooltipPopup side="top">
                    {t("sidebar.temporaryChat", { ns: "shell" })}
                  </TooltipPopup>
                </Tooltip>
              ) : null}
            </div>
        ...
        <PreviewCard>
          ...
          >
                ...
                <span
                  aria-label={
                    collapsedProjectStatus
                      ? t("sidebar.projectStatus", { status: collapsedProjectStatus.label })
                      : undefined
                  }
            ...
            <SidebarSectionToolbar placement="overlay" revealOnHover>
              <SidebarIconButton
                icon={TerminalIcon}
                label={t("sidebar.createTerminalIn", { project: project.name })}
                tooltip={
                  newTerminalThreadShortcutLabel
                    ? t("sidebar.newTerminalShortcut", {
              ...
              <SidebarIconButton
                icon={NewThreadIcon}
                label={t("sidebar.createThreadIn", { project: project.name })}
                tooltip={
                ...
                <SidebarMenuSubItem className="w-full">
                  <div className="flex w-full items-center gap-1">
                    {canShowMoreThreads && (
                      ...
                      >
                        <span>{t("actions.showMore", { ns: "common" })}</span>
                      </SidebarMenuSubButton>
                    )}
                    {canShowLessThreads && (
                      <SidebarMenuSubButton
                      ...
                      >
                        <span>{t("actions.showLess", { ns: "common" })}</span>
                      </SidebarMenuSubButton>
                    )}
                  </div>
                </SidebarMenuSubItem>
    ...
    (input: { title: string; description: string; state: DesktopUpdateState | null }) => {
      ...
      const fallbackProps = releaseUrl
        ? {
            ...
            actionProps: {
              children: t("sidebar.update.downloadManually"),
              onClick: () => {
      ...
      toastManager.add({
        type: "error",
        title: recommendManualDownload ? t("sidebar.update.downloadManuallyTitle") : input.title,
        description: recommendManualDownload
          ? t("sidebar.update.manualAfterFailures", {
              count: input.state?.installFailureCount ?? 0,
              version: input.state?.availableVersion ?? t("sidebar.update.theUpdate"),
            })
          : input.description,
        ...fallbackProps,
      });
    ...
    surfaceDesktopUpdateError({
      title:
        desktopUpdateState.errorContext === "install"
          ? t("sidebar.update.finishFailed")
          : t("sidebar.update.downloadFailed"),
      description: desktopUpdateState.message ?? t("sidebar.update.manualFallback"),
  ...
  const desktopUpdateTooltip = desktopUpdateState
    ...
    : t("sidebar.update.availableTitle");
  ...
  const searchPaletteActions = useMemo<SidebarSearchAction[]>(
    () => [
      {
        id: "new-chat",
        label: t("sidebar.paletteActions.newChat.label"),
        description: t("sidebar.paletteActions.newChat.description"),
        keywords: ["chat", "new", "home"],
        shortcutLabel: newChatShortcutLabel,
      },
      {
        id: "new-thread",
        label: t("sidebar.paletteActions.newThread.label"),
        description: t("sidebar.paletteActions.newThread.description"),
        keywords: ["thread", "new", "project"],
        shortcutLabel: newThreadShortcutLabel,
      },
      {
        id: "add-project",
        label: t("sidebar.paletteActions.addProject.label"),
        description: t("sidebar.paletteActions.addProject.description"),
        keywords: ["folder", "repo", "repository", "open"],
        shortcutLabel: addProjectShortcutLabel,
      },
      {
        id: "import-thread",
        label: t("sidebar.paletteActions.importThread.label"),
        description: t("sidebar.paletteActions.importThread.description"),
        keywords: [
          "import",
          "resume",
          "thread",
      ...
      {
        id: "settings",
        label: t("sidebar.paletteActions.settings.label"),
        description: t("sidebar.paletteActions.settings.description"),
        keywords: ["preferences", "config"],
      },
      {
        id: "usage-settings",
        label: t("sidebar.paletteActions.usage.label"),
        description: t("sidebar.paletteActions.usage.description"),
      ...
      void bridge
        ...
        .then((nextState) => {
          ...
          if (nextState.status === "available") {
            toastManager.add({
              type: "info",
              title: t("sidebar.update.preparing"),
              description: t("sidebar.update.preparingVersion", {
                version: nextState.availableVersion ?? t("sidebar.update.available"),
              }),
            });
            return;
          }
          ...
          if (nextState.status === "downloading") {
            toastManager.add({
              type: "info",
              title: t("sidebar.update.preparing"),
              description: t("sidebar.update.downloading"),
            });
            return;
          }
          ...
          if (nextState.status === "up-to-date") {
            toastManager.add({
              type: "info",
              title: t("sidebar.update.current"),
              description: t("sidebar.update.currentDescription", {
          ...
          if (nextState.status === "error") {
            surfaceDesktopUpdateError({
              title: t("sidebar.update.checkFailed"),
              description: nextState.message ?? t("sidebar.errors.generic"),
              state: nextState,
            });
          }
        })
      ...
      void bridge
        ...
        .then((result) => {
          ...
          if (alreadyCurrentNotice) {
            toastManager.add({
              type: "info",
              title: t("sidebar.update.alreadyCurrent"),
          ...
          surfaceDesktopUpdateError({
            title: t("sidebar.update.downloadFailed"),
        ...
        .catch((error) => {
          surfaceDesktopUpdateError({
            title: t("sidebar.update.downloadStartFailed"),
            description: error instanceof Error ? error.message : t("sidebar.errors.generic"),
            state: desktopUpdateState,
          });
        });
    ...
    if (desktopUpdateButtonAction === "install") {
      ...
      void bridge
        ...
        .then((result) => {
          ...
          if (!actionError) return;
          surfaceDesktopUpdateError({
            title: t("sidebar.update.installFailed"),
            description: actionError,
            state: result.state,
          });
        })
  ...
  return (
    <>
        ...
        {showArm64IntelBuildWarning && arm64IntelBuildWarningDescription ? (
          <SidebarGroup className="px-2 pt-2 pb-0">
            <Alert variant="warning" className="rounded-2xl border-warning/40 bg-warning/8">
              <TriangleAlertIcon />
              <AlertTitle>{t("sidebar.armWarning.title", { ns: "shell" })}</AlertTitle>
                ...
                <AlertAction>
                  ...
                  >
                    {desktopUpdateButtonAction === "download"
                      ? t("sidebar.armWarning.preparing")
                      : desktopUpdateButtonAction === "install"
                        ? t("sidebar.armWarning.update")
                        : t("sidebar.armWarning.check")}
                  </Button>
                </AlertAction>
              ) : null}
            </Alert>
                ...
                <SidebarMenu className="gap-0.5">
                  {isOnWorkspace ? (
                    <SidebarPrimaryAction
                      icon={TerminalIcon}
                      label={t("sidebar.actions.newWorkspace", { ns: "shell" })}
                    ...
                    <>
                      <SidebarPrimaryAction
                        icon={NewThreadIcon}
                        label={t("sidebar.actions.newTask", { ns: "shell" })}
                        onClick={handleCreateStudioChat}
                      />
                      <SidebarPrimaryAction
                        icon={BrainIcon}
                        label={t("aiDetector.nav", { ns: "work" })}
                        active={pathname === "/work/ai-writing-check"}
                        onClick={() => {
                          void navigate({ to: "/work/ai-writing-check" });
                        }}
                      />
                      <SidebarPrimaryAction
                        icon={SearchIcon}
                    ...
                    <>
                      <SidebarPrimaryAction
                        icon={NewThreadIcon}
                        label={t("sidebar.actions.newThread", { ns: "shell" })}
                      ...
                      <SidebarPrimaryAction
                        icon={ClockIcon}
                        label={t("sidebar.actions.automations", { ns: "shell" })}
              ...
              {isOnWorkspace ? (
                              ...
                              {(dragHandleProps) =>
                                isRenaming ? (
                                  ...
                                  <>
                                      ...
                                      {workspace.terminalStatus && (
                                        <span
                                          className={cn(
                                            ...
                                            workspace.terminalStatus.kind === "attention"
                                              ? "bg-amber-500 dark:bg-amber-300/90"
                                              : workspace.terminalStatus.kind === "running"
                                    ...
                                    <SidebarIconButton
                                      icon={Trash2}
                                      label={t("sidebar.actions.deleteWorkspace", { ns: "shell" })}
                                      glyph="meta"
                ...
                <SidebarGroup className="px-1.5 py-1.5">
                  ...
                  {renderListSectionHeader(
                    t("sidebar.views.work"),
                    <>
                      <SidebarIconButton
                        icon={NewThreadIcon}
                        label={t("sidebar.actions.newTask", { ns: "shell" })}
                        tooltip={t("sidebar.actions.newTask", { ns: "shell" })}
                        tooltipSide="top"
                        onClick={handleCreateStudioChat}
                      />
                      <ChatSortMenu
                  ...
                  <SidebarMenu ref={attachProjectListAutoAnimateRef} className="gap-1">
                    {studioChatThreadRows.length > 0 ? (
                      ...
                      <div className="px-2 pt-4 text-center text-[length:var(--app-font-size-ui,12px)] text-muted-foreground/58">
                        {threadsHydrated
                          ? t("sidebar.studio.noTasks")
                          : t("sidebar.studio.loading")}
                      </div>
                    )}
                  </SidebarMenu>
                </SidebarGroup>
                      ...
                      {standardProjects.length > 0 ? (
                        <SidebarIconButton
                          ...
                          label={
                            allProjectsExpanded
                              ? focusedProjectId
                                ? t("sidebar.projects.collapseExceptActive")
                                : t("sidebar.projects.collapseAll")
                          ...
                          tooltip={
                            allProjectsExpanded
                              ? focusedProjectId
                                ? t("sidebar.projects.collapseExceptActiveChat")
                                : t("sidebar.projects.collapseAll")
                              : t("sidebar.projects.expandAll")
                          }
                          tooltipSide="bottom"
                      ...
                      {!showManualPathInput ? (
                        ...
                        >
                          ...
                          <button
                            ...
                            aria-label={t("sidebar.addProject", { ns: "shell" })}
                          >
                            {isAddingProject ? "..." : "↵"}
                          </button>
                        </div>
            ...
            <div className="group/collapsible">
              <div className="group/project-header relative">
                ...
                <SidebarSectionToolbar placement="overlay" revealOnHover>
                  ...
                  <SidebarIconButton
                    icon={NewThreadIcon}
                    label={t("sidebar.openNewChatHome", { ns: "shell" })}
                    onClick={(event) => {
                      event.preventDefault();
                      event.stopPropagation();
                    ...
                    tooltip={
                      newChatShortcutLabel
                        ? t("sidebar.newChatShortcut", { shortcut: newChatShortcutLabel })
                        : t("sidebar.newChat")
                    }
                    tooltipSide="top"
                  />
              ...
              <div className="flex items-center gap-2">
                {!isOnSettings && (
                  ...
                  >
                    ...
                    <span>{t("sidebar.settings", { ns: "shell" })}</span>
                  </SidebarMenuButton>
                )}
          ...
          >
            <MenuGroup>
              ...
              >
                ...
                <span>{t("sidebar.projectMenu.openFinder", { ns: "shell" })}</span>
              </MenuItem>
              <MenuItem
                className={PROJECT_CONTEXT_MENU_ITEM_CLASS_NAME}
                onClick={() =>
              ...
              >
                ...
                <span>{t("sidebar.projectMenu.copyPath", { ns: "shell" })}</span>
              </MenuItem>
              <MenuSeparator />
              {projectContextMenuIsRunning ? (
                ...
                >
                  ...
                  <span>{t("sidebar.projectMenu.stopDev", { ns: "shell" })}</span>
                </MenuItem>
              ) : (
                <MenuItem
                  className={PROJECT_CONTEXT_MENU_ITEM_CLASS_NAME}
                ...
                >
                  ...
                  <span>{t("sidebar.projectMenu.startDev", { ns: "shell" })}</span>
                </MenuItem>
              )}
              {projectContextMenuHasOpenServer ? (
                ...
                >
                  ...
                  <span>{t("sidebar.projectMenu.openDevServer", { ns: "shell" })}</span>
                </MenuItem>
              ) : null}
              <MenuSeparator />
              <MenuItem
              ...
              >
                ...
                <span>{t("sidebar.projectMenu.editName", { ns: "shell" })}</span>
              </MenuItem>
              <MenuItem
                className={PROJECT_CONTEXT_MENU_ITEM_CLASS_NAME}
                onClick={() =>
              ...
              {projectContextMenuHasArchivableThreads ? (
                ...
                >
                  ...
                  <span>{t("sidebar.projectMenu.archiveThreads", { ns: "shell" })}</span>
                </MenuItem>
              ) : null}
              {projectContextMenuHasAnyThreads ? (
                ...
                >
                  ...
                  <span>{t("sidebar.projectMenu.deleteThreads", { ns: "shell" })}</span>
                </MenuItem>
              ) : null}
              <MenuSeparator />
              <MenuItem
              ...
              >
                ...
                <span>{t("sidebar.projectMenu.remove", { ns: "shell" })}</span>
              </MenuItem>
            </MenuGroup>
          </ComposerPickerMenuPopup>
        </Menu>
        ...
        <DialogPopup surface="solid" className="max-w-md">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2 text-base">
              ...
              {t("sidebar.projectMenu.startDev", { ns: "shell" })}
            </DialogTitle>
            <DialogDescription>
          ...
          <DialogPanel className="space-y-2">
            ...
            >
              {t("sidebar.runDialog.command", { ns: "shell" })}
            </label>
            <Input
              ...
              autoCapitalize="off"
              autoCorrect="off"
              placeholder={t("sidebar.projectMenu.devCommandPlaceholder", { ns: "shell" })}
              className="font-mono"
            ...
            {projectRunDialogCommandIsValid ? null : (
              <p className="text-[length:var(--app-font-size-ui-sm,11px)] text-destructive">
                {t("sidebar.runDialog.required", { ns: "shell" })}
              </p>
            )}
          </DialogPanel>
          <DialogFooter>
            ...
            >
              ...
              {t("sidebar.runDialog.run", { ns: "shell" })}
            </Button>
          </DialogFooter>
        </DialogPopup>
      </Dialog>
      ...
      <RenameDialog
        ...
        title={t("sidebar.renameProject.title", { ns: "shell" })}
        description={t("sidebar.renameProject.description", { ns: "shell" })}
        initialValue={
...
function SidebarSearchPaletteController(props: {
      ...
      return [
        {
          ...
          projectName:
            props.projectById.get(thread.projectId)?.name ?? t("sidebar.palette.unknownProject"),
          projectRemoteName:
            props.projectById.get(thread.projectId)?.remoteName ??
            t("sidebar.palette.unknownProject"),
  ...
  }, [props.projectById, sidebarDisplayThreads, t, threads]);
