...
describe("estimateTimelineMessageHeight", () => {
  const browserFinding: BrowserFindingDraft = {
    version: 1,
    id: "finding-1",
    imageId: "image-1",
    screenshotName: "finding.png",
    markerNumber: 1,
  ...
  it("counts a findings-only chip without double-counting its linked screenshot", () => {
    expect(
      estimateTimelineMessageHeight({
        role: "user",
        text: appendBrowserFindingsBlock("", [browserFinding]),
        attachments: [{ id: "image-1", name: "finding.png", type: "image" }],
      }),
    ).toBe(133);
  });

  it("counts the findings chip separately from visible user text", () => {
    expect(
      estimateTimelineMessageHeight({
        role: "user",
        text: appendBrowserFindingsBlock("Please fix this", [browserFinding]),
        attachments: [{ id: "image-1", name: "finding.png", type: "image" }],
      }),
    ).toBe(152.5);
  });
