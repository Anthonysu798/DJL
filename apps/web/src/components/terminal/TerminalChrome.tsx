    ...
    <div className="flex min-h-9 min-w-0 items-center gap-1 bg-[var(--color-background-surface)] px-1.5 py-1">
      <div className="flex min-w-0 flex-1 items-center gap-1 overflow-x-auto [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
        {props.terminalGroups.map((terminalGroup) => {
          ...
          const groupTitle = visualIdentity?.title ?? t("terminal.name");
          const closeTabLabel = t("terminal.closeNamed", { name: groupTitle });
          return (
            <SurfaceTabChip
            ...
            <div key={terminalGroup.id} className="pb-0.5">
              ...
              >
                {terminalGroup.terminalIds.map((terminalId) => {
                  ...
                  const terminalTitle = visualIdentity?.title ?? t("terminal.name");
