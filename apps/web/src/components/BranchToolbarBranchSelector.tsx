...
import { formatLocaleNumber, useSelectedLocale } from "~/i18n/intl";
...
type StashDiscardDialogState = {
  cwd: string;
  error: string | null;
  errorKey: "nativeUnavailable" | "loadStash" | null;
...
export function formatDirtyWorktreeDescription(t: TFunction<"workspace">, files: string[]): string {
  if (files.length <= 3) {
    return t("branch.errors.dirtyFiles", { files: files.join(", "), count: files.length });
  }
  const remaining = files.length - 2;
  return t("branch.errors.dirtyMany", {
    files: files.slice(0, 2).join(", "),
    count: remaining,
  });
}

export function resolveGitIndexLockPath(error: unknown): string {
  return parseGitIndexLockError(error)?.lockPath ?? ".git/index.lock";
}
...
function handleCheckoutError(
  ...
  const addGitIndexLockToast = (error: unknown): void => {
    ...
    if (!lockError) return;
    const lockFileLabel = resolveGitIndexLockPath(error);
    addBranchRecoveryToast({
      type: "error",
      title: input.t("branch.errors.indexLocked"),
      description: input.t("branch.errors.indexLockedDescription", { path: lockFileLabel }),
      data: { copyText: toBranchActionErrorMessage(error) },
      actionProps: {
        children: input.t("branch.actions.removeLockRetry"),
        onClick: () => {
  ...
  const addGitIndexWriteToast = (error: unknown): void => {
    addBranchRecoveryToast({
      type: "error",
      title: input.t("branch.errors.indexWrite"),
      description: input.t("branch.errors.indexWriteDescription"),
      data: { copyText: toBranchActionErrorMessage(error) },
      actionProps: {
        children: input.t("branch.actions.retryStashSwitch"),
        onClick: () => {
          input.runBranchAction(async () => {
            try {
              await retryStashAndCheckout();
  ...
  if (dirtyWorktree) {
    ...
    const dirtyToastId = addBranchRecoveryToast({
      ...
      data: { copyText },
      actionProps: {
        children: input.t("branch.actions.stashSwitch"),
        onClick: () => {
          closeActiveBranchRecoveryToast();
          input.runBranchAction(async () => {
            try {
              ...
              if (isStashConflictError(stashError)) {
                ...
                const stashConflictToastId = addBranchRecoveryToast({
                  ...
                  actionProps: {
                    children: input.t("branch.actions.discardStash"),
                    className:
              ...
              if (parseDirtyWorktreeError(stashError)) {
                addBranchRecoveryToast({
                  type: "error",
                  title: input.t("branch.errors.cannotSwitch"),
                  description: input.t("branch.errors.uncoveredFiles"),
                  data: { copyText: toBranchActionErrorMessage(stashError) },
                });
                return;
              }
              addBranchRecoveryToast({
                type: "error",
                title: input.t("branch.errors.stashSwitchFailed"),
  ...
  addBranchRecoveryToast({
    ...
    title: isUnresolvedIndexError(error)
      ? input.t("branch.errors.unresolvedConflicts")
      : input.fallbackTitle,
...
function getBranchTriggerLabel(
  ...
  if (!resolvedActiveBranch) {
    return t("branch.select");
  }
  if (effectiveEnvMode === "worktree" && !activeWorktreePath) {
    return t("branch.fromNamed", { branch: resolvedActiveBranch });
  }
  return resolvedActiveBranch;
}

function getCreateBranchActionLabel(t: TFunction<"workspace">, trimmedBranchQuery: string): string {
  return trimmedBranchQuery.length > 0
    ? t("branch.createCheckoutNamed", { branch: trimmedBranchQuery })
    : t("branch.createCheckoutNew");
}
...
export function BranchToolbarBranchSelector({
  ...
  const openStashDiscardDialog = useCallback((input: { cwd: string }) => {
    ...
    setStashDiscardDialog({
      cwd: input.cwd,
      error: null,
      errorKey: api ? null : "nativeUnavailable",
      info: null,
      loading: Boolean(api),
    });
    if (!api) return;
      ...
      (info) => {
        setStashDiscardDialog((current) =>
          current?.cwd === input.cwd
            ? { ...current, error: null, errorKey: null, info, loading: false }
            : current,
        );
      },
      (error) => {
    ...
    runBranchAction(async () => {
      ...
      try {
        ...
        handleCheckoutError(error, {
          ...
          fallbackTitle: t("branch.errors.checkoutFailed"),
          onSuccess: () => {
            setOptimisticBranch(selectedBranchName);
            onSetThreadWorkspace({
  ...
  const createBranch = (rawName: string) => {
    ...
    runBranchAction(async () => {
      ...
      try {
        ...
        toastManager.add({
          type: "error",
          title: t("branch.errors.createFailed"),
          description: toBranchActionErrorMessage(error),
        });
        return;
      }
  ...
  const triggerLabel = getBranchTriggerLabel(t, {
    activeWorktreePath,
    effectiveEnvMode,
  ...
  function renderPickerItem(itemValue: string, index: number, style?: CSSProperties) {
      ...
      return (
        ...
        >
          <div className="flex min-w-0 flex-col items-start py-1">
            <span className="truncate font-medium">{t("pullRequest.title")}</span>
    ...
    const badge = branch.current
      ? t("branch.badges.current")
      : hasSecondaryWorktree
        ? t("branch.badges.worktree")
        : branch.isRemote
          ? t("branch.badges.remote")
          : branch.isDefault
            ? t("branch.badges.default")
            : null;
    return (
      <ComboboxItem
        hideIndicator
            ...
            {currentBranchChangeSummary ? (
              <div className="mt-0.5 flex flex-wrap items-center gap-1.5 text-[11px] leading-4">
                <span className="text-muted-foreground">
                  {t("branch.uncommittedFiles", {
                    count: currentBranchChangeSummary.fileCount,
                  })}
                </span>
                <span className="font-mono tabular-nums text-success">
                  +{formatLocaleNumber(currentBranchChangeSummary.insertions, locale)}
                </span>
                <span className="font-mono tabular-nums text-destructive">
                  -{formatLocaleNumber(currentBranchChangeSummary.deletions, locale)}
                </span>
              </div>
            ) : null}
          </div>
    ...
    >
        ...
        {!isSelectingWorktreeBase ? (
          <div className="border-t border-[color:var(--color-border-light)] p-1">
            ...
            >
              ...
              <span className="truncate">{getCreateBranchActionLabel(t, trimmedBranchQuery)}</span>
            </button>
          </div>
        ) : null}
      </ComboboxPopup>
          ...
          <DialogPanel className="space-y-3">
            ...
            >
              ...
              {branchByName.has(createBranchName.trim()) ? (
                <p className="text-destructive text-sm">{t("branch.alreadyExists")}</p>
              ) : null}
      ...
      >
        <DialogPopup className="max-w-xl">
          <DialogHeader>
            <DialogTitle>{t("branch.discardTitle")}</DialogTitle>
            <DialogDescription>{t("branch.discardDescription")}</DialogDescription>
          </DialogHeader>
          <DialogPanel className="space-y-4">
            {stashDiscardDialog?.loading ? (
              <p className="text-muted-foreground text-sm">{t("branch.loadingStash")}</p>
            ) : stashDiscardDialog?.error || stashDiscardDialog?.errorKey ? (
              <p className="rounded-md border border-destructive/30 bg-destructive/5 px-3 py-2 text-destructive text-sm">
                <span>{t(`branch.errors.${stashDiscardDialog.errorKey ?? "loadStash"}`)}</span>
              ...
              <>
                <div className="grid gap-2 rounded-lg border border-[color:var(--color-border-light)] bg-[var(--color-background-elevated-secondary)] p-3 text-sm">
                  <div className="flex min-w-0 gap-2">
                    <span className="w-20 shrink-0 text-muted-foreground">{t("branch.name")}</span>
                    <span className="min-w-0 truncate font-medium">
                      {stashDiscardDialog.info.branch ??
                        currentGitBranch ??
                        t("branch.detachedHead")}
                    </span>
                  </div>
                  <div className="flex min-w-0 gap-2">
                  ...
                  <div className="flex min-w-0 gap-2">
                    <span className="w-20 shrink-0 text-muted-foreground">{t("branch.stash")}</span>
                    <span className="min-w-0 truncate font-mono text-xs">
                      {stashDiscardDialog.info.stashRef}
                    </span>
                  </div>
                  <div className="flex min-w-0 gap-2">
                    <span className="w-20 shrink-0 text-muted-foreground">{t("scripts.name")}</span>
                    <span className="min-w-0 truncate">{stashDiscardDialog.info.message}</span>
                  </div>
                </div>
                <div className="space-y-2">
                  <p className="font-medium text-sm">
                    {t("branch.changedFiles", { count: stashDiscardDialog.info.files.length })}
                  </p>
          ...
          <DialogFooter>
            ...
            >
              {isDroppingStash ? t("branch.actions.discarding") : t("branch.actions.discardStash")}
            </Button>
          </DialogFooter>
        </DialogPopup>
      </Dialog>
