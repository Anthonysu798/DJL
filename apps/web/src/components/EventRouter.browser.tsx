...
async function mountApp(options?: {
  ...
  await vi.waitFor(
    () => {
      expect(wsClient, "WebSocket fixture should be connected").not.toBeNull();
      ...
      const hydratedThread = getThreadFromState(useStore.getState(), expectedThreadId);
      expect(hydratedThread).not.toBeNull();
      expect(hydratedThread?.messages).toHaveLength(expectedThread?.messages.length ?? 0);
...
describe("EventRouter scoped orchestration sync", () => {
  ...
  it("replays a missed Work task transition for the active thread", async () => {
    const planningTask = {
      threadId: THREAD_ID,
      phase: "planning" as const,
      condition: "active" as const,
      status: "planning" as const,
      resumePhase: "planning" as const,
      progress: 10,
      statusReason: null,
    ...
    fixture = {
      ...fixture,
      snapshot: createSnapshot({ workTask: planningTask }),
    };
    const mounted = await mountApp();
    ...
    try {
      const transition = {
        sequence: 2,
        eventId: EventId.makeUnsafe("event-replay-work-needs-review"),
        aggregateKind: "thread",
        aggregateId: THREAD_ID,
        occurredAt: "2026-03-04T12:00:05.000Z",
        commandId: CommandId.makeUnsafe("work:event-replay-work-needs-review:submit_review"),
        causationEventId: null,
        correlationId: null,
        metadata: {},
        ...
        payload: {
          ...
          task: {
            ...
            resumePhase: "review",
            progress: 90,
            lastTransitionCommandId: CommandId.makeUnsafe(
              "work:event-replay-work-needs-review:submit_review",
            ),
            updatedAt: "2026-03-04T12:00:05.000Z",
          },
        },
      } satisfies Extract<OrchestrationEvent, { type: "thread.work-task-transitioned" }>;
      replayEvents = [transition];
      ...
      await vi.waitFor(
        () => {
          expect(getThreadFromState(useStore.getState(), THREAD_ID)?.workTask?.status).toBe(
            "needs_review",
          );
        },
        { timeout: 4_000, interval: 16 },
