          ...
          >
            {DIFF_PANEL_PICKER_SCOPE_OPTIONS.map((scope) => (
              <MenuRadioItem key={scope} value={scope}>
                <span className="min-w-0 flex-1 truncate">{t(`diff.scope.${scope}`)}</span>
                ...
                return (
                  <MenuRadioItem key={summary.turnId} value={summary.turnId}>
                    <span className="min-w-0 flex-1 truncate">
                      {t("diff.turnNumber", { number: turnNumber })}
                    </span>
                    <span className="shrink-0 text-[10px] text-muted-foreground tabular-nums">
                      {formatShortTimestamp(summary.completedAt, props.timestampFormat, locale)}
                    </span>
                  </MenuRadioItem>
                );
              })}
...
export default function DiffPanel({
  ...
  const gitRepoStatusError = gitBranchesQuery.error ? t("diff.errors.checkRepository") : null;
      ...
      activeThreadId
        ? {
            onReferenceInChat: (filePath: string) => {
              appendChatFileReference(activeThreadId, { path: filePath }, t);
            },
            onAskWhyChanged: (filePath: string) => {
              appendComposerPromptText(activeThreadId, buildWhyChangedPrompt(filePath, t));
            },
          }
        : undefined,
    [activeThreadId, t],
  ...
  const commitDiffSelection = useCallback(
    (payload: { filePath: string; text: string }) => {
      if (activeThreadId) {
        appendComposerPromptText(
          activeThreadId,
          buildDiffSelectionReference(payload.filePath, payload.text, t),
        );
      }
    },
    [activeThreadId, t],
  ...
  return (
    <DiffPanelShell mode={mode} header={shellHeader}>
      {!activeThreadContext ? (
        ...
        <PanelStateMessage density="compact" fill="flex">
          {t("diff.notRepository")}
        </PanelStateMessage>
      ) : gitRepoStatusError ? (
        <PanelStateMessage density="compact" fill="flex">
          <p>{gitRepoStatusError}</p>
          {gitRepoStatusErrorDetail ? (
            <pre className="whitespace-pre-wrap text-[10px]">{gitRepoStatusErrorDetail}</pre>
          ) : null}
        </PanelStateMessage>
      ) : gitRepoStatus === undefined && diffQueriesEnabled && activeCwd ? (
        <DiffPanelLoadingState label={t("diff.checkingRepository")} />
      ) : diffEnvironmentPending ? (
        <PanelStateMessage density="compact" fill="flex">
          {t("diff.environmentPending")}
        </PanelStateMessage>
      ) : (
        <div className="flex min-h-0 min-w-0 flex-1 overflow-hidden">
          ...
          >
            <DiffPanelPatchViewport
              ...
              loadingLabel={
                diffViewKind === "repo"
                  ? t("diff.loadingScope", { scope: t(`diff.scope.${repoDiffScope}`) })
              ...
              unavailableLabel={t("diff.unavailable")}
            />
