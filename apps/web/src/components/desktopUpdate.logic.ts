...
const updateCopy = (key: string, defaultValue: string) =>
  translateRendererCopy(`shell:sidebar.updateButton.${key}`, defaultValue);
...
export function getDesktopUpdateButtonPresentation(
  ...
  if (options?.installing) {
    return {
      label: updateCopy("updating", "Updating..."),
      secondaryLabel: null,
    };
  }
  ...
  if (!state) {
    return {
      label: updateCopy("action", "Update"),
      secondaryLabel: null,
    };
  }
  ...
  if (state.status === "checking") {
    return {
      label: updateCopy("checking", "Checking..."),
      secondaryLabel: null,
    };
  }
  ...
  if (state.status === "downloading") {
    return {
      label: updateCopy("preparingAction", "Preparing"),
      secondaryLabel: null,
    };
  }
  ...
  if (action === "check") {
    return {
      label: updateCopy("check", "Check updates"),
      secondaryLabel: null,
    };
  }
  return {
...
export function getArm64IntelBuildWarningDescription(state: DesktopUpdateState): string {
  ...
  const action = resolveDesktopUpdateButtonAction(state);
  if (action === "download") {
    return "This Mac has Apple Silicon, but DJL is still running the Intel build under Rosetta. DJL is preparing the native Apple Silicon update.";
  }
  if (action === "install") {
    return "This Mac has Apple Silicon, but DJL is still running the Intel build under Rosetta. Click Update to restart into the native Apple Silicon build.";
  }
  return "This Mac has Apple Silicon, but DJL is still running the Intel build under Rosetta. The next app update will replace it with the native Apple Silicon build.";
}
...
export function getDesktopUpdateButtonTooltip(
  ...
  if (state.errorContext === "install" && !state.downloadedVersion && state.availableVersion) {
    return `DJL restarted, but update ${state.availableVersion} was not installed. Click to try again.`;
  }
