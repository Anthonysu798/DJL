...
describe("normalizeComposerBrowserFindings", () => {
  it("normalizes legacy and mocked drafts that predate browser findings", () => {
    expect(normalizeComposerBrowserFindings(undefined)).toEqual([]);
  });
});
...
describe("shouldShowComposerModelBootstrapSkeleton", () => {
  it("keeps the real controls visible while a provider refreshes runtime-discovered models", () => {
    expect(
      shouldShowComposerModelBootstrapSkeleton({
  ...
  it("keeps controls visible while provider discovery resolves a persisted thread model", () => {
    expect(
      shouldShowComposerModelBootstrapSkeleton({
  ...
  it("keeps controls visible when the provisional provider differs from persisted state", () => {
    expect(
      shouldShowComposerModelBootstrapSkeleton({
