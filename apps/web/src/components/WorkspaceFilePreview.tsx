...
import { NativeDocumentPreview } from "./work/NativeDocumentPreview";
...
function FilePreviewLoadingState() {
  ...
  return (
    <div
      ...
      role="status"
      aria-label={t("preview.loadingFile")}
    >
      ...
      <span className="sr-only">{t("preview.loadingFile")}</span>
    </div>
  );
}
...
export function WorkspaceFilePreview(props: WorkspaceFilePreviewProps) {
  ...
  const fileIsDocx = filePath !== null && lowerCaseExtensionOf(filePath) === ".docx";
  const fileIsPptx = filePath !== null && lowerCaseExtensionOf(filePath) === ".pptx";
  const fileIsOfficePreview = fileIsDocx || fileIsPptx;
  ...
  const fileNeedsLocalPreviewGrant =
    filePath !== null && fileIsLocalAbsolute && !fileIsScratchBinaryPreview && !fileIsOfficePreview;
  ...
  const handleContentsContextMenu = useCallback(
    ...
    [onAskWhyInChat, onReferenceInChat, filePath, readPreviewSelection, t],
  );
    ...
    if (localPreviewGrantQuery.error) {
      return (
        <PanelStateMessage density="compact" fill="flex" className="items-start justify-start p-3">
          <p className="text-left text-[11px] font-medium text-destructive/85">
            {t("preview.grantError")}
          </p>
          {localPreviewGrantQuery.error instanceof Error ? (
            <pre className="max-w-full whitespace-pre-wrap text-left text-[10px] text-destructive/75">
  ...
  if (fileIsOfficePreview) {
    ...
    return (
      <NativeDocumentPreview
        ...
        filePath={filePath}
        sourceType={fileIsPptx ? "pptx" : "docx"}
      ...
      {fileIsImage ? (
        ...
        >
          ...
          {!showMarkdownPreview && lineCount > 0 ? (
            <span className="sr-only">{t("preview.lineCount", { count: lineCount })}</span>
          ) : null}
          ...
          {lineCommentingEnabled && hoveredCommentLine && !activeCommentLine ? (
            <button
              ...
              aria-label={t("preview.commentOnLine", {
                number: hoveredCommentLine.lineNumber,
              })}
