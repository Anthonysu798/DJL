...
export default function WorkspaceView({ workspaceId }: { workspaceId: string }) {
  ...
  const [draftTitle, setDraftTitle] = useState(workspace?.title ?? t("title"));
  ...
  return (
    <RouteInsetSurface>
          ...
          <div className={cn("flex items-center gap-2 sm:gap-3", CHAT_SURFACE_HEADER_HEIGHT_CLASS)}>
              ...
              {renaming ? (
                <input
                  ...
                  onKeyDown={(event) => {
                    ...
                    if (event.key === "Escape") {
                      event.preventDefault();
                      setDraftTitle(workspace?.title ?? t("title"));
                      setRenaming(false);
                    }
                  }}
                ...
                <h2
                  ...
                  title={t("actions.doubleClickRename")}
            ...
            <div className="flex shrink-0 items-center gap-1.5 [-webkit-app-region:no-drag]">
              ...
              >
                ...
                <span className="hidden sm:inline">{t("terminal.name")}</span>
              </Button>
              <Button
                size="icon-xs"
                variant="outline"
          ...
          {!homeDir ? (
            <div className="flex h-full items-center justify-center px-6">
              <div className="max-w-sm text-center">
                <div className="text-sm font-medium text-foreground/85">{t("loading.title")}</div>
                <div className="mt-1 text-sm text-muted-foreground">
                  {t("loading.homeDirectory")}
                </div>
              </div>
            </div>
          ) : terminalState.terminalOpen ? (
            ...
            <div className="flex h-full items-center justify-center px-6">
              <div className="max-w-sm rounded-3xl border border-border/70 bg-card/40 p-6 text-center shadow-sm">
                <div className="text-base font-medium text-foreground/88">
                  {t("empty.noTerminalsTitle")}
                </div>
                <div className="mt-2 text-sm leading-6 text-muted-foreground">
                  {t("empty.noTerminalsSummary")}
                </div>
                <div className="mt-5">
      ...
      <WorkspaceSettingsSheet
        ...
        workspaceTitle={workspace?.title ?? t("title")}
      />
    </RouteInsetSurface>
  );
}
