...
export function normalizeComposerBrowserFindings(
  browserFindings: BrowserFindingDraft[] | undefined,
): BrowserFindingDraft[] {
  ...
  return {
    ...
    title:
      draftThread.entryPoint === "terminal"
        ? translateRendererCopy("chat:draft.newTerminal", "New terminal")
        : translateRendererCopy("chat:draft.newThread", "New thread"),
...
export function describeVoiceRecordingStartError(error: unknown): string {
  ...
  if (errorName === "NotAllowedError" || errorName === "PermissionDeniedError") {
    return "Microphone access was denied. Enable it in macOS Privacy & Security > Microphone for DJL, then try again.";
  }
...
export function shouldShowComposerModelBootstrapSkeleton(input: {
  ...
  // Discovery progress belongs inside the picker menu. The footer controls themselves stay
  // mounted so surface switches never replace their labels with a spinner or skeleton.
  void input;
  return false;
}
...
function baseWorktreeSetupStepDefinitions(): ReadonlyArray<{
  ...
  return [
    {
      id: "create-worktree",
      label: translateRendererCopy("chat:worktreeSetup.creating", "Creating branch and worktree"),
    },
    {
      id: "prepare-thread",
      label: translateRendererCopy("chat:worktreeSetup.linking", "Linking thread workspace"),
    },
    {
      id: "start-session",
      label: translateRendererCopy("chat:worktreeSetup.starting", "Starting session"),
    },
  ];
}
...
function worktreeSetupStepDefinitions(
  ...
  if (!includeSetupStep) {
    return baseWorktreeSetupStepDefinitions();
  }
  return [
    {
      id: "create-worktree",
    ...
    {
      id: "run-setup-action",
      label: setupScriptName
        ? translateRendererCopy(
            "chat:worktreeSetup.runningNamed",
            "Running setup action: {{name}}",
            {
              name: setupScriptName,
            },
          )
        : translateRendererCopy("chat:worktreeSetup.running", "Running setup action"),
    },
    {
      id: "start-session",
      label: translateRendererCopy("chat:worktreeSetup.starting", "Starting session"),
...
export function buildExpiredTerminalContextToastCopy(
  ...
  if (variant === "empty") {
    return {
      title: translateRendererCopy(
        "chat:terminalContext.expiredNotSent",
        count === 1
          ? "Expired terminal context won't be sent"
          : "Expired terminal contexts won't be sent",
        { count },
      ),
      description: translateRendererCopy(
        "chat:terminalContext.expiredNotSentDescription",
  ...
  return {
    title: translateRendererCopy(
      "chat:terminalContext.expiredOmitted",
      count === 1
    ...
    description: translateRendererCopy(
      "chat:terminalContext.expiredOmittedDescription",
...
function deriveSubagentStatus(thread: Thread | undefined): {
  ...
  if (thread.error || thread.session?.status === "error") {
    return {
      isActive: false,
      label: translateRendererCopy("chat:subagent.status.error", "Error"),
    };
  }
  if (thread.session?.status === "connecting") {
    return {
      isActive: true,
      label: translateRendererCopy("chat:subagent.status.connecting", "Connecting"),
    };
  }
  if (
    ...
    return {
      isActive: true,
      label: translateRendererCopy("chat:subagent.status.running", "Running"),
    };
  }
  if (thread.session?.status === "closed") {
    return {
      isActive: false,
      label: translateRendererCopy("chat:subagent.status.closed", "Closed"),
    };
  }

  return {
    isActive: false,
    label: thread.session ? translateRendererCopy("chat:subagent.status.idle", "Idle") : undefined,
  };
}
