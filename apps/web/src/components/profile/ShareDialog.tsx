...
export function ShareDialog({
  ...
  const handleCopy = useCallback(async () => {
    ...
    try {
      ...
      setStatus(t(`profile.share.status.copy.${copyResult}`));
    } finally {
      setBusy(null);
    }
  ...
  const handleShare = useCallback(
    async (target: ShareTarget) => {
      ...
      try {
        const copyResult = await copyCardToClipboard();
        openExternalUrl(shareIntentUrl(target, t));
        setStatus(t(`profile.share.status.share.${copyResult}`));
      } finally {
        setBusy(null);
      }
    },
  ...
  const handleSave = useCallback(async () => {
    ...
    try {
      ...
      if (blob) {
        ...
        setStatus(t("profile.share.status.saved"));
      } else {
        setStatus(t("profile.share.status.renderFailed"));
      }
    } finally {
      setBusy(null);
    }
      ...
      <DialogPopup surface="solid" className="sm:max-w-[560px]">
        ...
        <div className="mt-5 flex flex-col items-center gap-7 px-2 pb-3">
          ...
          <div className="flex flex-wrap items-start justify-center gap-x-6 gap-y-4">
            <ShareButton
              label={t("actions.copy", { ns: "common" })}
              ariaLabel={t("profile.share.copyAriaLabel")}
              busy={busy === "copy"}
            ...
            <ShareButton
              ...
              ariaLabel={t("profile.share.saveAriaLabel")}
              busy={busy === "save"}
              disabled={actionsDisabled}
  ...
  return (
    <div className="flex flex-col items-center gap-2">
      <button
        ...
        onClick={onClick}
        disabled={disabled}
        aria-label={ariaLabel ?? t("profile.share.shareTo", { target: label })}
        className={cn(
