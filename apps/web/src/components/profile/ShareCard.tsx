...
import { formatCompact } from "./profileFormatting";
  ...
  const tiles: Tile[] = [
    {
      ...
      value: (
        <span className={VALUE_CLASS}>
          {formatCompact(tokenStats?.lifetimeTotalTokens ?? null, locale)}
        </span>
      ),
    ...
    },
    {
      ...
      value: (
        <span className={VALUE_CLASS}>
          {formatCompact(tokenStats?.peakDayTokens ?? null, locale)}
        </span>
      ),
    ...
    },
    {
      ...
      value: (
        <span className={VALUE_CLASS}>
          {t("profile.stats.dayCount", { count: stats.activity.currentStreakDays })}
        </span>
      ),
    ...
    },
    {
      ...
      value: (
        <span className={VALUE_CLASS}>
          {t("profile.stats.dayCount", { count: stats.activity.longestStreakDays })}
        </span>
      ),
    ...
    },
    {
      ...
      label: t("profile.stats.topProvider"),
    },
  ];
    ...
    >
      ...
      <div className="flex min-w-0 items-center justify-between gap-6">
        ...
        <div className="flex shrink-0 items-center gap-2 text-slate-600">
          <DjlLogo className="size-6 text-slate-700" />
          <span className="text-xl font-normal tracking-tight">DJL</span>
        </div>
      </div>
