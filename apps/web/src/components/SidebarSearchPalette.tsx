...
function createThemeCommandItem(
  ...
  if (mode === "system") {
    return {
      ...
      labelKey: "sidebar.palette.theme.system",
      descriptionKey: "sidebar.palette.theme.systemDescription",
      mode,
  ...
  return {
    ...
    labelKey: `sidebar.palette.theme.${mode}`,
    descriptionKey: `sidebar.palette.theme.${mode}Description`,
    mode,
    isActive: activeMode === mode,
  };
}
...
function threadMatchLabel(input: {
  ...
  if (input.matchKind === "message") {
    return input.t("sidebar.search.chatHitCount", { count: input.messageMatchCount });
  }
  if (input.matchKind === "project") {
    return input.t("sidebar.search.projectMatch");
  }
  return null;
}
...
export function SidebarSearchPalette(props: SidebarSearchPaletteProps) {
  ...
  const [importError, setImportError] = useState<{ key: string; detail?: string } | null>(null);
  ...
  const currentCodeThemeItems = useMemo<SidebarSearchTheme[]>(
    () =>
      getAvailableCodeThemes(resolvedTheme).map((option) => ({
        ...
        type: "code-theme",
        label: option.label,
        description: t("sidebar.palette.theme.codeDescription", {
          ns: "shell",
          theme: t(`sidebar.palette.theme.names.${resolvedTheme}`, { ns: "shell" }),
        }),
  ...
  const importFieldLabel =
    importProvider === "codex"
      ? t("sidebar.palette.threadId", { ns: "shell" })
      : t("sidebar.palette.sessionId", { ns: "shell" });
  const importPlaceholder =
    importProvider === "claudeAgent"
      ? t("sidebar.palette.pasteSession", { ns: "shell", provider: "Claude" })
      : importProvider === "cursor"
        ? t("sidebar.palette.pasteSession", { ns: "shell", provider: "Cursor" })
        : importProvider === "kilo"
          ? t("sidebar.palette.pasteSession", { ns: "shell", provider: "Kilo" })
          : importProvider === "opencode"
            ? t("sidebar.palette.pasteSession", { ns: "shell", provider: "DJL" })
            : t("sidebar.palette.pasteThread", { ns: "shell", provider: "Codex" });
  ...
  const browseSubmitLabel = willCreateMissingFolder
    ? t("sidebar.palette.createAndAdd", { ns: "shell" })
  ...
  const submitBrowsePath = async () => {
    ...
    if (trimmedQuery.length === 0 && !highlightedFolderPath) {
      setAddProjectError({ key: "sidebar.palette.errors.enterPath" });
      return;
    }
    if (unsupportedWindowsPath) {
      setAddProjectError({ key: "sidebar.palette.errors.windowsPath" });
      return;
    }
    if (!highlightedFolderPath && isExplicitRelativeProjectPath(trimmedQuery)) {
      setAddProjectError({ key: "sidebar.palette.errors.relativePath" });
      return;
    }
    ...
    try {
      ...
      setAddProjectError({
        key: "sidebar.palette.errors.addProject",
  ...
  const submitModifierLabel = isMac ? "⌘" : t("keyboard.control", { ns: "shell" });
  const enterKeyLabel = t("keyboard.enter", { ns: "shell" });
  ...
  const submitImport = async () => {
    ...
    try {
      ...
      setImportError({
        key: "sidebar.palette.errors.importThread",
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  <p className="text-sm font-medium text-foreground">
                    {t("sidebar.palette.importTitle", { ns: "shell" })}
                  </p>
                  <p className="mt-1 text-xs text-muted-foreground">
                    {t("sidebar.palette.importDescription", { ns: "shell" })}
                  </p>
                </div>
              </div>
            </div>
            <div className="space-y-4 px-4 py-4">
              <div className="space-y-2">
                <p className="text-xs font-medium text-muted-foreground">
                  {t("sidebar.palette.provider", { ns: "shell" })}
                </p>
                ...
                {props.importProviders.length === 0 ? (
                  <p className="text-xs text-muted-foreground">
                    {t("sidebar.palette.noImportProviders", { ns: "shell" })}
                  </p>
                ) : null}
              </div>
              <div className="space-y-2">
                ...
                <p className="text-xs text-muted-foreground">
                  {importProvider === "claudeAgent"
                    ? t("sidebar.palette.resumeSession", { ns: "shell", provider: "Claude" })
                    : importProvider === "cursor"
                      ? t("sidebar.palette.resumeSession", { ns: "shell", provider: "Cursor" })
                      : importProvider === "kilo"
                        ? t("sidebar.palette.resumeSession", { ns: "shell", provider: "Kilo" })
                        : importProvider === "opencode"
                          ? t("sidebar.palette.resumeSession", {
                              ns: "shell",
                              provider: "DJL",
                            })
                          : t("sidebar.palette.resumeThread", {
                              ns: "shell",
                              provider: "Codex",
                            })}
                </p>
              ...
              {importError ? (
                <p className="rounded-md border border-destructive/20 bg-destructive/5 px-3 py-2 text-sm text-destructive">
                  {t(importError.key, { ns: "shell" })}
                  {importError.detail ? <span className="block">{importError.detail}</span> : null}
                </p>
              ) : null}
              <div className="flex justify-end gap-2">
                ...
                >
                  {isImporting
                    ? t("sidebar.palette.importing", { ns: "shell" })
                    : t("sidebar.palette.import", { ns: "shell" })}
                </Button>
              </div>
            </div>
          </div>
              ...
              <CommandPanel className="overflow-hidden">
                <div className="relative">
                  <CommandInput
                    placeholder={
                      isBrowsing
                        ? t("sidebar.palette.projectPathPlaceholder", { ns: "shell" })
                        : t("sidebar.palette.searchPlaceholder", { ns: "shell" })
                    }
                    value={query}
                  ...
                  {isBrowsing ? (
                    <Button
                      ...
                      title={
                        hasHighlightedFolderItem
                          ? t("sidebar.palette.submitHighlightedFolder", {
                              ns: "shell",
                              action: browseSubmitLabel,
                              modifier: submitModifierLabel,
                              enter: enterKeyLabel,
                            })
                          : t("sidebar.palette.submit", {
                              ns: "shell",
                              action: browseSubmitLabel,
                              enter: enterKeyLabel,
                            })
                    ...
                    >
                      ...
                      <KbdGroup className="pointer-events-none -me-0.5 items-center gap-1">
                        <Kbd>
                          {hasHighlightedFolderItem
                            ? t("sidebar.palette.modifiedEnter", {
                                ns: "shell",
                                modifier: submitModifierLabel,
                                enter: enterKeyLabel,
                              })
                ...
                <CommandList className="max-h-[min(24rem,60vh)] not-empty:px-1.5 not-empty:pt-0 not-empty:pb-1.5">
                    ...
                    unsupportedWindowsPath ? (
                      <CommandEmpty className="py-10">
                        <div className="text-center text-sm text-muted-foreground/79">
                          {t("sidebar.palette.errors.windowsPath", { ns: "shell" })}
                        </div>
                      </CommandEmpty>
                    ) : (
                      <>
                        {canBrowseUp || filteredBrowseEntries.length > 0 ? (
                          ...
                          <div className="px-3 py-2 text-sm text-muted-foreground">
                            {t("sidebar.palette.noMatchingFolders", { ns: "shell" })}
                          </div>
                        ) : null}
                        {willCreateMissingFolder ? (
                          <div className="mx-1.5 mt-2 rounded-md border border-dashed border-[color:var(--color-border)] px-3 py-2 text-sm text-muted-foreground">
                            {t("sidebar.palette.createProjectHint", {
                              ns: "shell",
                              path: trimmedQuery,
                            })}
                          </div>
                        ...
                        {addProjectError ? (
                          <div className="mx-1.5 mt-2 rounded-md border border-destructive/20 bg-destructive/5 px-3 py-2 text-sm text-destructive">
                            {t(addProjectError.key, { ns: "shell" })}
                            {addProjectError.detail ? (
                              <span className="block">{addProjectError.detail}</span>
                            ) : null}
                          </div>
                        ) : null}
                      </>
                  ...
                  {!isBrowsing && matchedActions.length > 0 ? (
                    <CommandGroup>
                      <CommandGroupLabel className="pt-0 pb-1.5 pl-3">
                        {t("sidebar.search.suggested", { ns: "shell" })}
                      </CommandGroupLabel>
                  ...
                  {!isBrowsing && matchedThreads.length > 0 ? (
                    <CommandGroup>
                      <CommandGroupLabel className="py-1.5 pl-3">
                        {query
                          ? t("sidebar.palette.threads", { ns: "shell" })
                          : t("sidebar.palette.recent", { ns: "shell" })}
                      </CommandGroupLabel>
                      {matchedThreads.map(
                              ...
                              <div className="flex items-baseline gap-3">
                                <div className="min-w-0 flex-1 truncate text-[length:var(--app-font-size-ui,12px)] text-foreground">
                                  <HighlightedText
                                    text={
                                      thread.title ||
                                      t("sidebar.palette.untitledThread", { ns: "shell" })
                                    }
                                    query={query}
                                  />
                                </div>
                  ...
                  {!isBrowsing && matchedProjects.length > 0 ? (
                    <CommandGroup>
                      <CommandGroupLabel className="py-1.5 pl-3">
                        {t("sidebar.search.projects", { ns: "shell" })}
                      </CommandGroupLabel>
                        ...
                        >
                          ...
                          <div className="min-w-0 flex-1">
                            <div className="truncate text-[length:var(--app-font-size-ui,12px)] text-foreground">
                              {project.name ||
                                t("sidebar.palette.untitledProject", { ns: "shell" })}
                            </div>
                  ...
                  {showThemeSection ? (
                    <>
                      {themeCommandItems.length > 0 ? (
                        <CommandGroup>
                          <CommandGroupLabel className="py-1.5 pl-3">
                            {t("sidebar.search.configure", { ns: "shell" })}
                          </CommandGroupLabel>
                          {themeCommandItems.map((themeCommandItem) => (
                            ...
                            >
                              ...
                              <span className="min-w-0 flex-1 truncate text-[length:var(--app-font-size-ui,12px)] text-foreground">
                                {t(themeCommandItem.labelKey, { ns: "shell" })}
                              </span>
                              <span
                      ...
                      {matchedCurrentThemes.length > 0 ? (
                        <CommandGroup>
                          <CommandGroupLabel className="py-1.5 pl-3">
                            {t(
                              resolvedTheme === "dark"
                                ? "sidebar.palette.theme.darkGroup"
                                : "sidebar.palette.theme.lightGroup",
                              { ns: "shell" },
                            )}
                          </CommandGroupLabel>
                            ...
                            return (
                              ...
                              >
                                ...
                                <span className="shrink-0 text-[length:var(--app-font-size-ui-meta,10px)] text-muted-foreground/79">
                                  {t(
                                    resolvedTheme === "dark"
                                      ? "sidebar.palette.theme.darkDescription"
                                      : "sidebar.palette.theme.lightDescription",
                                    { ns: "shell" },
                                  )}
                                </span>
                                <span
                  ...
                  {!isBrowsing && !hasSearchResults ? (
                    <CommandEmpty className="py-10">
                      <div className="flex flex-col items-center justify-center gap-2 text-center text-sm text-muted-foreground/79">
                        ...
                        <div>{t("sidebar.search.noMatches", { ns: "shell" })}</div>
                      </div>
                    </CommandEmpty>
                  ) : null}
                </CommandList>
              ...
              <CommandFooter>
                {isBrowsing ? (
                  <>
                    <span>
                      {isAddingProject
                        ? t("sidebar.palette.addingProject", { ns: "shell" })
                        : t("sidebar.palette.browseHint", { ns: "shell" })}
                    </span>
                    <span>
                      {hasHighlightedFolderItem
                        ? t("sidebar.palette.openOrAddHint", {
                            ns: "shell",
                            enter: enterKeyLabel,
                            modifier: submitModifierLabel,
                          })
                        : hasHighlightedBrowseItem
                          ? t("sidebar.palette.goUpHint", {
                              ns: "shell",
                              enter: enterKeyLabel,
                            })
                          : t("sidebar.palette.addHint", {
                              ns: "shell",
                              enter: enterKeyLabel,
                            })}
                    </span>
                  ...
                  <>
                    <span>{t("sidebar.search.hint", { ns: "shell" })}</span>
                    <span>{t("sidebar.search.openHint", { ns: "shell" })}</span>
                  </>
                )}
              </CommandFooter>
            </Command>
