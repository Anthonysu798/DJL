...
import { EditorWorkspaceView as EditorWorkspaceViewComponent } from "./EditorWorkspaceView";
...
function EditorWorkspaceView(
  props: Omit<ComponentProps<typeof EditorWorkspaceViewComponent>, "threadId">,
) {
  return (
    <EditorWorkspaceViewComponent
      {...props}
      threadId={ThreadId.makeUnsafe("editor-workspace-test-thread")}
    />
  );
}
...
describe("EditorWorkspaceView", () => {
  ...
  it("renders PDF files through the in-app PDF viewer instead of the text preview", () => {
    ...
    // The local document viewer renders its own preparation surface (document
    // fetch runs in an effect) rather than the browser iframe or text preview.
    expect(markup).toContain('aria-label="Preparing document preview..."');
