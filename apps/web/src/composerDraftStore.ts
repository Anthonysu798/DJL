...
export interface ComposerPromptHistorySavedDraft {
  ...
  browserFindings: BrowserFindingDraft[];
...
const PersistedQueuedComposerChatTurn = Schema.Struct({
  ...
  browserFindings: Schema.optionalKey(Schema.Array(BrowserFindingDraft)),
...
export interface ComposerDraftStoreState {
  ...
  addBrowserFinding: (threadId: ThreadId, finding: BrowserFindingDraft) => boolean;
  updateBrowserFindingComment: (threadId: ThreadId, findingId: string, comment: string) => void;
  removeBrowserFinding: (threadId: ThreadId, findingId: string) => void;
...
export function captureComposerPromptHistorySavedDraft(input: {
  ...
  return {
    ...
    browserFindings: [...draft.browserFindings],
...
function buildTransferredComposerDraft(input: {
  ...
  return {
    ...
    browserFindings: [...sourceDraft.browserFindings],
...
function clonePromptHistorySavedDraft(
  ...
  return {
    ...
    browserFindings: [...savedDraft.browserFindings],
...
function normalizeRuntimeMode(value: unknown): RuntimeMode | null {
  return Schema.is(RuntimeMode)(value) ? value : null;
}
...
export function deriveEffectiveComposerModelState(input: {
  ...
  const deterministicOpenCodeFallback =
    input.selectedProvider === "opencode"
      ? input.availableModelOptionsByProvider?.opencode?.[0]?.slug
      : null;
  const selectedModel =
...
function normalizePersistedPromptHistorySavedDraft(
  ...
  const attachments = Array.isArray(candidate.attachments)
    ...
    : [];
  const attachmentIds = new Set(attachments.map((entry) => entry.id));
  const browserFindings = Array.isArray(candidate.browserFindings)
    ? candidate.browserFindings
        .filter(Schema.is(BrowserFindingDraft))
        .filter((finding) => attachmentIds.has(finding.imageId))
        .slice(0, 20)
    : [];
  ...
  return {
    prompt,
    attachments,
    ...(browserFindings.length > 0 ? { browserFindings } : {}),
...
function normalizePersistedQueuedTurns(
  ...
  for (const entry of rawQueuedTurns) {
    ...
    const sourceProposedPlan = Schema.is(PersistedSourceProposedPlanReference)(
      ...
      ? candidate.sourceProposedPlan
      : undefined;
    const runtimeMode = normalizeRuntimeMode(candidate.runtimeMode);
    if (
      id.length === 0 ||
      createdAt.length === 0 ||
    ...
    if (kind === "chat") {
      ...
      const browserFindings = Array.isArray(candidate.browserFindings)
        ? candidate.browserFindings
            .filter(Schema.is(BrowserFindingDraft))
            .filter((finding) => images.some((image) => image.id === finding.imageId))
            .slice(0, 20)
        : [];
      const terminalContexts = Array.isArray(candidate.terminalContexts)
...
function normalizePersistedDraftsByThreadId(
  ...
  for (const [threadId, draftValue] of Object.entries(rawDraftMap as Record<string, unknown>)) {
    ...
    const attachments = Array.isArray(draftCandidate.attachments)
      ...
      : [];
    const attachmentIds = new Set(attachments.map((entry) => entry.id));
    const browserFindings = Array.isArray(draftCandidate.browserFindings)
      ? draftCandidate.browserFindings
          .filter(Schema.is(BrowserFindingDraft))
          .filter((finding) => attachmentIds.has(finding.imageId))
          .slice(0, 20)
      : [];
...
function partializeComposerDraftStoreState(
  ...
  for (const [threadId, draft] of Object.entries(state.draftsByThreadId)) {
    ...
    for (const queuedTurn of draft.queuedTurns) {
      if (queuedTurn.kind === "chat") {
        ...
        persistedQueuedTurns.push({
          ...
          images,
          ...((queuedTurn.browserFindings?.length ?? 0) > 0
            ? { browserFindings: queuedTurn.browserFindings ?? [] }
            : {}),
    ...
    const persistedDraft: DeepMutable<PersistedComposerThreadDraftState> = {
      ...
      ...(draft.promptHistorySavedDraft !== null
        ? {
            promptHistorySavedDraft: {
              ...
              ...(draft.promptHistorySavedDraft.browserFindings.length > 0
                ? { browserFindings: draft.promptHistorySavedDraft.browserFindings }
                : {}),
      ...
      ...(draft.browserFindings.length > 0 ? { browserFindings: draft.browserFindings } : {}),
      ...(draft.assistantSelections.length > 0
        ? {
  ...
  return queuedTurns.map((queuedTurn) => {
    if (queuedTurn.kind === "chat") {
      return {
        ...
        browserFindings: (queuedTurn.browserFindings ?? []).filter((finding) =>
          queuedTurn.images.some((image) => image.id === finding.imageId),
        ),
        files: [],
...
function hydratePromptHistorySavedDraft(
  ...
  return {
    ...
    browserFindings: (savedDraft.browserFindings ?? []).filter((finding) =>
      attachments.some((attachment) => attachment.id === finding.imageId),
    ),
    assistantSelections: normalizeAssistantSelections(savedDraft.assistantSelections ?? []),
    terminalContexts:
...
function toHydratedThreadDraft(
  ...
  return {
    ...
    browserFindings: (persistedDraft.browserFindings ?? []).filter((finding) =>
      persistedDraft.attachments.some((attachment) => attachment.id === finding.imageId),
    ),
...
export const useComposerDraftStore = create<ComposerDraftStoreState>()(
  persist(
    (set, get) => ({
      ...
      removeImage: (threadId, imageId) => {
        ...
        set((state) => {
          ...
          const nextDraft: ComposerThreadDraftState = {
            ...
            browserFindings: current.browserFindings.filter(
              (finding) => finding.imageId !== imageId,
            ),
      ...
      addBrowserFinding: (threadId, finding) => {
        if (threadId.length === 0 || !Schema.is(BrowserFindingDraft)(finding)) return false;
        let accepted = false;
        set((state) => {
          const existing = state.draftsByThreadId[threadId] ?? createEmptyThreadDraft();
          if (
            existing.images.length + existing.files.length + existing.assistantSelections.length >
              PROVIDER_SEND_TURN_MAX_ATTACHMENTS ||
            existing.browserFindings.some((entry) => entry.id === finding.id) ||
            !existing.images.some((image) => image.id === finding.imageId)
          )
            return state;
          accepted = true;
          return {
            draftsByThreadId: {
              ...state.draftsByThreadId,
              [threadId]: { ...existing, browserFindings: [...existing.browserFindings, finding] },
            },
          };
        });
        return accepted;
      },
      updateBrowserFindingComment: (threadId, findingId, comment) => {
        const normalized = comment.trim().slice(0, 10_000);
        if (!normalized) return;
        set((state) => {
          const current = state.draftsByThreadId[threadId];
          if (!current || !current.browserFindings.some((entry) => entry.id === findingId))
            return state;
          return {
            draftsByThreadId: {
              ...state.draftsByThreadId,
      ...
      removeBrowserFinding: (threadId, findingId) => {
        const current = get().draftsByThreadId[threadId];
        const finding = current?.browserFindings.find((entry) => entry.id === findingId);
        const image = finding
          ? current?.images.find((entry) => entry.id === finding.imageId)
          : null;
        if (image) revokeObjectPreviewUrl(image.previewUrl);
        set((state) => {
          const draft = state.draftsByThreadId[threadId];
          const target = draft?.browserFindings.find((entry) => entry.id === findingId);
          if (!draft || !target) return state;
          const nextDraft: ComposerThreadDraftState = {
            ...draft,
            browserFindings: draft.browserFindings.filter((entry) => entry.id !== findingId),
            images: draft.images.filter((entry) => entry.id !== target.imageId),
            nonPersistedImageIds: draft.nonPersistedImageIds.filter((id) => id !== target.imageId),
            persistedAttachments: draft.persistedAttachments.filter(
              (entry) => entry.id !== target.imageId,
            ),
          };
