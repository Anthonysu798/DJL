...
import { APP_SETTINGS_STORAGE_KEY, readStoredAppLocalePreference } from "./appSettingsStorage";
...
describe("normalizeStoredAppSettings", () => {
  ...
  it("reads the stored language before React bootstrap and safely handles corrupt storage", () => {
    ...
    expect(readStoredAppLocalePreference(reader)).toBe("system");
    storage.set(APP_SETTINGS_STORAGE_KEY, JSON.stringify({ language: "ja" }));
    expect(readStoredAppLocalePreference(reader)).toBe("ja");
    storage.set(APP_SETTINGS_STORAGE_KEY, JSON.stringify({ language: "fr" }));
    expect(readStoredAppLocalePreference(reader, true)).toBe("system");
    storage.set(APP_SETTINGS_STORAGE_KEY, "not json");
    expect(readStoredAppLocalePreference(reader)).toBe("system");
  });

  it("defaults language to the system locale preference and round-trips explicit locales", () => {
    const decode = Schema.decodeSync(Schema.fromJsonString(AppSettingsSchema));
    const encode = Schema.encodeSync(Schema.fromJsonString(AppSettingsSchema));

    expect(decode("{}").language).toBe(DEFAULT_APP_LANGUAGE);
    const french = decode(JSON.stringify({ language: "fr" }));
    expect(french.language).toBe("fr");
    expect(decode(encode(french)).language).toBe("fr");
  });

  it("migrates invalid stored language values to system without dropping other settings", () => {
    const decodedSettings = Schema.decodeSync(Schema.fromJsonString(AppSettingsSchema))(
      JSON.stringify({ language: "not-a-locale", confirmThreadDelete: false }),
    );

    expect(decodedSettings.language).toBe("system");
    expect(decodedSettings.confirmThreadDelete).toBe(false);
  });
  ...
  it("drops default provider command names so they do not look like custom paths", () => {
    const decodedSettings = Schema.decodeSync(Schema.fromJsonString(AppSettingsSchema))(
      JSON.stringify({
        ...
        grokBinaryPath: "grok",
        kimiBinaryPath: "kimi",
        iflowBinaryPath: "iflow",
        qwenBinaryPath: "qwen",
        codebuddyBinaryPath: "codebuddy",
