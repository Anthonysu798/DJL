        ...
        if (!options?.silent) {
          toastManager.add({
            ...
            title: translateRendererCopy(
              "common:hardening.providerRefreshFailed",
            ...
            description:
              error instanceof Error
                ...
                : translateRendererCopy(
                    "common:hardening.providerRefreshFallback",
                    "Unknown error refreshing provider status.",
                  ),
          });
        }
        return null;
...
export function useProviderStatusRefresh(options: ProviderStatusRefreshOptions): void {
  ...
  useEffect(() => {
    ...
    let disposed = false;
    // Mounting the desktop renderer itself emits focus/visibility events. Treat
    // that moment as the start of the focus-refresh cooldown so a slow CLI
    // provider probe cannot block the cache-first startup RPCs.
    let lastRefreshAtMs = refreshOnFocus && minIntervalMs > 0 ? Date.now() : 0;
