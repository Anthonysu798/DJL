...
const slashCopy = (key: string, defaultValue: string, values?: Readonly<Record<string, unknown>>) =>
  translateRendererCopy(`chat:slash.${key}`, defaultValue, values);
...
export function useComposerSlashCommands(input: {
  ...
  const compactProviderThread = useCallback(async (): Promise<boolean> => {
    ...
    if (
      ...
      toastManager.add({
        type: "warning",
        title: slashCopy("compact.unavailable", "Compact is unavailable"),
        description: slashCopy(
          "compact.unavailableDescription",
    ...
    try {
      void api.provider
        ...
        .catch((error) => {
          toastManager.add({
            type: "error",
            title: slashCopy("compact.failed", "Could not compact thread"),
            description:
              error instanceof Error
                ? error.message
                : slashCopy(
                    "compact.failedFallback",
      ...
      toastManager.add({
        ...
        description:
          error instanceof Error
            ? error.message
            : slashCopy("compact.failedFallback", "An error occurred while compacting context."),
      });
      return false;
    }
  ...
  const runFastSlashCommand = useCallback(
    (text: string) => {
      ...
      if (!supportsFastSlashCommand) {
        toastManager.add({
          type: "warning",
          title: slashCopy("fast.unavailable", "Fast mode is unavailable"),
          description: slashCopy(
            "fast.unavailableDescription",
      ...
      if (action === "invalid") {
        toastManager.add({
          type: "warning",
          title: slashCopy("fast.invalid", "Invalid /fast command"),
          description: slashCopy(
            "fast.invalidDescription",
      ...
      if (action === "status") {
        toastManager.add({
          type: "info",
          title: slashCopy("fast.status", "Fast mode is {{status}}", {
            status: fastModeEnabled ? slashCopy("fast.on", "on") : slashCopy("fast.off", "off"),
          }),
        });
        return true;
      }
      ...
      toastManager.add({
        type: "success",
        title: slashCopy(
          nextEnabled ? "fast.enabled" : "fast.disabled",
          nextEnabled ? "Fast mode enabled" : "Fast mode disabled",
        ),
      });
      return true;
    },
  ...
  const createForkThreadFromSlashCommand = useCallback(
    async (inputOptions?: { target?: ForkSlashCommandTarget }) => {
      ...
      if (!api || !activeProject || !activeThread || !isServerThread) {
        toastManager.add({
          type: "warning",
          title: slashCopy("fork.unavailable", "Fork is unavailable"),
          description: slashCopy(
            "fork.unavailableDescription",
  ...
  const createSidechatFromSlashCommand = useCallback(
    async (inputOptions?: { initialPrompt?: string }) => {
      ...
      if (!api || !activeProject || !activeThread || !isServerThread || !canOfferSideCommand) {
        toastManager.add({
          type: "warning",
          title: slashCopy("side.unavailable", "Side is unavailable"),
          description: slashCopy(
            "side.unavailableDescription",
      ...
      await api.orchestration.dispatchCommand({
        ...
        projectId: activeProject.id,
        title: slashCopy("side.title", "Sidechat: {{title}}", { title: titleSeed }),
        modelSelection: selectedModelSelection,
  ...
  const runCodexReviewStart = useCallback(
    async (target: "changes" | "base-branch") => {
      ...
      if (!api || !activeThread || !activeProject) {
        toastManager.add({
          type: "warning",
          title: slashCopy("review.unavailable", "Review is unavailable"),
          description: slashCopy(
            "review.unavailableDescription",
      ...
      if (target === "base-branch" && !activeRootBranch) {
        toastManager.add({
          type: "warning",
          title: slashCopy("review.baseUnavailable", "Base branch unavailable"),
          description: slashCopy(
            "review.baseUnavailableDescription",
      ...
      const nextThreadId = newThreadId();
      const createdAt = new Date().toISOString();
      const nextThreadTitle = slashCopy("review.title", "{{title}} Review", {
        title: activeThread.title,
      });
      ...
      try {
        ...
        toastManager.add({
          type: "error",
          title: slashCopy("review.failed", "Could not start review"),
          description:
            error instanceof Error
              ? error.message
              : slashCopy("review.failedFallback", "An error occurred while starting review."),
        });
        return false;
      }
    },
  ...
  const handleForkTargetSelection = useCallback(
    async (target: ForkSlashCommandTarget) => {
      try {
        ...
        toastManager.add({
          type: "error",
          title: slashCopy("fork.failed", "Could not fork thread"),
          description:
            error instanceof Error
              ? error.message
              : slashCopy(
  ...
  const checkClaudeFastSlashCommandAvailability = useCallback(async (): Promise<boolean> => {
    ...
    if (!api || !providerCommandDiscoveryCwd) {
      ...
      toastManager.add({
        type: "warning",
        title: slashCopy("fast.checkFailed", "Fast mode could not be checked"),
        description: slashCopy(
          "fast.discoveryUnavailable",
    ...
    toastManager.add({
      ...
      title: slashCopy("fast.unavailable", "Fast mode is unavailable"),
      description: slashCopy(
        "fast.claudeUnavailableDescription",
  ...
  const runExportSlashCommand = useCallback(() => {
    ...
    if (!canOfferExportCommand) {
      toastManager.add({
        type: "warning",
        title: slashCopy("export.unavailable", "Export is unavailable"),
        description: slashCopy(
    ...
    void downloadUrlAsBlob({
      ...
      toastManager.add({
        type: "error",
        title: slashCopy("export.failed", "Could not export thread"),
        description:
          error instanceof Error
            ? error.message
            : slashCopy("export.failedFallback", "An error occurred while exporting the thread."),
      });
    });
    ...
    async (trimmed: string): Promise<boolean> => {
      ...
      if (slashInvocation.command === "review") {
        if (selectedProvider === "codex") {
          ...
          if (!target) {
            toastManager.add({
              type: "warning",
              title: slashCopy("review.invalid", "Invalid /review command"),
              description: slashCopy(
                "review.invalidDescription",
                "Use /review and then choose a review target.",
              ),
            });
            return true;
          }
      ...
      if (slashInvocation.command === "fork") {
        ...
        if (invalid) {
          toastManager.add({
            type: "warning",
            title: slashCopy("fork.invalid", "Invalid /fork command"),
            description: slashCopy(
              "fork.invalidDescription",
      ...
      if (slashInvocation.command === "side") {
        try {
          ...
          toastManager.add({
            type: "error",
            title: slashCopy("side.failed", "Could not start Side"),
            description:
              error instanceof Error
                ? error.message
                : slashCopy("side.failedFallback", "An error occurred while creating Side."),
          });
        }
        return true;
      }
