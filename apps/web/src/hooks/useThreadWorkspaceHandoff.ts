    ...
    async (targetMode: "local" | "worktree", options?: { preferredWorktreeName?: string }) => {
      ...
      try {
        ...
        toastManager.add({
          ...
          title:
            targetMode === "worktree" ? t("handoff.successWorktree") : t("handoff.successLocal"),
        ...
        toastManager.add({
          type: "error",
          title: targetMode === "worktree" ? t("handoff.failedWorktree") : t("handoff.failedLocal"),
          description: error instanceof Error ? error.message : t("handoff.failedFallback"),
        });
        return false;
      }
    },
    [handoffThreadMutation, input, t],
  );
