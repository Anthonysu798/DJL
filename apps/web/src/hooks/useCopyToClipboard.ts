...
export function useCopyPathToClipboard(): (path: string) => void {
  ...
  const { copyToClipboard } = useCopyToClipboard<{ path: string }>({
    onCopy: (ctx) =>
      toastManager.add({ type: "success", title: t("copy.pathCopied"), description: ctx.path }),
    onError: (error) =>
      toastManager.add({
        type: "error",
        title: t("copy.pathFailed"),
        description: error instanceof Error ? error.message : t("copy.failedFallback"),
      }),
  });
...
export function useCopyThreadIdToClipboard(): (threadId: string) => void {
  ...
  const { copyToClipboard } = useCopyToClipboard<{ threadId: string }>({
    onCopy: (ctx) =>
      toastManager.add({
        type: "success",
        title: t("copy.threadIdCopied"),
        description: ctx.threadId,
      }),
    onError: (error) =>
      toastManager.add({
        type: "error",
        title: t("copy.threadIdFailed"),
        description: error instanceof Error ? error.message : t("copy.failedFallback"),
      }),
  });
