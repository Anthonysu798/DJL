...
final class CodexSecurePairingStateTests: XCTestCase {
    ...
    func testExistingDeviceTrustKeysMigrateToThisDeviceOnlyWhenUnlockedAccessibility() {
        let phoneIdentity = CodexPhoneIdentityState(
            phoneDeviceId: "phone-\(UUID().uuidString)",
            phoneIdentityPrivateKey: Data(repeating: 7, count: 32).base64EncodedString(),
            phoneIdentityPublicKey: Data(repeating: 8, count: 32).base64EncodedString()
        )
        let macDeviceID = "mac-\(UUID().uuidString)"
        let registry = CodexTrustedMacRegistry(records: [
            macDeviceID: CodexTrustedMacRecord(
                macDeviceId: macDeviceID,
        ...
        SecureStore.writeCodable(phoneIdentity, for: CodexSecureKeys.phoneIdentityState)
        SecureStore.writeCodable(registry, for: CodexSecureKeys.trustedMacRegistry)

        XCTAssertEqual(codexPhoneIdentityStateFromSecureStore().phoneDeviceId, phoneIdentity.phoneDeviceId)
        XCTAssertEqual(codexTrustedMacRegistryFromSecureStore().records.keys.first, macDeviceID)
        XCTAssertEqual(
            keychainAccessibility(for: CodexSecureKeys.phoneIdentityState),
            kSecAttrAccessibleWhenUnlockedThisDeviceOnly as String
        )
        XCTAssertEqual(
            keychainAccessibility(for: CodexSecureKeys.trustedMacRegistry),
            kSecAttrAccessibleWhenUnlockedThisDeviceOnly as String
        )
    }
