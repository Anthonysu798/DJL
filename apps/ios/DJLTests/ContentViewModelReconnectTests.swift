...
// Depends on: XCTest, Foundation, DJL
...
final class ContentViewModelReconnectTests: XCTestCase {
    ...
    func testTrustedResolveURLCandidatesTryProxyRelativeThenRootRoute() {
        let candidates = CodexTrustedSessionResolveURLBuilder.candidates(
            from: "wss://relay.example.com/djl/relay?stale=1#old"
        )

        XCTAssertEqual(
            candidates.map(\.absoluteString),
            [
                "https://relay.example.com/djl/v1/trusted/session/resolve",
    ...
    func testPairingCodeResolveURLCandidatesKeepProxyPrefixFallbacks() {
        ...
        XCTAssertEqual(
            candidates.map(\.absoluteString),
            [
                "https://relay.example.com/djl/relay/v1/pairing/code/resolve",
                "https://relay.example.com/djl/v1/pairing/code/resolve",
                "https://relay.example.com/v1/pairing/code/resolve",
            ]
        )
    }

    func testManualPairingCodeConvertsRegistryBaseToWebSocketRelayRoute() {
        XCTAssertEqual(
            pairingCodeWebSocketRelayURL(from: "http://127.0.0.1:8787"),
            "ws://127.0.0.1:8787/relay"
        )
        XCTAssertEqual(
            pairingCodeWebSocketRelayURL(from: "https://relay.example.com/djl/relay?stale=1#old"),
            "wss://relay.example.com/djl/relay"
        )
        XCTAssertNil(pairingCodeWebSocketRelayURL(from: "ftp://relay.example.com"))
    }
