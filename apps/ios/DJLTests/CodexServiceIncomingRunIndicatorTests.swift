...
final class CodexServiceIncomingRunIndicatorTests: XCTestCase {
    ...
    func testBackgroundDiscoveryLifecycleUpdatesBadgeWithoutHydratingUnopenedThread() async {
        ...
        let lifecycleParams: [String: JSONValue] = [
            ...
            "djlDesktopMirror": .bool(true),
            "djlBackgroundDiscovery": .bool(true),
        ]
    ...
    func testDeletingRunningThreadClearsRuntimeState() {
        ...
        let turnID = "turn-\(UUID().uuidString)"
        service.threads = [
            CodexThread(id: threadID, title: "Running chat", cwd: "/tmp/djl"),
        ]
        service.runningThreadIDs.insert(threadID)
    ...
    func testDisplayIslandDoesNotMarkDisconnectedRunReadyWithoutTerminalState() {
        let service = makeService()
        let coordinator = DJLDisplayIslandCoordinator()
        let threadID = "thread-\(UUID().uuidString)"
    ...
    func testDisplayIslandDoesNotMarkActiveRunReadyAfterCompletion() {
        ...
        let startedAt = Date(timeIntervalSince1970: 1_000)
        service.threads = [
            CodexThread(id: threadID, title: "Active chat", cwd: "/tmp/djl"),
        ]
        service.activeThreadId = threadID
        service.runningThreadIDs.insert(threadID)
    ...
    // as running ("DJL is thinking") right after a trimmed thread/read.
    func testTurnStateSnapshotIgnoresHistoryCompactionMarkerTurn() {
        let service = makeService()
        let turnObjects: [RPCObject] = [
            [
                "id": .string("djl-history-compacted-turn-1"),
                "djlSynthetic": .bool(true),
                "djlHistoryCompacted": .bool(true),
            ],
            [
                "id": .string("turn-30"),
                "status": .string("completed"),
