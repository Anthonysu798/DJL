...
final class TurnFileAutocompleteTokenTests: XCTestCase {
    ...
    func testTrailingTokenParsesBareLowercaseSearchAfterAt() {
        let token = TurnViewModel.trailingFileAutocompleteToken(in: "paste @djl")

        XCTAssertEqual(token?.query, "djl")
    }
    ...
    func testProviderDiscoveryTextNormalizesCommonSeparators() {
        XCTAssertEqual(
