...
final class CodexApprovalStateTests: XCTestCase {
        ...
        service.handleIncomingRPCMessage(
            RPCMessage(
                ...
                params: .object([
                    ...
                    "djlActionSource": .string("desktop-ipc-action-follower"),
                ]),
                includeJSONRPC: false
            )
        )
    ...
    func testApprovalHandlingUsesThreadAccessModeInsteadOfGlobalDefault() {
        let service = makeService()
        service.setSelectedAccessMode(.fullAccess)
        service.setThreadAccessModeOverride(.onRequest, for: "thread-on-request")
        let requestID: JSONValue = .string("thread-aware-approval")
        ...
        service.handleIncomingRPCMessage(
            RPCMessage(
                ...
                method: "item/commandExecution/requestApproval",
                params: .object([
                    "threadId": .string("thread-on-request"),
                    "turnId": .string("turn-a"),
                    "command": .string("git status"),
                ]),
                includeJSONRPC: false
        ...
        XCTAssertEqual(service.pendingApproval(for: "thread-on-request")?.id, service.idKey(from: requestID))
    }
