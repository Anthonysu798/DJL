...
final class CodexThreadProjectRoutingTests: XCTestCase {
    ...
    func testStartThreadIfReadyWaitsForRuntimeInitializationDuringReconnect() async throws {
        ...
        let thread = try await service.startThreadIfReady(preferredProjectPath: "/tmp/djl-local")
    ...
    func testMoveThreadToProjectPathKeepsRebindWhenResumeFailsOnlyBecauseRolloutIsMissing() async throws {
        ...
        let movedThread = try await service.moveThreadToProjectPath(
            threadId: "thread-1",
            projectPath: "/tmp/djl-worktree"
        )

        XCTAssertEqual(resumeRequests.count, 1)
        XCTAssertEqual(resumeRequests.first?["threadId"]?.stringValue, "thread-1")
        XCTAssertEqual(resumeRequests.first?["cwd"]?.stringValue, "/tmp/djl-worktree")
        XCTAssertEqual(movedThread.gitWorkingDirectory, "/tmp/djl-worktree")
        XCTAssertEqual(service.thread(for: "thread-1")?.gitWorkingDirectory, "/tmp/djl-worktree")
        XCTAssertEqual(service.currentAuthoritativeProjectPath(for: "thread-1"), "/tmp/djl-worktree")
    ...
    func testManagedWorktreeAssociationPersistsAcrossLocalHandoffs() async throws {
        ...
        let worktreePath = "/Users/me/.codex/worktrees/a1b2/djl"
        _ = try await service.moveThreadToProjectPath(threadId: "thread-1", projectPath: worktreePath)
        _ = try await service.moveThreadToProjectPath(threadId: "thread-1", projectPath: "/tmp/djl-local")

        XCTAssertEqual(resumeResponses, [worktreePath, "/tmp/djl-local"])
