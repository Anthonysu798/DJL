...
final class CodexThreadForkTests: XCTestCase {
    ...
    func testForkAppliesSelectedAccessModeToThreadFork() async throws {
        ...
        let forkedThread = try await service.forkThreadIfReady(
            from: "source-thread",
            target: .projectPath("/tmp/djl-worktree")
        )
        ...
        XCTAssertEqual(capturedResumeParams["cwd"]?.stringValue, "/tmp/djl-worktree")
        XCTAssertEqual(forkedThread.id, "fork-local")
        XCTAssertEqual(forkedThread.gitWorkingDirectory, "/tmp/djl-worktree")
        XCTAssertEqual(service.activeThreadId, "fork-local")
    }
    ...
    func testUnsupportedThreadForkDisablesCapabilityAndShowsUpdatePrompt() async {
        ...
        do {
            ...
            XCTAssertEqual(service.bridgeUpdatePrompt?.title, "Update DJL on your Mac to use /fork")
        }
    }
    ...
    func testForkKeepsAuthoritativeProjectPathWhenResumeFallsBackToMissingRollout() async throws {
        ...
        XCTAssertEqual(forkedThread.gitWorkingDirectory, "/tmp/djl-worktree")
        XCTAssertEqual(service.currentAuthoritativeProjectPath(for: "fork-local"), "/tmp/djl-worktree")
        ...
        XCTAssertEqual(service.thread(for: "fork-local")?.gitWorkingDirectory, "/tmp/djl-worktree")
        XCTAssertEqual(service.currentAuthoritativeProjectPath(for: "fork-local"), "/tmp/djl-worktree")
