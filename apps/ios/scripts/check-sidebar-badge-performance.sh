...
PROJECT_PATH="$ROOT_DIR/DJL.xcodeproj"
SCHEME="${SCHEME:-DJL}"
...
Environment:
  SCHEME                   Xcode scheme. Default: DJL
...
xcodebuild \
  ...
  test \
  -only-testing:DJLTests/SidebarRunBadgePerformanceTests/testSidebarRunBadgeSnapshotPerformance \
  -only-testing:DJLTests/SidebarRunBadgePerformanceTests/testSidebarRunBadgeSnapshotWithLargeTimelinePerformance \
  2>&1 | tee "$run_log"
