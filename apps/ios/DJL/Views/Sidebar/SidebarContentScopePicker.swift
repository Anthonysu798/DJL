...
//             UserBubbleColor, DJLIcon, AdaptiveGlassModifier
...
struct SidebarContentScopePicker: View {
    ...
    private func scopeButton(_ scope: SidebarContentScope) -> some View {
        ...
        return HapticButton(hapticStyle: .light, action: {
            ...
            HStack(spacing: 5) {
                Text(scope.title)
                if scope == .attention, attentionCount > 0 {
                    Text("\(attentionCount)")
        ...
        .accessibilityIdentifier("sidebar.scope.\(scope.rawValue)")
    }

    private func accessibilityLabel(for scope: SidebarContentScope) -> String {
        guard scope == .attention, attentionCount > 0 else { return scope.title }
        return "Attention, \(attentionCount) \(attentionCount == 1 ? "item" : "items")"
    }
...
struct SidebarFolderExpansionToggleButton: View {
    ...
    var body: some View {
        HapticButton(hapticStyle: .light, action: action) {
            DJLIcon.image(systemName: iconSystemName)
