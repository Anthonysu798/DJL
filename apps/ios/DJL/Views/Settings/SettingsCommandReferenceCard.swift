...
// Purpose: Sheet with local DJL CLI commands for pairing and bridge management.
...
private struct SettingsCommandReference: Identifiable {
    static let commands = [
        ...
        SettingsCommandReference(
            command: "djl reset-pairing",
            detail: "Clears saved trust so the next connection needs a fresh QR."
        ),
        SettingsCommandReference(
            command: "djl resume",
            detail: "Reopens the last active DJL thread in Codex."
        ),
        SettingsCommandReference(
            command: "djl watch [threadId]",
            detail: "Tails a thread event log in real time."
        ),
        SettingsCommandReference(
            command: "djl --version",
            detail: "Prints the installed DJL CLI version."
        )
    ]
...
struct SettingsCommandReferenceSheet: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                Section {
                    Text("Run these in Terminal on your paired Mac when you need to start, repair, or inspect the local DJL bridge.")
