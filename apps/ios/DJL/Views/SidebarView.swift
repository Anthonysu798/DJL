...
struct SidebarView<ConnectionEmptyStatePanel: View, ConnectionEmptyStateFooter: View>: View {
    ...
    let onOpenAttentionItem: (SidebarAttentionItem) -> Void
    ...
    @State private var attentionItems: [SidebarAttentionItem] = []
    ...
    var body: some View {
        ...
        threadListWithBottomBar
            ...
            .onChange(of: attentionSourceFingerprint) { _, _ in
                Task { @MainActor in rebuildAttentionItems() }
            }
    ...
    private var attentionSourceFingerprint: Int {
        var hasher = Hasher()
        hasher.combine(codex.activeThreadId)
        hasher.combine(codex.readyThreadIDs)
        hasher.combine(codex.failedThreadIDs)
        for request in codex.pendingApprovals {
            hasher.combine(request.id)
            hasher.combine(request.threadId)
            hasher.combine(request.turnId)
            hasher.combine(request.reason)
            hasher.combine(request.command)
        }
        for thread in codex.threads {
            hasher.combine(thread.id)
            ...
            hasher.combine(codex.messageRevisionByThread[thread.id] ?? 0)
            if let goal = codex.goalByThreadID[thread.id] {
                hasher.combine(goal.status.rawValue)
    ...
    private func rebuildAttentionItems() {
        attentionItems = SidebarAttentionProjection.project(
            threads: codex.threads,
            pendingApprovals: codex.pendingApprovals,
            messagesByThread: codex.messagesByThread,
            readyThreadIDs: codex.readyThreadIDs,
            failedThreadIDs: codex.failedThreadIDs,
            goalsByThreadID: codex.goalByThreadID,
    ...
    func body(content: Content) -> some View {
        content
            ...
            .alert(
                ...
                Text("Chats for this project will be deleted only from DJL on this phone. Nothing is removed from your device or Codex observer.")
            }
            .alert(
                ...
                Text("This only removes the chat from DJL on this phone. Nothing is removed from your device or Codex observer.")
            }
