...
private let qrScannerBridgeUpdateCommand = "npm install -g djl@latest"
...
func validatePairingQRCode(_ code: String, now: Date = Date()) -> QRScannerPairingValidationResult {
    ...
    if looksLikeDJLPairingPayload(data) {
        return .bridgeUpdateRequired(
            makeScannerBridgeUpdatePrompt(
                message: "This QR code looks like it came from an older DJL bridge. Update the npm package on your Mac to the latest release before scanning a new QR code."
            )
        )
    }

    return .scanError("Not a valid secure pairing code. Make sure you're scanning a QR from the latest DJL bridge.")
}
...
private func looksLikeDJLPairingPayload(_ data: Data) -> Bool {
