...
struct CodexForkIcon: View {
    ...
    var body: some View {
        // `djl.fork` is a virtual key in DJLIcon mapped to
        // central-fork-code; routing through DJLIcon keeps Dynamic Type
        // scaling and the square anchor logic in one place.
        DJLIcon.image(systemName: "djl.fork", size: pointSize)
    }
}
...
struct CodexWorktreeIcon: View {
    ...
    var body: some View {
        // Synara uses Central's arrow-split-right for worktrees; keep DJL
        // on the same glyph through the shared icon resolver.
        DJLIcon.image(
            systemName: "djl.worktree",
            size: pointSize,
            weight: weight
        )
    }
    ...
    static func menuImage(pointSize: CGFloat = 13, weight _: UIImage.SymbolWeight = .regular) -> UIImage {
        guard let base = DJLIcon.uiImage(systemName: "djl.worktree") else {
            return UIImage()
        }
    ...
    // Matches the UIKit menu glyph metric used by `DJLIcon.menuUIImage`.
