...
// Depends on: Foundation, DJLIcon (icon name resolution)
...
///
/// Returned values are SF Symbol names that `DJLIcon` maps to the bundled
/// `central-*` template assets, so callers render them via
/// `DJLIcon.image(systemName:)` and tint them with a single `foregroundStyle`.
///
...
enum ToolCallIcon {
    ...
    static func systemName(forDescriptor descriptor: String?) -> String {
        ...
        if tokens.first == "mcp" {
            return tokens.contains("github") ? "djl.github" : "djl.skill"
        }
