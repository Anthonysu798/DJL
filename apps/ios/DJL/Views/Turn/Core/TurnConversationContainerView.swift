...
struct TurnConversationContainerView: View {
    ...
    var body: some View {
        ...
        let messageLayout = Self.buildMessageLayout(
            ...
            preferredStructuredPromptMessageID: focusTarget?.messageID
        )
        ...
        ZStack(alignment: .top) {
            TurnTimelineView(
                ...
                focusTarget: messageLayout.activeStructuredPromptMessage?.id == focusTarget?.messageID
                    ? nil
                    : focusTarget,
    ...
    private static func buildMessageLayout(
        ...
        preferredStructuredPromptMessageID: String?
        ...
        let preferredStructuredPromptMessage = preferredStructuredPromptMessageID.flatMap { preferredID in
            messages.first { message in
                message.id == preferredID && message.shouldDisplayComposerStructuredPrompt
            }
        }
        ...
        if canReplaceComposerWithPrompt, let preferredStructuredPromptMessage {
            activeStructuredPromptMessage = preferredStructuredPromptMessage
        }
...
private struct AccessoryBackedEmptyState: View {
    ...
    var body: some View {
        VStack(spacing: 0) {
            ...
            VStack(spacing: 14) {
                DJLIcon.image(systemName: systemImage)
