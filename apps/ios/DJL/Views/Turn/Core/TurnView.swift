...
struct TurnView: View {
    ...
    private let attentionTarget: AttentionNavigationTarget?
    ...
    private var approvalForThread: CodexApprovalRequest? {
        if let approvalRequestID = attentionTarget?.approvalRequestID,
           let targetedApproval = codex.pendingApprovals.first(where: { request in
               request.id == approvalRequestID && request.threadId == thread.id
           }) {
            return targetedApproval
        }
        return codex.pendingApproval(for: thread.id)
    }
