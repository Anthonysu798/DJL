...
struct NewChatDraftView: View {
        ...
        guard !projectChoices.isEmpty else {
            return UIMenu(children: [
                UIAction(
                    ...
                    image: DJLIcon.menuUIImage(systemName: "folder"),
                    attributes: [.disabled]
                ) { _ in },
            ])
        }

        // A paired phone cannot infer arbitrary Mac paths from abbreviated
        // project labels. Keep the fast recent-folder choices, but expose the
        // visible Mac folder browser so a user can select and verify an exact
        // workspace before the first remote turn.
        let browseAction =
            UIAction(
                title: "Browse local folders…",
                image: DJLIcon.menuUIImage(systemName: "folder.badge.plus")
            ) { _ in
                HapticFeedback.shared.triggerImpactFeedback(style: .light)
                activeSheet = .localFolderBrowser
            }
        let actions = [browseAction] + projectChoices.map { choice in
            UIAction(
                title: choice.label,
                image: DJLIcon.menuUIImage(systemName: choice.iconSystemName),
    ...
    // Uses the custom DJL folder glyph so the inline picker matches the rest of the sidebar.
    ...
    NavigationStack {
        NewChatDraftView(
            route: NewChatDraftRoute(
                id: "draft_preview",
                preferredProjectPath: "/Users/emanueledipietro/Developer/DJL",
                source: .generalChat
            ),
            onOpenThread: { _ in }
        )
