...
struct ComposerBottomBar: View {
    ...
    private func accessModeRow(_ mode: CodexAccessMode) -> some View {
        HStack(spacing: 14) {
            DJLIcon.image(systemName: accessModeIconSystemName(for: mode), size: 22)
    ...
    private var queueBadge: some View {
        HStack(spacing: 3) {
            if isQueuePaused {
                DJLIcon.image(systemName: "pause.fill")
...
private struct ComposerRuntimeMenuControl: View, Equatable {
    ...
    var body: some View {
        ...
        // Let the pill yield width before the fixed composer controls. Long
        // provider/model names must truncate instead of widening the composer
        // beyond the screen and shifting its page margins.
        .layoutPriority(-1)
        .tint(metaLabelColor)
    ...
    private func composerMenuLabel(
        ...
        HStack(spacing: 6) {
            if let leadingImageName {
                DJLIcon.image(systemName: leadingImageName)
...
private struct AllModelsSheet: View {
    ...
    private func modelRow(for model: CodexModelOption) -> some View {
        ...
        HStack(alignment: .top, spacing: 12) {
            DJLIcon.image(systemName: model.id == selectedModelID ? "checkmark.circle.fill" : "circle")
                .font(.system(size: 18))
            ...
            VStack(alignment: .leading, spacing: 2) {
                HStack(spacing: 6) {
                    ...
                    if modelSupportsFastMode(model) {
                        DJLIcon.image(systemName: CodexServiceTier.fast.iconName, size: fastModeIconSide)
