...
struct SlashCommandAutocompletePanel: View {
    ...
    private func commandIcon(for command: TurnComposerSlashCommand, isEnabled: Bool) -> some View {
        if command == .fork {
            DJLIcon.image(systemName: "djl.git-branch", size: 16)
                .foregroundStyle(commandPrimaryStyle(isEnabled: isEnabled))
                .frame(width: 22)
        } else {
            DJLIcon.image(systemName: command.symbolName)
    ...
    private func forkDestinationIcon(for destination: TurnComposerForkDestination) -> some View {
        switch destination {
        case .local:
            DJLIcon.image(systemName: destination.symbolName)
                .font(AppFont.system(size: 15, weight: .semibold))
                .foregroundStyle(.primary)
                .frame(width: 22)
        case .newWorktree:
    ...
    private func submenuHeader(
        ...
        HStack(spacing: 10) {
            ...
            Button(action: onClose) {
                DJLIcon.image(systemName: "xmark")
