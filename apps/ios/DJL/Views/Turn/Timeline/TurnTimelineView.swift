...
struct TurnTimelineView<EmptyState: View, Composer: View>: View {
    ...
    let focusTarget: AttentionNavigationTarget?
    ...
    @State private var lastFocusedAttentionTarget: AttentionNavigationTarget?
        ...
        if messages.isEmpty && !hasEarlierMessages && olderHistoryLoadErrorMessage == nil && !isLoadingRemoteEarlierMessages {
            ...
            ScrollViewReader { proxy in
                GeometryReader { viewport in
                    ...
                    .task(id: attentionFocusTaskID) {
                        await focusAttentionTargetIfNeeded(using: proxy)
                    }
    ...
    private var attentionFocusTaskID: String {
        guard let focusTarget else { return "none" }
        return [
            focusTarget.threadID,
            focusTarget.turnID ?? "",
            focusTarget.itemID ?? "",
            focusTarget.messageID ?? "",
            String(timelineChangeToken),
        ].joined(separator: "|")
    }

    @MainActor
    private func focusAttentionTargetIfNeeded(using proxy: ScrollViewProxy) async {
        guard let focusTarget,
              focusTarget.threadID == threadID,
              focusTarget != lastFocusedAttentionTarget,
              let messageID = focusTarget.resolvedMessageID(in: messages),
              let targetIndex = messages.firstIndex(where: { $0.id == messageID }) else {
            return
        }

        let targetMessage = messages[targetIndex]
        if targetMessage.kind == .userInputPrompt {
            // Active structured prompts are intentionally moved into the composer
            // replacement surface, which is already visible when the thread opens.
            lastFocusedAttentionTarget = focusTarget
            return
        }

        if !usesPaginatedHistory && targetIndex < messages.count - visibleTailCount {
            visibleTailCount = messages.count - targetIndex
            recomputeRenderItemsAndBlockInfoIfNeeded()
        }
