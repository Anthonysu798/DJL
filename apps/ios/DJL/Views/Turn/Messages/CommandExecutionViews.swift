...
struct CommandExecutionCardBody: View {
    ...
    var body: some View {
        HStack(spacing: 6) {
            ...
            if accent == .failed {
                DJLIcon.image(systemName: "exclamationmark.circle.fill", size: 15, relativeTo: .body)
                    .foregroundStyle(.red)
            }
...
enum CommandHumanizer {
    ...
    /// Leading icon (an SF Symbol name resolved to a Central asset by `DJLIcon`)
...
struct CommandExecutionDetailSheet: View {
    ...
    private var commandSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            DJLIcon.label("Command", systemName: "terminal.fill")
    ...
    private var outputSection: some View {
        VStack(alignment: .leading, spacing: 8) {
            Button {
                ...
                HStack(spacing: 6) {
                    DJLIcon.image(systemName: isOutputExpanded ? "chevron.down" : "chevron.right")
