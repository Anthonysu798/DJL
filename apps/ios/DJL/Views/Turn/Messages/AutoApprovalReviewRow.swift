...
struct AutoApprovalReviewRow: View {
            ...
            Button {
                ...
                HStack(spacing: 8) {
                    if review.status == .inProgress {
                        ...
                        DJLIcon.image(systemName: statusIconSystemName, size: 17, relativeTo: .body)
                            .foregroundStyle(statusColor)
                    }
                    Text(rowTitle)
                        .font(AppFont.body(weight: .regular))
                        .foregroundStyle(.secondary)
                    DJLIcon.image(systemName: "chevron.right", size: 13, relativeTo: .body)
                        .foregroundStyle(.secondary)
    ...
    // Every status name resolves to a bundled Central asset in DJLIcon
