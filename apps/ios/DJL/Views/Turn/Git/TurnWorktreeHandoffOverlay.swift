...
enum TurnWorktreeOverlayMode {
    ...
    var message: String {
        switch self {
        case .handoff:
            return "Create and check out a branch in a new worktree to continue in parallel. The branch is normalized with the djl/ prefix."
        case .fork:
...
struct TurnWorktreeHandoffOverlay: View {
    ...
    private var normalizedBranchName: String {
        djlNormalizedCreatedBranchName(branchName)
    }
