...
func djlNormalizedCreatedBranchName(_ rawName: String) -> String {
    let trimmedName = rawName.trimmingCharacters(in: .whitespacesAndNewlines)
    guard !trimmedName.isEmpty else { return "" }
    if trimmedName.hasPrefix("djl/") {
        return trimmedName
    }
    return "djl/\(trimmedName)"
}

// Leaves "open elsewhere" branches selectable so the caller can surface the right alert or git error.
func djlCurrentBranchSelectionIsDisabled(
    branch: String,
...
func djlSelectableDefaultBranch(
...
struct TurnGitBranchSelector: View, Equatable {
    ...
    private var localDefaultBranch: String? {
        ...
        return djlSelectableDefaultBranch(
...
struct TurnGitBranchPickerSheet: View {
    ...
    private var isNewBranchNameValid: Bool {
        ...
        guard !trimmed.isEmpty, trimmed != "djl/" else { return false }
        return true
    }
    ...
    private var suggestedCreateBranchName: String? {
        ...
        let candidate = djlNormalizedCreatedBranchName(searchText)
    ...
    var body: some View {
        List {
            Section(sectionTitle) {
                if showsDefaultBranchRow, let defaultBranch {
                    let isDisabled = djlCurrentBranchSelectionIsDisabled(
                ...
                if orderedBranches.isEmpty {
                    ContentUnavailableView {
                        DJLIcon.label("No branches found", systemName: "arrow.triangle.branch")
                    } description: {
            ...
            if allowsSelectingCurrentBranch {
                Section {
                    ...
                    Button {
                        let fromSearch = djlNormalizedCreatedBranchName(searchText)
                        newBranchName = fromSearch.isEmpty ? "djl/" : fromSearch
        ...
        .alert("New branch", isPresented: $isShowingCreateBranchPrompt) {
            TextField("djl/my-feature", text: $newBranchName)
            Button("Cancel", role: .cancel) {
                newBranchName = ""
            }
            Button("Create") {
                let branchName = djlNormalizedCreatedBranchName(newBranchName)
...
#Preview("Branch Picker") {
    TurnGitBranchPickerSheet(
        ...
        selectedBranch: "djl/onboarding-v2",
        defaultBranch: "main",
        currentBranch: "djl/onboarding-v2",
