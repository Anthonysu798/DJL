...
enum TurnVoiceRecoveryPresentationBuilder {
    static func presentation(for reason: CodexVoiceFailureReason) -> VoiceRecoveryPresentation {
        ...
        case .reconnectRequired:
            return VoiceRecoveryPresentation(
                snapshot: snapshot(
                    ...
                    detail: "Keep the DJL bridge running on your paired device, then try the microphone again.",
                    status: .interrupted,
        ...
        case .bridgeSessionUnsupported:
            return VoiceRecoveryPresentation(
                snapshot: snapshot(
                    ...
                    detail: "Restart DJL on your device, then reconnect this iPhone. If it still happens, update DJL on your device and pair again.",
        ...
        case .microphonePermissionRequired:
            return VoiceRecoveryPresentation(
                snapshot: snapshot(
                    summary: "Microphone access is off for DJL.",
                    detail: "Open iPhone Settings, allow Microphone for DJL, then try recording again.",
                    status: .actionRequired,
        ...
        case .recorderUnavailable:
            return VoiceRecoveryPresentation(
                snapshot: snapshot(
                    summary: "DJL could not start the recorder.",
