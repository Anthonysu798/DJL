// FILE: AboutDJLView.swift
// Purpose: Full-screen guide explaining how DJL works, styled as a blog page.
// Layer: View
// Exports: AboutDJLView

import SwiftUI

struct AboutDJLView: View {
    var body: some View {
        ScrollView {
    ...
    @ViewBuilder private var architectureDiagram: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            VStack(spacing: 0) {
                diagramStep(from: "DJL iOS", to: "Bridge (Device)", via: "WebSocket")
    ...
    private func diagramStep(from: String, to: String, via: String, isLast: Bool = false) -> some View {
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                ...
                VStack(spacing: 2) {
                    DJLIcon.image(systemName: "arrow.right")
    ...
    @ViewBuilder private var desktopSection: some View {
        VStack(alignment: .leading, spacing: 14) {
            ...
            calloutCard(
                ...
                color: .blue,
                text: "The desktop app doesn't live-reload external writes. Use the desktop app handoff button in DJL to continue the current thread on your device."
            )
        }
    }
