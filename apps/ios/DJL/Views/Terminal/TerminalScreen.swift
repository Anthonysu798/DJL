...
// Purpose: Full-page Ghostty SSH terminal route for DJL's remote terminal.
// Layer: View
// Exports: TerminalScreen
// Depends on: CodexService, GhosttyTerminalSurface, DJLTerminalModels
...
struct TerminalScreen: View {
    ...
    @State private var draftProfile = DJLTerminalProfileStore.load()
    @State private var connectionDraft = DJLTerminalProfileStore.load().connectionString
    @State private var privateKeyDraft = DJLTerminalPrivateKeyStore.loadPrivateKey()
    @State private var passphraseDraft = DJLTerminalPrivateKeyStore.loadPassphrase()
    ...
    private var profileResolvedFromConnection: DJLTerminalProfile {
    ...
    private var hasConnectionConfiguration: Bool {
        ...
        return !profile.host.trimmingCharacters(in: .whitespacesAndNewlines).isEmpty
            ...
            && DJLTerminalPrivateKeyStore.hasPrivateKey(privateKeyDraft)
    }
    ...
    private var activeSnapshot: DJLTerminalSnapshot {
    ...
    private func openTerminal() async {
        ...
        actionErrorMessage = nil
        DJLTerminalProfileStore.save(connectionProfile)
        DJLTerminalPrivateKeyStore.savePrivateKey(privateKeyDraft)
        DJLTerminalPrivateKeyStore.savePassphrase(passphraseDraft)
    ...
    private func resetKnownHost() {
        ...
        DJLSSHKnownHostStore.delete(host: profile.host, port: profile.port)
        actionErrorMessage = nil
    }
    ...
    private func adjustFontSize(_ delta: Double) {
        terminalFontSize = min(
            djlTerminalMaxFontSize,
            max(djlTerminalMinFontSize, terminalFontSize + delta)
        )
    }
    ...
    private func terminalErrorText(_ error: Error) -> String {
        ...
        return DJLNativeSSHTerminalError.userFacingDescription(for: error)
    }
