...
// Purpose: User-friendly setup guide for creating an SSH connection from DJL.
// Layer: View Component
// Exports: TerminalConnectionHelpSheet
// Depends on: SwiftUI, UIKit, DJLIcon, HapticFeedback
...
private enum TerminalConnectionHelpPlatform: String, CaseIterable, Identifiable {
    ...
    var masterPrompt: String {
        ...
        case .mac:
            return """
            I'm setting up DJL on my iPhone so I can open a terminal on my Mac over SSH.
            ...
            3. Create a dedicated SSH key pair for DJL and add the public key to this Mac.
            4. Tell me exactly what to enter in DJL: connection string (user@host), nickname, private key, and port.

            Keep it beginner-friendly. Tell me exactly what to click or type, and ask me one question at a time if you need more info.
            """
        case .windows:
            return """
            I'm setting up DJL on my iPhone so I can open a terminal on my Windows PC over SSH.
            ...
            3. Create a dedicated SSH key pair for DJL and add the public key to this PC.
            4. Tell me exactly what to enter in DJL: connection string (user@host), nickname, private key, and port.
        ...
        NavigationStack {
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    Text("SSH is the secure tunnel DJL uses to reach your Mac or PC. You need an address, a username, SSH enabled, and a private key.")
                        .font(.system(size: 14))
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 14) {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text("Run these in an Administrator PowerShell on the Windows PC, then create a terminal profile in DJL.")
                    .font(.system(size: 15))
                    .foregroundStyle(.secondary)
                    .fixedSize(horizontal: false, vertical: true)
            }
            ...
            Text("Recommended private-key format: OpenSSH Ed25519 with a passphrase, created by ssh-keygen above. DJL also accepts encrypted PKCS#8 and encrypted legacy PEM keys; unencrypted keys work only if you explicitly enable that option on the profile. In DJL, use the Windows username, PC IPv4 address, port 22, and the matching private key. Administrator accounts use C:\\ProgramData\\ssh\\administrators_authorized_keys instead.")
                .font(.system(size: 14))
                .foregroundStyle(.secondary)
                .fixedSize(horizontal: false, vertical: true)
                .padding(14)
    ...
    var body: some View {
        Button {
            ...
            HStack(alignment: .top, spacing: 10) {
                ...
                DJLIcon.image(systemName: didCopy ? "checkmark" : "doc.on.doc")
    ...
    var body: some View {
        HStack(spacing: 10) {
            ...
            Button {
                ...
                Group {
                    if didCopy {
                        DJLIcon.image(systemName: "checkmark")
                            .font(.system(size: 12, weight: .semibold))
                    } else {
                        Image("copy")
    ...
    var body: some View {
        HStack(alignment: .top, spacing: 12) {
            ...
            VStack(alignment: .leading, spacing: 3) {
                HStack(spacing: 6) {
                    DJLIcon.image(systemName: step.icon)
                        .font(.system(size: 12, weight: .semibold))
                        .foregroundStyle(.secondary)
    ...
    static func steps(for platform: TerminalConnectionHelpPlatform) -> [TerminalConnectionHelpStep] {
        ...
        case .mac:
            return [
                ...
                TerminalConnectionHelpStep(
                    number: 3,
                    title: "Create a DJL key",
                    icon: "key",
                    body: "Open Terminal and create a dedicated SSH key for DJL. The public half is allowed on the Mac; the private half is pasted into DJL and should not be shared anywhere else."
                ),
                TerminalConnectionHelpStep(
                    number: 4,
                    title: "Fill in the connection",
                    icon: "terminal",
                    body: "In DJL, enter the connection as user@address, give it a nickname, paste the private key, and leave the port as 22 unless you changed it."
                ),
                TerminalConnectionHelpStep(
                    number: 5,
        ...
        case .windows:
            return [
                ...
                TerminalConnectionHelpStep(
                    ...
                    command: "ssh-keygen -t ed25519 -f $HOME\\.ssh\\djl_ed25519"
                ),
                TerminalConnectionHelpStep(
                    number: 4,
                    title: "Add that public key for the Windows user you will log in as",
                    icon: "terminal",
                    body: "Paste the contents of djl_ed25519.pub into authorized_keys.",
                    command: """
                ...
                TerminalConnectionHelpStep(
                    number: 5,
                    title: "Find the PC IP, then use it in DJL",
                    icon: "checkmark.seal.fill",
                    body: "",
                    command: "ipconfig"
                )
