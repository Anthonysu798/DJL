...
// Depends on: SwiftUI, DJLTerminalTheme, TerminalUIModels, AdaptiveGlassModifier
    ...
    var body: some View {
        ZStack(alignment: .trailing) {
            ScrollView(.horizontal, showsIndicators: false) {
                ...
                .frame(minHeight: djlTerminalAccessoryHeight, alignment: .leading)
            }
        ...
        .padding(.vertical, 4)
        .frame(minHeight: djlTerminalAccessoryHeight)
...
private struct TerminalArmedModifierBadge: View {
    ...
    var body: some View {
        HStack(spacing: 5) {
            DJLIcon.image(systemName: modifier.menuSymbolName)
...
private struct TerminalRouteKeySegment: View {
        ...
        Menu {
            Picker(
                ...
                ForEach(TerminalPendingModifier.allCases, id: \.self) { modifier in
                    DJLIcon.menuLabel(modifier.menuTitle, systemName: modifier.menuSymbolName)
                        .tag(modifier)
                }
            }
            .pickerStyle(.inline)
    ...
    private var segmentLabel: some View {
        HStack(spacing: 3) {
            ...
            if action.isModifier {
                DJLIcon.image(systemName: "chevron.up.chevron.down")
...
private struct DPadArrows: View {
    ...
    private func arrow(_ direction: TerminalDPadDirection) -> some View {
        ZStack {
            ...
            DJLIcon.image(systemName: direction.symbolName)
