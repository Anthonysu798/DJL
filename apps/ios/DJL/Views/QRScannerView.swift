...
struct QRScannerView: View {
            ...
            VStack(alignment: .leading, spacing: 24) {
                ...
                VStack(alignment: .leading, spacing: 14) {
                    if let command = prompt.command, !command.isEmpty {
                        ...
                        bridgeUpdateStep(number: "1", title: "Update DJL", detail: "Install the latest DJL build on this iPhone.")
    ...
    private func backButton(action: @escaping () -> Void) -> some View {
        Button(action: action) {
            DJLIcon.image(systemName: "chevron.left")
    ...
    private var cameraPermissionView: some View {
        VStack(spacing: 20) {
            DJLIcon.image(systemName: "camera.fill")
                .font(.system(size: 48))
...
private final class QRCameraLifecycleCoordinator {
    ...
    private let queue = DispatchQueue(label: "app.djl.ios.qr-camera.lifecycle")
