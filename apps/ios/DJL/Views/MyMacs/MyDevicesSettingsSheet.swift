...
// Depends on: SwiftUI, UIKit, CodexService, MyDevicesPresentation, DJLIcon, UIKitMenuButton
...
struct MyDevicesSettingsSheet: View {
    ...
    var body: some View {
        NavigationStack {
            List {
                ...
                Section {
                    ForEach(MyDeviceConnectionAction.allCases, id: \.self) { action in
                        connectionActionRow(action)
                    }
                }
            }
        ...
        let switchActions: [UIMenuElement] = pickerDevices
            .map { candidate in
                UIAction(
                    ...
                    image: DJLIcon.menuUIImage(systemName: MyDevicesPresentation.macIconSystemName),
    ...
    private func connectionActionRow(_ action: MyDeviceConnectionAction) -> some View {
        Button {
    ...
    private func handleConnectionAction(_ action: MyDeviceConnectionAction) {
        switch action {
        case .scanQRCode:
            handleScanQRCode()
        case .pairWithCode:
