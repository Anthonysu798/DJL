    ...
    func reconcileLocalThreadsWithServer(_ serverThreads: [CodexThread]) {
        ...
        for serverThread in serverThreads {
            ...
            if let localThread = localByID[liveThread.id] {
                ...
                // A server-confirmed active thread revives a transient local "not found"
                // placeholder. Explicit user archives remain authoritative.
                liveThread.syncState = persistedArchivedIDs.contains(liveThread.id)
                    ? .archivedLocal
                    : .live
