...
enum AppEnvironment {
    private static let defaultRelayURLInfoPlistKey = "DJL_DEFAULT_RELAY_URL"
    ...
    // Enable from an Xcode scheme with DJL_VERBOSE_LOGGING=1.
    nonisolated static let verboseDiagnosticsEnabled: Bool = {
        #if DEBUG
        ProcessInfo.processInfo.environment["DJL_VERBOSE_LOGGING"] == "1"
        #else
        false
        #endif
    }()
    ...
    static let privacyPolicyURL = URL(
        string: "https://github.com/Emanuele-web04/djl/blob/main/Legal/PRIVACY_POLICY.md"
    )!
    static let termsOfUseURL = URL(
        string: "https://github.com/Emanuele-web04/djl/blob/main/Legal/TERMS_OF_USE.md"
    )!
    ...
    static func feedbackMailtoURL(
        ...
        var queryItems = [
            URLQueryItem(name: "subject", value: "Share Feedback on DJL with the Developer")
        ]
...
private extension AppEnvironment {
    ...
    static func feedbackBody(
        ...
        if let cliVersion = sanitizedCLIVersion(cliVersion) {
            lines.append("- DJL CLI: \(cliVersion)")
        }
        lines.append("")
        lines.append("Notes:")
