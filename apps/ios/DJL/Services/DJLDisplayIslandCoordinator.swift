// FILE: DJLDisplayIslandCoordinator.swift
// Purpose: Owns DJL Live Activity state, coalescing, and expiration.
// Layer: Service

import ActivityKit
import Foundation

struct DJLDisplayIslandSnapshot: Equatable {
    let runningConversations: [DJLDisplayIslandConversation]
    let completedConversations: [DJLDisplayIslandConversation]
    let failedConversations: [DJLDisplayIslandConversation]
...
@MainActor
final class DJLDisplayIslandCoordinator {
    ...
    private var lastSnapshot: DJLDisplayIslandSnapshot?
    ...
    func makeReconciledSnapshot(codex: CodexService, now: Date) -> DJLDisplayIslandSnapshot {
    ...
    private func makeSnapshot(codex: CodexService, now: Date) -> DJLDisplayIslandSnapshot {
        let currentRunningIDs = currentRunningThreadIDs(codex: codex)
        ...
        return DJLDisplayIslandSnapshot(
    ...
    private func runningState(for threadId: String, codex: CodexService) -> DJLDisplayIslandConversationState {
    ...
    private func conversation(
        ...
        state: DJLDisplayIslandConversationState,
        runningStartedAt: Date? = nil,
        codex: CodexService
    ) -> DJLDisplayIslandConversation? {
        ...
        return DJLDisplayIslandConversation(
            id: threadId,
    ...
    private func detail(for thread: CodexThread?) -> String {
        ...
        return lastPathComponent.isEmpty ? "DJL" : lastPathComponent
    }
    ...
    private func scheduleNextTimedSyncIfNeeded(
        codex: CodexService,
        snapshot: DJLDisplayIslandSnapshot,
        now: Date
    ) {
    ...
    private func nextTimedSyncDate(for snapshot: DJLDisplayIslandSnapshot, now: Date) -> Date? {
        [
    ...
    private func staleDeadlineRefreshDate(for snapshot: DJLDisplayIslandSnapshot, now: Date) -> Date? {
    ...
    private func apply(snapshot: DJLDisplayIslandSnapshot, now: Date) async {
        ...
        let content = ActivityContent(
            state: DJLDisplayIslandAttributes.ContentState(
        ...
        if let activity {
            ...
            do {
                let activity = try Activity<DJLDisplayIslandAttributes>.request(
                    attributes: DJLDisplayIslandAttributes(title: "DJL"),
                    content: content,
                    pushType: nil
                )
                activityID = activity.id
    ...
    private func activityStaleDate(for snapshot: DJLDisplayIslandSnapshot, now: Date) -> Date {
        min(
    ...
    private var currentActivity: Activity<DJLDisplayIslandAttributes>? {
        if let activityID,
           let matchingActivity = Activity<DJLDisplayIslandAttributes>.activities.first(where: { $0.id == activityID }) {
            return matchingActivity
        }

        return Activity<DJLDisplayIslandAttributes>.activities.first
    }

    private func endAllActivities() async {
        for activity in Activity<DJLDisplayIslandAttributes>.activities {
