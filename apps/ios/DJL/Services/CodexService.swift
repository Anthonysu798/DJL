...
struct CodexThreadRuntimeOverride: Codable, Equatable, Sendable {
    ...
    var accessModeRawValue: String?
    var overridesAccessMode: Bool
    ...
    init(
        ...
        accessModeRawValue: String? = nil,
        overridesAccessMode: Bool = false,
        ...
        self.accessModeRawValue = accessModeRawValue
        self.overridesAccessMode = overridesAccessMode
    ...
    private enum CodingKeys: String, CodingKey {
        ...
        case accessModeRawValue
        case overridesAccessMode
    ...
    init(from decoder: Decoder) throws {
        ...
        accessModeRawValue = try container.decodeIfPresent(String.self, forKey: .accessModeRawValue)
        overridesAccessMode = try container.decodeIfPresent(Bool.self, forKey: .overridesAccessMode) ?? false
    ...
    var accessMode: CodexAccessMode? {
        guard overridesAccessMode, let accessModeRawValue else {
            return nil
        }
        return CodexAccessMode(rawValue: accessModeRawValue)
    }

    var isEmpty: Bool {
        !overridesModel && !overridesReasoning && !overridesServiceTier && !overridesAccessMode
    }
}
...
final class CodexService {
    static let minimumSupportedBridgePackageVersion = "0.1.0"
    ...
    var supportsThreadRuntimeModeSync = false
    @ObservationIgnored var confirmedThreadAccessModesByThreadID: [String: CodexAccessMode] = [:]
    @ObservationIgnored var pendingThreadAccessModesByThreadID: [String: CodexAccessMode] = [:]
    @ObservationIgnored var threadAccessModeSyncGenerationByThreadID: [String: Int] = [:]
    ...
    var terminalSnapshot: DJLTerminalSnapshot = .idle
    var terminalSnapshotsById: [String: DJLTerminalSnapshot] = [:]
    ...
    let webSocketQueue = DispatchQueue(label: "DJL.WebSocket", qos: .userInitiated)
    ...
    func startNetworkPathMonitor() {
        ...
        monitor.start(queue: DispatchQueue(label: "DJL.NetworkPathMonitor", qos: .utility))
    }
    ...
    var preferredTrustedMacDeviceId: String? {
        if let normalizedCurrentTrustedMacDeviceId {
            return normalizedCurrentTrustedMacDeviceId
        }
        return normalizedLastTrustedMacDeviceId
            .flatMap { trustedMacRegistry.records[$0] == nil ? nil : $0 }
    }
