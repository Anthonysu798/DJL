...
extension CodexService {
    ...
    func handleServerRequest(method: String, requestID: JSONValue, params: JSONValue?) {
        ...
        if method == "item/commandExecution/requestApproval"
            ...
            let isDesktopMirroredApproval = paramsObject?["djlActionSource"]?.stringValue == desktopIpcActionSource
            if effectiveAccessMode(for: request.threadId) == .fullAccess && !isDesktopMirroredApproval {
    ...
    func handleNotification(method: String, params: JSONValue?) {
        ...
        switch method {
        case "djl/thread/runtimeMode/updated":
            if let threadId = paramsObject?["threadId"]?.stringValue,
               let runtimeMode = paramsObject?["runtimeMode"]?.stringValue,
               let accessMode = CodexAccessMode(electronRuntimeMode: runtimeMode) {
                applyRemoteThreadAccessMode(accessMode, for: threadId)
            }
    ...
    private static let nonActivityDesktopMirrorMethods: Set<String> = [
        ...
        "thread/goal/updated",
        "thread/goal/cleared",
        "djl/thread/runtimeMode/updated",
    ...
    private func handleTurnStarted(_ paramsObject: IncomingParamsObject?) {
        ...
        let preservesRunningSourceEpoch = paramsObject?["djlTurnIdentityContinuity"]?.boolValue == true
    ...
    func isReplayedBridgeEvent(_ paramsObject: IncomingParamsObject?) -> Bool {
        paramsObject?["djlReplayedEvent"]?.boolValue == true
    }
    ...
    func isDesktopMirroredBridgeEvent(_ paramsObject: IncomingParamsObject?) -> Bool {
        paramsObject?["djlDesktopMirror"]?.boolValue == true
            || paramsObject?["djlRolloutLiveMirror"]?.boolValue == true
    }
    ...
    func isBackgroundDiscoveryBridgeEvent(_ paramsObject: IncomingParamsObject?) -> Bool {
        paramsObject?["djlBackgroundDiscovery"]?.boolValue == true
    }
    ...
    func isRolloutBootstrapBridgeEvent(_ paramsObject: IncomingParamsObject?) -> Bool {
        paramsObject?["djlRolloutBootstrapReplay"]?.boolValue == true
    }

    func isRolloutBootstrapCompleteEvent(_ paramsObject: IncomingParamsObject?) -> Bool {
        paramsObject?["djlRolloutBootstrapComplete"]?.boolValue == true
    }
    ...
    func isBufferedReplayCompleteEvent(_ paramsObject: IncomingParamsObject?) -> Bool {
        paramsObject?["djlBufferedReplayComplete"]?.boolValue == true
    }
    ...
    func isBufferedReplayGapEvent(_ paramsObject: IncomingParamsObject?) -> Bool {
        paramsObject?["djlBufferedReplayGap"]?.boolValue == true
    }

    func isBufferedReplayResetEvent(_ paramsObject: IncomingParamsObject?) -> Bool {
        paramsObject?["djlBufferedReplayReset"]?.boolValue == true
    }
