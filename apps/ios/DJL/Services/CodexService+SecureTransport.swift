...
extension CodexService {
    ...
    func performSecureHandshake() async throws {
        ...
        guard serverHello.protocolVersion == codexSecureProtocolVersion else {
            presentBridgeUpdatePrompt(
                message: "This bridge is using a different secure transport version. Update the DJL package on your device and try again."
            )
            throw CodexSecureTransportError.incompatibleVersion(
                "This bridge is using a different secure transport version. Update DJL on the iPhone or paired device and try again."
            )
        }
    ...
    func secureWireText(for plaintext: String) throws -> String {
        guard var secureSession else {
            throw CodexSecureTransportError.invalidHandshake(
                "The secure DJL session is not ready yet. Try reconnecting."
            )
        }
        ...
        guard let text = String(data: data, encoding: .utf8) else {
            throw CodexSecureTransportError.invalidHandshake("Unable to encode the secure DJL envelope.")
        }
        return text
    }
    ...
    func resolvePairingCode(_ code: String) async throws -> CodexPairingQRPayload {
        ...
        // A manual code can be resolved through an HTTP(S) registry base URL, but the
        // resulting pairing must always connect to the relay's WebSocket route. QR
        // payloads already contain that route; construct the same canonical route here.
        guard let pairingRelayURL = pairingCodeWebSocketRelayURL(from: relayURL) else {
...
extension CodexService {
    ...
    func trustMac(
        ...
        codexPersistTrustedMacRegistry(trustedMacRegistry)
    ...
    private func isStaleGenericPresentationRecord(_ trustedMac: CodexTrustedMacRecord, now: Date) -> Bool {
        ...
        guard !protectedDeviceIds.contains(trustedMac.macDeviceId),
              ...
              now.timeIntervalSince(trustedMacActivityDate(trustedMac)) > TimeInterval(24 * 60 * 60) else {
            return false
        }
...
/// Converts a pairing-code registry URL into the canonical relay WebSocket base used by
/// QR payloads. The resolver never supplies an endpoint, so this transformation stays
/// local and cannot weaken the pairing response's cryptographic identity checks.
func pairingCodeWebSocketRelayURL(from relayURL: String) -> String? {
...
private extension CodexService {
    ...
    func sendWireControlMessage<Value: Encodable>(_ value: Value) async throws {
        ...
        guard let text = String(data: data, encoding: .utf8) else {
            throw CodexSecureTransportError.invalidHandshake("Unable to encode the secure DJL control payload.")
        }
    ...
    func waitForSecureControlMessage(kind: String, timeoutSeconds: TimeInterval = 12) async throws -> String {
        ...
        let waiterID = UUID()
        let timeoutMessage = "Timed out waiting for the secure DJL \(kind) message."
    ...
    func handleEncryptedEnvelopeText(_ text: String) {
        ...
        guard let envelope = try? decodeSecureControl(SecureEnvelope.self, from: text),
            ...
            lastErrorMessage = "The secure DJL payload could not be verified."
            secureConnectionState = .rePairRequired
            return
        }
