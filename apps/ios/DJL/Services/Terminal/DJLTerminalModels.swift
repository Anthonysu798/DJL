// FILE: DJLTerminalModels.swift
// Purpose: Defines the native SSH terminal profile and runtime snapshot models.
// Layer: Service Model
// Exports: DJLTerminalProfile, DJLTerminalSnapshot, DJLTerminalStatus
// Depends on: Foundation, JSONValue

import Foundation

private let djlTerminalMaxBufferCharacters = 200_000
private let djlTerminalMaxBufferBytes = 200_000
private let djlDefaultTerminalId = "term-1"

enum DJLTerminalStatus: String, Codable, Equatable, Sendable {
    case idle
    case starting
    case running
    case exited
...
struct DJLTerminalProfile: Codable, Equatable, Sendable {
    ...
    var normalizedForSave: DJLTerminalProfile {
        DJLTerminalProfile(
...
struct DJLTerminalSnapshot: Equatable, Sendable {
    var terminalId: String
    var instanceId: String?
    var status: DJLTerminalStatus
    var buffer: String
    var bufferData: Data
    var cwd: String
    var cols: Int
    ...
    static let idle = DJLTerminalSnapshot(
        terminalId: djlDefaultTerminalId,
        instanceId: nil,
        status: .idle,
        buffer: "",
        bufferData: Data(),
    ...
    static func idleSnapshot(terminalId: String) -> DJLTerminalSnapshot {
        var snapshot = idle
    ...
    init(resultObject: [String: JSONValue]) {
        let rawStatus = resultObject["status"]?.stringValue ?? DJLTerminalStatus.idle.rawValue
        ...
        self.init(
            terminalId: resultObject["terminalId"]?.stringValue ?? resultObject["terminal_id"]?.stringValue ?? djlDefaultTerminalId,
            instanceId: Self.instanceId(in: resultObject),
            status: DJLTerminalStatus(rawValue: rawStatus) ?? .idle,
    ...
    mutating func applyTerminalEvent(_ paramsObject: [String: JSONValue]) {
        ...
        if let rawStatus = paramsObject["status"]?.stringValue,
           let status = DJLTerminalStatus(rawValue: rawStatus) {
            self.status = status
        }
    ...
    private static func trimmedBufferData(_ value: Data) -> Data {
        guard value.count > djlTerminalMaxBufferBytes else {
            return value
        }
        return Data(value.suffix(djlTerminalMaxBufferBytes))
    }
