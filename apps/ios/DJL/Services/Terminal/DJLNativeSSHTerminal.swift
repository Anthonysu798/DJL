// FILE: DJLNativeSSHTerminal.swift
// Purpose: Owns the phone-side SSH client and bridges raw TTY bytes to Ghostty.
// Layer: Service
// Exports: DJLNativeSSHTerminal, DJLNativeSSHTerminalError
...
enum DJLNativeSSHTerminalError: LocalizedError {
    ...
    static func userFacingDescription(for error: Error) -> String {
        if let terminalError = error as? DJLNativeSSHTerminalError,
    ...
    private static func userFacingDescription(for error: SSHClientError) -> String {
        ...
        case .unsupportedPasswordAuthentication:
            return "This SSH server is asking for password authentication, but DJL terminal currently uses a private key."
        case .unsupportedHostBasedAuthentication:
            return "This SSH server requires host-based authentication, which DJL terminal does not support."
...
@MainActor
final class DJLNativeSSHTerminal {
            ...
            sessionTask = Task { [weak self] in
                ...
                do {
                    let sshClient = try await SSHClient.connect(
                        ...
                        hostKeyValidator: .custom(DJLSSHKnownHostValidator(
                            host: profile.host,
                            port: profile.port
                        )),
                        reconnect: .never
    ...
    func write(_ data: Data) async throws {
        guard let writer else {
            throw DJLNativeSSHTerminalError.sessionNotRunning
        }
    ...
    private static func authenticationMethod(
        ...
        guard !normalizedKey.isEmpty else {
            throw DJLNativeSSHTerminalError.missingPrivateKey
        }
        ...
        default:
            throw DJLNativeSSHTerminalError.unsupportedPrivateKey(keyType.description)
        }
    }
}

private struct DJLSSHKnownHostValidator: NIOSSHClientServerAuthenticationDelegate, Sendable {
    let host: String
    let port: Int
    ...
    nonisolated func validateHostKey(
        ...
        if let storedHostKey = DJLSSHKnownHostStore.load(host: host, port: port) {
            if storedHostKey == currentHostKey {
                validationCompletePromise.succeed(())
            } else {
                validationCompletePromise.fail(DJLNativeSSHTerminalError.hostKeyChanged)
            }
            return
        }

        DJLSSHKnownHostStore.save(currentHostKey, host: host, port: port)
        validationCompletePromise.succeed(())
    }
}
