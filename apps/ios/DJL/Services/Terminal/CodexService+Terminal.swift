...
// Depends on: CodexService, DJLNativeSSHTerminal, DJLTerminalModels
...
extension CodexService {
    ...
    func terminalSnapshot(for terminalId: String) -> DJLTerminalSnapshot {
        if terminalId == Self.defaultTerminalId, terminalSnapshotsById[terminalId] == nil {
            return terminalSnapshot
        }
        return terminalSnapshotsById[terminalId] ?? DJLTerminalSnapshot.idleSnapshot(terminalId: terminalId)
    }

    func knownTerminalSnapshots() -> [DJLTerminalSnapshot] {
    ...
    func openTerminal(
        ...
        let privateKey = DJLTerminalPrivateKeyStore.loadPrivateKey()
        let passphrase = DJLTerminalPrivateKeyStore.loadPassphrase()
        do {
    ...
    private func nativeTerminal(for terminalId: String) -> DJLNativeSSHTerminal {
        ...
        let terminal = DJLNativeSSHTerminal()
    ...
    private func terminalErrorText(_ error: Error) -> String {
        DJLNativeSSHTerminalError.userFacingDescription(for: error)
    }
}
