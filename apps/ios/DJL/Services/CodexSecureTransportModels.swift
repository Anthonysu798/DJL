...
// Purpose: Defines the wire payloads, device trust records, and crypto helpers for DJL E2EE.
...
let codexSecureHandshakeTag = "djl-e2ee-v1"
let codexSecureHandshakeLabel = "client-auth"
let codexSecureClockSkewToleranceSeconds: TimeInterval = 60
let codexTrustedSessionResolveTag = "djl-trusted-session-resolve-v1"
...
enum CodexSecureTransportError: LocalizedError {
    ...
    var errorDescription: String? {
        ...
        case .invalidQR(let message),
            ...
            return message
        case .decryptFailed:
            return "Unable to decrypt the secure DJL payload."
        }
    }
}
...
func codexPhoneIdentityStateFromSecureStore() -> CodexPhoneIdentityState {
    if let existing: CodexPhoneIdentityState = SecureStore.readCodable(
        ...
        codexPersistPhoneIdentityState(existing)
        return existing
    }
    ...
    codexPersistPhoneIdentityState(next)
    return next
}

func codexTrustedMacRegistryFromSecureStore() -> CodexTrustedMacRegistry {
    guard let existing = SecureStore.readCodable(
        CodexTrustedMacRegistry.self,
        for: CodexSecureKeys.trustedMacRegistry
    ) else {
        return .empty
    }
    codexPersistTrustedMacRegistry(existing)
    return existing
}

func codexPersistPhoneIdentityState(_ state: CodexPhoneIdentityState) {
...
func codexPersistTrustedMacRegistry(_ registry: CodexTrustedMacRegistry) {
    SecureStore.writeCodable(
        registry,
        for: CodexSecureKeys.trustedMacRegistry,
        accessibility: kSecAttrAccessibleWhenUnlockedThisDeviceOnly
