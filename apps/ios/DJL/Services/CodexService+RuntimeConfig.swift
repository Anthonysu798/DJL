...
extension CodexService {
    func learnThreadRuntimeModeSyncSupport(from response: RPCMessage) {
        supportsThreadRuntimeModeSync = response.result?.objectValue?["capabilities"]?
            .objectValue?["djlThreadRuntimeModeSync"]?.boolValue == true
    }

    func runtimeAccessConfiguration(threadId: String? = nil) -> RuntimeAccessConfiguration {
        RuntimeAccessConfiguration(mode: effectiveAccessMode(for: threadId))
    }
    ...
    func effectiveAccessMode(for threadId: String?) -> CodexAccessMode {
        guard let normalizedThreadID = normalizedInterruptIdentifier(threadId),
              let override = threadRuntimeOverride(for: normalizedThreadID),
              let accessMode = override.accessMode else {
            return selectedAccessMode
        }
        return accessMode
    }

    func setThreadAccessModeOverride(_ accessMode: CodexAccessMode, for threadId: String) {
        guard let normalizedThreadID = normalizedInterruptIdentifier(threadId) else {
            return
        }
        mutateThreadRuntimeOverride(for: normalizedThreadID) { override in
            override.accessModeRawValue = accessMode.rawValue
            override.overridesAccessMode = true
        }
    }

    func confirmThreadAccessMode(_ accessMode: CodexAccessMode, for threadId: String) {
        guard let normalizedThreadID = normalizedInterruptIdentifier(threadId) else {
            return
        }
        confirmedThreadAccessModesByThreadID[normalizedThreadID] = accessMode
    }

    func applyRemoteThreadAccessMode(_ accessMode: CodexAccessMode, for threadId: String) {
        guard let normalizedThreadID = normalizedInterruptIdentifier(threadId) else {
            return
        }
        if let pendingMode = pendingThreadAccessModesByThreadID[normalizedThreadID],
           pendingMode != accessMode {
            debugRuntimeLog(
                "ignoring stale runtime mode update thread=\(normalizedThreadID) mode=\(accessMode.rawValue) pending=\(pendingMode.rawValue)"
            )
            return
        }
        setThreadAccessModeOverride(accessMode, for: normalizedThreadID)
        confirmThreadAccessMode(accessMode, for: normalizedThreadID)
    }

    func applyRemoteThreadAccessMode(from thread: CodexThread) {
        guard let runtimeMode = thread.runtimeMode,
              let accessMode = CodexAccessMode(electronRuntimeMode: runtimeMode) else {
            return
        }
        applyRemoteThreadAccessMode(accessMode, for: thread.id)
    }

    func setThreadAccessMode(_ accessMode: CodexAccessMode, for threadId: String) async {
        guard let normalizedThreadID = normalizedInterruptIdentifier(threadId) else {
            return
        }
        let confirmedMode = confirmedThreadAccessModesByThreadID[normalizedThreadID]
            ?? effectiveAccessMode(for: normalizedThreadID)
        confirmThreadAccessMode(confirmedMode, for: normalizedThreadID)

        let generation = (threadAccessModeSyncGenerationByThreadID[normalizedThreadID] ?? 0) + 1
        threadAccessModeSyncGenerationByThreadID[normalizedThreadID] = generation
        pendingThreadAccessModesByThreadID[normalizedThreadID] = accessMode
        setThreadAccessModeOverride(accessMode, for: normalizedThreadID)

        guard supportsThreadRuntimeModeSync, isConnected, isInitialized else {
            confirmThreadAccessMode(accessMode, for: normalizedThreadID)
            pendingThreadAccessModesByThreadID.removeValue(forKey: normalizedThreadID)
            return
        }
        ...
        do {
            let response = try await sendRequest(
                ...
                params: .object([
                    ...
                    "runtimeMode": .string(accessMode.electronRuntimeMode),
                ])
            )
            guard threadAccessModeSyncGenerationByThreadID[normalizedThreadID] == generation else {
                return
            }
            guard let appliedRuntimeMode = response.result?.objectValue?["runtimeMode"]?.stringValue,
                  let appliedAccessMode = CodexAccessMode(electronRuntimeMode: appliedRuntimeMode),
                  appliedAccessMode == accessMode else {
                throw CodexServiceError.invalidResponse(
                    "DJL Electron did not confirm the selected permission mode."
                )
            }
            setThreadAccessModeOverride(appliedAccessMode, for: normalizedThreadID)
            confirmThreadAccessMode(appliedAccessMode, for: normalizedThreadID)
            pendingThreadAccessModesByThreadID.removeValue(forKey: normalizedThreadID)
        } catch {
            guard threadAccessModeSyncGenerationByThreadID[normalizedThreadID] == generation else {
                return
            ...
            pendingThreadAccessModesByThreadID.removeValue(forKey: normalizedThreadID)
            setThreadAccessModeOverride(confirmedMode, for: normalizedThreadID)
            let detail = error.localizedDescription.trimmingCharacters(in: .whitespacesAndNewlines)
            lastErrorMessage = detail.isEmpty
                ? "Unable to update this chat's permission mode."
                : detail
            debugRuntimeLog(
                "runtime mode sync failed thread=\(normalizedThreadID): \(lastErrorMessage ?? "unknown error")"
            )
        }
    }
    ...
    func validateAppliedAccessConfiguration(
        ...
        if let runtimeMode = result["runtimeMode"]?.stringValue,
           let appliedAccessMode = CodexAccessMode(electronRuntimeMode: runtimeMode) {
            guard appliedAccessMode == expected.mode else {
                throw CodexServiceError.invalidResponse(
                    "\(context) applied runtime mode \(runtimeMode) instead of \(expected.mode.electronRuntimeMode)."
                )
            }
            return
        }
        ...
        if result["djlDesktopIpcMirror"]?.boolValue == true {
            return
        }
    ...
    func applyRemoteRuntimeSettings(from thread: CodexThread) {
        applyRemoteThreadAccessMode(from: thread)
        ...
        let existingAccessMode = threadRuntimeOverride(for: thread.id)?.accessMode
        let runtimeOverride = CodexThreadRuntimeOverride(
            ...
            accessModeRawValue: existingAccessMode?.rawValue,
            overridesAccessMode: existingAccessMode != nil,
