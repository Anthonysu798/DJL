...
extension CodexService {
    ...
    func initializeSession() async throws {
        ...
        do {
            ...
            learnThreadRuntimeModeSyncSupport(from: initializeResponse)
    ...
    func incompatibleBridgeAppVersionError(from error: Error) -> CodexServiceError? {
        ...
        if !message.isEmpty {
            ...
            promptMessage =
                "This device bridge is running DJL \(bridgeVersion), which requires DJL iPhone \(minimumSupportedAppVersion) or newer. Update the iPhone app, then reconnect."
        } else if let minimumSupportedAppVersion, !minimumSupportedAppVersion.isEmpty {
            promptMessage =
                "This device bridge requires DJL iPhone \(minimumSupportedAppVersion) or newer. Update the iPhone app, then reconnect."
        } else {
            promptMessage = "This device bridge requires a newer DJL iPhone app. Update the app, then reconnect."
        }
        ...
        return .invalidInput("This device bridge requires a newer DJL iPhone app. Update the app, then reconnect.")
    }
...
extension CodexService {
    ...
    func userFacingConnectError(error: Error, attemptedURL: String, host: String?) -> String {
        ...
        if nsError.domain == NSURLErrorDomain,
           ...
           requiresLocalNetworkAuthorization(for: URL(string: attemptedURL) ?? URL(fileURLWithPath: "/")) {
            return "DJL cannot open the local relay connection on this iPhone. Check Local Network and the app's Wi-Fi/Cellular access in Settings, then retry."
        }
    ...
    func retryableSessionUnavailableMessage(for closeCode: NWProtocolWebSocket.CloseCode?) -> String? {
        ...
        return "Trying to reach your saved device. DJL will keep retrying. If you restarted the bridge on that device, scan the new QR code."
    }
    ...
    func requestLocalNetworkAuthorizationIfNeeded(for url: URL) async throws {
        ...
        guard status != .denied else {
            let message =
                "DJL is not allowed to access your local network. Enable Local Network for DJL in iPhone Settings and try again."
            lastErrorMessage = message
