// FILE: DJLQuickActionCenter.swift
// Purpose: Publishes Home Screen quick actions and routes selected shortcuts
//          back into the SwiftUI root.
// Layer: App
// Exports: DJLQuickAction, DJLQuickActionCenter

import Foundation
import UIKit

enum DJLQuickAction: Equatable, Sendable {
    case newChat
    case thread(id: String)
}

@MainActor
enum DJLQuickActionCenter {
    static let didReceiveQuickAction = Notification.Name("djl.didReceiveQuickAction")

    private static let newChatType = "app.djl.ios.quickAction.newChat"
    private static let threadType = "app.djl.ios.quickAction.thread"
    private static let threadIDUserInfoKey = "threadId"
    private static var pendingAction: DJLQuickAction?
    ...
    static func consumePendingAction() -> DJLQuickAction? {
    ...
    private static func action(from shortcutItem: UIApplicationShortcutItem) -> DJLQuickAction? {
