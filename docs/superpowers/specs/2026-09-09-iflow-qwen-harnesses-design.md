...
the CLI's own stored login, the same way Codex, Claude Code, Cursor, Grok Build
and Kimi Code work.
...
| Auth methods advertised         | `oauth-iflow` (browser login), `iflow` (API key), `openai-compatible` | `openai` only; Qwen OAuth free tier discontinued 2026-04-15                                 |
...
| Login command                   | none; interactive CLI shows its auth dialog                           | none; interactive CLI shows its auth dialog, `/auth` when signed in                         |
