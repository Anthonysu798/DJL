...
| Env stripped | `CODEBUDDY_API_KEY`, `CODEBUDDY_AUTH_TOKEN`, `CODEBUDDY_BASE_URL`, `CODEBUDDY_INTERNET_ENVIRONMENT` (+ misspelt variants), `CODEBUDDY_MODEL`, `CODEBUDDY_CUSTOM_HEADERS`; sets `DISABLE_AUTOUPDATER=1`, `CODEBUDDY_DISABLE_IDE=1` | none removed (Pi's own auth file already outranks env keys); sets `PI_SKIP_VERSION_CHECK=1` (full offline mode would also stop Pi downloading its `fd` helper, seen live) |
